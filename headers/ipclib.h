...
#define pid_to_taskid(tid, pid) (((uint32_t)(tid) << PID_OFFSET) | ((uint32_t)(pid) & PID_MASK))
...
#define taskid_to_cid(task_id, ch_num) ((task_id) | ((uint32_t)(ch_num) << CID_OFFSET))
...
int32_t ipc_create_channel(const char *name, int32_t ch_cnt, cref_t *pp_ch[], struct reg_items_st reg_items);

int32_t ipc_remove_channel(uint32_t task_id, const char *name, int32_t ch_num, cref_t ch);
...
/* Use with ipc_get_ch_from_taskid to release resources on the client and clear the cache. */
...
/* Register exist channel to pathmgr. Return 0 for success, and -1 for error */
int32_t ipc_register_ch_path(const char *path, cref_t ch);

int32_t ipc_msg_call(cref_t channel, void *send_buf, size_t send_len, void *reply_buf, size_t reply_len, int32_t timeout);
