 ...
 */
#ifndef HISEE_TRY_CATCH_H
#define HISEE_TRY_CATCH_H
#include "tee_log.h"

#define hisee_tpdu_print tloge

struct hisee_mntn_info {
    uint32_t errorcode;
    uint32_t errorline;
    uint32_t logpara1;
};

#define __TRY struct hisee_mntn_info mntn_info_value = {0, 0xFFFFFFFF, 0};

/* Here write as follow, checkpatch mischeck, no use do - while loop */
#define __CATCH \
    __tabErr: \
        hisee_tpdu_print("[%s] line(%d),error(%u),para(%u)\n", __func__, mntn_info_value.errorline,\
