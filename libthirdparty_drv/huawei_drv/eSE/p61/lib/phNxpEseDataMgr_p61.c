 ...
 *
 ******************************************************************************/
ESESTATUS phNxpEse_GetData_p61(uint32_t* data_len, uint8_t** pbuffer) {
  uint32_t total_data_len = 0;
  uint8_t* pbuff = NULL;

  if (total_len == 0) {
    tloge("%s total_len = %d", __FUNCTION__, total_len);
    return ESESTATUS_FAILED;
  }
  pbuff = (uint8_t*)phNxpEse_memalloc_p61(total_len);
  if (NULL == pbuff) {
    tloge("%s Error in malloc ", __FUNCTION__);
    return ESESTATUS_NOT_ENOUGH_MEMORY;
  }

  if (ESESTATUS_SUCCESS != phNxpEse_GetDataFromList(&total_data_len, pbuff)) {
    tloge("%s phNxpEse_GetDataFromList", __FUNCTION__);
    phNxpEse_free_p61(pbuff);
    return ESESTATUS_FAILED;
  }
  if (total_data_len != total_len) {
    tloge("%s Mismatch of len total_data_len %d total_len %d", __FUNCTION__,
...
/******************************************************************************
 * Function         phNxpEse_StoreDatainList_p61
 *
 ...
 *
 ******************************************************************************/
ESESTATUS phNxpEse_StoreDatainList_p61(uint32_t data_len, uint8_t* pbuff) {
  phNxpEse_sCoreRecvBuff_List_t* newNode = NULL;

  newNode = (phNxpEse_sCoreRecvBuff_List_t*)phNxpEse_memalloc_p61(
  ...
  phNxpEse_memcpy_p61(newNode->tData.sbuffer, pbuff, data_len);
  total_len += data_len;
  if (head == NULL) {
    head = newNode;
    current = newNode;
...
static ESESTATUS phNxpEse_GetDataFromList(uint32_t* data_len, uint8_t* pbuff) {
  ...
  new_node = head;
  while (new_node != NULL) {
    phNxpEse_memcpy_p61((pbuff + offset), new_node->tData.sbuffer,
                    new_node->tData.wLen);
 ...
 *
 ******************************************************************************/
static ESESTATUS phNxpEse_DeletList(phNxpEse_sCoreRecvBuff_List_t* ph_head) {
    ESESTATUS status = ESESTATUS_SUCCESS;
    phNxpEse_sCoreRecvBuff_List_t *ph_current, *next;
    ph_current = ph_head;
    ...
    while (ph_current != NULL) {
        next = ph_current->pNext;
        phNxpEse_free_p61(ph_current);
        ph_current = NULL;
        ph_current = next;
    }
    ph_head = NULL;
