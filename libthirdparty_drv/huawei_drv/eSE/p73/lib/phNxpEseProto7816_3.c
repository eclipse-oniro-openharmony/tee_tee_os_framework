...
static ESESTATUS phNxpEseProto7816_SendRawFrame(uint32_t data_len,
  ...
  HISI_PRINT_INFO("Enter phNxpEseProto7816_SendRawFrame");
  status = phNxpEse_WriteFrame(data_len, p_data);
  if (ESESTATUS_SUCCESS != status) {
    HISI_PRINT_ERROR("%s Error phNxpEse_WriteFrame\n", __FUNCTION__);
  } else {
    HISI_PRINT_INFO( "%s phNxpEse_WriteFrame Success \n",
             __FUNCTION__);
  }
  HISI_PRINT_INFO("Exit %s ", __FUNCTION__);
  return status;
}
...
static ESESTATUS phNxpEseProto7816_GetRawFrame(uint32_t* data_len,
  ...
  if (ESESTATUS_SUCCESS != status) {
    HISI_PRINT_ERROR("%s phNxpEse_read failed , status : 0x%x", __FUNCTION__, status);
  }
  return status;
}
...
static ESESTATUS phNxpEseProto7816_CheckLRC(uint32_t data_len, uint8_t* p_data) {
  ...
  uint8_t calc_crc = 0;
  uint8_t recv_crc = 0;
  HISI_PRINT_INFO("Enter %s len %d", __FUNCTION__, data_len);
  if(data_len > 0) {
    recv_crc = p_data[data_len - 1];

    /* calculate the CRC after excluding CRC  */
    calc_crc = phNxpEseProto7816_ComputeLRC(p_data, 1, (data_len - 1));
    HISI_PRINT_INFO( "Received LRC:0x%x Calculated LRC:0x%x",
             recv_crc, calc_crc);
    if (recv_crc != calc_crc) {
      status = ESESTATUS_FAILED;
      HISI_PRINT_ERROR("%s LRC failed Received:0x%x Calculated:0x%x",
          __FUNCTION__, recv_crc, calc_crc);
    }
  } else {
    status = ESESTATUS_FAILED;
    HISI_PRINT_ERROR("%s LRC failed length = 0", __FUNCTION__);
  }
  HISI_PRINT_INFO("Exit %s ", __FUNCTION__);
  return status;
}
...
static ESESTATUS phNxpEseProto7816_SendSFrame(sFrameInfo_t sFrameData) {
  ...
  switch (sframeData.sFrameType) {
    ...
    case ATR_REQ:
      ...
      break;
    default:
      HISI_PRINT_ERROR("Invalid S-block");
      break;
  }
  if (NULL != p_framebuff) {
    /* frame the packet */
    ...
    HISI_PRINT_ERROR("Invalid S-block or malloc for s-block failed");
  }
  HISI_PRINT_INFO("Exit %s ", __FUNCTION__);
  return status;
}
...
static ESESTATUS phNxpEseProto7816_sendRframe(rFrameTypes_t rFrameType) {
  ...
  recv_ack[1] |=
      ((phNxpEseProto7816_3_Var.phNxpEseRx_Cntx.lastRcvdIframeInfo.seqNo ^ 1)
       << 4);
  HISI_PRINT_ERROR( "%s recv_ack[1]:0x%x", __FUNCTION__, recv_ack[1]);
  recv_ack[3] =
...
static ESESTATUS phNxpEseProto7816_SendIframe(iFrameInfo_t iFrameData) {
  ...
  if (0 == iFrameData.sendDataLen) {
    HISI_PRINT_ERROR("I frame Len is 0, INVALID");
    return ESESTATUS_FAILED;
  }
  /* This update is helpful in-case a R-NACK is transmitted from the MW */
  ...
  p_framebuff = (uint8_t*)phNxpEse_memalloc(frame_len * sizeof(uint8_t));
  if (NULL == p_framebuff) {
    HISI_PRINT_ERROR("Heap allocation failed");
    return ESESTATUS_FAILED;
  }
...
static ESESTATUS phNxpEseProto7816_SetFirstIframeContxt(void) {
  ...
  HISI_PRINT_ERROR( "I-Frame Data Len: %d Seq. no:%d",
           phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx.IframeInfo.sendDataLen,
           phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx.IframeInfo.seqNo);
  HISI_PRINT_INFO( "Exit %s ", __FUNCTION__);
...
static ESESTATUS phNxpEseProto7816_SetNextIframeContxt(void) {
  ...
  if (phNxpEseProto7816_3_Var.phNxpEseLastTx_Cntx.IframeInfo.totalDataLen >
      phNxpEseProto7816_3_Var.phNxpEseLastTx_Cntx.IframeInfo.currentDataLenIFS) {
    HISI_PRINT_ERROR( "Process Chained Frame");
    phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx.IframeInfo.isChained = true;
    phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx.IframeInfo.sendDataLen =
        phNxpEseProto7816_3_Var.phNxpEseLastTx_Cntx.IframeInfo.currentDataLenIFS;
    phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx.IframeInfo.totalDataLen =
  ...
  HISI_PRINT_ERROR( "I-Frame Data Len: %d",
           phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx.IframeInfo.sendDataLen);
  HISI_PRINT_INFO("Exit %s ", __FUNCTION__);
  return ESESTATUS_SUCCESS;
}
...
static ESESTATUS phNxpEseProro7816_SaveIframeData(uint8_t* p_data,
  ...
  ESESTATUS status = ESESTATUS_FAILED;
  HISI_PRINT_INFO("Enter %s ", __FUNCTION__);
  HISI_PRINT_INFO( "Data[0]=0x%x len=%d Data[%d]=0x%x", p_data[0],
           data_len, data_len - 1, p_data[data_len - 1]);
  if (ESESTATUS_SUCCESS != phNxpEse_StoreDatainList(data_len, p_data)) {
    HISI_PRINT_ERROR("%s - Error storing chained data in list", __FUNCTION__);
  } else {
...
static ESESTATUS phNxpEseProto7816_RecoverySteps(void) {
  if (phNxpEseProto7816_3_Var.recoveryCounter <= GET_FRAME_RETRY_COUNT()) {
    ...
    HISI_PRINT_ERROR("%s Recovery failed", __FUNCTION__);
  }
  return ESESTATUS_SUCCESS;
}
...
static void phNxpEseProto7816_DecodeSFrameIFSData(uint8_t* p_data) {
  ...
  if(ifsd_data == phNxpEseProto7816_3_Var.currentIFSDSize) {
    ...
    HISI_PRINT_ERROR( "%s IFS adjustment: Max DataLen=%d \n",
             __FUNCTION__,
             phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx.IframeInfo
                 .currentDataLenIFS);
  } else {
    HISI_PRINT_ERROR("%s ERROR IFS adjustment: Max DataLen=%d \n",
             __FUNCTION__,
             phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx.IframeInfo
                 .currentDataLenIFS);
  }
...
static void phNxpEseProto7816_DecodeSFrameATRData(uint8_t* p_data) {
  ...
  HISI_PRINT_ERROR(
      ...
      phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx.IframeInfo
          .defaultDataLenIFSC);
  HISI_PRINT_ERROR( "ATR Data Follows");
  HISI_PRINT_ERROR( "======================");
  HISI_PRINT_ERROR( "ATR Length = %d",
           phNxpEseProto7816_3_Var.atrInfo.len);
  HISI_PRINT_ERROR( "Vendor ID = 0x%.2x%.2x%.2x%.2x%.2x",
  ...
  if (phNxpEseProto7816_3_Var.atrInfo.vendorID[4] == PH_SE_OS_VERSION_11) {
    ...
    HISI_PRINT_ERROR("Channel Number = 0x%x",
             phNxpEseProto7816_3_Var.extndAtrInfo.channelNo);
    HISI_PRINT_ERROR("OS Type = %s",
...
static void phNxpEseProto7816_DecodeSFrameSecureTimerData(uint8_t* p_data) {
  ...
  while (maxSframeLen > frameOffset) {
    ...
    HISI_PRINT_INFO( "%s frameoffset=%d value=0x%x\n", __FUNCTION__,
  ...
  HISI_PRINT_INFO( "secure timer t1 = 0x%x t2 = 0x%x t3 = 0x%x",
...
static ESESTATUS phNxpEseProto7816_DecodeFrame(uint8_t* p_data,
  ...
  HISI_PRINT_INFO("Enter %s ", __FUNCTION__);
  HISI_PRINT_INFO( "Retry Counter = %d\n",
  ...
  if (0x00 == pcb_bits.msb) /* I-FRAME decoded should come here */
  {
    if (phNxpEseProto7816_3_Var.phNxpEseLastTx_Cntx.FrameType == SFRAME &&
        phNxpEseProto7816_3_Var.phNxpEseLastTx_Cntx.SframeInfo.sFrameType ==
            INTF_RESET_REQ) {
      HISI_PRINT_ERROR("%s Unexpected Frame, shall retry interface reset",
               __FUNCTION__);
      phNxpEse_memcpy(&phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx,
        ...
        HISI_PRINT_INFO( "%s I-Frame Received", __FUNCTION__);
        ...
        {
            HISI_PRINT_ERROR( "%s I-Frame lastRcvdIframeInfo.seqNo:0x%x",
  ...
  {
    HISI_PRINT_INFO( "%s R-Frame Received", __FUNCTION__);
    phNxpEseProto7816_3_Var.wtx_counter = 0;
  ...
  {
    HISI_PRINT_INFO( "%s S-Frame Received", __FUNCTION__);
    ...
    switch (frameType) {
      ...
      case WTX_REQ:
        ...
        HISI_PRINT_ERROR( "%s Wtx_counter value - %lu", __FUNCTION__,
                 phNxpEseProto7816_3_Var.wtx_counter);
        HISI_PRINT_INFO( "%s Wtx_counter wtx_counter_limit - %lu",
    ...
    case ATR_RSP:
        ...
        break;
      default:
        HISI_PRINT_ERROR("%s Wrong S-Frame Received", __FUNCTION__);
        break;
    }
    ...
    HISI_PRINT_ERROR( "%s Wrong-Frame Received", __FUNCTION__);
  }
  HISI_PRINT_INFO("Exit %s ", __FUNCTION__);
  return status;
}
...
static ESESTATUS phNxpEseProto7816_ProcessResponse(void) {
  ...
  HISI_PRINT_INFO("%s len ----> 0x%x", __FUNCTION__, data_len);
  if (ESESTATUS_SUCCESS == status) {
    ...
    if (status == ESESTATUS_SUCCESS) {
      ...
      HISI_PRINT_ERROR("%s LRC Check failed", __FUNCTION__);
    ...
    HISI_PRINT_ERROR( "%s phNxpEseProto7816_GetRawFrame failed",
             __FUNCTION__);
    if ((SFRAME == phNxpEseProto7816_3_Var.phNxpEseLastTx_Cntx.FrameType) &&
      ...
      if (phNxpEseProto7816_3_Var.timeoutCounter <
        ...
        HISI_PRINT_ERROR( "%s re-transmitting the previous frame",
                 __FUNCTION__);
        phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx =
            phNxpEseProto7816_3_Var.phNxpEseLastTx_Cntx;
      } else {
        ...
        phNxpEseProto7816_3_Var.timeoutCounter = PH_PROTO_7816_VALUE_ZERO;
        HISI_PRINT_INFO( "%s calling phNxpEse_StoreDatainList",
                 __FUNCTION__);
        phNxpEse_StoreDatainList(data_len, p_data);
      }
    }
  }
  HISI_PRINT_INFO("Exit %s Status 0x%x", __FUNCTION__, status);
  return status;
}
...
static ESESTATUS TransceiveProcess(void) {
  ...
  while (phNxpEseProto7816_3_Var.phNxpEseProto7816_nextTransceiveState !=
         IDLE_STATE) {
    HISI_PRINT_ERROR( "%s nextTransceiveState %x", __FUNCTION__,
             phNxpEseProto7816_3_Var.phNxpEseProto7816_nextTransceiveState);
    ...
    if (ESESTATUS_SUCCESS == status) {
      ...
      HISI_PRINT_ERROR("%s Transceive send failed, going to recovery!", __FUNCTION__);
      phNxpEseProto7816_3_Var.phNxpEseProto7816_nextTransceiveState =
          IDLE_STATE;
    }
  };
...
ESESTATUS phNxpEseProto7816_Transceive(phNxpEse_data* pCmd,
  ...
  phNxpEse_data pRes;
  uint8_t osuAid[] = {0x00, 0xA4, 0x04, 0x00, 0x09, 0x4F, 0x70, 0x80, 0x13, 0x04, 0xDE, 0xAD, 0xBE, 0xEF};
  HISI_PRINT_INFO( "Enter %s ", __FUNCTION__);
  ...
  if(pCmd->len == sizeof(osuAid) && (!memcmp(pCmd->p_data, osuAid, pCmd->len))) {
    pRes.p_data = (uint8_t*)phNxpEse_memalloc(2);
    pRes.p_data[0]=0x90;
    pRes.p_data[1]=0x00;
    pRes.len = 2;
    pRsp->len = pRes.len;
    ...
    HISI_PRINT_INFO( "joe get osu aid Exit %s ", __FUNCTION__);
    return ESESTATUS_SUCCESS;
  }
  ...
  phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx.IframeInfo.totalDataLen =
      pCmd->len;
  HISI_PRINT_INFO( "Transceive data ptr 0x%p len:%d", pCmd->p_data,
           pCmd->len);
  ...
  if (ESESTATUS_FAILED == status || ESESTATUS_TRANSCEIVE_FAILED == status) {
    ...
    HISI_PRINT_ERROR("Transceive failed, hard reset to proceed");
...
ESESTATUS phNxpEseProto7816_Open(phNxpEseProto7816InitParam_t initParam) {
  ...
  HISI_PRINT_INFO("%s: First open completed, Congratulations", __FUNCTION__);
  ...
  if(phNxpEseProto7816_3_Var.atrInfo.vendorID[PH_PROTO_ATR_RSP_VENDOR_ID_LEN-1]
    ...
    if(buffer != NULL) {
      ...
      if (ESESTATUS_FAILED == status) {
        /* reset all the structures */
        HISI_PRINT_ERROR( "%s EndOfSession failed ", __FUNCTION__);
      }
...
ESESTATUS phNxpEseProto7816_SetIfs(uint16_t IFS_Size) {
  ...
  if(IFS_Size > phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx.IframeInfo.maxDataLenIFSC) {
    ...
    HISI_PRINT_ERROR( "%s : IFSD greater than IFSC ,  set max IFSC as IFSD  ",__FUNCTION__);
  } else {
...
uint16_t phNxpEseProto7816_GetIfs(void) {
  HISI_PRINT_INFO("Enter %s current IFSC = %d", __FUNCTION__,
      phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx.IframeInfo.currentDataLenIFS);
...
phNxpEseProto7816_OsType_t phNxpEseProto7816_GetOsMode(void) {
    ...
    if (GET_CHIP_OS_VERSION() == OS_VERSION_5_2_2) {
        mode = phNxpEseProto7816_3_Var.extndAtrInfo.osType;
        HISI_PRINT_INFO("Enter %s OS Mode = %d", __FUNCTION__, mode);
    } else {
        HISI_PRINT_ERROR("%s function not supported", __FUNCTION__);
    }
    return mode;
