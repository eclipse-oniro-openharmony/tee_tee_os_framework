...
ESESTATUS phNxpEse_7816_Transceive(pphNxpEse_7816_cpdu_t pCmd,
  ...
  if (NULL == pCmd || NULL == pRsp) {
    ...
    HISI_PRINT_ERROR(" %s Invalid cpdu type \n", __FUNCTION__);
    ...
    HISI_PRINT_ERROR(" %s Invalid response buffer \n", __FUNCTION__);
    ...
    if (ESESTATUS_SUCCESS == status) {
      ...
      if (ESESTATUS_SUCCESS != status) {
        HISI_PRINT_ERROR(" %s phNxpEse_Transceive Failed \n", __FUNCTION__);
        ...
        if ((pRspTrans.len > 0) && (pRspTrans.p_data != NULL)) {
          ...
          pRspTrans.len--;
          pRsp->len = pRspTrans.len;
          HISI_PRINT_INFO("pRsp->len %d", pRsp->len);
          if (pRspTrans.len > 0 && NULL != pRsp->pdata) {
            ...
            HISI_PRINT_ERROR("Invalid Res buffer");
            status = ESESTATUS_FAILED;
          }
          HISI_PRINT_INFO("Freeing memroy pRspTrans.p_data ");
          phNxpEse_free(pRspTrans.p_data);
          pRspTrans.p_data = NULL;
          pRspTrans.len = 0;
        } else {
          HISI_PRINT_ERROR("pRspTrans.len error = %d", pRspTrans.len);
          status = ESESTATUS_FAILED;
        }
      }
      if (pCmd_data != NULL) {
        HISI_PRINT_ERROR("Freeing memory pCmd_data");
...
static ESESTATUS phNxpEse_7816_FrameCmd(pphNxpEse_7816_cpdu_t pCmd,
  ...
  HISI_PRINT_INFO("%s  pCmd->lc = %d, pCmd->le_type = %d", __FUNCTION__, pCmd->lc, pCmd->le_type);
  ...
  if (pCmd->lc > 0) {
    ...
    if (pCmd->pdata == NULL) {
      HISI_PRINT_ERROR("%s Invalide data buffer from application ", __FUNCTION__);
      return ESESTATUS_INVALID_BUFFER;
    }
  } else {
    lc_len = 0;
    HISI_PRINT_INFO("%s lc (data) field is not present %d", __FUNCTION__, pCmd->lc);
  }
  ...
  if (pCmd->le_type > 0) {
    if (pCmd->le_type == 1) {
      ...
      if ((pCmd->le_type == 3) && (lc_len == 0)) {
        ...
        HISI_PRINT_ERROR("%s wrong LE type  %d", __FUNCTION__, pCmd->le_type);
        cmd_total_len += pCmd->le_type;
        le_len = pCmd->le_type;
      }
    } else {
      HISI_PRINT_ERROR("%s wrong cpdu_type value %d", __FUNCTION__, pCmd->cpdu_type);
      return ESESTATUS_INVALID_CPDU_TYPE;
    }
  } else {
    le_len = 0;
    HISI_PRINT_INFO("%s le field is not present", __FUNCTION__);
  }
  HISI_PRINT_INFO("%s cmd_total_len = %d, le_len = %d, lc_len = %d", __FUNCTION__, cmd_total_len, le_len, lc_len);

  pbuff = (uint8_t*)phNxpEse_calloc(cmd_total_len, sizeof(uint8_t));
  if (pbuff == NULL) {
    HISI_PRINT_ERROR("%s Error allocating memory", __FUNCTION__);
  ...
  *(pbuff + index) = pCmd->p2;
  ...
  HISI_PRINT_INFO("Exit %s cmd_total_len = %d, index = %d", __FUNCTION__, index, cmd_total_len);
