...
*/
typedef enum {
	CC_AES_ENCRYPT = 0,	/*!< Encrypt mode. */
	CC_AES_DECRYPT = 1,	/*!< Decrypt mode. */
...
*/
typedef enum {
	CC_AES_USER_KEY          = 0,		/*!< user key. */
	CC_AES_PLATFORM_KEY      = 1,		/*!< Kplt hardware key. */
	CC_AES_CUSTOMER_KEY      = 2,		/*!< Kcst hardware key. */
	CC_AES_CEK_AUDIO         = 3,
	CC_AES_CEK_VIDEO         = 4,
...
typedef struct CCAesUserContext_t {
	uint32_t buff[CC_AES_USER_CTX_SIZE_IN_WORDS];		/*!< Buffer for the AES context. */
}CCAesUserContext_t;
...
CIMPORT_C CCError_t  CC_AesSetKey(
	...
	CCAesKeyType_t       keyType,         /*!< [in]  The type of key to be used for the AES operation.
							   Currently only CC_AES_USER_KEY is supported - the key is plaintext and provided in the pKeyData parameter. */
...
CIMPORT_C CCError_t  CC_AesFinish(
	...
	size_t                 dataSize,       /*!< [in]  The size of the input data in bytes.
							   <ul><li> For CBC-CTS mode, must be > 16. Can be <=16 only if this is the only data (no previous calls were
							            made to CC_AesBlock with the same context).</li>
