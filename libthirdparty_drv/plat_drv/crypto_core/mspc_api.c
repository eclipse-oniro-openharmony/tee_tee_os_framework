...
#define MSPC_APDU_WAIT_TIME             10 /* 10s */
#define MSPC_WAIT_READY_TIMEOUT         10 /* 10s */
...
static int32_t mspc_set_send_work(uint8_t *p_cmd, uint32_t cmd_len)
{
    int32_t ret;
...
int32_t mspc_send_apdu(uint8_t *p_cmd, uint32_t cmd_len)
{
    ...
    (void)pthread_mutex_lock(&g_apdu_work_mutex);
    ret = mspc_set_send_work(p_cmd, cmd_len);
