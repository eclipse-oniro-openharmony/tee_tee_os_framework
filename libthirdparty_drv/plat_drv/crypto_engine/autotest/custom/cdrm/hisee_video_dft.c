...
PRIVATE err_bsp_t hisee_video_output_result(struct hisee_video_cfg *pcfg)
{
	...
	if (pcfg->video_type == HISEE_VIDEO_TYPE_NOPATTERN) {
		...
		return BSP_RET_OK;
		   ...
		   pcfg->pattern_ratio == HISEE_VIDEO_PATTERN_RATION_1_9) {
		cipherlen = PATTERN_1_9_CIPHERLEN;
		skip_len = cipherlen + PATTERN_1_9_PLAINLEN;
					...
					struct hisee_video_init_param *param)
{
	...
	param->pattern_ratio = pcfg->pattern_ratio;
...
PRIVATE err_bsp_t hisee_video_decrypt_frame(struct hisee_video_cfg *pcfg)
{
	...
	for (i = 0; i < pcfg->dinlen_array_size - 1; i++) {
		...
		coutlen = outbuflen;
		PAL_ERROR("video_update,i=%d\n", i);
		ret = hisee_video_update(&ctx, pdin, dinlen, pdout, &coutlen);
		if (PAL_CHECK(ret != BSP_RET_OK))
			goto end;
		pdin      += dinlen;
		pdout     += coutlen;
	...
	dinlen = pcfg->dinlen_array[i];
	coutlen = outbuflen;
	ret = hisee_video_dofinal(&ctx, pdin, dinlen, pdout, &coutlen);
	if (PAL_CHECK(ret != BSP_RET_OK))
		goto end;
	toutlen += coutlen;
