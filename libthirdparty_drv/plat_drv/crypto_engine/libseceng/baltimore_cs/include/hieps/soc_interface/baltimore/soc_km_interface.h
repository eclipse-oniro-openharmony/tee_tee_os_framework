 ...
 ******************************************************************************
  ...
  1.日    期   : 2019年10月26日
    作    者   : l00249396
    修改内容   : 从《HiEPS V210 nManager寄存器手册_KM.xml》自动生成
...
/* 寄存器说明：KM MODE和KM_MODE1的选择信号
   位域定义UNION结构:  SOC_KM_MODE_IND_UNION */
#define SOC_KM_MODE_IND_ADDR(base)                    ((base) + (0x0400))

/* 寄存器说明：密钥路由寄存器1
   位域定义UNION结构:  SOC_KM_MODE1_UNION */
#define SOC_KM_MODE1_ADDR(base)                       ((base) + (0x0404))

/* 寄存器说明：AES_KEY_SEL和AES_KEY_SEL1的选择信号
   位域定义UNION结构:  SOC_KM_AES_KEY_SEL_IND_UNION */
#define SOC_KM_AES_KEY_SEL_IND_ADDR(base)             ((base) + (0x0408))

/* 寄存器说明：AES_KEY_SEL1
   位域定义UNION结构:  SOC_KM_AES_KEY_SEL1_UNION */
#define SOC_KM_AES_KEY_SEL1_ADDR(base)                ((base) + (0x040c))

/* 寄存器说明：根据KDR解密出来的明文的CPU配置key清零信号
   位域定义UNION结构:  SOC_KM_TA_KEY_PLA_CLR_UNION */
#define SOC_KM_TA_KEY_PLA_CLR_ADDR(base)              ((base) + (0x0410))

/* 寄存器说明：派生密钥清零信号
   位域定义UNION结构:  SOC_KM_CEK_CLR_UNION */
#define SOC_KM_CEK_CLR_ADDR(base)                     ((base) + (0x0414))

/* 寄存器说明：根据KDR解密出来的CPU配置key的明文
   位域定义UNION结构:  SOC_KM_TA_KEY_PLA_UNION */
#define SOC_KM_TA_KEY_PLA_ADDR(base, n)               ((base) + (0x0420+(n)*4))

/* 寄存器说明：根据KDR解密出来的CPU配置key的明文的掩码
   位域定义UNION结构:  SOC_KM_TA_KEY_PLA_M_UNION */
#define SOC_KM_TA_KEY_PLA_M_ADDR(base, n)             ((base) + (0x0440+(n)*4))

/* 寄存器说明：CEK的明文
   位域定义UNION结构:  SOC_KM_CEK_PLA_UNION */
#define SOC_KM_CEK_PLA_ADDR(base, n)                  ((base) + (0x0460+(n)*4))

/* 寄存器说明：CEK的明文的掩码
   位域定义UNION结构:  SOC_KM_CEK_PLA_M_UNION */
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  ddrenc_key_derive : 1;  /* bit[14]   : 指示当前的密钥派生是否为DDR加密KEY的派生
                                                             0: 非DDR加密KEY的派生
                                                             1: DDR加密KEY的派生 */
        unsigned int  kdr_inv           : 1;  /* bit[15]   : 用于指示派生出来的密钥是否可读：
                                                             0: 派生密钥可读。
                                                             1: 派生密钥不可读。当派生的主密钥是kdr时，会以kdr的取反值来产生派生key。 */
...
/*****************************************************************************
 结构名    : SOC_KM_MODE_IND_UNION
 结构说明  : MODE_IND 寄存器结构定义。地址偏移量:0x0400，初值:0x00000000，宽度:32
 寄存器说明: KM MODE和KM_MODE1的选择信号
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  km_mode_ind : 4;  /* bit[0-3] : KM MODE和KM_MODE1的选择信号
                                                      4'h0:选择KM_MODE
                                                      4'h5:选择KM_MODE1
                                                      其它reserv */
        unsigned int  reserved    : 28; /* bit[4-31]:  */
...
/*****************************************************************************
 结构名    : SOC_KM_MODE1_UNION
 结构说明  : MODE1 寄存器结构定义。地址偏移量:0x0404，初值:0x00000000，宽度:32
 寄存器说明: 密钥路由寄存器1
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  km_mode1 : 2;  /* bit[0-1] : 密钥管理的模式1
                                                   0: 使用根密钥加密
                                                   1: 使用根密钥解密或使用RTL_KEY1解密
                                                   （AES_KEY_SEL_IND为5且aes_key_sel1为3时选择使用RTL_KEY1解密）
                                                   其他配置值保留 */
        unsigned int  reserved : 30; /* bit[2-31]:  */
    } reg;
} SOC_KM_MODE1_UNION;
...
/*****************************************************************************
 结构名    : SOC_KM_AES_KEY_SEL_IND_UNION
 结构说明  : AES_KEY_SEL_IND 寄存器结构定义。地址偏移量:0x0408，初值:0x00000000，宽度:32
 寄存器说明: AES_KEY_SEL和AES_KEY_SEL1的选择信号
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  aes_key_sel_ind : 4;  /* bit[0-3] : AES_KEY_SEL和AES_KEY_SEL1的选择信号
                                                          4'h0:选择AES_KEY_SEL
                                                          4'h5:选择AES_KEY_SEL1
                                                          其他配置值保留 */
        unsigned int  reserved        : 28; /* bit[4-31]:  */
...
/*****************************************************************************
 结构名    : SOC_KM_AES_KEY_SEL1_UNION
 结构说明  : AES_KEY_SEL1 寄存器结构定义。地址偏移量:0x040c，初值:0x0000，宽度:32
 寄存器说明: AES_KEY_SEL1
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  aes_key_sel1 : 4;  /* bit[0-3] : AES_KEY_SEL1
                                                       1.选择kdr的明文来直接加解密
                                                       2: 选择解密出的明文的CPU配置key用来做密钥
                                                       3: 选择RTL_KEY1做为密钥
                                                       4: 选择解密出的明文CEK用来做密钥
                                                       其他配置值:选择CPU配置key */
        unsigned int  reserved     : 28; /* bit[4-31]:  */
...
/*****************************************************************************
 结构名    : SOC_KM_TA_KEY_PLA_CLR_UNION
 结构说明  : TA_KEY_PLA_CLR 寄存器结构定义。地址偏移量:0x0410，初值:0x00000000，宽度:32
 寄存器说明: 根据KDR解密出来的明文的CPU配置key清零信号
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  ta_key_pla_clr : 1;  /* bit[0]   : 对根据KDR解密出来的明文的cpu配置key进行清零
                                                         写任意值进行清除 */
        unsigned int  reserved       : 31; /* bit[1-31]:  */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_KM_CEK_CLR_UNION
 结构说明  : CEK_CLR 寄存器结构定义。地址偏移量:0x0414，初值:0x00000000，宽度:32
 寄存器说明: 派生密钥清零信号
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  cek_clr  : 1;  /* bit[0]   : 对CEK清零
                                                   写任意值进行清除 */
        unsigned int  reserved : 31; /* bit[1-31]:  */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_KM_TA_KEY_PLA_UNION
 结构说明  : TA_KEY_PLA 寄存器结构定义。地址偏移量:0x0420+(n)*4，初值:0x0000，宽度:32
 寄存器说明: 根据KDR解密出来的CPU配置key的明文
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  ta_key_pla_sec : 32; /* bit[0-31]: n=0时，对应key的高32位,n的范围0-7，所有key为小端方式。 */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_KM_TA_KEY_PLA_M_UNION
 结构说明  : TA_KEY_PLA_M 寄存器结构定义。地址偏移量:0x0440+(n)*4，初值:0x0000，宽度:32
 寄存器说明: 根据KDR解密出来的CPU配置key的明文的掩码
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  ta_key_pla_m : 32; /* bit[0-31]: n=0时，对应key的高32位,n的范围0-7，所有key为小端方式。 */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_KM_CEK_PLA_UNION
 结构说明  : CEK_PLA 寄存器结构定义。地址偏移量:0x0460+(n)*4，初值:0x0000，宽度:32
 寄存器说明: CEK的明文
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  cek_pla_sec : 32; /* bit[0-31]: n=0时，对应key的高32位,n的范围0-7，所有key为小端方式。 */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_KM_CEK_PLA_M_UNION
 结构说明  : CEK_PLA_M 寄存器结构定义。地址偏移量:0x0480+(n)*4，初值:0x0000，宽度:32
 寄存器说明: CEK的明文的掩码
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  cek_pla_m : 32; /* bit[0-31]: n=0时，对应key的高32位,n的范围0-7，所有key为小端方式。 */
    } reg;
