...
/*
 * @brief      : hieps_is_bypassed:teeos->atf get efuse lite chip flag
 * @param[out] : lite_chip_val:lite chip flag
 * @param[out] : modem_bypass_val:modem bypass flag
 * @return     : succ:HIEPS_OK;
 */
static uint32_t hieps_get_efuse_lite_chip(uint32_t *lite_chip_val, uint32_t *modem_bypass_val)
{
    uint32_t ret;
    uint32_t efuse_val;

    ret = hieps_smc_send_process(HIEPS_GET_EFUSE_LITE_CMD, 0, 0, 0);
    if (ret != HIEPS_OK) {
        tloge("hieps: send smc failed! ret = 0x%x\n", ret);
        return ret;
    }

    /* get efuse lite chip value. */
    efuse_val = read32(HIEPS_POWER_RESULT_ADDR);

    /* bit 0: lite chip flag.0 is normal chip, 1 is lite chip */
    *lite_chip_val = (efuse_val & HIEPS_EFUSE_LITE_FLAG_MASK) ? 1 : 0;
    /* bit 1: modem bypass flag. 0 is not bypass, 1 is bypass modem */
    *modem_bypass_val = (efuse_val & HIEPS_EFUSE_MODEM_FLAG_MASK) ? 1 : 0;
    return ret;
}

static int32_t hieps_is_bypassed(void)
{
    uint32_t lite_chip = 0;
    uint32_t modem_bypass = 0;
    int32_t ret;

    ret = hieps_get_efuse_lite_chip(&lite_chip, &modem_bypass);
    if (ret != HIEPS_OK) {
        tloge("hieps: get efuse lite chip failed! ret = 0x%x\n", ret);
        return FALSE;
    }

    /* Return OK when chip is lite chip and modem is not bypass */
    if (lite_chip == IS_LITE_CHIP_FLAG &&
        modem_bypass == MODEM_NOT_BYPASS_FLAG)
        return TRUE;
...
#ifdef CONFIG_HIEPS_BYPASS_LITE_CHIP
    /* bypass hieps for lite chip */
    if (hieps_is_bypassed() == TRUE) {
        tloge("%s-%d:bypass hieps success\n", __func__, __LINE__);
        return HIEPS_OK;
    }
#endif
	switch (cmd) {
