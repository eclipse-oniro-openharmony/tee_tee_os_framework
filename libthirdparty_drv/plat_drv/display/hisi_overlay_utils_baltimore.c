...
void hisi_dss_mcu_interrupt_mask(struct hisifb_data_type *hisifd)
{
	...
	mask = ~0;
	outp32(hisifd->mipi_dsi0_base + MIPI_LDI_MCU_ITF_INT_MSK, mask);
...
void hisi_dss_mcu_interrupt_unmask(struct hisifb_data_type *hisifd)
{
	...
	outp32(hisifd->mipi_dsi0_base + MIPI_LDI_MCU_ITF_INT_MSK, unmask);
}
...
int hisi_vactive0_start_config(struct hisifb_data_type *hisifd)
{
	...
	if (count == TIME_OUT) {
		...
		isr_s2_mask = inp32(hisifd->mipi_dsi0_base + MIPI_LDI_MCU_ITF_INT_MSK);
		isr_s2      = inp32(hisifd->mipi_dsi0_base + MIPI_LDI_MCU_ITF_INTS);
		...
		HISI_FB_ERR("wait vactive0_start timeout: vactive_start_flag = %d,"
				...
				isr_s1_mask, isr_s1,
				isr_s2_mask, isr_s2,
				inp32(hisifd->mipi_dsi0_base + MIPI_LDI_CTRL), inp32(hisifd->mipi_dsi0_base + MIPI_LDI_FRM_MSK));
		return -1;
	}
		...
		do {
			if (count == 0) {
				HISI_FB_ERR("------ MCTRL_CTL0[0x%x]: 0x%x \t",
						hisifd->dss_base + DSS_MCTRL_CTL0_OFFSET + step,
						inp32(hisifd->dss_base + DSS_MCTRL_CTL0_OFFSET + step));
			} else {
				HISI_FB_PRINTF(" 0x%x \t", inp32(hisifd->dss_base + DSS_MCTRL_CTL0_OFFSET + step));
			}
			step += 0x4;
			count++;
		} while (count < 4);
		...
		do {
			if (count == 0) {
				HISI_FB_ERR("------ MCTRL_CTL4[0x%x]: 0x%x \t",
						hisifd->dss_base + DSS_MCTRL_CTL4_OFFSET + step,
						inp32(hisifd->dss_base + DSS_MCTRL_CTL4_OFFSET + step));
			} else {
...
static int hisi_fb_underflow_clear(struct hisifb_data_type *hisifd)
{
	...
	isr_s1 = inp32(dss_base + GLB_MCU_PDP_INTS);
	isr_s2 = inp32(hisifd->mipi_dsi0_base + MIPI_LDI_MCU_ITF_INTS);
	outp32(hisifd->mipi_dsi0_base + MIPI_LDI_MCU_ITF_INTS, isr_s2);
...
int hisi_fb_irq_handle(uint32_t ptr)
{
	...
	if (isr_s2 & BIT_LDI_UNFLOW) {
		...
		if (hisifd->disp_debug_dump == 1) {
			...
			hisifd->set_reg(dss_base + DSS_OVL0_OFFSET + OV8_RD_SHADOW_SEL, 0x1, 1, 0);
			dump_dss_reg_info(hisifd);

			hisifd->set_reg(dss_base + DSS_SEC_RCH_DMA_OFFSET + CH_RD_SHADOW, 0x0, 1, 0);
			hisifd->set_reg(dss_base + DSS_OVL0_OFFSET + OV8_RD_SHADOW_SEL, 0x0, 1, 0);
...
int hisi_dss_mif_config(struct hisifb_data_type *hisifd, uint32_t chn_idx, int securemode)
{
	...
	mif_ch_base = hisifd->dss_base + g_dss_module_base[chn_idx][MODULE_MIF_CHN];
...
int hisi_dss_smmu_config(struct hisifb_data_type *hisifd, uint32_t chn_idx, int securemode)
{
	...
	if (securemode == 1) {
		...
		for (i = 0; i < g_dss_chn_sid_num[chn_idx]; i++) {
			idx = g_dss_smmu_smrx_idx[chn_idx] + i;
			hisifd->set_reg(smmu_base + SMMU_SMRx_P + idx * 0x4, 0x1, 1, 0);
			 if (idx < 32) {
				hisifd->set_reg(smmu_base + SMMU_RLD_EN0_S, 0x1, 1, (uint8_t)idx);
				hisifd->set_reg(smmu_base + SMMU_RLD_EN0_P, 0x1, 1, (uint8_t)idx);
				hisifd->set_reg(smmu_base + SMMU_RLD_EN0_NS, 0x1, 1, (uint8_t)idx);
		...
		for (i = 0; i < g_dss_chn_sid_num[chn_idx]; i++) {
			...
			hisifd->set_reg(smmu_base + SMMU_SMRx_P + idx * 0x4, 0x0, 1, 0);
			if (idx < 32) {
				hisifd->set_reg(smmu_base + SMMU_RLD_EN0_S, 0x1, 1, (uint8_t)idx);
				hisifd->set_reg(smmu_base + SMMU_RLD_EN0_P, 0x1, 1, (uint8_t)idx);
				hisifd->set_reg(smmu_base + SMMU_RLD_EN0_NS, 0x1, 1, (uint8_t)idx);
...
void hisi_dss_ovl_layer_config(struct hisifb_data_type *hisifd, dss_layer_t *layer)
{
	...
	hisifd->set_reg(ovl_base + OVL_LAYER7_POS, (layer->dst_rect.x) |(layer->dst_rect.y << 16), 32, 0);
	hisifd->set_reg(ovl_base + OVL_LAYER7_SIZE, DSS_WIDTH(layer->dst_rect.x + layer->dst_rect.w)
		| (DSS_HEIGHT(layer->dst_rect.y + layer->dst_rect.h) << 16), 32, 0);

	hisifd->set_reg(ovl_base + OVL_LAYER7_ALPHA_MODE, 0x4000, 32, 0);
	hisifd->set_reg(ovl_base + OVL_LAYER7_ALPHA_A, 0x3ff03ff, 32, 0);
	hisifd->set_reg(ovl_base + OVL_LAYER7_CFG, 0x1, 1, 0);
