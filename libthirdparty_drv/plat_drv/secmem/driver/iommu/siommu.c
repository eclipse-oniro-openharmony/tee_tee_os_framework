...
static inline u64 smmu_pte_none_lpae(smmu_pte_t pte)
{
	return !(pte ? pte : 0);
}
...
#ifdef SECMEM_UT
static s32 print_pte_within_one_pmd(const struct smmu_domain *sdomain, u32 iova,
				u32 size)
{
	u32 pte_h;
	u32 pte_l;
...
s32 hisi_dump_pgtable(const struct smmu_domain *sdomain, u32 iova, u32 size)
{
	s32 ret;
	u64 *pmd_base = NULL;
	...
	do {
		...
		if (size >= SMMU_PMDIR_SIZE) {
			ret = print_pte_within_one_pmd(sdomain,
							iova, SMMU_PMDIR_SIZE);
		} else {
			ret = print_pte_within_one_pmd(sdomain, iova, size);
			break;
		}
		/* overflow */
		if (iova + SMMU_PMDIR_SIZE < iova)
...
unmap_pmd:
	(void)sre_unmap((u32)(sdomain->pmd), SZ_4K);
unmap_pgd:
	(void)sre_unmap((u32)(sdomain->pgd), SZ_4K);
free_mem:
	SRE_MemFree(OS_MID_SYS, sdomain);
	return NULL;
}

static u64 smmu_build_pteval(int prot, u64 attr)
{
	u64 pteval = attr;
				...
				struct smmu_addr_pair addr_pair, int prot)
{
	...
	ppte = (smmu_pte_t *)gen_pool_alloc(sdomain->pool, PAGE_SIZE);
	if (!ppte) {
...
pte_ready:
	*ppmd &= ~SMMU_PMD_NS;
	...
	do {
		if (!pte_is_valid_lpae(*ppte)) {
			*ppte = (u64)(addr_pair.paddr | pteval);
		} else {
			tloge("map to same VA more times!, iova:0x%llx\n",
				addr_pair.addr);
			return -EINVAL;
		}
		ppte++;
				...
				struct smmu_addr_pair addr_pair, int prot)
{
	...
	/* Allocate a new set of tables */
	ppmd = (smmu_pmd_t *)gen_pool_alloc(sdomain->pool, PAGE_SIZE);
	if (!ppmd) {
		tloge("%s: alloc page fail\n", __func__);
		return -ENOMEM;
	}
...
pmd_ready:
	*ppgd &= ~SMMU_PGD_NS;
	...
	pmdp = (smmu_pmd_t *)(u32)(uintptr_t)pool_va(
		start, sdomain->pgtable_addr, sdomain->va_base);
...
struct smmu_domain *siommu_domain_alloc(u32 pgtable_addr, u32 pgtable_size)
{
	...
	(void)memset_s((void *)vaddr, pgtable_size, 0, pgtable_size);
	...
	/* alloc pgd first */
	sdomain->pgd = (u64 *)gen_pool_alloc(sdomain->pool, SZ_4K);
	if (!sdomain->pgd) {
