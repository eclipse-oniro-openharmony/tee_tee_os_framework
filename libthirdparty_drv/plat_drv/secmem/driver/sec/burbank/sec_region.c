...
struct sub_rgn_info g_sub_rgn_cfg_table[SUB_RGN_MAX_NUM] = {
	{
		/* DDR_SEC_TUI */
...
static int mddrc_sec_check_secandrgn(unsigned int asi_num, unsigned int rgn_num)
{
	int ret = OK;
...
static int check_rgn_usable(unsigned int asi_num, unsigned int rgn_num)
{
	...
	ret = mddrc_sec_check_secandrgn(asi_num, rgn_num);
...
static void sec_region_clean(unsigned int asi_num, unsigned int rgn_num)
{
	unsigned int value;
...
static int is_not_covered_by_normal_region(u64 start_addr, u64 end_addr)
{
	...
	for (asi_num = 0; asi_num < ASI_NUM_MAX; asi_num++) {
		if ((bit(asi_num) & DDR_SEC_ALL_ASI_MASK) == 0)
			continue;
		for (rgn_num = 1 + SUB_RGN_MAX_NUM; rgn_num < g_rgn_num[asi_num]; rgn_num++) {
...
static int sub_rgn_cfg_asi(struct sub_rgn_info *sub_pro_cfg,
...
static int sub_rgn_cfg(struct sub_rgn_info *sub_pro_cfg, u32 sub_rgn_cfg_addr, u32 sub_rgn_cfg_size, DDR_CFG_TYPE ddr_cfg_type)
{
	int loop_flag;
...
static int check_segment(TEE_PAGEINFO *segment_info)
{
	...
	for (feature_num = 0; feature_num < SUB_RGN_MAX_NUM; feature_num++) {
...
static bool sec_face_check_uuid_fail(void)
{
	return (cmp_uuid(&g_cur_uuid, &g_sec_uuid[SEC_ISP_UUID_NUM]) != OK &&
		...
		cmp_uuid(&g_cur_uuid, &g_sec_uuid[SEC_FACE3D_AE_AC_UUID_NUM]) != OK &&
...
static int sub_rgn_feature_cfg(struct sglist *sglist, enum SEC_FEATURE feature_id, DDR_CFG_TYPE ddr_cfg_type)
{
	...
	if (feature_num >= SUB_RGN_MAX_NUM) {
