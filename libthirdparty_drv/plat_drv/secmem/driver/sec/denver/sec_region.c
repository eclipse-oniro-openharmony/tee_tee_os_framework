...
TEE_UUID g_sec_uuid[] = { HIAI_UUID, TUI_UUID, SEC_ISP_UUID, SEC_FACE_UUID,
			SECBOOT_UUID, EID1_UUID, EID3_UUID, ION_UUID };
...
NORMAL_RGN_CFG g_normal_rgn_cfg_table[NORMAL_RGN_MAX_NUM] = {
	{
		...
		.sec_rgn = {
			...
			{	/* ASI_NUM_TYPE_3n4 */
				...
				.start_addr = 0x0,
				.end_addr = 0x0,
				.mid_wr = ISP_ALL | BIT(IPP_SUBSYS_CPE_MID),
				.mid_rd = ISP_ALL | BIT(IPP_SUBSYS_CPE_MID),
			},
			{	/* ASI_NUM_TYPE_5 */
				.rgn_en = 0x0,
				.attri = SEC_RD | SEC_WR,
				.start_addr = 0x0,
				.end_addr = 0x0,
				.mid_wr = BIT(CC712_MID),
				.mid_rd = BIT(CC712_MID),
			},
			{	/* ASI_NUM_TYPE_6n7 */
				.rgn_en = 0x0,
				.attri = SEC_RD | SEC_WR,
...
static int sec_region_cfg(unsigned int asi_num, unsigned int rgn_num, const SEC_RGN_CFG *sec_rgn_cfg)
{
	...
	sec_rgn_map0 = (volatile SOC_DMSS_ASI_SEC_RGN_MAP0_UNION *)((unsigned long)SOC_DMSS_ASI_SEC_RGN_MAP0_ADDR(SOC_ACPU_DMSS_BASE_ADDR, rgn_num, asi_num));
	sec_rgn_map1 = (volatile SOC_DMSS_ASI_SEC_RGN_MAP1_UNION *)((unsigned long)SOC_DMSS_ASI_SEC_RGN_MAP1_ADDR(SOC_ACPU_DMSS_BASE_ADDR, rgn_num, asi_num));
	sec_mid_wr = (volatile SOC_DMSS_ASI_SEC_MID_WR_UNION *)((unsigned long)SOC_DMSS_ASI_SEC_MID_WR_ADDR(SOC_ACPU_DMSS_BASE_ADDR, rgn_num, asi_num));
	sec_mid_rd = (volatile SOC_DMSS_ASI_SEC_MID_RD_UNION *)((unsigned long)SOC_DMSS_ASI_SEC_MID_RD_ADDR(SOC_ACPU_DMSS_BASE_ADDR, rgn_num, asi_num));
...
static inline void sec_region_clean(unsigned int asi_num, unsigned int rgn_num)
{
	...
	writel((~(1UL << SOC_DMSS_ASI_SEC_RGN_MAP0_rgn_en_START)) & value, SOC_DMSS_ASI_SEC_RGN_MAP0_ADDR(SOC_ACPU_DMSS_BASE_ADDR, rgn_num, asi_num));
...
static int normal_rgn_feature_cfg(struct sglist *sglist, enum SEC_FEATURE feature_id, DDR_CFG_TYPE ddr_cfg_type)
{
	...
	segment_end_addr = segment_start_addr + sglist->info->npages * PAGE_SIZE;
	region_en = 0x1;
...
int check_unsec_sglist(struct sglist *sglist)
{
	...
	for (segment_num = 0; segment_num < (sglist->infoLength); segment_num++) {
		segment_info = (TEE_PAGEINFO *)(sglist->info + segment_num);
		segment_start_addr = segment_info->phys_addr;
		segment_end_addr = segment_start_addr + segment_info->npages * PAGE_SIZE;
...
static int check_cma_addr(u64 start_addr, u64 end_addr)
{
	...
	if ((start_addr == HISI_CMA_START && end_addr >= HISI_CMA_END) ||
		...
		PRINT_ERROR("check_cma_addr error\n");
		return ERROR;
	}
}
...
#ifdef CONFIG_HISI_SEC_DDR_SUB_RGN
	ret = sub_region_init();
#endif
