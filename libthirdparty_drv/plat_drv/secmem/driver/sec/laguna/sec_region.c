...
static struct sec_rgn_cfg g_hifi_reset_sec_rgn = { 0 };
#endif
pthread_mutex_t g_sec_pthread_lock = PTHREAD_MUTEX_INITIALIZER;
TEE_UUID g_sec_uuid[] = {
	HIAI_UUID, TUI_UUID, SEC_ISP_UUID, SEC_FACE_UUID,
	SECBOOT_UUID, EID1_UUID, EID3_UUID, ION_UUID, VLTMM_UUID
};
TEE_UUID g_cur_uuid = UUID_INITIALIZER;
u32 g_uuid_get_flag = UUID_NOT_GET;

struct sub_rgn_cfg g_sub_rgn_cfg_table[SUB_RGN_FEATURE_MAX] = {
	{ /* DDR_SEC_TUI */
	...
	{ /* DDR_SEC_FACE */
		...
		.region_num = FACE_TA_ALGO_RGN_NUM,
		.start_addr = 0x0,
		.end_addr = 0x0,
		.granularity_size = 0x0,
	},
	{ /* DDR_SEC_FACE */
		.sec_feature = DDR_SEC_FACE,
		.region_num = FACE_CAMERA_RGN_NUM,
		.start_addr = 0x0,
		.end_addr = 0x0,
		.granularity_size = 0x0,
	},
	{ /* DDR_SEC_ID */
		...
		.region_num = EID_FACE_2D_RGN_NUM,
		.start_addr = 0x0,
		.end_addr = 0x0,
		.granularity_size = 0x0,
	},
...
#define mddrc_check_secandrgn(ret, asi_num, rgn_num) \
do { \
...
static int check_rgn_usable(unsigned int asi_num, unsigned int rgn_num)
{
	...
	mddrc_check_secandrgn(ret, asi_num, rgn_num);
...
/* one region */
static int sec_region_cfg(unsigned int asi_num, unsigned int rgn_num, const struct sec_rgn_cfg *sec_rgn_cfg)
{
	int ret;
	...
	sec_rgn_map0 =
		(volatile SOC_DMSS_ASI_SEC_RGN_MAP0_UNION *)(unsigned long)
		SOC_DMSS_ASI_SEC_RGN_MAP0_ADDR(SOC_ACPU_DMSS_BASE_ADDR, rgn_num, asi_num);
	sec_rgn_map1 =
		(volatile SOC_DMSS_ASI_SEC_RGN_MAP1_UNION *)(unsigned long)
		SOC_DMSS_ASI_SEC_RGN_MAP1_ADDR(SOC_ACPU_DMSS_BASE_ADDR, rgn_num, asi_num);
	sec_mid_wr =
		(volatile SOC_DMSS_ASI_SEC_MID_WR_UNION *)(unsigned long)
		SOC_DMSS_ASI_SEC_MID_WR_ADDR(SOC_ACPU_DMSS_BASE_ADDR, rgn_num, asi_num);
	sec_mid_rd =
		(volatile SOC_DMSS_ASI_SEC_MID_RD_UNION *)(unsigned long)
		SOC_DMSS_ASI_SEC_MID_RD_ADDR(SOC_ACPU_DMSS_BASE_ADDR, rgn_num, asi_num);
...
static int sec_region_cfg_mask(unsigned int asi_mask, unsigned int reserved_reg, const struct sec_rgn_cfg *sec_rgn_cfg)
{
...
static int is_not_covered_by_normal_region(u64 start_addr, u64 end_addr)
{
	...
	for (asi_num = 0; asi_num < ASI_NUM_MAX; asi_num++) {
		...
		for (rgn_num = 1 + SUB_RGN_USED; rgn_num < g_rgn_num[asi_num]; rgn_num++) {
			rgn_map0.value = readl((u32)SOC_DMSS_ASI_SEC_RGN_MAP0_ADDR(SOC_ACPU_DMSS_BASE_ADDR,
										   rgn_num, asi_num));
			if (rgn_map0.reg.rgn_en == 0)
				continue;
			rgn_map1.value = readl((u32)SOC_DMSS_ASI_SEC_RGN_MAP1_ADDR(SOC_ACPU_DMSS_BASE_ADDR,
										   rgn_num, asi_num));
...
static int ddr_sec_cfg_for_hifi_reset(u64 start_addr, u64 end_addr)
{
	...
	ret = sec_region_cfg_mask(MODEM_ASI_MASK | VDEC_IVP_ASI_MASK | ISP_DSS_ASI_MASK | NPU_ASI_MASK | GPU_ASI_MASK,
				  HIFI_REBOOT_RGN_NUM, &cfg_data);
	check_ret(ret);
...
static int sub_rgn_cfg_asi(struct sub_rgn_cfg *sub_pro_cfg,
			   ...
			   u32 sub_rgn_start_bit)
{
	...
	for (asi_num = 0; asi_num < ASI_NUM_MAX; asi_num++) {
		...
		case DDR_UNSET_SEC:
			...
			break;
		case DDR_CHECK_SEC:
			reg_value_check = reg_value & VALUE_SHIFT_WIDTH(BITS_WIDTH_MASK(sub_bit_length_reg),
											sub_rgn_start_bit);
			/* 0 is sec */
		...
		case DDR_CHECK_UNSEC:
			...
			reg_value_check = reg_value & VALUE_SHIFT_WIDTH(BITS_WIDTH_MASK(sub_bit_length_reg),
											sub_rgn_start_bit);
			if (reg_value_check != VALUE_SHIFT_WIDTH(BITS_WIDTH_MASK(sub_bit_length_reg),
										 sub_rgn_start_bit)) {
...
static int sub_rgn_cfg(struct sub_rgn_cfg *sub_pro_cfg, u32 sub_rgn_cfg_addr, u32 sub_rgn_cfg_size, DDR_CFG_TYPE ddr_cfg_type)
{
	int loop_flag;
	...
	/* when the granularity_size of feature is cfg as 2M, the feature checked is 1M, then sub_bit_length_total need add 1 */
	...
	do {
		...
		if (sub_rgn_cfg_asi(sub_pro_cfg, ddr_cfg_type, sub_rgn_num,
				    sub_bit_length_reg, sub_rgn_start_bit) != OK)
			return ERROR;
...
static int check_segment(TEE_PAGEINFO *segment_info)
{
	...
	for (feature_num = 0; feature_num < SUB_RGN_FEATURE_MAX; feature_num++) {
		...
		if (segment_start_addr >= feature_start_addr && segment_start_addr < feature_end_addr) {
			...
			ret = sub_rgn_cfg(&(g_sub_rgn_cfg_table[feature_num]), segment_cfg_addr,
					  segment_info->npages * PAGE_SIZE, DDR_CHECK_UNSEC);
			if (ret != OK)
				return ERROR;
		}
	}
...
static int check_sglist(struct sglist *sglist, u32 feature_num, DDR_CFG_TYPE ddr_cfg_type)
{
	...
	for (segment_num = 0; segment_num < (sglist->infoLength); segment_num++) {
		/* add segment_num to get segment_num addr */
...
static int face_feature_cfg(struct sglist *sglist, enum SEC_FEATURE feature_id, DDR_CFG_TYPE ddr_cfg_type)
{
	...
	for (segment_num = 0; segment_num < (sglist->infoLength); segment_num++) {
		...
		ret = sub_rgn_cfg(&g_sub_rgn_cfg_table[segment_feature_num], sub_rgn_cfg_addr,
				  segment_size, ddr_cfg_type);
		if (ret != OK) {
			PRINT_ERROR("sub_rgn_cfg fail\n");
...
static int sub_region_init(void)
{
	...
	for (feature_num = 0; feature_num < SUB_RGN_FEATURE_MAX; feature_num++) {
		rgn_map0.value =
			readl((u32)SOC_DMSS_ASI_SEC_RGN_MAP0_ADDR(SOC_ACPU_DMSS_BASE_ADDR,
								  g_sub_rgn_cfg_table[feature_num].region_num, CPU_ASI_NUM));
		rgn_map1.value =
			readl((u32)SOC_DMSS_ASI_SEC_RGN_MAP1_ADDR(SOC_ACPU_DMSS_BASE_ADDR,
								  g_sub_rgn_cfg_table[feature_num].region_num, CPU_ASI_NUM));
