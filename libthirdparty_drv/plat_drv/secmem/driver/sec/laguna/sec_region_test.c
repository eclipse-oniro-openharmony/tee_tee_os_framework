...
extern struct sub_rgn_cfg g_sub_rgn_cfg_table[SUB_RGN_FEATURE_MAX];
extern struct normal_rgn_cfg g_normal_rgn_cfg_table[NORMAL_RGN_MAX_NUM];
...
static void sglist_init(u32 feature_num, u32 sgment_num)
{
	...
	for (i = 0; i < sgment_num; i++) {
		(g_sglist->info[i]).phys_addr = g_sub_rgn_cfg_table[feature_num].start_addr +
						g_feature_npages[feature_num] * PAGE_SIZE * i;
...
static int sub_rgn_cfg_test(u32 feature_num, u32 segment_num)
{
	...
	reg3_val = readl(SOC_DMSS_ASI_SEC_SUB_RGN3_ADDR(SOC_ACPU_DMSS_BASE_ADDR,
							SUB_RGN_NUM(g_sub_rgn_cfg_table[feature_num].region_num),
							CPU_ASI_NUM));
	reg2_val = readl(SOC_DMSS_ASI_SEC_SUB_RGN2_ADDR(SOC_ACPU_DMSS_BASE_ADDR,
							SUB_RGN_NUM(g_sub_rgn_cfg_table[feature_num].region_num),
							CPU_ASI_NUM));
...
static int sub_rgn_test_case002(void)
{
	...
	for (feature_num = 0; feature_num < SUB_RGN_FEATURE_MAX; feature_num++) {
		...
		g_sglist->info[0].phys_addr = g_sub_rgn_cfg_table[feature_num].start_addr +
					      g_feature_npages[feature_num] * (SUB_BIT_MAX_NUM + 1);
		g_sglist->info[0].npages = g_feature_npages[feature_num] * (SUB_BIT_MAX_NUM + 1);
		if (ddr_sec_cfg(g_sglist, g_sub_rgn_cfg_table[feature_num].sec_feature, DDR_SET_SEC) != ERROR) {
			PRINT_ERROR("[%s] fail!\n", __func__);
			return ERROR;
...
static int normal_rgn_test_base(u32 feature_num, u32 segment_num, u32 start_addr, u32 npages)
{
	...
	reg_map0_val = readl(SOC_DMSS_ASI_SEC_RGN_MAP0_ADDR(SOC_ACPU_DMSS_BASE_ADDR,
							    g_normal_rgn_cfg_table[feature_num].region_num,
							    CPU_ASI_NUM));
	reg_map1_val = readl(SOC_DMSS_ASI_SEC_RGN_MAP1_ADDR(SOC_ACPU_DMSS_BASE_ADDR,
							    g_normal_rgn_cfg_table[feature_num].region_num,
							    CPU_ASI_NUM));
