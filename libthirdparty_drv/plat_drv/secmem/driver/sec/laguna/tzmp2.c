...
s32 ddrc_ca_rd_cfg(s32 ca_rd_enable)
{
	...
	for (i = 0; i < DDRC_NUM_MAX; i++) {
		for (j = 0; j < DMC_NUM_MAX; j++) {
			ddrc_cfg_ca =
				(volatile SOC_DDRC_DMC_DDRC_CFG_CA_UNION *)
				SOC_DDRC_DMC_DDRC_CFG_CA_ADDR(DDR_REG_DMC(i, j));
			ddrc_chg_caeps =
				(volatile SOC_DDRC_DMC_DDRC_CHG_CAEPS_UNION *)
				SOC_DDRC_DMC_DDRC_CHG_CAEPS_ADDR(DDR_REG_DMC(i, j));
...
static void cpu_addr_shift(u64 *phys)
{
	...
	u32 addr_shift_mode =
		(readl(SOC_DMSS_ASI_ADDR_SHIFT_ADDR(SOC_ACPU_DMSS_BASE_ADDR, CPU_ASI_NUM))) & ADDR_SHIFT_MODE_MASK;
