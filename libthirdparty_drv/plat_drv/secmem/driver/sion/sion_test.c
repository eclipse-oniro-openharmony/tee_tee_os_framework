...
static struct smmu_domain *__test_siommu_map(struct sglist *sglist, u32 size)
{
	int ret;
	struct smmu_domain *sdomain = NULL;

	sdomain = create_siommu_domain();
	if (!sdomain) {
...
static int test_siommu_map(struct mem_chunk_list *mcl)
{
	...
	if (!pg_sg) {
		...
		ddr_sec_cfg(pg_sg, DDR_SEC_FACE, DDR_UNSET_SEC);
		ret = -EINVAL;
		goto destory_sglist;
	}

	sdomain = __test_siommu_map(sglist, size);
	if (!sdomain) {
		tloge("%s: create sdomain fail!\n", __func__);
		ret = -EINVAL;
		goto free_dom;
...
free_dom:
	ddr_sec_cfg(sglist, DDR_SEC_FACE, DDR_UNSET_SEC);
	ddr_sec_cfg(pg_sg, DDR_SEC_PGTABLE, DDR_UNSET_SEC);
...
static int test_sion_mmap(struct mem_chunk_list *mcl)
{
	...
	sglist_size = sizeof(*array) * mcl->nents + sizeof(struct sglist);
	...
	ddr_sec_cfg(sglist, DDR_SEC_FACE, DDR_SET_SEC);
