 ...
 *        The certificate must be loaded to the RAM prior to this function
 *
 * @param[in] certPtr - the certificate address (after it has been loaded to memory)
 * @param[in] address - the new storage address to change to
 * @param[in] indexOfAddress - the index of the sw image in the content certificate, starting from 0
 *
 * @return DxError_t - On success the value DX_OK is returned,
 *         on failure - a value from BootImagesVerifier_error.h
 */
DxError_t DX_SB_SwImageStoreAddrChange(uint32_t *certPtr, uint64_t address, uint32_t indexOfAddress);
 ...
 *         on failure - a value from bootimagesverifier_error.h
 */
DxError_t DX_SB_GetCertSize(uint32_t *pCert, uint32_t *pCertSizeWords);
