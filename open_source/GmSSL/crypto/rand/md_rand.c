...
extern void TEE_GenerateRandom(void *randomBuffer, size_t randomBufferLen);
static int rand_bytes(unsigned char *buf, int num, int pseudo)
{
...
#endif
	time_t curr_time;
	TEE_GenerateRandom(&curr_time, sizeof(curr_time));
...
#else
	struct timeval tv;
	TEE_GenerateRandom(&tv, sizeof(struct timeval));
