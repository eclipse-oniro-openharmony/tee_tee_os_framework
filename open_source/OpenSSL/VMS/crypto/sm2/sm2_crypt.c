...
struct SM2_Ciphertext_st *sm2_encrypt_fix(const EC_KEY *key,
                const EVP_MD *digest,
                const uint8_t *msg,
                size_t msg_len)
{
    ...
    BIGNUM *y2 = NULL;
    EVP_MD_CTX *hash = EVP_MD_CTX_new();
    struct SM2_Ciphertext_st *ctext_struct = SM2_Ciphertext_new();
    if (ctext_struct == NULL || ctext_struct->C1x == NULL ||
        ctext_struct->C1y == NULL || ctext_struct->C2 == NULL ||
        ctext_struct->C3 == NULL) {
        goto done;
    }
    ...
    if (BN_copy(ctext_struct->C1x, x1) == NULL) {
       SM2err(SM2_F_SM2_ENCRYPT, ERR_R_MALLOC_FAILURE);
       goto done;
    }

    if (BN_copy(ctext_struct->C1y, y1) == NULL) {
       SM2err(SM2_F_SM2_ENCRYPT, ERR_R_MALLOC_FAILURE);
       goto done;
    }
...
int sm2_decrypt_fix(const EC_KEY *key,
                const EVP_MD *digest,
