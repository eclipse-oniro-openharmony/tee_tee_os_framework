...
#ifdef OPENSSL_RAND_SEED_TEE
size_t rand_acquire_entropy_from_tee(RAND_POOL *pool)
{
    size_t bytes_needed;
    unsigned char *buffer;
    int ret;
    ...
    if (bytes_needed > 0) {
        buffer = rand_pool_add_begin(pool, bytes_needed);
        if (buffer != NULL) {
            ret = crypto_driver_get_entropy(buffer, bytes_needed);
            if (ret == 0)
                rand_pool_add_end(pool, bytes_needed, 8 * bytes_needed);
        }
    }
