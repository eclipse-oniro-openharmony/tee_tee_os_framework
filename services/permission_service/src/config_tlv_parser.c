 ...
 * Description: permission tlv parser
...
static dlist_head(g_task_list);
static pthread_mutex_t g_task_list_lock = PTHREAD_MUTEX_INITIALIZER;
...
static int32_t parser_callee_ta_uuid(const uint8_t *buff, uint32_t len, TEE_UUID *uuid)
{
    ...
    ret = convert_str_to_uuid((char *)buff + sizeof(value_len), value_len, uuid);
    return ret;
}
...
static int32_t parser_callee_ta_command_id(const uint8_t *buff, uint32_t len, struct callee_ta_info *callee)
{
    ...
    if (is_invalid) {
        tloge("invalid command 0x%x", len);
        return PERMSRV_ERROR;
    }
...
static int32_t parser_fill_callee(const uint8_t *buff, uint32_t len, uint32_t value_len, struct config_info *config)
{
    ...
    is_invalid = (ret != PERMSRV_OK) || (is_duplicate_callee(config, callee_info) != 0);
    if (is_invalid) {
    ...
    uint32_t *offset, int32_t *tag, uint16_t *value_len)
{
    ...
    *offset = TLV_VALUE_OFFSET + (uint32_t)inner_tlv_len;
    ...
    if (tlv_value_len <= inner_tlv_len) {
        tloge("tlv value len 0x%x", tlv_value_len);
        return PERMSRV_ERROR;
    }
...
static int32_t get_tag_uuid(const uint8_t *buff, uint32_t len, struct config_info *config)
{
    ...
    ret = convert_str_to_uuid((char *)buff, len, &(config->uuid));
    if (ret != PERMSRV_OK) {
        tloge("invalid uuid");
        return PERMSRV_ERROR;
    }
...
static void config_node_info(const uint8_t *buff, uint32_t len, int32_t tag, struct config_info *config)
{
    switch (tag) {
    ...
    case TLV_TAG_SYS_VERIFY_TA:
        config->manifest_info.sys_verify_ta = (bool)(*buff);
        break;
#endif
    default:
        break;
...
static int32_t config_tlv_parser_manifest_info(const uint8_t *buff, uint32_t len, struct config_info *config)
{
    ...
    while (offset < len) {
        ...
        config_node_info(buff + offset + child_offset, value_len, tag, config);
...
static TEE_Result check_cn_validation(const uint8_t *cn, size_t cn_size, const struct config_info *info)
{
    ...
    if (convert_uuid_to_str(&(info->uuid), (char *)buff, sizeof(buff)) != TEE_SUCCESS)
        return TEE_ERROR_GENERIC;
...
static void remove_duplicate_config(const TEE_UUID *uuid)
{
    ...
    dlist_for_each_safe(pos, tmp, &g_config_list) {
        ...
        if (TEE_MemCompare(&entry->uuid, uuid, sizeof(*uuid)) == 0) {
            dlist_delete(&entry->head);
            release_callee_info(entry->control_info.callee_info);
            TEE_Free(entry);
            entry = NULL;
            break;
        }
...
static uint32_t check_contain_dynconf(const struct perm_config *perm_config)
{
    ...
    tlogd("no dynconf tlv data\n");

    return 0;
}

static uint32_t parse_dyntlv_buf(const TEE_UUID *uuid, const struct perm_config *perm_config,
                                 const struct config_info *config, uint32_t dynconf_len)
{
    if (dynconf_len != 0) {
        if (TEE_MemCompare(uuid, &(config->uuid), sizeof(*uuid)) == 0) {
            dynconf_len += 1;
            int32_t res = tee_secure_img_parse_manifest_v3(perm_config->tlv_buf + perm_config->tlv_len,
...
static int32_t parser_python_tlv_to_ta_config(struct perm_config *perm_config, struct config_info *config)
{
    struct dyn_conf_t dyn_conf;
    dyn_conf.dyn_conf_buffer = (char *)perm_config->tlv_buf;
    dyn_conf.dyn_conf_size = perm_config->tlv_len;
    return register_conf(&dyn_conf, install_ta_config, config, sizeof(*config));
}

static int32_t parse_ta_tlv_config(struct perm_config *perm_config, struct config_info *config)
{
    int32_t ret;
    if ((perm_config->policy_version & XML2TLV_PARSE_BIT_MAP) == XML2TLV_JAR_VALUE)
        ...
        ret = parser_python_tlv_to_ta_config(perm_config, config);
    else
        ret = TEE_ERROR_BAD_PARAMETERS;
    if (ret != PERMSRV_OK)
        tloge("parse failed for tlv type:%d, 0-jar, 1-python", perm_config->policy_version & XML2TLV_PARSE_BIT_MAP);
    return ret;
}

TEE_Result parse_conf_body(const TEE_UUID *uuid, struct perm_config *perm_config)
{
    bool param_invalid = (uuid == NULL || perm_config == NULL || perm_config->tlv_buf == NULL ||
        perm_config->tlv_len == 0 || perm_config->cn_size == 0 || perm_config->policy_version == 0 ||
    ...
    if (config == NULL) {
        tloge("fail to malloc mem for config\n");
        return TEE_ERROR_OUT_OF_MEMORY;
    }

    int32_t res = parse_ta_tlv_config(perm_config, config);
    if (res != PERMSRV_OK) {
        tloge("parse config tlv content fail\n");
        release_callee_info(config->control_info.callee_info);
        TEE_Free(config);
        return TEE_ERROR_GENERIC;
    }

    if (check_cn_validation(perm_config->cn, perm_config->cn_size, config) != TEE_SUCCESS) {
        release_callee_info(config->control_info.callee_info);
        TEE_Free(config);
        return TEE_ERROR_GENERIC;
    }
    ...
    if ((config->manifest_info.target_type == TA_TARGET_TYPE ||
        config->manifest_info.target_type == SRV_TARGET_TYPE) && add_timer_perm_by_config(config) != TEE_SUCCESS) {
        tloge("add timer permission failed\n");
        release_callee_info(config->control_info.callee_info);
        TEE_Free(config);
        return TEE_ERROR_GENERIC;
    }
    ...
    /*
     * remove previous one with same uuid if it exists
     * then insert the new one to list
     */
    remove_duplicate_config(&(config->uuid));

    res = parse_dyntlv_buf(uuid, perm_config, config, dynconf_len);
    if (res != TEE_SUCCESS) {
        TEE_Free(config);
        return TEE_ERROR_GENERIC;
    }

    dlist_insert_tail(&config->head, &g_config_list);
    return TEE_SUCCESS;
}
...
TEE_Result get_config_by_taskid(uint32_t taskid, struct config_info *config)
{
    ...
    if (pthread_mutex_lock(&g_task_list_lock) != 0) {
        tloge("Failed to get task list lock\n");
        return TEE_ERROR_BAD_STATE;
    }

    dlist_for_each(pos, &g_task_list) {
        entry = dlist_entry(pos, struct task_config, list);
        if (taskid != entry->taskid)
            continue;
        if (entry->con_info != NULL) {
            if (memcpy_s(config, sizeof(*config), entry->con_info, sizeof(*(entry->con_info))) != EOK) {
                tloge("memcpy operation failed for config info\n");
    ...
    (void)pthread_mutex_unlock(&g_task_list_lock);
    return ret;
}
...
static TEE_Result get_register_ta_entry(struct config_info **entry, TEE_UUID tmp_uuid, bool *new_config_entry)
{
...
static TEE_Result ta_add_list(struct task_config *tmp_task, struct config_info *entry, bool new_config_entry)
{
    if (pthread_mutex_lock(&g_task_list_lock) != 0) {
        tloge("Failed to get task list lock\n");
        return TEE_ERROR_BAD_STATE;
    ...
    dlist_insert_tail(&tmp_task->list, &g_task_list);
    (void)pthread_mutex_unlock(&g_task_list_lock);

    if (new_config_entry)
        dlist_insert_tail(&entry->head, &g_config_list);
...
{
    TEE_UUID tmp_uuid = uuid;
    struct config_info *entry = NULL;
    struct task_config *tmp_task = NULL;
    bool new_config_entry = false;
    TEE_Result ret;

    ret = get_register_ta_entry(&entry, tmp_uuid, &new_config_entry);
    if (ret != TEE_SUCCESS)
        return ret;
    ...
    tmp_task->taskid = task_pid;
    tmp_task->userid = userid;
    tmp_task->con_info = entry;

    ret = ta_add_list(tmp_task, entry, new_config_entry);
    if (ret != TEE_SUCCESS)
        goto clean_entry;
...
TEE_Result unregister_ta_pid(uint32_t task_pid)
{
    ...
    dlist_for_each_prev(pos, &g_task_list) {
        entry = dlist_entry(pos, struct task_config, list);
        if (entry->taskid == task_pid) {
            dlist_delete(&entry->list);
            entry->con_info = NULL;
            TEE_Free(entry);
            entry = NULL;
            (void)pthread_mutex_unlock(&g_task_list_lock);
...
static void unset_ta_config(const TEE_UUID *uuid)
{
    struct dlist_node *pos = NULL;
    struct dlist_node *tmp = NULL;
...
void clear_ta_permissions(const TEE_UUID *uuid)
{
    ...
    if (pthread_mutex_lock(&g_task_list_lock) != 0) {
        tlogw("Failed to get task list lock\n");
        return;
    }

    dlist_for_each_safe(pos, tmp, &g_task_list) {
        task_entry = dlist_entry(pos, struct task_config, list);
        if (TEE_MemCompare(&task_entry->con_info->uuid, &tmp_uuid, sizeof(tmp_uuid)) == 0) {
            dlist_delete(&task_entry->list);
            task_entry->con_info = NULL;
            TEE_Free(task_entry);
            task_entry = NULL;
        }
    }
    ...
    unset_ta_config(uuid);
}

static bool is_right_cmd(const TEE_UUID *callee_uuid, uint32_t cmd, const struct callee_ta_info *callee_info)
{
    int32_t rc;
    uint32_t i;

    rc = TEE_MemCompare(callee_uuid, &callee_info->uuid, sizeof(callee_info->uuid));
    if (rc == 0) {
        for (i = 0; i < callee_info->command_num; i++) {
            if (cmd == *(callee_info->command_id + i))
                return true;
        }
    }
    return false;
...
TEE_Result query_ta2ta_perm_by_uuid(const TEE_UUID *caller_uuid, const TEE_UUID *callee_uuid, uint32_t cmd)
{
    ...
    bool check = (caller_uuid == NULL || callee_uuid == NULL);
    if (check)
        return TEE_ERROR_BAD_PARAMETERS;

    ret = get_config_by_uuid(caller_uuid, &config);
    if (ret != TEE_SUCCESS) {
        tloge("get config info by uuid failed\n");
        return TEE_ERROR_GENERIC;
    }

    struct callee_ta_info *callee_info = config.control_info.callee_info;

    while (callee_info != NULL) {
        if (is_right_cmd(callee_uuid, cmd, callee_info))
            return TEE_SUCCESS;

        callee_info = callee_info->next;
    }
