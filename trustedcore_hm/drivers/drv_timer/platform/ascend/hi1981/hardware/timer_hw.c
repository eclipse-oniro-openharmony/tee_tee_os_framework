...
#define TIMER_200M_FRQ       0x01000000
...
static void timer_clk_enable(void)
{
    const uint32_t val = 0x1;

    write32(SC_OS_TIMER0_RESET_DREQ, val);
    write32(SC_OS_TIMER1_RESET_DREQ, val);
}

static void timer_freq_init(void)
{
    timer_interrupt_choose();
    uint32_t ctrl = read32((void *)SC_SEC_TIMER_CLK_SEL);
    ctrl |= TIMER_200M_FRQ;
...
uint64_t timer_get_value(uint32_t timer_base, uint32_t tim_mode)
{
    uint32_t time_low;
    uint32_t time_high;
    (void)timer_base;
    ...
    if (tim_mode == FREE_RUNNING_TIMER_NUM) {
...
void timer_set_value(uint32_t timer_base, uint32_t tim_mode, uint32_t mode, uint64_t usecs)
{
    void *ptr_load = NULL;
...
void timer_enable(uint32_t timer_base, uint32_t tim_mode)
{
    uint32_t ctrl;
    void *ptr_ctrl = NULL;
    (void)timer_base;
...
void timer_disable(uint32_t timer_base, uint32_t tim_mode)
{
    uint32_t ctrl;
    void *ptr_ctrl = NULL;
    (void)timer_base;
...
uint32_t secure_timer_mis_read(uint32_t timer_base, uint32_t tim_mode)
{
    uint32_t ctrl;
    void *ptr_ctrl = NULL;
    (void)timer_base;
...
{
    hm_debug("1981 platform do nothing\n");
}
...
void secure_timer_irq_clear(uint32_t timer_base, uint32_t tim_mode)
{
    void *ptr_ctrl = NULL;
    (void)timer_base;
...
void resume_freerunning_timer(uint32_t flag)
{
    ...
    timer_set_value(FREE_RUNNING_TIMER_BASE, FREE_RUNNING_TIMER_NUM, MODE_FREE_RUNNING, g_free_time_val);
    timer_enable(FREE_RUNNING_TIMER_BASE, FREE_RUNNING_TIMER_NUM);
}
