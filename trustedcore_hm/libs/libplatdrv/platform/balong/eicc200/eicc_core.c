...
{
    if (pchannel->const_flags & EICC_CHN_SF_REMOTE_UNSEC) {
        eicc_ipipe_unsec(pdev, EICC_GET_PIPE_ID(pchannel->ldrvchn_id));
        eicc_core_unsec(pdev, EICC_GET_RCORE_ID(pchannel->ldrvchn_id));
    }
...
/* 按照pipe强停通道，用于复位时 啥都不管了的强停，存在当前pipe被其他核正使用的情况 */
/* 复位时 该函数可能处于fiq上下文，所以无锁 */
int eicc_opipe_rst_halt(struct eicc_device *pdev, u32 opipe_id)
{
    ...
    if (!eicc_opipe_is_en(pdev->base_va, opipe_id)) {
        return 0;
    }
    eicc_opipe_abort_set(pdev->base_va, opipe_id);
    for (query_times = 0; query_times < EICC_HALT_QUERY_WAIT_TIMES; query_times++) {
        if (EICC_INT_WR_ABORT & eicc_opipe_devint_raw(pdev->base_va, opipe_id)) {
            break;
        }
    }
    ...
    eicc_opipe_devint_clr(pdev->base_va, opipe_id, EICC_INT_WR_ABORT);
    eicc_opipe_local_dis(pdev->base_va, opipe_id);
    /* 只要abort上报了，就不会搬数了，所以 复位场景下，处理到这步就足够了，再多处理会增加单独复位时间 */
    return 0;
}
...
static void ipipe_pkt_ts_set(struct eicc_channel *pchannel)
{
    ...
    /*lint -e413*/
    ts_off = stru_member_offset(struct eicc_packet, recvtime1);
    /*lint +e413*/
    ...
    *ts = pchannel->irq_ts;
    /*lint -e413*/
    ts_off = stru_member_offset(struct eicc_packet, recvtime2);
    /*lint +e413*/
    if (r_off + ts_off >= bufsz) {
        ts = (u32 *)(ringbuf + r_off + ts_off - bufsz);
    } else {
...
static int eicc_chn_attr_check(struct eicc_channel *pchannel, eicc_chn_attr_t *pattr,
    struct compat_attr *pcompat_attr)
{
...
int bsp_eicc_chn_open(eicc_chn_t *pchn_hdl, eicc_chn_attr_t *pattr)
{
    ...
    osl_spin_lock_irqsave(&pchannel->lock, flags);
    if (pchannel->state != (u32)EICC_CHN_INIT) {
        osl_spin_unlock_irqrestore(&pchannel->lock, flags);
        ...
        if (ret == 0) {
            ...
            if ((phelper->recv_flags & EICC_RECV_FLAGS_DATAPEEK) == 0) {
                ...
                eicc_print_trace("eicc_ipipe_rptr_update rptr: 0x%x , wptr: 0x%x\n",
                                 (unsigned)pchannel->ipipe_cfg->rptr,
                                 (unsigned)*(volatile u32 *)(pchannel->ipipe_cfg->ptr_vaddr));
            }
        } else {
            eicc_print_error("ipipe_get_from_ringbuf failed @%d\n", __LINE__);
        }
...
int bsp_eicc_chn_ioctl(eicc_chn_t chn_hdl, unsigned req, void *arg, u32 size)
{
    ...
    if (req == EICC_IOCTL_CHN_NXTPKT_INF) {
        ...
        osl_spin_lock_irqsave(&pchannel->lock, lockflags);
        ...
        osl_spin_unlock_irqrestore(&pchannel->lock, lockflags);
        return 0;
