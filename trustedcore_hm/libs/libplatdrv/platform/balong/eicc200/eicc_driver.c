...
irqreturn_t eicc_localsend_handler(void *arg)
{
    u32 int_pipe_status;
    u32 pipe_id;
...
irqreturn_t eicc_localrecv_handler(void *arg)
{
    u32 int_pipe_status;
    u32 pipe_id;
    struct irq_bundle *bundle = (struct irq_bundle *)arg;
...
/* pdev should be a remote eicc device( and must poweon) */
irqreturn_t eicc_remote_handler(void *arg)
{
    u32 int_pipe_status;
    u32 pipe_id;
    struct irq_bundle *bundle = (struct irq_bundle *)arg;
...
int eicc_irqs_local_init(struct eicc_device *pdev)
{
    ...
    for (idx = 0; idx < sizeof(pdev->maps) / sizeof(pdev->maps[0]); idx++) {
        ...
        bundle->pdev = pdev;
        uret = 0;
        uret |= (u32)eicc_request_irq(bundle->irq[0x0], eicc_localsend_handler, "eicc_irq_lsend", (void *)bundle);
        uret |= (u32)eicc_request_irq(bundle->irq[0x1], eicc_localrecv_handler, "eicc_irq_lrecv", (void *)bundle);
        if (uret == 0) {
            uret |= (u32)eicc_enable_irq(bundle->irq[0x0]);
            uret |= (u32)eicc_enable_irq(bundle->irq[0x1]);
        }
        if (uret) {
            ret = -1;
...
int eicc_irqs_remote_init(struct eicc_device *pdev)
{
    ...
    for (idx = 0; idx < sizeof(pdev->maps) / sizeof(pdev->maps[0]); idx++) {
        ...
        bundle->pdev = pdev;
        uret = 0;
        uret |= (u32)eicc_request_irq(bundle->irq[0x0], eicc_remote_handler, "eicc_irq_remote", (void *)bundle);
        if (uret == 0) {
            uret |= (u32)eicc_enable_irq(bundle->irq[0x0]);
        }
        if (uret) {
