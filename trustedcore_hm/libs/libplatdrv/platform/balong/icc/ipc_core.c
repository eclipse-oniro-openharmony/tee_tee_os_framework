...
{
    if (ulLvl >= IPC_INT_BUTTOM) {
        tloge("[%s]Wrong param , line:%d,param = %d\n", __FUNCTION__, __LINE__, ulLvl);
        return BSP_ERROR;
    }
    ...
    for (i = 0; i < INTSRC_NUM; i++) {
        ...
        if (u32BitValue != 0) {
            ...
            if (g_ipc_ctrl.ipc_int_table[i].routine != NULL) {
                ...
                tloge("BSP_DRV_IpcIntHandler:No IntConnect,BSP_ERROR!.int num =%d\n", i);
            }
...
int bsp_ipc_init(void)
{
    ...
    if (ret) {
        tloge("ipc int handler error,init failed\n");
        return -1;
    }
...
int bsp_ipc_resume(void)
{
    ...
    if (ret) {
        tloge("SRE_HwiResume modem IPC irq %u failed, error %d\n", irq, ret);
        return -1;
    }

    ret = SRE_HwiEnable(irq);
    if (ret) {
        tloge("SRE_HwiEnable modem IPC irq %u failed, error %d\n", irq, ret);
        return -1;
    }
...
/*lint -e528 -esym(528,*)*/
DECLARE_TC_DRV(ipc_driver, 0, 0, 0, TC_DRV_MODULE_INIT, bsp_ipc_init, NULL, NULL, NULL, bsp_ipc_resume); /*lint -e528 +esym(528,*)*/
