...
/* CPU ID 分配 */
enum CPU_ID { ICC_CPU_MIN = 0, ICC_CPU_APP = 0, ICC_CPU_MODEM = 1, ICC_CPU_NRCCPU = 7, ICC_CPU_MAX };
...
enum ICC_S_CHN_ID {
    ...
    ICC_CHN_SEC_IFC_NR = 32,         /* 共享IPC无任务直接回调处理函数，安全OS与nrcore共享物理通道 */
    ICC_CHN_SEC_VSIM_NR = 33,  /* 私有IPC无任务直接回调处理函数，天际通安全OS与ccore物理通道 */
    ICC_CHN_ID_MAX
};
...
*             4) malloc()
*****************************************************************************/
int bsp_icc_event_register(unsigned int channel_id, read_cb_func read_cb, void *read_context, write_cb_func write_cb,
                           void *write_context);
...
#define ICC_SEC_IFC_NR_SIZE (4 * 1024)
#define ICC_SEC_VSIM_NR_SIZE (16 * 1024)
