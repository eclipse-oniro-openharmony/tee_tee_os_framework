 ...
 * Description: Api for crypto syscall pbkdf2 api.
 * Create: 2020-06-26
 */
...
static void pbkdf2_map_init_param0(struct call_params *map_param)
{
    ...
    if (map_param[0].args[ARG4_INDEX] != 0) {
        map_param[0].mmaped_ptrs[MMAP_PTR2_INDEX].addr.addr_64 = map_param[0].args[ARG4_INDEX];
...
static void pbkdf2_map_init_param1(struct call_params *map_param, const uint64_t *tmp_addr)
{
...
static void pbkdf2_build_param1(const struct call_params *map_param, uint64_t *tmp_addr)
{
    if (map_param[0].mmaped_ptrs[MMAP_PTR0_INDEX].addr.addr_64)
    ...
    uint64_t *tmp_addr, uint32_t tmp_addr_count)
{
    ...
    pbkdf2_map_init_param0(map_param);
    ...
    pbkdf2_build_param1(map_param, tmp_addr);

    pbkdf2_map_init_param1(map_param, tmp_addr);
