 ...
 * Description: TUI secure display general data struct and definition
...
#define DUMP_REG_ALWAYS false
#define DUMP_REG_ONCE true

#define DUMP_REG_NOSHADOW false
#define DUMP_REG_SHADOW true
...
extern unsigned int g_hisi_fb_msg_level;

/* ========== log print ============= */
#define HISI_FB_ERR(msg, ...) \
	do { \
		if (g_hisi_fb_msg_level >= 1) \
			uart_printf_func("[hisifb]%s: "msg, __func__, ## __VA_ARGS__); \
	} while (0)
...
#define HISI_FB_PRINTF(msg, ...) \
	do { \
		if (g_hisi_fb_msg_level >= 1) \
			uart_printf_func(""msg, ## __VA_ARGS__); \
	} while (0)

/* ========== parameter check ============= */
#define HISI_ERR_CHECK_RETURN(condition, ret, msg, ...) \
	do { \
		if (condition) { \
...
#define HISI_ERR_CHECK_NO_RETVAL(condition, msg, ...) \
	do { \
		if (condition) { \
			HISI_FB_ERR(msg, ## __VA_ARGS__); \
			return; \
...
#define HISI_CHECK_AND_CALL_FUNC(fun_ptr, ...) \
	do { \
		if ((fun_ptr) != NULL) \
			fun_ptr(__VA_ARGS__); \
	} while (0)
