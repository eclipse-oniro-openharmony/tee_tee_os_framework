...
static hi_s32 cenc_config(hi_handle handle, hi_cipher_data_dir data_dir, hi_cipher_work_mode *word_mode)
{
    hi_s32 ret;
    ...
    chn = HI_HANDLE_GET_CHNID(handle);

    drv_symc_set_data_dir(chn, data_dir);
 ...
 */
hi_s32 cenc_decrypt_sample(hi_handle handle,
                           hi_cipher_cenc *cenc,
                           compat_addr inputaddr,
                           compat_addr outputaddr)
{
    ...
    hi_cipher_work_mode word_mode = HI_CIPHER_WORK_MODE_MAX;
    hi_u32 i;

    hi_log_func_enter();

    chn_num = HI_HANDLE_GET_CHNID(handle);

    ret = cenc_config(handle,  cenc->data_dir, &word_mode);
    if (ret != HI_SUCCESS) {
