...
static irqreturn_t hash_interrupt_isr(hi_u32 arg)
{
    hi_u32 mask;
    hi_u32 i;
    ...
    hi_log_debug("hash_interrupt_isr devId 0x%x\n", arg);
...
static hi_s32 drv_hash_register_interrupt(void)
{
    ...
    drv_symc_register_hash_interrupt(hash_interrupt_isr);
...
static void drv_hash_unregister_interrupt(void)
{
    ...
    drv_symc_unregister_hash_interrupt();

    for (i = 0; i < CRYPTO_HARD_CHANNEL_MAX; i++) {
        ctx = g_hash_hard_channel[i].ctx;
        crypto_queue_destroy(ctx->queue);
    }
...
static void hash_set_entry(hi_u32 chn, compat_addr dma_addr, void *cpu_addr)
{
    ...
    hash_chn_ctrl.bits.hash_chn_ss = 0x01; /* use sec table */
...
static void drv_hash_smmu_base_addr(void)
{
    ...
    symc_write (SPACC_MMU_SEC_TLB, (hi_u32)(table_addr >> 4)); /* bit[35:4]: mmu base address, bit[3:0] fixed 0 */
    symc_write (SPACC_MMU_IN_HASH_SEC_EADDR_H, (hi_u32)(err_raddr >> 32)); /* high 32 bit of addr */
    symc_write (SPACC_MMU_IN_HASH_SEC_EADDR_L, (hi_u32)(err_raddr));       /* low 32 bit of addr */
    hi_log_debug("SPACC_MMU_SEC_TLB[0x%x]  : 0x%x\n", SPACC_MMU_SEC_TLB, (hi_u32)table_addr);
...
static hi_s32 drv_hash_check_mmu(hi_void)
{
    ...
    if (va_tab.bits.mmu_in_hash_sec_unvld_va_vld == 1) {
        hi_log_error("mmu_in_hash_sec_unvld_va_vld:\n");
        hi_log_error("mmu_in_hash_sec_unvld_chn_id %d !!!\n", va_tab.bits.mmu_in_hash_sec_unvld_chn_id);
        hi_log_error("mmu_in_hash_sec_unvld_va_tab 0x%x !!!\n",
            va_tab.bits.mmu_in_hash_sec_unvld_va_tab << SMMU_PAGE_BIT);
        hi_log_error("SPACC_MMU_SEC_TLB[0x%x] 0x%x\n", SPACC_MMU_SEC_TLB, symc_read(SPACC_MMU_SEC_TLB));
        ret = HI_ERR_CIPHER_INVALID_ADDR;
    }
    if (pa_tab.bits.mmu_in_hash_sec_unvld_pa_vld == 1) {
        hi_log_error("mmu_in_hash_sec_unvld_pa_vld:\n");
        hi_log_error("mmu_in_hash_sec_unvld_vld_err %d !!!\n", pa_tab.bits.mmu_in_hash_sec_unvld_vld_err);
        hi_log_error("mmu_in_hash_sec_unvld_pa_tab 0x%x !!!\n",
...
hi_s32 drv_hash_init(void)
{
    ...
    ret = crypto_mem_create(&g_hash_dma, SEC_MMZ, "CIPHER_SHA_NODE", HASH_NODE_LIST_SIZE);
    if (ret != HI_SUCCESS) {
    ...
    hi_log_debug("HASH DMA buffer, MMU 0x%x, MMZ 0x%x, VIA %p, size 0x%x\n",
                 ADDR_L32(g_hash_dma.dma_addr), ADDR_L32(g_hash_dma.mmz_addr),
                 g_hash_dma.dma_virt, g_hash_dma.dma_size);

    hi_log_debug("hash secure channel configure\n");
    ret = drv_hash_enable_secure();
    if (ret != HI_SUCCESS) {
        hi_log_print_func_err(drv_hash_enable_secure, ret);
        crypto_mem_destory(&g_hash_dma);
        goto __error1;
    }
...
void drv_hash_resume(void)
{
    ...
    hi_log_debug("hash secure channel configure\n");
    if (drv_hash_enable_secure() != HI_SUCCESS) {
        hi_log_func_exit();
        return;
    }
...
static hi_s32 drv_hash_wait_ready(hi_u32 chn_num)
{
    ...
    if (int_valid) {
        ...
        /* wait interrupt */
        ret = crypto_queue_wait_timeout(ctx->queue, HI_FALSE != ctx->done, CRYPTO_TIME_OUT);
        if (ret != HI_SUCCESS) {
...
hi_s32 drv_hash_config(hi_u32 chn_num, hash_mode mode, hi_u32 state[HASH_RESULT_MAX_SIZE_IN_WORD])
{
    ...
    crypto_assert(g_hash_initialize == HI_TRUE);
