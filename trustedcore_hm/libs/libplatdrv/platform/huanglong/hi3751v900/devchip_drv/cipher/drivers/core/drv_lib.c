...
static hi_u32 g_secure_cpu = HI_FALSE;
#endif
...
hi_s32 module_addr_map(void)
{
...
#ifdef CRYPTO_SWITCH_CPU
    g_secure_cpu = drv_symc_is_secure();
    if (g_secure_cpu == HI_TRUE) {
    ...
    /* use non-secure info */
    ret = memcpy_s(&g_hard_info_table, sizeof(g_hard_info_table),
