...
const char *g_err_string[PKE_FAIL_FLAG_CNT] = {
...
static hi_u32 g_pke_initialize = HI_FALSE;
...
static irqreturn_t pke_interrupt_isr(hi_u32 arg)
{
    hi_u32 mask;

    hi_log_debug("pke_interrupt_isr devId 0x%x\n", arg);

    mask = drv_pke_done_notify();
    if (mask != 0x00) {
        g_pke_info.done = HI_TRUE;
        hi_log_debug("pke wake up\n");
        crypto_queue_wake_up(g_pke_info.queue);
    }
...
static hi_s32 drv_pke_register_interrupt(void)
{
    ...
    /* request irq */
    hi_log_debug("pke request irq, num %d, name %s\n", int_num, name);
...
hi_s32 drv_pke_init(void)
{
    ...
    g_pke_initialize = HI_TRUE;
    g_pke_info.done = HI_TRUE;
    g_pke_info.attacked = HI_FALSE;
...
static hi_s32 drv_pke_wait_done(void)
{
    ...
    if (int_valid) {
        /* wait interrupt */
        ret = crypto_queue_wait_timeout(g_pke_info.queue, g_pke_info.done != HI_FALSE, PKE_TIME_OUT);
        if (ret != 0x00) {
...
hi_s32 drv_pke_add_mod(const hi_u8 *a, const hi_u8 *b, const hi_u8 *p, hi_u8 *c, hi_u32 klen)
{
    ...
    hi_log_check_inited(g_pke_initialize);
    hi_log_check_param(g_pke_info.attacked);
    crypto_assert(a != HI_NULL);
    crypto_assert(b != HI_NULL);
    crypto_assert(p != HI_NULL);
    crypto_assert(c != HI_NULL);

    g_pke_info.done = HI_FALSE;
...
hi_s32 drv_ifep_rsa_exp_mod(hi_u8 *n, hi_u8 *k, hi_u8 *in, hi_u8 *out, rsa_key_width width)
{
    ...
    /* key xor random */
    (void)memcpy_s(key, klen, k, klen);
    (void)memcpy_s(key + klen, klen, n, klen);
