...
/* ! Channel of cipher */
static symc_hard_context *g_hard_context = HI_NULL;
...
static func_hash_interrupt_isr g_hash_isr_func = HI_NULL;
...
hi_void drv_symc_register_hash_interrupt(func_hash_interrupt_isr hash_isr)
{
    g_hash_isr_func = hash_isr;
}

hi_void drv_symc_unregister_hash_interrupt(hi_void)
{
    g_hash_isr_func = HI_NULL;
}
...
static irqreturn_t drv_symc_interrupt_isr(hi_u32 arg)
{
    hi_u32 mask;
    hi_u32 i;
    hi_bool finish;
    ...
    hi_log_debug("drv_symc_interrupt_isr devId 0x%x\n", arg);
 ...
 */
    if (drv_hash_done_test() != 0) {
        if (g_hash_isr_func != HI_NULL) {
            ret = g_hash_isr_func(arg);
        }
    }
...
static void drv_symc_unregister_interrupt(void)
{
    ...
    for (i = 0; i < CRYPTO_HARD_CHANNEL_MAX; i++) {
        crypto_queue_destroy(g_hard_context[i].queue);
    }
...
static void drv_symc_smmu_base_addr(void)
{
	...
	hi_tee_drv_hal_printf("SPACC_MMU_SEC_TLB[0x%x]  : 0x%llx\n", SPACC_MMU_SEC_TLB, table_addr);
    symc_write (SPACC_MMU_SEC_TLB, (hi_u32)(table_addr >> 4));             /* bit[35:4]: mmu base address */
    symc_write (SPACC_MMU_IN_SYM_SEC_EADDR_H, (hi_u32)(err_raddr >> 32));  /* high 32 bit of addr */
    symc_write (SPACC_MMU_IN_SYM_SEC_EADDR_L, (hi_u32)(err_raddr));        /* low 32 bit of addr */
    symc_write (SPACC_MMU_OUT_SYM_SEC_EADDR_H, (hi_u32)(err_waddr >> 32)); /* high 32 bit of addr */
    symc_write (SPACC_MMU_OUT_SYM_SEC_EADDR_L, (hi_u32)(err_waddr));       /* low 32 bit of addr */
#endif
    hi_log_func_exit();
    return;
}
...
static hi_void drv_symc_get_dir(hi_cipher_data_dir data_dir, in_sym_chn_ctrl *in_ctrl)
{
    switch (data_dir) {
...
static hi_s32 drv_symc_check_nosec_mmu(hi_void)
{
...
static hi_s32 drv_symc_check_sec_mmu(hi_void)
{
    hi_s32 ret = HI_SUCCESS;
...
static void drv_symc_entry_init(crypto_mem mem)
{
    ...
    for (i = 0; i < CRYPTO_HARD_CHANNEL_MAX; i++) {
        if ((CIPHER_HARD_CHANNEL_MASK >> i) & 0x01) { /* valid channel */
            ...
            ADDR_U64(mem.mmz_addr) += SYMC_NODE_SIZE;      /* move to next channel */
...
static hi_s32 drv_symc_mem_init(void)
{
    ...
    ret = crypto_mem_create(&g_symc_dma, SEC_MMZ, "CIPHER_NODE",
...
hi_s32 drv_symc_alloc_chn(hi_u32 *chn_num, hi_cipher_type type)
{
    ...
    crypto_assert(g_symc_initialize == HI_TRUE);
    CRYPTO_UNUSED(type);
    ...
    for (i = 0; i < CRYPTO_HARD_CHANNEL_MAX; i++) {
        if (g_hard_context[i].open == HI_TRUE) {
            continue;
        }
...
hi_s32 drv_symc_init(void)
{
    ...
    if (g_hard_context == HI_NULL) {
        hi_log_print_func_err(crypto_calloc, HI_ERR_CIPHER_FAILED_MEM);
        return HI_ERR_CIPHER_FAILED_MEM;
    }
    ret = memset_s(&g_symc_dma, sizeof(g_symc_dma), 0, sizeof(g_symc_dma));
    if (ret != HI_SUCCESS) {
...
#ifdef CRYPTO_OS_INT_SUPPORT
    hi_tee_drv_hal_printf("symc interrupt configure\n");
...
void drv_symc_set_data_dir(hi_u32 chn_num, hi_cipher_data_dir data_dir)
{
    symc_hard_context *ctx = HI_NULL;
...
hi_s32 drv_symc_start(hi_u32 chn_num)
{
    ...
    drv_symc_get_dir(ctx->data_dir, &in_ctrl);
