...
static hi_s32 cryp_hash_create_mem(void)
{
    ...
    ret = crypto_mem_try_create_max("CIPHER_SHA_MSG", SEC_MMZ, HASH_PHY_MEM_SIZE, &g_hash_mem);
    if (ret != HI_SUCCESS) {
...
static hi_s32 cryp_hash_update(void *ctx, hi_u8 *chunk, hi_u32 chunk_len, hash_chunk_src src)
{
    hi_s32 ret;
    ...
    chunk += inverse_len;
    chunk_len -= inverse_len; /* the chunkLen may be zero */

    hi_log_debug("new1: total 0x%x, tail_len %d, chunkLen 0x%x, inverse_len 0x%x\n",
        hisi_ctx->total, hisi_ctx->tail_len, chunk_len, inverse_len);
    ...
    hi_log_debug("new2: total 0x%x, tail_len %d, chunkLen 0x%x\n", hisi_ctx->total,
                 hisi_ctx->tail_len, chunk_len);
...
static void *cryp_hash_clone(const void *ctx)
{
    s32 ret;
    cryp_hash_context *hisi_ctx = HI_NULL;
    ...
    hi_log_func_enter();
    if (ctx == HI_NULL) {
        hi_log_error("cryp hash clone with invalid point, ctx is null!");
        return HI_NULL;
    }
    hisi_ctx = crypto_calloc(1, sizeof(cryp_hash_context));
    if (hisi_ctx == HI_NULL) {
        hi_log_print_func_err(crypto_calloc, HI_NULL);
        return HI_NULL;
    }

    ret = memcpy_s(hisi_ctx, sizeof(cryp_hash_context), ctx, sizeof(cryp_hash_context));
    if (ret != HI_SUCCESS) {
        hi_log_print_func_err(memcpy_s, ret);
        crypto_free(hisi_ctx);
        hisi_ctx = HI_NULL;
