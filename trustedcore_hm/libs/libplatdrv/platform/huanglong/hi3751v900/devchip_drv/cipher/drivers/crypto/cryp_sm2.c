...
static const hi_u8 g_sm2_p[SM2_LEN_IN_BYTE] = {
...
static const hi_u8 g_sm2_n[SM2_LEN_IN_BYTE] = {
                     ...
                     hi_u32 length, ecc_param_t *sm2_id)
{
    ...
    while (1) {
        ...
        check_exit(drv_sm2_mul_modn(r, g_sm2_n, mr, HI_FALSE));
        hi_log_debug_msg("mr", mr, SM2_LEN_IN_BYTE);

        hi_log_info("10. mk=k*N mod n\n");
        check_exit(drv_sm2_mul_modn(k, g_sm2_n, mk, HI_FALSE));
        ...
        check_exit(drv_sm2_mul_modn(ta, g_sm2_n, m_ta, HI_FALSE));
        hi_log_debug_msg("mTA", m_ta, SM2_LEN_IN_BYTE);

        hi_log_info("15. mTy = (mTA + my) mod n\n");
        check_exit(drv_sm2_add_mod(m_ta, my, sm2_id->n, m_ty));
        hi_log_debug_msg("mTy", m_ty, SM2_LEN_IN_BYTE);

        hi_log_info("16. minv = mTy^-1 mod n\n");
        check_exit(drv_sm2_inv_mod(m_ty, sm2_id->n, minv));
        ...
        check_exit(drv_sm2_mul_modn(mrky, minv, m_tz, HI_FALSE));
        hi_log_debug_msg("mTz", m_tz, SM2_LEN_IN_BYTE);

        hi_log_info("21. ms = (mTz - mr) mod n\n");
        check_exit(drv_sm2_sub_mod(m_tz, mr, sm2_id->n, ms));
                                 ...
                                 hi_u32 check, ecc_param_t *sm2_id)
{
    ...
    check_exit(drv_sm2_mul_modp(x, g_sm2_p, mx, HI_FALSE));
    hi_log_info("4. my=C1y*P mod p\n");
    check_exit(drv_sm2_mul_modp(y, g_sm2_p, my, HI_FALSE));
