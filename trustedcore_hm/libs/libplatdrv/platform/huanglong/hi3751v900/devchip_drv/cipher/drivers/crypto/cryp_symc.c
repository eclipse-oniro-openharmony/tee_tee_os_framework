...
void cryp_symc_set_data_dir(void *ctx, hi_cipher_data_dir data_dir)
{
    cryp_symc_context *hisi_ctx = HI_NULL;
    int i;
    ...
    for (i = 0; i < CRYPTO_HARD_CHANNEL_MAX; i++) {
        ...
        if (ctx == &g_symc_context[i]) {
            hisi_ctx = ctx;
            drv_symc_set_data_dir(hisi_ctx->hard_chn, data_dir);
            break;
        }
    }
                                   ...
                                   hi_u32 pkg_num, hi_u32 wait)
{
    ...
    ret = cyp_aead_ccm_n(n, (hi_u8 *)hisi_ctx->iv,  hisi_ctx->ivlen, hisi_ctx->alen,
                         hisi_ctx->enclen, hisi_ctx->tlen);
    if (ret != HI_SUCCESS) {
