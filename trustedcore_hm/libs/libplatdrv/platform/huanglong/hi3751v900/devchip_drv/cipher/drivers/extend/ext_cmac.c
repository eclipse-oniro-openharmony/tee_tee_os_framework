...
static hi_s32 ext_aes_cmac_block(hi_u32 id, hi_u8 *input, hi_u32 inlen, hi_u32 from_user, hi_u8 *output)
{
    ...
    /* malloc dma buffer */
    ret = crypto_mem_try_create_max("CIPHER_AES_EXT", SEC_MMZ, cipher_min(MAX_EXT_DATA_LEN, inlen), &mem);
    if (ret != HI_SUCCESS) {
    ...
    for (i = 0; i < blocknum; i++) {
        ...
        check_exit(kapi_symc_crypto(id, phy, phy, enc_len, HI_FALSE, HI_FALSE, HI_CIPHER_DATA_DIR_TEE2TEE));
...
8. Return T.
***********************************************************/
hi_s32 ext_aes_cmac_init(hi_u32 id, hi_cipher_key_length klen)
{
    hi_s32 ret;
    ...
    ret = kapi_symc_config(id, HI_CIPHER_ALG_AES,
        HI_CIPHER_WORK_MODE_CBC, HI_CIPHER_BIT_WIDTH_128BIT,
        klen, iv, AES_IV_SIZE, CIPHER_IV_CHANGE_ONE_PKG,
        ADDR_NULL, 0, 0);
    if (ret != HI_SUCCESS) {
