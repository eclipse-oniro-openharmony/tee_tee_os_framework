...
hi_s32 mbedtls_hash_update(void *ctx, hi_u8 *chunk, hi_u32 chunk_len, hash_chunk_src src)
{
    hi_u8 *ptr = HI_NULL;
    hi_s32 ret;
...
void *mbedtls_hash_clone(const void *ctx)
{
    hi_s32 ret;

    mbedtls_md_context_t *clone = HI_NULL;
    mbedtls_md_context_t *parent = HI_NULL;
    ...
    clone = crypto_malloc(sizeof(mbedtls_md_context_t));
    if (clone == HI_NULL) {
        hi_log_error("malloc hash context buffer failed!");
        hi_log_print_err_code(HI_ERR_CIPHER_FAILED_MEM);
        return HI_NULL;
    ...
    parent = (mbedtls_md_context_t *)ctx;
    mbedtls_md_init(clone);
    ret = mbedtls_md_setup(clone, parent->md_info, HI_FALSE);
    if (ret != HI_SUCCESS) {
        crypto_free(clone);
        clone = HI_NULL;
        hi_log_print_func_err(mbedtls_md_setup, ret);
    ...
    ret = mbedtls_md_clone(clone, parent);
    if (ret != HI_SUCCESS) {
        crypto_free(clone);
        clone = HI_NULL;
        hi_log_print_func_err(mbedtls_md_clone, ret);
        return HI_NULL;
    }
