                            ...
                            unsigned char *output )
{
    ...
    MBEDTLS_MPI_CHK( memset_s( output, olen, 0, olen ) );
    ...
    *p++ = 1;
    MBEDTLS_MPI_CHK( memcpy_s( p, ilen, input, ilen ) );
                                 ...
                                 unsigned char *output )
{
    ...
    *p++ = 0;
    ret = memcpy_s( p, ilen, input, ilen );
    if( ret !=0 )
    {
        return( ret );
    }
                            ...
                            size_t output_max_len )
{
    ...
    *olen = ilen - (p - buf);
    MBEDTLS_MPI_CHK( memcpy_s( output, *olen, p, *olen ) );
    ret = 0;
                         ...
                         unsigned char *sig )
{
    ...
    ret = memset_s(&hisi_type, sizeof(hisi_type), 0, sizeof(hisi_type));
    if( ret != 0 )
    {
        return( ret );
    }
                                        ...
                                        unsigned char *dst )
{
    ...
    *p++ = 0;
    *p++ = MBEDTLS_RSA_SIGN;
    ret = memset_s( p, nb_pad, 0xFF, nb_pad );
    if( ret != 0 )
    {
        return( ret );
    }
    ...
    *p++ = 0;
    ...
    if( md_alg == MBEDTLS_MD_NONE )
    {
        ret = memcpy_s( p, hashlen, hash, hashlen );
        if( ret != 0 )
        {
            return( ret );
        }
    ...
    *p++ = (unsigned char) oid_size;
    ret = memcpy_s( p, oid_size, oid, oid_size );
    if( ret != 0 )
    {
        return( ret );
    }
                               ...
                               const unsigned char *sig )
{
    ...
    ret = memset_s( zeros, sizeof( zeros ), 0, sizeof( zeros ) );
    if( ret != 0 )
    {
        return( ret );
    }
...
int mbedtls_rsa_self_test( int verbose )
{
    ...
    MBEDTLS_MPI_CHK( memcpy_s( rsa_plaintext, sizeof(rsa_plaintext), RSA_PT, PT_LEN ) );
