...
static hi_s32 dispatch_symc_config(hi_void *argp)
{
    ...
    aad_is_sec = HI_TRUE;
    ret = crypto_bufhandle_to_phys(&aad_phy, config->aad, aad_is_sec);
    if (ret != HI_SUCCESS) {
        hi_log_print_func_err(crypto_bufhandle_to_phys, ret);
        return ret;
    }
    ADDR_U64(aad) = aad_phy;
    ret = kapi_symc_config(config->id, config->alg, config->mode, config->width, config->klen,
        (hi_u8 *)config->iv, config->ivlen, config->iv_usage, aad, config->alen, config->tlen);
    if (ret != HI_SUCCESS) {
...
static hi_s32 dispatch_symc_encrypt(hi_void *argp)
{
    ...
    hi_log_func_enter();
    ret = crypto_judge_sec_mem(encrypt->data_dir, &in_is_sec, &out_is_sec);
    if (ret != HI_SUCCESS) {
        hi_log_print_func_err(crypto_judge_sec_mem, ret);
        return ret;
    }
    ret = crypto_bufhandle_to_phys(&input_phy, encrypt->input, in_is_sec);
    if (ret != HI_SUCCESS) {
        hi_log_print_func_err(crypto_bufhandle_to_phys, ret);
        return ret;
    }
    ret = crypto_bufhandle_to_phys(&output_phy, encrypt->output, out_is_sec);
    if (ret != HI_SUCCESS) {
        hi_log_print_func_err(crypto_bufhandle_to_phys, ret);
        return ret;
    }
    ADDR_U64(in) = input_phy;
    ADDR_U64(out) = output_phy;
    ret = kapi_symc_crypto(encrypt->id, in, out, encrypt->length,
        encrypt->operation, HI_FALSE, encrypt->data_dir);
    if (ret != HI_SUCCESS) {
...
static hi_s32 dispatch_symc_encrypt_multi(hi_void *argp)
{
    ...
    ret = kapi_symc_crypto_multi(encrypt_mutli->id, (const void *)(ADDR_VIA(encrypt_mutli->pkg)),
        encrypt_mutli->pkg_num, encrypt_mutli->operation, encrypt_mutli->data_dir);
    if (ret != HI_SUCCESS) {
...
#endif
        || HI_CIPHER_HASH_TYPE_HMAC_SM3 == start->type
    ) {
...
static hi_s32 dispatch_trng_get_random(hi_void *argp)
{
    ...
    hi_log_check_param(ADDR_VIA(trng->randnum) == HI_NULL);
    hi_tee_drv_hal_write_right_check(ADDR_VIA(trng->randnum), trng->size);

    buffer = ADDR_VIA(trng->randnum);
    ret = hi_tee_drv_hal_user_mmap(&buffer, trng->size);
    if (ret != HI_SUCCESS) {
        hi_log_print_func_err(hi_tee_drv_hal_user_mmap, ret);
        return ret;
    }

    ret = kapi_trng_get_rand_byte(buffer, trng->size, trng->timeout);
    if (ret != HI_SUCCESS) {
        return ret;
    }

    ret = hi_tee_drv_hal_user_munmap(buffer, trng->size);
    if (ret != HI_SUCCESS) {
...
static hi_s32 dispatch_cenc_decrypt(hi_void *argp)
{
    ...
    hi_tee_drv_hal_read_right_check(ADDR_VIA(cenc->subsample), cenc->subsample_num * sizeof(hi_cipher_subsample));

    ret = crypto_judge_sec_mem(cenc->data_dir, &in_is_sec, &out_is_sec);
    if (ret != HI_SUCCESS) {
        hi_log_print_func_err(crypto_judge_sec_mem, ret);
        return ret;
    }
    ret = crypto_bufhandle_to_phys(&input_phy, cenc->inphy, in_is_sec);
    if (ret != HI_SUCCESS) {
        hi_log_print_func_err(crypto_bufhandle_to_phys, ret);
        return ret;
    }
    ret = crypto_bufhandle_to_phys(&output_phy, cenc->outphy, out_is_sec);
    if (ret != HI_SUCCESS) {
        hi_log_print_func_err(crypto_bufhandle_to_phys, ret);
        return ret;
    }
    ...
    cipher_cenc.data_dir = cenc->data_dir;
...
static hi_s32 dispatch_pbkdf_hmac256(hi_void *argp)
{
    hi_s32 ret;
    pbkdf_hmac256_t *pfkdf = argp;
    hi_cipher_pbkdf2 pbkdf_info = {0};
    hi_u8 *output = HI_NULL;

    hi_log_func_enter();

    hi_log_check_param(MAX_MALLOC_BUF_SIZE < pfkdf->hmac_key_len);
    hi_log_check_param(MAX_MALLOC_BUF_SIZE < pfkdf->slen);
    hi_log_check_param(MAX_MALLOC_BUF_SIZE < pfkdf->outlen);

    pbkdf_info.hmac_key = crypto_calloc(1, pfkdf->hmac_key_len + pfkdf->slen + pfkdf->outlen);
    if (pbkdf_info.hmac_key == HI_NULL) {
        hi_log_error("error, malloc for password failed\n");
        hi_log_print_func_err(crypto_calloc, HI_ERR_CIPHER_FAILED_MEM);
        return HI_ERR_CIPHER_FAILED_MEM;
    }

    pbkdf_info.salt = pbkdf_info.hmac_key + pfkdf->hmac_key_len;
    output = pbkdf_info.salt + pfkdf->slen;
    pbkdf_info.hmac_key_len = pfkdf->hmac_key_len;
    pbkdf_info.slen = pfkdf->slen;
    pbkdf_info.iteration_count = pfkdf->iteration_count;
    pbkdf_info.key_length = pfkdf->outlen;

    check_exit(crypto_copy_from_user(pbkdf_info.hmac_key, pfkdf->hmac_key_len,
                                     ADDR_VIA(pfkdf->hmac_key), pfkdf->hmac_key_len));
    check_exit(crypto_copy_from_user(pbkdf_info.salt, pfkdf->slen, ADDR_VIA(pfkdf->salt), pfkdf->slen));
    check_exit(crys_pkcs5_pbkdf2_hmac256(&pbkdf_info, output));
    check_exit(crypto_copy_to_user(ADDR_VIA(pfkdf->output), pfkdf->outlen, output, pfkdf->outlen));

    if (pbkdf_info.hmac_key != HI_NULL) {
        crypto_free(pbkdf_info.hmac_key);
        pbkdf_info.hmac_key = HI_NULL;
    }
    hi_log_func_exit();
    return HI_SUCCESS;
...
static hi_s32 dispatch_self_test(hi_void *argp)
{
    hi_s32 ret;
    ...
    hi_tee_drv_hal_read_right_check(ADDR_VIA(test->param), test->size);
    hi_tee_drv_hal_write_right_check(ADDR_VIA(test->param), test->size);

    ret = crypto_self_test(test->cmd, ADDR_VIA(test->param), test->size);
    if (ret != HI_SUCCESS) {
        hi_log_print_func_err(crypto_self_test, ret);
        return ret;
    }
#endif
...
static hi_s32 dispatch_init(void *argp)
{
    return HI_SUCCESS;
}

static s32 dispatch_deinit(void *argp)
{
    return HI_SUCCESS;
}
...
static crypto_dispatch_func g_dispatch_func[CRYPTO_CMD_COUNT] = {
    ...
    {"PBKDF",         dispatch_pbkdf_hmac256,       CRYPTO_CMD_PBKDF2},
    {"TEST",          dispatch_self_test,           CRYPTO_CMD_TEST},
    {"INIT",          dispatch_init,                CRYPTO_CMD_INIT},
    {"DEINIT",        dispatch_deinit,              CRYPTO_CMD_DEINIT},
};
...
hi_s32 crypto_ioctl(hi_u32 cmd, hi_void *argp)
{
    ...
    hi_log_check_param(g_dispatch_func[nr].cmd != cmd);
    hi_log_check_param(g_dispatch_func[nr].func == HI_NULL);
