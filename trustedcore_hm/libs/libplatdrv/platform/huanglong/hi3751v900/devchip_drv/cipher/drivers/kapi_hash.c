...
static hi_s32 kapi_check_hash_handle(hi_u32 handle)
{
                                       ...
                                       hash_mode *mode, hi_u32 *hmac)
{
    *hmac = HI_FALSE;
    ...
    switch (type) {
        ...
        case HI_CIPHER_HASH_TYPE_HMAC_SM3: {
            *hmac = HMAC_HASH;
            *mode = HASH_MODE_SM3;
            break;
        }
...
hi_s32 kapi_hash_clone(hi_u32 *id, hi_u32 parents)
{
    hi_s32 ret;
    kapi_hash_ctx *ctx = HI_NULL;
    kapi_hash_ctx *parents_ctx = HI_NULL;
    hi_u32 soft_hash_id;

    hi_log_func_enter();

    hi_log_check_param(id == HI_NULL);
    ret = kapi_check_hash_handle(parents);
    if (ret != HI_SUCCESS) {
        return ret;
    }

    parents_ctx = kapi_hash_get_ctx(HI_HANDLE_GET_CHNID(parents));
    if (parents_ctx == NULL) {
        hi_log_print_u32(parents);
        hi_log_print_func_err(kapi_hash_get_ctx, 0);
    ...
    if (parents_ctx->func->clone == HI_NULL) {
        hi_log_error("error, clone function is null\n");
        hi_log_print_err_code(HI_ERR_CIPHER_UNSUPPORTED);
        ret = HI_ERR_CIPHER_UNSUPPORTED;
        goto error1;
    }

    ret = memcpy_s(ctx, sizeof(kapi_hash_ctx), parents_ctx, sizeof(kapi_hash_ctx));
    if (ret != HI_SUCCESS) {
        hi_log_print_func_err(memcpy_s, ret);
        goto error1;
    }

    ctx->cryp_ctx = ctx->func->clone((const void *)parents_ctx->cryp_ctx);
    if (ctx->cryp_ctx == HI_NULL) {
        hi_log_print_func_err(ctx->func->clone, 0);
        hi_log_print_err_code(HI_ERR_CIPHER_UNSUPPORTED);
        ret = HI_ERR_CIPHER_UNSUPPORTED;
        goto error1;
    }
                        ...
                        hash_chunk_src src)
{
    ...
    if (ret != HI_SUCCESS) {     /* release resource */
        ctx->func->destroy((const void *)ctx->cryp_ctx);
        ctx->cryp_ctx = HI_NULL;
        kapi_hash_destroy(soft_hash_id);
        hi_log_print_func_err(ctx->func->update, ret);
