...
#define BYTE_BITS       8
#define SM2_ENC_PAD_LEN (0x01 + SM2_LEN_IN_BYTE * 3)

static ecc_param_t g_sm2_id = {
                     ...
                     hi_u32 r[SM2_LEN_IN_WROD], hi_u32 s[SM2_LEN_IN_WROD])
{
    ...
    if ((idlen == SM3_RESULT_SIZE) && (msglen == 0x00)) {
        ...
        check_exit(kapi_hash_update(sm3id, (hi_u8 *)g_sm2_id.a, SM2_LEN_IN_BYTE, HASH_CHUNCK_SRC_LOCAL));
        check_exit(kapi_hash_update(sm3id, (hi_u8 *)g_sm2_id.b, SM2_LEN_IN_BYTE, HASH_CHUNCK_SRC_LOCAL));
        check_exit(kapi_hash_update(sm3id, (hi_u8 *)g_sm2_id.gx, SM2_LEN_IN_BYTE, HASH_CHUNCK_SRC_LOCAL));
        check_exit(kapi_hash_update(sm3id, (hi_u8 *)g_sm2_id.gy, SM2_LEN_IN_BYTE, HASH_CHUNCK_SRC_LOCAL));
    ...
    ****************************************/
    ret = func->sign(e, d, r, s, SM2_LEN_IN_BYTE, &g_sm2_id);
    if (ret != HI_SUCCESS) {
                       ...
                       hi_u32 r[SM2_LEN_IN_WROD], hi_u32 s[SM2_LEN_IN_WROD])
{
    ...
    ****************************************/
    ret = func->verify(e, px, py, r, s, SM2_LEN_IN_BYTE, &g_sm2_id);
    if (ret != HI_SUCCESS) {
        crypto_zeroize(e, sizeof(e));
                        ...
                        hi_u8 *msg, hi_u32 msglen, hi_u8 *enc, hi_u32 *enclen)
{
    ...
    c1_index = 0;
    enc[c1_index++] = SM2_PC_UNCOMPRESS;
    ...
    while (trycnt++ < SM2_ENCRYPT_TRY_CNT) {
        ...
        ret = func->encrypt(px, py, c1x, c1y, x2, y2, SM2_LEN_IN_BYTE, &g_sm2_id);
        if (ret != HI_SUCCESS) {
                        ...
                        hi_u32 enclen, hi_u8 *msg, hi_u32 *msglen)
{
    ...
    c1_index = 0;
    if (enc[c1_index++] != SM2_PC_UNCOMPRESS) {
        hi_log_error("error, sm2 decrypt failed, C1[0] != 0x04!\n");
    ...
    ret = func->decrypt(d, c1x, c1y, x2, y2, SM2_LEN_IN_BYTE, &g_sm2_id);
    if (ret != HI_SUCCESS) {
    ...
    if (memcmp(&enc[c3_index], h, SM2_LEN_IN_BYTE) != 0) {
...
hi_s32 kapi_sm2_gen_key(hi_u32 d[SM2_LEN_IN_WROD], hi_u32 px[SM2_LEN_IN_WROD], hi_u32 py[SM2_LEN_IN_WROD])
{
    ...
    ret = func->genkey(d, px, py, SM2_LEN_IN_BYTE, &g_sm2_id);
    if (ret != HI_SUCCESS) {
