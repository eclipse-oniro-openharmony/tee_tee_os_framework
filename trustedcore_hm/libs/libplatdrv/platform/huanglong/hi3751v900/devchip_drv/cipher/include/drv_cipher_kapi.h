...
\retval     NA.
 */
hi_s32 kapi_symc_get_user_uuid(hi_u32 id, TEE_UUID *uuid);
...
* \retval     On success, HI_SUCCESS is returned.  On error, HI_FAILURE is returned.
 */
hi_s32 kapi_ecdsa_verify_hash(ecc_param_t *ecc, hi_u8 *px, hi_u8 *py, hi_u8 *hash, hi_u32 hlen, hi_u8 *r,
                              hi_u8 *s);

hi_u32 crys_pkcs5_pbkdf2_hmac256(hi_cipher_pbkdf2 *info, hi_u8 *output);
hi_s32 crypto_self_test(hi_u32 cmd, hi_void *param, hi_u32 size);
