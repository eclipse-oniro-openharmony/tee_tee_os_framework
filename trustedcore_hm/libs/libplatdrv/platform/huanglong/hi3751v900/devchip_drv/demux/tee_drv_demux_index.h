...
#ifndef __TEE_FIDX_EXT_HEADER__
#define __TEE_FIDX_EXT_HEADER__
...
typedef struct {
    hi_u8   sub_layer_profile_present_flag[0x6];
    hi_u8   sub_layer_level_present_flag[0x6];
...
typedef struct {
    ...
    hi_u32  list_entry_l0[32]; /* list_entry num is 32 */
    hi_u32  list_entry_l1[32]; /* list_entry num is 32 */
...
typedef struct {
    ...
    hi_u32 listx_size[0x2];
    hi_u32 num_ref_idx[0x3];
    hi_s32 entry_point_offset[256]; /* entry_point_offset num is 256 */
    ...
    hi_s32 poc_ls_blt[HEVC_MAX_NUM_REF_PICS];
    hi_s32 delta_poc_msb_cycle_t[HEVC_MAX_NUM_REF_PICS];
    hi_s32 check_lt_msb[HEVC_MAX_NUM_REF_PICS];
...
typedef struct {
    ...
    struct  hevc_frame_store *frame_store;
} hevc_storable_pic;
...
typedef struct {
    ...
    hevc_frame_store *fs[HEVC_MAX_DPB_NUM];
    hevc_frame_store *fs_negative_ref[HEVC_MAX_DPB_NUM];
    hevc_frame_store *fs_positive_ref[HEVC_MAX_DPB_NUM];
    hevc_frame_store *fs_ltref[HEVC_MAX_DPB_NUM];
} hevc_dpb;
...
typedef struct {
    hi_u8  new_sequence;
    hi_u8  no_out_put_of_prior_pics_flag;
    hi_u8  no_rasl_out_put_flag;
    hi_u8  prev_rap_is_bla;
    ...
    hi_u32 scaling_list_size[0x4];
    hi_u32 scaling_list_size_x[0x4];
    hi_u32 sig_last_scan_cg32x32[HEVC_MAX_MATRIX_COEF_NUM];
    hi_u32 scaling_list_num[SCALING_LIST_SIZE_NUM];
    hi_u32 aui_sig_last_scan[0x4][HEVC_MAX_CU_DEPTH][128 * 128]; /* 128 * 128 */

    hi_s32 hevc_frm_cnt;
    hi_s32 hevc_frm_poc;
    hi_s32 hevc_frm_type;
    hi_s32 hevc_ref_num;
    hi_s32 hevc_ref_poc[HEVC_MAX_NUM_REF_PICS];

    hi_s32 dmx_hevc_frm_cnt;
    hi_s32 dmx_hevc_frm_poc;
    hi_s32 dmx_hevc_frm_type;
    hi_s32 dmx_hevc_ref_num;
    hi_s32 dmx_hevc_ref_poc[HEVC_MAX_NUM_REF_PICS];

    bs *bs_p;
    hevc_video_param_set    vps[HEVC_MAX_NUM_REF_PICS];
    hevc_seq_param_set      sps[HEVC_MAX_NUM_REF_PICS];
    hevc_pic_param_set      pps[HEVC_MAX_PIC_PARAM_SET_ID];
    hevc_nalu              *curr_nal;
    hevc_storable_pic      *list_x[0x2][HEVC_MAX_LIST_SIZE];
    ...
    hevc_seq_param_set         sei_sps;
    hevc_pic_param_set         curr_pps;
    hevc_curr_pic              curr_pic;
    hevc_slice_segment_header  curr_slice;
    hevc_nalu                  nal_array;
    hevc_frame_store           frame_store[HEVC_MAX_FRAME_STORE];
} hevc_ctx;
...
typedef struct {
    ...
    hi_s32        cur_poc;
    hi_s32        ref_num;
    hi_s32        ref_poc[HEVC_MAX_NUM_REF_PICS];
} frame_pos;


/* global init, clear residual information, and register call back. */
hi_s32  fidx_init(hi_void (*out_put_frame_position)(hi_u32 *param, const frame_pos *sc_info));
 ...
 *  open an instance
 * @param
 *  vid_standard:  video standard type
 * @return
 ...
 * @param
 *  inst_idx:  the ID of the instance to be closed
 */
 ...
 */
hi_s32 fidx_is_sc_useful(hi_s32 inst_idx, hi_u8 startcode);
