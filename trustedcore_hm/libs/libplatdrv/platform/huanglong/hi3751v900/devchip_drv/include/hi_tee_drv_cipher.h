...
typedef enum {
    ...
    HI_CIPHER_WORK_MODE_CBC_CTS,
    HI_CIPHER_WORK_MODE_MAX
} hi_cipher_work_mode;
...
/** encryption/decryption type selecting */
typedef enum {
    HI_CIPHER_DATA_DIR_REE2REE = 0x0,
    HI_CIPHER_DATA_DIR_REE2TEE = 0x1,
    HI_CIPHER_DATA_DIR_TEE2REE = 0x2,
    HI_CIPHER_DATA_DIR_TEE2TEE = 0x3,
    HI_CIPHER_DATA_DIR_BUTT,
    HI_CIPHER_DATA_DIR_INVALID = 0xffffffff,
} hi_cipher_data_dir;
...
typedef struct {
    ...
    hi_mem_handle a_phy_addr;            /* memory handle of Associated data for CCM and GCM */
...
/* Cipher data */
typedef struct {
    hi_mem_handle src_phy_addr;  /* memory handle of the original data */
    hi_mem_handle dest_phy_addr; /* memory handle of the purpose data */
...
typedef struct {
    hi_handle handle;
    hi_u32 key[12]; /* Key input, EK||AK||SK for SM1 */                          /**< CNcomment:0ê7, SM1¦ÄAK||EK||SK */
    hi_u32 odd_key[8]; /* Key input, Old Key */                       /**< CNcomment:0ê7, 0ê7 */
    hi_u32 iv[4]; /* Initialization vector (IV) */                            /**< CNcomment:0¶3 */
    hi_cipher_alg alg; /* Cipher algorithm */                       /**< CNcomment:‚1ï1 */
    hi_cipher_bit_width bit_width; /* Bit width for encryption or decryption */           /**< CNcomment:1Ÿ51ž9¦Ë */
    hi_cipher_work_mode work_mode; /* Operating mode */           /**< CNcomment:000¶4 */
    hi_cipher_key_length key_len; /* Key length */            /**< CNcomment:0ê7 */
    hi_cipher_ctrl_change_flag change_flags; /* control information exchange choices, we default all woulde be change except they have been in the choices */ /**< CNcomment:0Î40Ô5ƒ6Ý50Ô50‹4§Ò0µ7080¨0 */
    hi_u32 iv_len; /* IV lenght for CCM/GCM, which is an element of {4,6,8,10,12,14,16} for CCM, and is an element of [1-16] for GCM */                           /**< CNcomment: CCM/GCMIV0§2CCM0§00ö5¦¶{7, 8, 9, 10, 11, 12, 13} GCM0§00ö5¦¶[1-16] */
    hi_u32 tag_len; /* Tag lenght for CCM which is an element of {4,6,8,10,12,14,16} */                          /**< CNcomment: CCMTAG0§20§00ö5¦¶{4,6,8,10,12,14,16} */
    hi_u32 a_len; /* Associated data for CCM and GCM */                            /**< CNcomment: CCM/GCM0‘11¥1 */
    hi_u64 a_phy_addr; /* Physical address of Associated data for CCM and GCM */                       /**< CNcomment: CCM/GCM0‘11¥1 */
...
typedef struct _hi_cipher_pbkdf2 {
    hi_u8 *hmac_key;
    hi_u32 hmac_key_len;
    hi_u8 *salt;
    hi_u32 slen;
...
hi_s32 hi_drv_cipher_encrypt(hi_handle handle, hi_u64 src_phy_addr, hi_u64 dest_phy_addr,
                             hi_u32 data_length, hi_cipher_data_dir dir);
hi_s32 hi_drv_cipher_decrypt(hi_handle handle, hi_u64 src_phy_addr, hi_u64 dest_phy_addr,
                             hi_u32 data_length, hi_cipher_data_dir dir);
