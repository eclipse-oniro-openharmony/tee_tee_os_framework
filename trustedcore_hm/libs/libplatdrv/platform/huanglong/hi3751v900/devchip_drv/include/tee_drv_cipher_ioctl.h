...
typedef struct {
    ...
    hi_u32 reserve;            /* !<  reserve to make align at 64bit */
    hi_mem_handle aad;         /* !<  Associated Data */
...
typedef struct {
    ...
    hi_mem_handle input;  /* !<  Buffer handle of the input data */
    hi_mem_handle output; /* !<  Buffer handle of the output data */
...
/* ! \struct of pbkdf */
typedef struct {
    compat_addr hmac_key;   /* !<  Buffer of hamc key */
    compat_addr salt;       /* !<  Buffer of salt */
    compat_addr output;     /* !<  Buffer of ouput */
    hi_u32 hmac_key_len;    /* !<  Length of hamc key */
...
typedef struct
{
    hi_u32 cmd;        /* !<  command id */
    compat_addr param; /* !<  command params */
    hi_u32 size;       /* !<  command size */
} test_t;
...
#define CRYPTO_CMD_TRNG               CRYPTO_IOW(0x0f, sizeof(trng_t))
#define CRYPTO_CMD_SM2_SIGN           CRYPTO_IOWR(0x10, sizeof(sm2_sign_t))
#define CRYPTO_CMD_SM2_VERIFY         CRYPTO_IOWR(0x11, sizeof(sm2_verify_t))
#define CRYPTO_CMD_SM2_ENCRYPT        CRYPTO_IOWR(0x12, sizeof(sm2_encrypt_t))
#define CRYPTO_CMD_SM2_DECRYPT        CRYPTO_IOWR(0x13, sizeof(sm2_decrypt_t))
#define CRYPTO_CMD_SM2_GEN_KEY        CRYPTO_IOWR(0x14, sizeof(sm2_key_t))
#define CRYPTO_CMD_SYMC_GET_KEYSLOT   CRYPTO_IOWR(0x15, sizeof(symc_keyslot_t))
#define CRYPTO_CMD_SYMC_GET_CONFIG    CRYPTO_IOWR(0x16, sizeof(symc_get_config_t))
#define CRYPTO_CMD_PBKDF2             CRYPTO_IOWR(0x17, sizeof(pbkdf_hmac256_t))
#define CRYPTO_CMD_TEST               CRYPTO_IOWR(0x18, sizeof(test_t))
#define CRYPTO_CMD_INIT               CRYPTO_IOWR(0x19, 0)
#define CRYPTO_CMD_DEINIT             CRYPTO_IOWR(0x1a, 0)
#define CRYPTO_CMD_COUNT              0x1B
