...
static hi_s32 __hkl_com_func_content_start(common_klad *com_klad, hi_u32 level, hi_u32 com_kl_num,
                                           common_klad_crc *com_klad_crc)
{
    hi_s32 ret;
...
static hi_s32 __com_hkl_func_start(common_klad *com_klad, hi_s32 level,
                                   hi_u32 com_kl_num, common_klad_crc *com_klad_crc)
{
    hi_s32 ret;
    hi_s32 i = 0;
    ...
    ret = __hkl_com_func_content_start(com_klad, level, com_kl_num, com_klad_crc);
    if (ret != HI_SUCCESS) {
        print_err_func(__hkl_com_func_content_start, ret);
    }
...
static hi_s32 __hkl_fp_start(fp_klad *fpk_klad, hi_u32 com_kl_num, hi_u8 *fpk_crc)
{
    ...
    hkl_set_data_in(fpk_klad->fpk_ext_klad.key, HKL_KEY_LEN);
    if (fpk_klad->fpk_ext_klad.operation == HI_KLAD_FP_OPT_DECRYPT) {
        is_dec = HI_TRUE;
    }
    hkl_set_fp_lev1(is_dec, com_kl_num);
...
static hi_s32 __fp_func_generate(hi_u8 *klad_enc, hi_u32 len)
{
    hi_s32 ret;
    ...
    ret = hkl_generate_fp_enc_key(klad_enc, len);
    if (ret != HI_SUCCESS) {
        print_err_func(hkl_generate_fp_enc_key, ret);
    }
out:
    return ret;
}
...
static hi_s32 __hkl_klad_cfg(const hi_klad_attr *attr, hi_handle target, hi_bool odd, common_klad *com_klad)
{
    ...
    com_klad->cfg.key_slot_num = HI_HANDLE_GET_CHNID(target);
    ...
    com_klad->cfg.tgt_sec_cfg.key_sec = (attr->key_sec_cfg.key_sec == HI_KLAD_SEC_ENABLE) ? HI_TRUE : HI_FALSE;
    ...
    */
    com_klad->cfg.tgt_sec_cfg.dest_nsec = attr->key_sec_cfg.dest_buf_non_sec_support;
    com_klad->cfg.tgt_sec_cfg.dest_sec = attr->key_sec_cfg.dest_buf_sec_support;
    com_klad->cfg.tgt_sec_cfg.src_nsec = attr->key_sec_cfg.src_buf_non_sec_support;
    com_klad->cfg.tgt_sec_cfg.src_sec = attr->key_sec_cfg.src_buf_sec_support;
...
hi_s32 __hkl_fp_klad_cfg(const hi_klad_fp_key *fp_key, fp_ext_klad *fpk_ext_klad)
{
    fpk_ext_klad->key_alg = fp_key->alg;
    fpk_ext_klad->operation = fp_key->operation;
    if (memcpy_s(fpk_ext_klad->key, sizeof(fpk_ext_klad->key), fp_key->key, HKL_KEY_LEN) != EOK) {
        print_err_func(memcpy_s, HI_ERR_KLAD_SEC_FAILED);
...
static hi_s32 __hkl_rkp_cfg(struct klad_r_base_attr *base_attr, rkp_deob_kdf *deob_kdf)
{
    ...
    */
    if (base_attr->is_secure_key == HI_TRUE) {
        deob_kdf->kdf.cas_kdf_static = HI_RKP_CAS_KDF_STATIC_SECURE;
    } else {
...
static hi_s32 __hkl_klad_com_key(const hi_klad_com_entry *instance, common_klad *com_klad)
{
    ...
    for (i = 0; i < HI_KLAD_LEVEL_MAX; i++) {
        ...
        if (memcpy_s(&com_klad->session_key[i], sizeof(com_klad->session_key[i]),
                     &instance->session_key[i].key, HKL_KEY_LEN) != EOK) {
            print_err_func(memcpy_s, HI_ERR_KLAD_SEC_FAILED);
            return HI_ERR_KLAD_SEC_FAILED;
        }
    }
    ...
    if (memcpy_s(&com_klad->session_key[i], sizeof(com_klad->session_key[i]),
                 &instance->content_key.key, HKL_KEY_LEN) != EOK) {
        print_err_func(memcpy_s, HI_ERR_KLAD_SEC_FAILED);
        return HI_ERR_KLAD_SEC_FAILED;
    }
    return HI_SUCCESS;
...
static hi_s32 __hkl_klad_fp_key(const hi_klad_fp_entry *instance, fp_klad *fp_klad)
{
    hi_u32 i;
    ...
    for (i = 0; i < HI_KLAD_LEVEL_MAX; i++) {
        ...
        fp_klad->com_klad.klad_alg[i] = __hkl_alg(instance->session_key[i].alg);
        if (memcpy_s(&fp_klad->com_klad.session_key[i], sizeof(fp_klad->com_klad.session_key[i]),
                     &instance->session_key[i].key, HKL_KEY_LEN) != EOK) {
            print_err_func(memcpy_s, HI_ERR_KLAD_SEC_FAILED);
            return HI_ERR_KLAD_SEC_FAILED;
        }
    ...
    if ((instance->fp_cnt == 0) || (instance->fp_key.key_size != HKL_KEY_LEN)) {
        print_err_code(HI_ERR_KLAD_INVALID_PARAM);
        return HI_ERR_KLAD_INVALID_PARAM;
    }

    fp_klad->fpk_ext_klad.operation = instance->fp_key.operation;
    fp_klad->fpk_ext_klad.key_alg = instance->fp_key.alg;
    if (memcpy_s(&fp_klad->fpk_ext_klad.key, sizeof(fp_klad->fpk_ext_klad.key),
                 &instance->fp_key.key, HKL_KEY_LEN) != EOK) {
        print_err_func(memcpy_s, HI_ERR_KLAD_SEC_FAILED);
        return HI_ERR_KLAD_SEC_FAILED;
    }
    return HI_SUCCESS;
...
static hi_void __hkl_isr_nonce_lock_impl(struct hkl_mgmt *mgmt)
{
    ...
    sem_post(&mgmt->kl_nonce_lock.sem);
    return;
}
...
static irqreturn_t _hkl_isr(hi_void *priv)
{
    hi_info_klad("_hkl_isr enter\n");
    if (priv == HI_NULL) {
...
static struct hkl_mgmt g_hkl_mgmt = {
    .lock      = __mutex_initializer(g_hkl_mgmt.lock),
...
static hi_void __hkl_mgmt_irq_init(struct hkl_mgmt *mgmt)
{
    ...
    mutex_init(&mgmt->kl_alg_ip.lock);
    sem_init(&mgmt->kl_alg_ip.sem, 0, 0);

    mutex_init(&mgmt->kl_csgk2_lock.lock);
    sem_init(&mgmt->kl_csgk2_lock.sem, 0, 0);

    mutex_init(&mgmt->kl_ta_lock.lock);
    sem_init(&mgmt->kl_ta_lock.sem, 0, 0);

    mutex_init(&mgmt->kl_fp_lock.lock);
    sem_init(&mgmt->kl_fp_lock.sem, 0, 0);

    mutex_init(&mgmt->kl_clr_lock.lock);
    sem_init(&mgmt->kl_clr_lock.sem, 0, 0);

    mutex_init(&mgmt->kl_nonce_lock.lock);
    sem_init(&mgmt->kl_nonce_lock.sem, 0, 0);

    mutex_init(&mgmt->kl_com_lock.lock);
    sem_init(&mgmt->kl_com_lock.sem, 0, 0);
}
#endif


static hi_void __hkl_perm_multi2_cfg(hi_void)
{
    reg_write(PERM_MULTI2_CFG, 0x20); /* set fixed value, 0x20, come from logic engineer. */
...
hi_s32 hkl_mgmt_init(hi_void)
{
    ...
    if (hi_request_irq(HKL_IRQ_NUMBER, _hkl_isr, (hi_void *)mgmt) != 0) {
...
#ifdef HI_INT_SUPPORT
static hi_void __hkl_mgmt_irq_exit(struct hkl_mgmt *mgmt)
{
    mutex_deinit(&mgmt->kl_alg_ip.lock);
    sem_destroy(&mgmt->kl_alg_ip.sem);

    mutex_deinit(&mgmt->kl_csgk2_lock.lock);
    sem_destroy(&mgmt->kl_csgk2_lock.sem);

    mutex_deinit(&mgmt->kl_ta_lock.lock);
    sem_destroy(&mgmt->kl_ta_lock.sem);

    mutex_deinit(&mgmt->kl_fp_lock.lock);
    sem_destroy(&mgmt->kl_fp_lock.sem);

    mutex_deinit(&mgmt->kl_clr_lock.lock);
    sem_destroy(&mgmt->kl_clr_lock.sem);

    mutex_deinit(&mgmt->kl_nonce_lock.lock);
    sem_destroy(&mgmt->kl_nonce_lock.sem);

    mutex_deinit(&mgmt->kl_com_lock.lock);
    sem_destroy(&mgmt->kl_com_lock.sem);
}
#endif
...
#ifdef HI_INT_SUPPORT
    __hkl_mgmt_irq_exit(mgmt);

    hi_free_irq(HKL_IRQ_NUMBER, HKL_IRQ_NAME, (hi_void *)mgmt);
                         ...
                         common_klad_crc *com_klad_crc, rkp_deob_kdf_rst *deob_kdf_rst)
{
    ...
    ret = __com_hkl_func_start(com_klad, level, com_kl_num, com_klad_crc);
    if (ret != HI_SUCCESS) {
        print_err_func(__com_hkl_func_start, ret);
        goto out;
    }
out:
...
hi_s32 hi_klad_fp_generate(hi_u8 *klad_enc, hi_u32 len)
{
    ...
    ret = __fp_func_generate(klad_enc, len);
    if (ret != HI_SUCCESS) {
...
hi_s32 hi_klad_clr_entry_start(struct klad_r_clr_route *clr_route, const hi_klad_clr_entry *instance)
{
    ...
    if (memcpy_s(com_klad.session_key, sizeof(com_klad.session_key),
                 instance->clr_key.key, HKL_KEY_LEN) != EOK) {
...
hi_s32 hi_klad_rkp_entry_start(struct klad_r_base_attr *base_attr, klad_hw_id hw_id)
{
    ...
    ret = __hkl_rkp_module_id(base_attr->module_id, MODULE_ID_CNT, &deob_kdf);
    if (ret != HI_SUCCESS) {
        print_err_func(__hkl_rkp_module_id, ret);
        goto out;
    }

    ret = __hkl_rkp_cfg(base_attr, &deob_kdf);
    if (ret != HI_SUCCESS) {
        print_err_func(__hkl_rkp_cfg, ret);
        goto out;
    }
    mutex_lock(&mgmt->lock);
    ret = __rkp_func_start(&deob_kdf, hw_id, &deob_kdf_rst);
    if (ret != HI_SUCCESS) {
        print_err_func(__rkp_func_start, ret);
    }
    mutex_unlock(&mgmt->lock);
...
hi_s32 hi_klad_com_entry_start(struct klad_r_com_hkl *com_hkl, const hi_klad_com_entry *instance)
{
    ...
    mutex_lock(&mgmt->lock);
    ret = __com_hkl_func_start(&com_klad, level, com_hkl->base.hw_id, &com_klad_crc);
    if (ret != HI_SUCCESS) {
        print_err_func(__com_hkl_func_start, ret);
    }
    mutex_unlock(&mgmt->lock);
...
hi_s32 hi_klad_fp_entry_start(struct klad_r_fp_hkl *fp_hkl, hi_klad_fp_entry *instance)
{
    hi_s32 ret;
    fp_klad fp_klad = {0};
    fp_klad_crc fp_klad_crc = {0};
    struct hkl_mgmt *mgmt = __get_hkl_mgmt();

    ret = __hkl_klad_cfg(&instance->attr, instance->target_handle, HI_FALSE, &fp_klad.com_klad);
    if (ret != HI_SUCCESS) {
        print_err_func(__hkl_rkp_module_id, ret);
        goto out;
    }

    ret = __hkl_fp_klad_cfg(&instance->fp_key, &fp_klad.fpk_ext_klad);
    if (ret != HI_SUCCESS) {
        print_err_func(__hkl_fp_klad_cfg, ret);
        goto out;
    }

    ret = __hkl_klad_fp_key(instance, &fp_klad);
    if (ret != HI_SUCCESS) {
        print_err_func(__hkl_klad_fp_key, ret);
        goto out;
    }
    mutex_lock(&mgmt->lock);
    ret = __fp_func_start(&fp_klad, fp_hkl->base.hw_id, HI_KLAD_LEVEL4, &fp_klad_crc);
    if (ret != HI_SUCCESS) {
        print_err_func(__fp_func_start, ret);
        goto out;
    }
    if (fp_klad.fpk_ext_klad.operation == HI_KLAD_FP_OPT_DECRYPT) {
        ret = __fp_func_send();
        if (ret != HI_SUCCESS) {
            print_err_func(__fp_func_send, ret);
        }
    } else {
        ret = __fp_func_generate(instance->fp_key.enc_key, HKL_KEY_LEN);
        if (ret != HI_SUCCESS) {
            print_err_func(__fp_func_generate, ret);
        }
        instance->fp_key.enc_key_size = HKL_KEY_LEN;
    }
out:
    mutex_unlock(&mgmt->lock);
    return ret;
...
hi_s32 hi_klad_clrcw_lock(void)
{
    ...
    mutex_lock(&mgmt->kl_clr_lock.lock);
    ret = hkl_clrcw_lock();
    if (ret != HI_SUCCESS) {
        print_err_func(hkl_clrcw_lock, ret);
    }
    mutex_unlock(&mgmt->kl_clr_lock.lock);
...
hi_s32 hi_klad_com_lock(hi_u32 *com_kl_num)
{
    ...
    mutex_lock(&mgmt->kl_com_lock.lock);

    ret = hkl_com_lock(com_kl_num);
    if (ret != HI_SUCCESS) {
        print_err_func(hkl_com_lock, ret);
    }
    mutex_unlock(&mgmt->kl_com_lock.lock);
...
hi_s32 hi_klad_ta_lock(void)
{
    ...
    mutex_lock(&mgmt->kl_ta_lock.lock);

    ret = hkl_ta_lock();
    if (ret != HI_SUCCESS) {
        print_err_func(hkl_ta_lock, ret);
    }
    mutex_unlock(&mgmt->kl_ta_lock.lock);
...
hi_s32 hi_klad_fp_lock(void)
{
    ...
    mutex_lock(&mgmt->kl_fp_lock.lock);

    ret = hkl_fp_lock();
    if (ret != HI_SUCCESS) {
        print_err_func(hkl_fp_lock, ret);
    }
    mutex_unlock(&mgmt->kl_fp_lock.lock);
...
hi_s32 hi_klad_nonce_lock(void)
{
    ...
    mutex_lock(&mgmt->kl_nonce_lock.lock);

    ret = hkl_nonce_lock();
    if (ret != HI_SUCCESS) {
        print_err_func(hkl_nonce_lock, ret);
    }
    mutex_unlock(&mgmt->kl_nonce_lock.lock);
...
hi_s32 hi_klad_csgk2_lock(void)
{
    ...
    mutex_lock(&mgmt->kl_csgk2_lock.lock);

    ret = hkl_csgk2_lock();
    if (ret != HI_SUCCESS) {
        print_err_func(hkl_csgk2_lock, ret);
    }
    mutex_unlock(&mgmt->kl_csgk2_lock.lock);
