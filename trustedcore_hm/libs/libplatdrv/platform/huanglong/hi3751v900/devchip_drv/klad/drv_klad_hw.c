...
hi_s32 hi_drv_hw_fp_klad_route(struct klad_entry_key *attrs)
{
    hi_s32 ret;
    struct klad_r_fp_hkl *obj = HI_NULL;
    struct time_ns time_b;
    ...
    ret = klad_hw_fp_hkl_route(obj);
    if (ret != HI_SUCCESS) {
        print_err_func(klad_hw_fp_hkl_route, ret);
    }

    ret = klad_hw_fp_hkl_close(obj);
    if (ret != HI_SUCCESS) {
        print_err_func(klad_hw_fp_hkl_close, ret);
    }
...
{
    hi_tee_log_level log_level = HI_LOG_LEVEL_DBG;
    hi_s32 ret;
