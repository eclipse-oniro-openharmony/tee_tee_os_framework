...
hi_void timestamp_tag(struct klad_r_base *obj, enum klad_timetamp_tag step)
{
    ...
    if (t != HI_NULL) {
        t->tv_sec = tv.tval.sec;
        t->tv_nsec = tv.tval.nsec;
    }
...
hi_void timestamp_clean(struct klad_r_base *obj)
{
    ...
    if (memset_s(&obj->timestamp, sizeof(struct klad_timestamp), 0, sizeof(obj->timestamp)) != EOK) {
...
hi_s32 klad_hw_fp_hkl_open(struct klad_entry_key *attrs, struct klad_r_fp_hkl **obj)
{
    ...
    ret = fp_hkl->ops->attach(com_hkl, fp_hkl);
    if (ret != HI_SUCCESS) {
        goto out4;
    }
...
hi_s32 klad_hw_fp_hkl_start(struct klad_r_fp_hkl *fp_hkl, hi_klad_fp_entry *instance)
{
    hi_s32 ret;
    ...
    ret = klad_r_get_raw((struct klad_r_base *)fp_hkl->com_obj);
    if (ret != HI_SUCCESS) {
        goto out1;
    }
...
out2:
    klad_r_put((struct klad_r_base *)fp_hkl->com_obj);
out1:
    klad_r_put((struct klad_r_base *)fp_hkl);
out0:
    return ret;
...
hi_s32 klad_hw_fp_hkl_route(struct klad_r_fp_hkl *fp_hkl)
{
    hi_s32 ret;
...
static hi_s32 __klad_r_com_hkl_open_impl(struct klad_r_com_hkl *com_hkl)
{
    ...
    com_hkl->base.rkp_ready = HI_FALSE;
    return ret;
}
...
static hi_s32 klad_r_com_hkl_rkp_impl(struct klad_r_com_hkl *com_hkl)
{
    ...
    if (com_hkl->base.rkp_ready == HI_TRUE) {
        ret = HI_SUCCESS; /* do nothing~. */
        goto out;
    }
    ret = hi_klad_rkp_entry_start(&com_hkl->base_attr, com_hkl->base.hw_id);
    if (ret != HI_SUCCESS) {
        print_err_func(hi_klad_rkp_entry_start, ret);
        goto out;
    }

    com_hkl->base.rkp_ready = HI_TRUE;
    ret = HI_SUCCESS;
...
static hi_void klad_r_hkl_rkp_restart(struct klad_r_base *base, struct klad_r_base_attr *base_attr)
{
    hi_s32 ret;

    warn_on(__hkl_is_unlocked(base));

    base->rkp_ready = HI_FALSE;
    ret = hi_klad_rkp_entry_start(base_attr, base->hw_id);
    if (ret != HI_SUCCESS) {
        print_err_func(hi_klad_rkp_entry_start, ret);
        return;
    }
    base->rkp_ready = HI_TRUE;
}
...
static hi_s32 klad_r_com_hkl_start_impl(struct klad_r_com_hkl *com_hkl, const hi_klad_com_entry *instance)
{
    ...
    if (ret != HI_SUCCESS) {
        ...
        klad_r_hkl_rkp_restart(&com_hkl->base, &com_hkl->base_attr);
    }
...
static hi_s32 klad_r_fp_hkl_attach_impl(struct klad_r_com_hkl *com_hkl, struct klad_r_fp_hkl *fp_hkl)
{
    __mutex_lock(&fp_hkl->lock);
    __mutex_lock(&com_hkl->lock);

    if (fp_hkl->com_obj == HI_NULL_PTR && com_hkl->associate_obj == HI_NULL_PTR) {
        fp_hkl->com_obj = com_hkl;
        com_hkl->associate_obj = (struct klad_r_base *)fp_hkl;
    }
...
static hi_s32 klad_r_fp_hkl_rkp_impl(struct klad_r_fp_hkl *fp_hkl)
{
    ...
    if (fp_hkl->base.rkp_ready == HI_TRUE) {
        ret = HI_SUCCESS; /* do nothing~. */
        goto out;
    }
    ret = hi_klad_rkp_entry_start(&fp_hkl->base_attr, fp_hkl->base.hw_id);
    if (ret != HI_SUCCESS) {
        print_err_func(hi_klad_rkp_entry_start, ret);
        goto out;
    }

    fp_hkl->base.rkp_ready = HI_TRUE;
    ret = HI_SUCCESS;
...
static hi_s32 klad_r_fp_hkl_start_impl(struct klad_r_fp_hkl *fp_hkl, hi_klad_fp_entry *instance)
{
    hi_s32 ret;

    __mutex_lock(&fp_hkl->lock);
    __mutex_lock(&fp_hkl->com_obj->lock);

    timestamp_tag(&fp_hkl->base, TIMETAMP_START_I);

    warn_on(__hkl_is_unlocked(&fp_hkl->base));
    warn_on(__hkl_is_unlocked(&fp_hkl->com_obj->base));

    ret = hi_klad_fp_entry_start(fp_hkl, instance);
    if (ret != HI_SUCCESS) {
        print_err_func(hi_klad_fp_entry_start, ret);
        klad_r_hkl_rkp_restart(&fp_hkl->base, &fp_hkl->base_attr);
    }

    timestamp_tag(&fp_hkl->base, TIMETAMP_START_O);
    __mutex_unlock(&fp_hkl->com_obj->lock);
    __mutex_unlock(&fp_hkl->lock);

    return ret;
}

static hi_s32 klad_r_fp_hkl_route_impl(struct klad_r_fp_hkl *fp_hkl)
{
    hi_s32 ret;
    ...
    ret = hi_klad_fp_entry_route(fp_hkl);
    if (ret != HI_SUCCESS) {
        print_err_func(hi_klad_fp_entry_start, ret);
    }
...
static hi_s32 klad_r_fp_hkl_close_impl(struct klad_r_fp_hkl *fp_hkl)
{
    ...
    if (ret == HI_SUCCESS) {
        fp_hkl->base.rkp_ready = HI_FALSE;
    } else {
        hi_err_klad("Unlock fp keyladder %d failed.\n", fp_hkl->base.hw_id);
    }
...
static hi_s32 klad_r_ta_hkl_rkp_impl(struct klad_r_ta_hkl *ta_hkl)
{
    ...
    if (ta_hkl->base.rkp_ready == HI_TRUE) {
        ret = HI_SUCCESS; /* do nothing~. */
        goto out;
    }

    /* do something: */
    ta_hkl->base.rkp_ready = HI_TRUE;
    ret = HI_SUCCESS;
...
static hi_s32 klad_r_ta_hkl_close_impl(struct klad_r_ta_hkl *ta_hkl)
{
    ...
    if (ret == HI_SUCCESS) {
        ta_hkl->base.rkp_ready = HI_FALSE;
    } else {
...
static hi_s32 klad_r_nonce_hkl_rkp_impl(struct klad_r_nonce_hkl *nonce_hkl)
{
    ...
    if (nonce_hkl->base.rkp_ready == HI_TRUE) {
        ret = HI_SUCCESS; /* do nothing~. */
        goto out;
    }
    /* do something: */
    nonce_hkl->base.rkp_ready = HI_TRUE;
    ret = HI_SUCCESS;
...
static hi_s32 klad_r_nonce_hkl_close_impl(struct klad_r_nonce_hkl *nonce_hkl)
{
    ...
    if (ret == HI_SUCCESS) {
        nonce_hkl->base.rkp_ready = HI_FALSE;
    } else {
...
{
    if (memcpy_s(base_attr, sizeof(struct klad_r_base_attr),
                 &hkl_attr->hkl_base_attr, sizeof(hkl_attr->hkl_base_attr)) != EOK) {
                                              ...
                                              struct klad_r_com_hkl **com_hkl)
{
    ...
    if (memset_s(new_com_hkl, sizeof(struct klad_r_com_hkl), 0, sizeof(*new_com_hkl)) != EOK) {
    ...
    new_com_hkl->base.handle  = HI_HANDLE_MAKEHANDLE(HI_ID_KLAD, HI_KLAD_COM, id);
    new_com_hkl->base.mgmt    = mgmt;
    mutex_init(&new_com_hkl->lock);
                                             ...
                                             struct klad_r_ta_hkl **ta_hkl)
{
    ...
    if (memset_s(new_ta_hkl, sizeof(struct klad_r_ta_hkl), 0, sizeof(*new_ta_hkl)) != EOK) {
        ret = HI_ERR_KLAD_SEC_FAILED;
        goto out;
    }
    ...
    new_ta_hkl->base.handle  = HI_HANDLE_MAKEHANDLE(HI_ID_KLAD, HI_KLAD_TA, 1);
    new_ta_hkl->base.mgmt    = mgmt;

    mutex_init(&new_ta_hkl->lock);
...
static hi_s32 __klad_mgmt_destroy_ta_hkl_impl(struct klad_mgmt *mgmt, struct klad_r_ta_hkl *ta_hkl)
{
    ...
    if (memset_s(ta_hkl, sizeof(struct klad_r_ta_hkl), 0, sizeof(*ta_hkl)) != EOK) {
                                             ...
                                             struct klad_r_fp_hkl **fp_hkl)
{
    ...
    if (memset_s(new_fp_hkl, sizeof(struct klad_r_fp_hkl), 0, sizeof(*new_fp_hkl)) != EOK) {
        ret = HI_ERR_KLAD_SEC_FAILED;
        goto out;
    }
    ...
    new_fp_hkl->base.handle  = HI_HANDLE_MAKEHANDLE(HI_ID_KLAD, HI_KLAD_FP, 1);
    new_fp_hkl->base.mgmt    = mgmt;

    mutex_init(&new_fp_hkl->lock);
    new_fp_hkl->ops = &g_fp_klad_ops;

    new_fp_hkl->com_obj = HI_NULL_PTR;
...
static hi_s32 __klad_mgmt_destroy_fp_hkl_impl(struct klad_mgmt *mgmt, struct klad_r_fp_hkl *fp_hkl)
{
    ...
    if (memset_s(fp_hkl, sizeof(struct klad_r_fp_hkl), 0, sizeof(*fp_hkl)) != EOK) {
                                           ...
                                           struct klad_r_fp_hkl **fp_hkl)
{
    ...
    ret = __klad_mgmt_create_fp_hkl_impl(mgmt, hkl_attr, fp_hkl);
    if (ret != HI_SUCCESS) {
        print_err_func(__klad_mgmt_create_fp_hkl_impl, ret);
        goto out;
    }

    ret = (*fp_hkl)->ops->open(*fp_hkl);
    if (ret != HI_SUCCESS) {
        print_err_func(fp_hkl->ops->open, ret);
        goto out1;
    }
...
out1:
    ...
    if (ret != HI_SUCCESS) {
        print_err_func(__klad_mgmt_destroy_fp_hkl_impl, ret);
    }
out:
    timestamp_tag(&(*fp_hkl)->base, TIMETAMP_CREATE_O);
    timestamp_add(&time_b, &(*fp_hkl)->base, TIMETAMP_CREATE_I);
                                                ...
                                                struct klad_r_nonce_hkl **nonce_hkl)
{
    ...
    if (memset_s(new_nonce_hkl, sizeof(struct klad_r_nonce_hkl), 0, sizeof(*new_nonce_hkl)) != EOK) {
        ret = HI_ERR_KLAD_SEC_FAILED;
        goto out;
    }
    ...
    new_nonce_hkl->base.handle  = HI_HANDLE_MAKEHANDLE(HI_ID_KLAD, HI_KLAD_NONCE, 1);
    new_nonce_hkl->base.mgmt    = mgmt;

    mutex_init(&new_nonce_hkl->lock);
...
static hi_s32 __klad_mgmt_destroy_nonce_hkl_impl(struct klad_mgmt *mgmt, struct klad_r_nonce_hkl *nonce_hkl)
{
    ...
    if (memset_s(nonce_hkl, sizeof(struct klad_r_nonce_hkl), 0, sizeof(*nonce_hkl)) != EOK) {
        hi_dbg_klad("clear ta hkl buffer failed.\n");
        return HI_ERR_KLAD_SEC_FAILED;
    }
                                                ...
                                                struct klad_r_clr_route **clr_route)
{
    ...
    if (memset_s(new_clr_route, sizeof(struct klad_r_clr_route), 0, sizeof(*new_clr_route)) != EOK) {
        ret = HI_ERR_KLAD_SEC_FAILED;
        goto out;
    }
    ...
    new_clr_route->base.handle  = HI_HANDLE_MAKEHANDLE(HI_ID_KLAD, HI_KLAD_CLR, 1);
    new_clr_route->base.mgmt    = mgmt;

    mutex_init(&new_clr_route->lock);
...
static hi_s32 __klad_mgmt_destroy_clr_route_impl(struct klad_mgmt *mgmt, struct klad_r_clr_route *clr_route)
{
    ...
    if (memset_s(clr_route, sizeof(struct klad_r_clr_route), 0, sizeof(*clr_route)) != EOK) {
        hi_dbg_klad("clear ta hkl buffer failed.\n");
        return HI_ERR_KLAD_SEC_FAILED;
    }
...
static hi_void __klad_mgmt_show_info_impl(struct klad_mgmt *mgmt)
{
    ...
    list_for_each(node, &mgmt->com_hkl_head) {
        struct klad_r_com_hkl *com_hkl = list_entry(node, struct klad_r_com_hkl, node);
        com_hkl = com_hkl;
    }
...
static struct klad_mgmt g_klad_mgmt = {
    .lock      = __mutex_initializer(g_klad_mgmt.lock),
    .ref_count = hi_atomic_init(0),
