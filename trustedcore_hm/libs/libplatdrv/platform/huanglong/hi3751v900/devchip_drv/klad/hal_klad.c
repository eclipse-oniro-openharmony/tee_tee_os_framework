...
hi_s32 hkl_wait_idle(void)
{
    ...
    ret = sem_wait(&mgmt->kl_alg_ip.sem);
    if (ret == -1) {
        hi_err_klad("rkp is busy, IRQ : %d, status : 0x%08x\n", HKL_IRQ_NUMBER, hkl_read_reg(HKL_ERROR));
...
{
    hkl_crc crc __attribute__((unused));
    crc.u32 = hkl_read_reg(HKL_CRC);
...
static hi_s32 _hkl_com_lock_wait_idle(hkl_com_lock_info *lock_info)
{
    ...
    ret = sem_wait(&mgmt->kl_com_lock.sem);
...
hi_s32 hkl_generate_fp_enc_key(hi_u8 *klad_enc, hi_u32 len)
{
    hkl_fp_status reg;
    hi_u32 i;
    hi_u32 dout;
    ...
    if (reg.bits.kl_fp_lv1_enc_rdy != 0x01) {
        return HI_FAILURE;
    }
    for (i = 0; i < len; i += sizeof(hi_u32)) {
        dout = hkl_read_reg(HKL_FP_ENC_RSLT_0 + i);
        word_2_char(dout, klad_enc[i], klad_enc[i + 0x1], klad_enc[i + 0x2], klad_enc[i + 0x3]);
    }
    return HI_SUCCESS;
}
...
static hi_s32 _hkl_fp_lock_wait_idle(hkl_fp_lock_info *lock_info)
{
    ...
    ret = sem_wait(&mgmt->kl_fp_lock.sem);
...
static hi_s32 _hkl_ta_lock_wait_idle(hkl_ta_lock_info *lock_info)
{
    ...
    ret = sem_wait(&mgmt->kl_ta_lock.sem);
...
static hi_s32 _hkl_nonce_lock_wait_idle(hkl_nonce_lock_info *lock_info)
{
    ...
    ret = sem_wait(&mgmt->kl_nonce_lock.sem);
...
static hi_s32 _hkl_clrcw_lock_wait_idle(hkl_clr_lock_info *lock_info)
{
    ...
    ret = sem_wait(&mgmt->kl_clr_lock.sem);
    ...
    if (ret == -1) {
        hi_err_klad("timtout %06ld.%09ld s\n", mgmt->sem_time_out.tv_sec, mgmt->sem_time_out.tv_nsec);
        hi_err_klad("begin   %06ld.%09ld s\n", time_b.tv_sec, time_b.tv_nsec);
        hi_err_klad("end     %06ld.%09ld s\n", time_e.tv_sec, time_e.tv_nsec);
...
static hi_s32 _hkl_csgk2_lock_wait_idle(hkl_csgk2_lock_info *lock_info)
{
    ...
    ret = sem_wait(&mgmt->kl_csgk2_lock.sem);
...
hi_void hkl_low_power_en(hi_void)
{
    hkl_power_en reg;

    reg.u32 = 0;
    reg.bits.kl_power_en = 0x05; /* low power enable */

    hkl_write_reg(HKL_POWER_EN, reg.u32);
}
...
hi_u32 get_provision_key(hi_u8 *oemkey, size_t key_size)
{
    (void)oemkey;
    return HI_FAILURE;
}
