...
ESESTATUS phNxpEse_GetData(uint32_t* data_len, uint8_t** pbuffer) {
      ...
      if (ESESTATUS_SUCCESS == phNxpEse_GetDataFromList(&total_data_len, pbuff)) {
        if(total_data_len == total_len){
          ...
          *data_len = total_data_len;
          ...
          HISI_PRINT_ERROR("%s Mismatch of len total_data_len %d total_len %d", __FUNCTION__,
              total_data_len, total_len);
          phNxpEse_free(pbuff);
        }
      } else {
        HISI_PRINT_ERROR("%s phNxpEse_GetDataFromList failed", __FUNCTION__);
        phNxpEse_free(pbuff);
      }
    } else {
      HISI_PRINT_ERROR("%s Error in malloc ", __FUNCTION__);
      status = ESESTATUS_NOT_ENOUGH_MEMORY;
    }
  } else {
    HISI_PRINT_ERROR("%s total_len = %d", __FUNCTION__, total_len);
  }
  ...
  HISI_PRINT_INFO("%s exit status = %d", __FUNCTION__, status);
  return status;
}
