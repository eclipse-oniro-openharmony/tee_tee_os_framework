...
int p73_p61_factory_test(void)
{
    ...
    HISI_PRINT_ERROR("%s: eSE factory test begin ~~~~~~~~\n", __func__);

    p73_scard_connect(1, NULL, NULL);
    ret = p73_scard_transmit(1, test_cmd, sizeof(test_cmd), response, &response_len);
    p73_scard_disconnect(1);
    ...
    for (i = 0; i < response_len; i++) {
        HISI_PRINT_ERROR("%s: i = %u, response[%u] = 0x%x\n", __func__, i, i, response[i]);
    }
...
int p73_scard_connect(int reader_id, void *p_atr, unsigned int *atr_len)
{
    ...
    if (status != ESESTATUS_SUCCESS && ESESTATUS_BUSY != status) {
        HISI_PRINT_ERROR("%s(%d): invalid status(0x%x)!\n", __func__, __LINE__, status);
        goto EndOfTests2;
    }
                ...
                unsigned char *p_rsp, unsigned int *rsp_len)
{
    ...
    (void)reader_id;
    HISI_PRINT_INFO("p73_scard_transmit cmd_len=0x%x, p_cmd[0]=0x%x, p_cmd[1]=0x%x\n", cmd_len, p_cmd[0], p_cmd[1]);
    ...
    HISI_PRINT_INFO("p73_scard_transmit cmdData.len=0x%x, cmdData.p_data[0]=0x%x, cmdData.p_data[1]=0x%x\n",
        cmdData.len, cmdData.p_data[0], cmdData.p_data[1]);
    status = phNxpEse_Transceive(&cmdData, &rspData);
    if (status != ESESTATUS_SUCCESS) {
        HISI_PRINT_ERROR("%s: transmit failed!!!\n", __FUNCTION__);
    }
...
int p73_EseProto7816_Reset()
{
    ...
    HISI_PRINT_INFO("%s %d: exit status %d\n", __func__, __LINE__, status);
    return status;
}
