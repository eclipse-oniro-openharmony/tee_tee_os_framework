...
ESESTATUS phNxpEse_SPM_EnablePwr(void) {
  ...
  ESESTATUS wSpmStatus = ESESTATUS_SUCCESS;
  spm_state_t current_spm_state = SPM_STATE_INVALID;
  /*ALOGD( "%s : phNxpEse_SPM_EnablePwr is set to  = 0x%d",
           __FUNCTION__, 0);*/
...
ESESTATUS phNxpEse_SPM_DisablePwr(void) {
  int32_t ret = -1;
  ESESTATUS status = ESESTATUS_SUCCESS;
  /*ALOGD( "%s : phNxpEse_SPM_DisablePwr is set to  = 0x%d",
           __FUNCTION__, 1);*/
...
ESESTATUS phNxpEse_SPM_SetPwrScheme(long arg) {
  ...
  /*ALOGD( "%s : Power scheme is set to  = 0x%ld",
           __FUNCTION__, arg);*/
...
ESESTATUS phNxpEse_SPM_DisablePwrControl(unsigned long arg) {
  ...
  /*ALOGD( "%s : Inhibit power control is set to  = 0x%ld",
           __FUNCTION__, arg);*/
...
ESESTATUS phNxpEse_SPM_SetJcopDwnldState(long arg) {
  ...
  /*ALOGD( "%s :phNxpEse_SPM_SetJcopDwnldState  = 0x%ld",
           __FUNCTION__, arg);*/
...
ESESTATUS phNxpEse_SPM_GetAccess(long timeout) {
  ESESTATUS status = ESESTATUS_SUCCESS;
  /*ALOGD( "phTmlEse_get_ese_access(), timeout  %ld",
           timeout);*/
  ...
  //ALOGD( "phTmlEse_get_ese_access(), exit  %d", status);
#endif
  return status;
}
/*******************************************************************************
...
#if ((NFC_NXP_ESE_VER == JCOP_VER_3_1) || (NFC_NXP_ESE_VER == JCOP_VER_3_2))
  int ret = -1;
  //ALOGD( "phNxpEse_SPM_RelAccess(): enter");

  ret = phPalEse_ioctl(phPalEse_e_ChipRst, pEseDeviceHandle, 5);
  if (ret < 0) {
    status = ESESTATUS_FAILED;
  }
  //ALOGD( "phNxpEse_SPM_RelAccess(): exit  %d", status);
#endif
  return status;
}
/** @} */
