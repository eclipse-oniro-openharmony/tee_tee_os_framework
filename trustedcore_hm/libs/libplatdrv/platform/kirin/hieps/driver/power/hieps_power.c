...
static void hieps_set_power_status(uint32_t id, hieps_smc_cmd_type cmd)
{
	...
	case DICE:
		...
		break;
	case GP:
		g_hieps_power_status.status.gp = value;
		break;
	case SELF_CTRL:
		g_hieps_power_status.status.self_ctrl = value;
		break;
	case SELF_CTRL_MULTI:
		g_hieps_power_status.status.self_ctrl_muti = value;
		break;
	default:
		tloge("hieps: power status set id error, id is %x!\n", id);
		break;
	}
...
uint32_t hieps_power_on(uint32_t id, uint32_t profile_id)
{
	...
	if (HIEPS_POWEROFF_STATUS == hieps_get_power_status()) {
		...
		/* power on in 0.8v when profile is keep profile */
		...
		/* needn't do dvfs when profile is profile keep */
		if (profile_id == PROFILE_KEEP)
...
uint32_t hieps_power_off(uint32_t id, uint32_t profile_id)
{
	...
	if (profile_id == PROFILE_KEEP)
		profile_id = g_hieps_profile_status.profile_vote[id];
