...
typedef enum {
	...
	SEC_BOOT,
	DICE,
	GP,                   /* for GP interface, used by TA or HUKS SDK */
	SELF_CTRL,            /* for normal interface, used in eps driver */
	SELF_CTRL_MULTI,       /* for cipher and hash update interface, used in eps driver */
	MAX_POWER_ID,
} hieps_power_id;
...
typedef union {
	...
	struct {
		...
		uint32_t dice:4;
		uint32_t gp:4;
		uint32_t self_ctrl:4;
		uint32_t self_ctrl_muti:4;
		uint32_t reserved:8;
	} status;
