...
PRIVATE u32 hisee_video_get_pattern_outlen(u32 dinlen, struct hisee_video_init_param *param)
{
	u32 leftlen = dinlen % param->cipher_blk_size;
	u32 m;

	m = (dinlen - leftlen) / param->cipher_blk_size;
	return (param->cipher_blk_size + param->plain_blk_size) * m + leftlen;
}
					     ...
					     u32 dinlen)
{
	...
	param->cipher_blk_size  = 16;   /* 16byte ciphered data */
	param->plain_blk_size   = 144;  /* 144byte plain data */
	...
	if (video_type == HISEE_VIDEO_TYPE_PATTERN)
		g_video_perf_doutlen = hisee_video_get_pattern_outlen(dinlen, param);
	else
...
err_bsp_t hisee_video_perf_test(u32 video_type, u32 algorithm, u32 dinlen, u32 *timecostus)
{
	...
	ret = hisee_video_dofinal(&ctx, g_video_perf_pdin, g_video_perf_dinlen,
				  g_video_perf_pdout, &doutlen, g_video_perf_pdout);
	if (PAL_CHECK(ret != BSP_RET_OK))
		goto end;
	end = (u32)pal_timer_value();
