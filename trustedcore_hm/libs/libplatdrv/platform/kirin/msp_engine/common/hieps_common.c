...
#ifdef FEATURE_DFT_ENABLE
    ret = mspe_test_callback_init();
    if (ret != HIEPS_OK) {
        tloge("hieps:Intialize msp channel failed!ret=0x%x\n", ret);
        return ret;
    }
#endif
...
#ifdef CONFIG_HISI_MSPE_POWER_SCHEME
static int32_t hieps_suspend(void)
{
    err_bsp_t ret;
    tloge("mspe suspend!\n");

    ret = mspe_power_suspend();
    if (ret != BSP_RET_OK)
        tloge("mspe_power_suspend fail!\n");
