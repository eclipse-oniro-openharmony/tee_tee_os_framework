				       ...
				       struct hisee_video_init_param *vparam)
{
	...
	vparam->cipher_blk_size = sparam->cipher_blk_size;
	vparam->plain_blk_size  = sparam->plain_blk_size;
...
static int hisee_video_syscall_handle(int swi_id, struct drv_param *params, uint64_t permissions)
{
	...
	HANDLE_SYSCALL(swi_id) {
		/* hisee_video_init */
		SYSCALL_PERMISSION(SW_SYSCALL_SEE_VIDEO_INIT, permissions, MSPE_VIDEO_GROUP_PERMISSION)
		...
		/* hisee_video_update */
		SYSCALL_PERMISSION(SW_SYSCALL_SEE_VIDEO_UPDATE, permissions, MSPE_VIDEO_GROUP_PERMISSION)
			/* ctx: map + copy, allow drv read/write */
			ACCESS_CHECK_A64(args[0], sizeof(struct hisee_video_ctx));
			ACCESS_WRITE_RIGHT_CHECK(args[0], sizeof(struct hisee_video_ctx));
			...
			doutlen = *(uint32_t *)(uintptr_t)args[4];
			...
			if (video_type == HISEE_VIDEO_TYPE_PATTERN) {
				...
				ACCESS_CHECK_NOCPY_A64(pdout_va, doutlen);
			}
		...
		/* hisee_video_dofinal */
		SYSCALL_PERMISSION(SW_SYSCALL_SEE_VIDEO_DOFINAL, permissions, MSPE_VIDEO_GROUP_PERMISSION)
			/* ctx: map + copy, allow drv read/write */
			ACCESS_CHECK_A64(args[0], sizeof(struct hisee_video_ctx));
			ACCESS_WRITE_RIGHT_CHECK(args[0], sizeof(struct hisee_video_ctx));
		...
		/* hisee_video_deinit */
		SYSCALL_PERMISSION(SW_SYSCALL_SEE_VIDEO_DEINIT, permissions, MSPE_VIDEO_GROUP_PERMISSION)
			/* ctx: map + copy, allow drv read/write */
			ACCESS_CHECK_A64(args[0], sizeof(struct hisee_video_ctx));
			ACCESS_WRITE_RIGHT_CHECK(args[0], sizeof(struct hisee_video_ctx));
