...
export HI_PROJECT="mspe"
export HI_PLATFORM="teeos"
export HI_TARGET="libseceng"
source "${BUILD_ROOT}/build/build.sh"
...
export OBB_PRODUCT_NAME="${HI_CHIP_NAME}"
export chip_type="${HI_CHIP_TYPE}"
VRL_TOOL_PATH="${ANDROID_TOP_ABS_DIR}/vendor/thirdparty/hisi/vrl_creater_for_local"
make
rm -rf "${VRL_TOOL_PATH}/*trustedcore.img"
cp -rf "output/stage/kirin/trustedcore.img" "${VRL_TOOL_PATH}/"
cd "${VRL_TOOL_PATH}"
chmod +x create_cert.sh
./create_cert.sh trustedcore trustedcore.img "$TARGET_BOARD_PLATFORM"
cd -
cp -rf ${VRL_TOOL_PATH}/*trustedcore.img ./
