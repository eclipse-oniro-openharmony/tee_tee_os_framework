...
#define MSPC_VOTE_STATUS_OFF          0x5555555555555
...
static uint64_t g_mspc_power_on_trace[MSPC_MAX_VOTE_ID];
static uint64_t g_mspc_power_off_trace[MSPC_MAX_VOTE_ID];
...
static int32_t mspc_power_off_ctrl(void)
{
    ...
    time.tval64 = 0;
    time.tval.sec = MSPC_POWER_OFF_DELAY;
...
int32_t mspc_power_off(uint32_t vote_id)
{
    ...
    g_mspc_power_off_trace[vote_id]++;
    (void)pthread_mutex_lock(&g_power_mutex);
...
int32_t mspc_wait_state(uint32_t state, uint32_t timeout)
{
    ...
    while (ready != state) {
        ...
        if (end_ts.tv_sec < start_ts.tv_sec ||
            ...
            tloge("%s:timeout!state:%u, end:%d, start:%d,time:%ds\n",
                  __func__, ready, end_ts.tv_sec, start_ts.tv_sec, timeout);
...
int32_t mspc_power_suspend(void)
{
    ...
    if (g_mspc_timer) {
        /* Stop mspc timer, then power off mspc. */
        ret = __SRE_TimerEventStop(g_mspc_timer);
        if (ret != SRE_OK)
            tloge("%s: stop timer :%d\n", __func__, ret);
...
void mspc_power_status_dump(void)
{
    uint32_t i;
    int32_t state;
    uint32_t atf_status, lpm3_status;
    uint32_t addr;
    uint64_t vote_status;

    vote_status = mspc_get_vote_status();
    state = mspc_check_ready();
    tloge("%s: vote status:%llx\n", __func__, vote_status);
    tloge("mspc state is :%d\n", state);
    for (i = 0; i < MSPC_MAX_VOTE_ID; i++) {
        tloge("mspc: vote id :%d, on count:%llx, off count:%llx!\n",
              i, g_mspc_power_on_trace[i], g_mspc_power_off_trace[i]);
    }
    if (g_share_mem_vir != 0) {
        addr = g_share_mem_vir + MSPC_ATF_VOTE_STATUS_OFFSET;
        atf_status = read32(addr);
        addr = g_share_mem_vir + MSPC_LPMCU_VOTE_STATUS_OFFSET;
        lpm3_status = read32(addr);
        tloge("mspc vote status, atf:0x%x, lpm3:0x%x!\n", atf_status, lpm3_status);
    }
}
