...
s32 bitmap_create(struct bitmap *sbitmap, u32 size, u32 order)
{
	...
	map = SRE_MemAlloc(OS_MID_SYS, OS_MEM_DEFAULT_FSC_PT,
		ALIGN(bits, BITS_PER_LONG) / BITS_PER_CHAR);
	if (!map) {
		tloge("%s:count = %x, unity = %x\n", __func__, bits, order);
		return -1;
	}
...
{
	SRE_MemFree(OS_MID_SYS, sbitmap->map);
}
...
again:
	...
	index = find_next_bit(map, max_bits, start_ibit, ~0UL);
	if (index >= max_bits)
		return -1;
...
void bitmap_clear_ll(struct bitmap *sbitmap, u32 start_ibits, u32 size)
{
	...
	nr_bits = nbits % BITS_PER_LONG;
	if (nr_words) {
		memset_s(&map[sword], nr_words * sizeof(u32), 0UL,
			nr_words * sizeof(u32));
		sword += nr_words;
	}
