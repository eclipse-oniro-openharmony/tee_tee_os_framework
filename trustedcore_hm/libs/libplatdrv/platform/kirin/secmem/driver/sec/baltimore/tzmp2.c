...
#ifdef DDR_CA_RD_PRINT
void ddrc_ca_rd_info_dump(void)
{
	int i;
	u32 ddrc_cfg_ca, ddrc_chg_caeps;
#ifdef CONFIG_HISI_DDR_AUTO_FSGT
	(void)ddr_autofsgt_ctrl(DDR_AUTOFSGT_CA_RD_CFG, DDR_AUTOFSGT_LOGIC_DIS);
#endif
	for (i = 0; i < CHANNEL_MAX_NUM; i++) {
		ddrc_cfg_ca = readl(SOC_DDRC_DMC_DDRC_CFG_CA_ADDR(DDR_REG_DMC(i)));
		ddrc_chg_caeps = readl(SOC_DDRC_DMC_DDRC_CHG_CAEPS_ADDR(DDR_REG_DMC(i)));
		tloge("di %d %x %x %d\n", i, ddrc_cfg_ca, ddrc_chg_caeps, g_ca_rd_cfg_flag);
	}
#ifdef CONFIG_HISI_DDR_AUTO_FSGT
