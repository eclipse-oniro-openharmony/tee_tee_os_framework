...
u32 g_feature_npages[SUB_RGN_MAX_NUM] = {0};
u32 g_sub_rgn_zone[SUB_RGN_ZONE_MAX_NUM] = { SUB_GRN_ZONE_1M, SUB_GRN_ZONE_2M };
struct sglist *g_sglist = NULL;

extern struct sub_rgn_info g_sub_rgn_cfg_table[SUB_RGN_MAX_NUM];
...
static void sglist_init(u32 feature_num, u32 sgment_num)
{
	...
	for (i = 0; i < sgment_num; i++) {
		...
		PRINT_ERROR("%u, phys_addr:0x%llx\n", i, g_sglist->info[i].phys_addr);
	}
}
...
static u32 set_and_check_sec_attri(u32 feature_num)
{
	sglist_init(feature_num, 0x1);
	if (ddr_sec_cfg(g_sglist, g_sub_rgn_cfg_table[feature_num].sec_feature, DDR_SET_SEC) != OK) {
		PRINT_ERROR("[%s] set sec fail\n", __func__);
		return ERROR;
	}
	return OK;
}

static u32 unset_and_check_sec_attri(u32 feature_num)
{
	sglist_init(feature_num, 0x1);
	if (ddr_sec_cfg(g_sglist, g_sub_rgn_cfg_table[feature_num].sec_feature, DDR_UNSET_SEC) != OK) {
		PRINT_ERROR("[%s] unset sec fail\n", __func__);
		return ERROR;
	}
	return OK;
}
...
static int sub_rgn_test_case006(void)
{
	...
	for (i = 0; i < SUB_RGN_ZONE_MAX_NUM; i++) {
		feature_num = get_feature_num_by_gran(g_sub_rgn_zone[i]);
		if (feature_num == ERROR_MAGIC)
			continue;
		ret += set_and_check_sec_attri(feature_num);
		if (ret != OK) {
			PRINT_ERROR("feature num: 0x%x set sec fail!\n", feature_num);
			return ERROR;
		}
	}
	...
	for (i = 0; i < SUB_RGN_ZONE_MAX_NUM; i++) {
		...
		if (feature_num == ERROR_MAGIC)
			continue;
		ret += unset_and_check_sec_attri(feature_num);
		if (ret != OK) {
			PRINT_ERROR("feature num: 0x%x unset sec fail!\n", feature_num);
			return ERROR;
		}
	}
	return OK;
