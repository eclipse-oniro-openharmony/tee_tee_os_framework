...
int ddr_autofsgt_bypass(unsigned int client, unsigned int bypass)
{
	...
	if (bypass) {
		...
		if (g_autofsgt_dis_cnt == 1)
			/* first time */
			writel(bit(DDR_AUTOFSGT_CLIENT_SECURE_OS + WRITE_MASK_SHIFT)| bit(DDR_AUTOFSGT_CLIENT_SECURE_OS),
			       SOC_CRGPERIPH_PERI_COMMON_CTRL1_ADDR(SOC_ACPU_PERI_CRG_BASE_ADDR));
	} else {
		...
		if (g_autofsgt_dis_cnt == 0)
			/* last time */
			writel(bit(DDR_AUTOFSGT_CLIENT_SECURE_OS + WRITE_MASK_SHIFT),
			       SOC_CRGPERIPH_PERI_COMMON_CTRL1_ADDR(SOC_ACPU_PERI_CRG_BASE_ADDR));
	}
