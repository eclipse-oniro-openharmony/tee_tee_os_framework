...
TEE_UUID g_sec_uuid[] = { HIAI_UUID, TUI_UUID, SEC_ISP_UUID, SEC_FACE_UUID, SECBOOT_UUID, EID1_UUID, EID3_UUID, ION_UUID };
...
NORMAL_RGN_CFG g_normal_rgn_cfg_table[NORMAL_RGN_MAX_NUM] = {
	{
		/* DDR_SEC_PGTABLE */
		.sec_feature = DDR_SEC_PGTABLE,
		.region_num = PGTABLE_RGN_NUM,
		.sec_rgn = {
			{	/* ASI_NUM_TYPE_0 */
				.rgn_en = 0x0,
				.attri = RW_FORBID,
			...
			{	/* ASI_NUM_TYPE_3n4 */
				...
				.start_addr = 0x0,
				.end_addr = 0x0,
				.mid_wr = ISP_ALL | (1U << IPP_SUBSYS_CPE_MID),
				.mid_rd = ISP_ALL | (1U << IPP_SUBSYS_CPE_MID),
			},
			{	/* ASI_NUM_TYPE_5 */
				.rgn_en = 0x0,
				.attri = SEC_RD | SEC_WR,
	...
	},
	{
		/* DDR_SEC_3D_FACE2 */
		.sec_feature = DDR_SEC_FACE,
		.region_num = FACE_RGN_NUM_3D_ALGO,
		.sec_rgn = {
			{	/* ASI_NUM_TYPE_0 */
				.rgn_en = 0x0,
				.attri = RW_FORBID,
...
static int ddr_sec_cfg_for_hifi_reset(u64 start_addr, u64 end_addr)
{
	...
	if (ERROR == is_not_covered_by_normal_region(start_addr, end_addr))
	{
...
int kernel_read_only_enable(u64 start_addr, u64 end_addr)
{
	...
	rgn_index = KERNEL_PROTECT_RGN_NUM;
	...
	ret = sec_region_cfg_mask(CPU_ASI_MASK, KERNEL_READ_ONLY_RGN_NUM, &cfg_data);
err_proc:
#if defined(CONFIG_HISI_DDR_AUTO_FSGT)
	(void)ddr_autofsgt_ctrl(DDR_AUTOFSGT_CLIENT_SECURE_OS, DDR_AUTOFSGT_LOGIC_EN);
#endif
...
int ddr_sec_check_for_feature(u64 start_addr, u64 end_addr, enum SEC_FEATURE feature_id){
	...
	switch (feature_id)
	{
		case DDR_SEC_TUI:
			...
			break;
		default:
			PRINT_ERROR("check, error feature, feature_id:0x%x!\n", feature_id);
			break;
	}
...
static int sub_rgn_cfg(SUB_RGN_CFG *sub_pro_cfg, u32 sub_rgn_cfg_addr, u32 sub_rgn_cfg_size, DDR_CFG_TYPE ddr_cfg_type)
{
	...
	/* sub_rgn的排布是从后往前，即配置顺序是sub_rgn3-sub_rgn0 */
	...
	/* 用于业务粒度是1M的地址校验；如果已配置的业务是2M，则sub_bit_length_total要加1 */
...
static int check_uuid(int feature_id)
{
	...
	switch (feature_id) {
		case DDR_SEC_TINY:
			if (cmp_uuid(&g_cur_uuid, &g_sec_uuid[HIAI_UUID_NUM]) != OK &&
				cmp_uuid(&g_cur_uuid, &g_sec_uuid[ION_UUID_NUM]) != OK) {
				ret = ERROR;
			}
			break;
		case DDR_SEC_FACE:
			if (cmp_uuid(&g_cur_uuid, &g_sec_uuid[SEC_ISP_UUID_NUM]) != OK &&
	...
	if (ret != OK) {
		PRINT_ERROR("verify error, %x-%hx-%hx-%llx", g_cur_uuid.timeLow, \
			g_cur_uuid.timeMid, g_cur_uuid.timeHiAndVersion, *(u64 *)(&(g_cur_uuid.clockSeqAndNode[0])));
	}
...
static void update_addr_to_cfg_table(u32 feature_num, u64 segment_start_addr, u64 segment_end_addr, u32 region_en)
{
	u32 asi_type;

	for (asi_type = 0; asi_type < ASI_NUM_TYPE_MAX; asi_type++) {
		g_normal_rgn_cfg_table[feature_num].sec_rgn[asi_type].start_addr = segment_start_addr;
		g_normal_rgn_cfg_table[feature_num].sec_rgn[asi_type].end_addr = segment_end_addr;
		g_normal_rgn_cfg_table[feature_num].sec_rgn[asi_type].rgn_en = region_en;
	}
}

static int normal_rgn_set_sec(NORMAL_RGN_CFG *normal_rgn_cfg)
{
	int ret = OK;
	u32 asi_type;

	u32 rgn_num = normal_rgn_cfg->region_num;

	for (asi_type = 0; asi_type < ASI_NUM_TYPE_MAX; asi_type++) {
		switch (asi_type) {
			case ASI_NUM_TYPE_0:
				ret = sec_region_cfg(ASI_NUM_0, rgn_num, &(normal_rgn_cfg->sec_rgn[asi_type]));
				break;
			case ASI_NUM_TYPE_1n2:
				ret = sec_region_cfg(ASI_NUM_1, rgn_num, &(normal_rgn_cfg->sec_rgn[asi_type]));
				break;
			case ASI_NUM_TYPE_3n4:
				ret = sec_region_cfg(ASI_NUM_3, rgn_num, &(normal_rgn_cfg->sec_rgn[asi_type]));
				break;
			case ASI_NUM_TYPE_5:
				ret = sec_region_cfg(ASI_NUM_5, rgn_num, &(normal_rgn_cfg->sec_rgn[asi_type]));
				break;
			case ASI_NUM_TYPE_6n7:
				ret = sec_region_cfg(ASI_NUM_6, rgn_num, &(normal_rgn_cfg->sec_rgn[asi_type]));
				break;
			case ASI_NUM_TYPE_8n9:
				ret = sec_region_cfg(ASI_NUM_8, rgn_num, &(normal_rgn_cfg->sec_rgn[asi_type]));
				break;
#ifndef WITH_CHIP_CS2
			case ASI_NUM_TYPE_10n11n12n13:
				ret = sec_region_cfg(ASI_NUM_10, rgn_num, &(normal_rgn_cfg->sec_rgn[asi_type]));
				break;
#else
			case ASI_NUM_TYPE_10:
				ret = sec_region_cfg(ASI_NUM_10, rgn_num, &(normal_rgn_cfg->sec_rgn[asi_type]));
				break;
			case ASI_NUM_TYPE_11:
				ret = sec_region_cfg(ASI_NUM_11, rgn_num, &(normal_rgn_cfg->sec_rgn[asi_type]));
				break;
			case ASI_NUM_TYPE_12:
				ret = sec_region_cfg(ASI_NUM_12, rgn_num, &(normal_rgn_cfg->sec_rgn[asi_type]));
				break;
			case ASI_NUM_TYPE_13n14n15n16:
				ret = sec_region_cfg(ASI_NUM_13, rgn_num, &(normal_rgn_cfg->sec_rgn[asi_type]));
				break;
#endif
			default:
				PRINT_ERROR("[%s_%d]:Never be here!\n", __func__, __LINE__);
				return ERROR;
			}
		}
...
static void normal_rgn_clean(NORMAL_RGN_CFG *normal_rgn_cfg)
{
	u32 asi_num;

	/*clean sec region*/
	for (asi_num = 0; asi_num < ASI_NUM_MAX; asi_num++) {
		sec_region_clean(asi_num, normal_rgn_cfg->region_num);
	}
}

static int normal_rgn_check(u32 feature_num, u64 segment_start_addr, u64 segment_end_addr)
{
	u64 feature_start_addr, feature_end_addr;

	feature_start_addr = g_normal_rgn_cfg_table[feature_num].sec_rgn[ASI_NUM_TYPE_0].start_addr;
	feature_end_addr = g_normal_rgn_cfg_table[feature_num].sec_rgn[ASI_NUM_TYPE_0].end_addr;
	if (segment_start_addr >= feature_start_addr && segment_end_addr <= feature_end_addr) {
		return OK;
	}

	PRINT_ERROR("normal_rgn_check fail\n");
	PRINT_DEBUG("[%s]feature_num:0x%x, s_s_addr:0x%llx, s_e_addr:0x%llx\n", __func__, feature_num, segment_start_addr, segment_end_addr);
	return ERROR;
}

static int normal_rgn_feature_cfg(struct sglist *sglist, enum SEC_FEATURE feature_id, DDR_CFG_TYPE ddr_cfg_type)
{
	int ret = OK;
	int feature_num;
	u32 region_en;
	...
	if (sglist->infoLength != 1) {
		PRINT_ERROR("feature_id:0x%x, infoLength is not one\n", feature_id);
		return ERROR;
	}

	/* get feature_num */
	for (feature_num = 0; feature_num < NORMAL_RGN_MAX_NUM; feature_num++) {
		if (feature_id == g_normal_rgn_cfg_table[feature_num].sec_feature) {
			break;
		}
	}

	if (feature_num >= NORMAL_RGN_MAX_NUM) {
		PRINT_ERROR("wrong feature_num: %u\n", feature_num);
		return ERROR;
	}

	segment_start_addr = sglist->info->phys_addr;
	segment_end_addr = segment_start_addr + (u64)sglist->info->npages * (u64)PAGE_SIZE;
	region_en = 0x1;
	PRINT_INFO("segment_start_addr: 0x%llx, segment_end_addr: 0x%llx\n", segment_start_addr, segment_end_addr);
	...
	switch (ddr_cfg_type) {
		case DDR_SET_SEC:
			if (is_not_covered_by_normal_region(segment_start_addr, segment_end_addr) == ERROR) {
				PRINT_ERROR("normal rgn cfg, addr has been covered!\n");
				ret = ERROR;
				goto err_proc;
			}
			update_addr_to_cfg_table(feature_num, segment_start_addr, segment_end_addr, region_en);
			ret = normal_rgn_set_sec(&(g_normal_rgn_cfg_table[feature_num]));
			break;
		case DDR_UNSET_SEC:
			normal_rgn_clean(&(g_normal_rgn_cfg_table[feature_num]));
			update_addr_to_cfg_table(feature_num, 0x0, 0x0, 0x0);
			break;
		case DDR_CHECK_SEC:
			ret = normal_rgn_check(feature_num, segment_start_addr, segment_end_addr);
			break;
		default:
			PRINT_ERROR("wrong ddr_cfg_type :%d\n", ddr_cfg_type);
			ret = ERROR;
...
{
#ifndef CONFIG_PRODUCT_ARMPC
	u32 segment_size, segment_feature_num;
	u64 segment_start_addr, segment_end_addr;
	...
	if (segment_start_addr >= HISI_CMA_START && segment_end_addr <= HISI_CMA_END) {
		...
		PRINT_DEBUG("s_addr:0x%llx, size:0x%x\n", segment_start_addr, segment_size);
		return ERROR;
	}
...
static int face_feature_cfg(struct sglist *sglist, enum SEC_FEATURE feature_id, DDR_CFG_TYPE ddr_cfg_type)
{
	...
	if (sglist->infoLength == 1) {
		segment_info = (TEE_PAGEINFO *)(sglist->info);
		segment_start_addr = segment_info->phys_addr;
		segment_size = segment_info->npages * PAGE_SIZE;
		segment_end_addr = segment_start_addr + segment_size;
		PRINT_INFO("segment_start_addr: 0x%llx, segment_end_addr: 0x%llx\n", segment_start_addr, segment_end_addr);
		...
		if (is_not_covered_by_cma_region(segment_start_addr, segment_end_addr) == OK) {
			/* 3D_FACE use normal rgn */
			return normal_rgn_feature_cfg(sglist, DDR_SEC_FACE, ddr_cfg_type);
		}
	}
...
static int ddr_sec_cfg_internel(struct sglist *sglist, int feature_id, int ddr_cfg_type, int ta_state)
{
	...
	switch (feature_id) {
		...
		case DDR_SEC_SION: /* for old plan */
			...
			break;
		case DDR_SEC_PGTABLE:
			ret = normal_rgn_feature_cfg(sglist, feature_id, ddr_cfg_type);
			break;
		case DDR_DRM_PRO:
...
int ddr_sec_cfg(struct sglist *sglist, int feature_id, int ddr_cfg_type)
{
	...
	(void)feature_id;
	(void)ddr_cfg_type;
	PRINT_ERROR("ddr sec cfg is not support\n");
	return ERROR;
}
#endif
...
static int check_cma_addr(u64 start_addr, u64 end_addr)
{
	...
	if ((start_addr != HISI_CMA_START) && (start_addr != FACE_CMA_START) && (start_addr != TINY_CMA_START)) {
		PRINT_ERROR("wrong start_addr\n");
		return ERROR;
	}

	/* region cfg end_addr may beyond cma end_addr for aligning */
	if ((end_addr < HISI_CMA_END) && (end_addr < FACE_CMA_END) && (end_addr < TINY_CMA_END)) {
...
static int sub_region_init()
{
	...
	for (feature_num = 0; feature_num < SUB_RGN_FEATURE_MAX; feature_num++) {
		rgn_map0.value = readl((u32)SOC_DMSS_ASI_SEC_RGN_MAP0_ADDR(REG_BASE_DMSS, g_sub_rgn_cfg_table[feature_num].region_num, CPU_ASI_NUM));
		rgn_map1.value = readl((u32)SOC_DMSS_ASI_SEC_RGN_MAP1_ADDR(REG_BASE_DMSS, g_sub_rgn_cfg_table[feature_num].region_num, CPU_ASI_NUM));
		cma_start_addr = (u64)(rgn_map0.reg.rgn_base_addr) << SEC_RGN_ADDR_SHIFT;
		cma_end_addr = (((u64)(rgn_map1.reg.rgn_top_addr) << SEC_RGN_ADDR_SHIFT) | SEC_RGN_ADDR_MASK) + 0x1;
...
static int pg_table_cfg(u64 start_addr, u64 end_addr, DDR_CFG_TYPE ddr_cfg_type)
{
	int ret = OK;
	int feature_num;
	...
	/* get feature_num */
	for (feature_num = 0; feature_num < NORMAL_RGN_MAX_NUM; feature_num++) {
		if (g_normal_rgn_cfg_table[feature_num].sec_feature == DDR_SEC_PGTABLE) {
			break;
		}
	}
	...
	switch (ddr_cfg_type) {
		case DDR_SET_SEC:
			if (is_not_covered_by_normal_region(start_addr, end_addr) == ERROR) {
				PRINT_ERROR("normal_rgn cfg, addr has been covered!\n");
				ret = ERROR;
				goto err_proc;
			}
			update_addr_to_cfg_table(feature_num, start_addr, end_addr, 0x1);
			ret = normal_rgn_set_sec(&(g_normal_rgn_cfg_table[feature_num]));
			break;
		case DDR_UNSET_SEC:
			normal_rgn_clean(&(g_normal_rgn_cfg_table[feature_num]));
...
{
	PRINT_INFO("mddrc_sec_clean\n");
	return pg_table_cfg(start_addr, end_addr, DDR_UNSET_SEC);
}

int mddrc_sec_cfg(u64 start_addr, u64 end_addr)
{
	PRINT_INFO("mddrc_sec_cfg\n");
	return pg_table_cfg(start_addr, end_addr, DDR_SET_SEC);
}
