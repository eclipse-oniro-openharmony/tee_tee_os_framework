...
#define DDRC_NUM_MAX (1)
#define DMC_NUM_MAX (2)
...
typedef union
{
    ...
    struct
    {
        unsigned int  oneop_chg_caeps  : 1;  /* bit[0]    : 是否使用硬件一键改写解密使能的功能，使用的话，不需要软件参与反压主通路，软件直接
改写解密使能即可；不使用的话，需要软件先将主通路反压，然后再改写解密使能。
                                                            1'b0: 不使用
                                                            1'b1: 使用 */
