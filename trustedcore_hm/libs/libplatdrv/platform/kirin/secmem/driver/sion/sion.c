...
#ifndef CONFIG_WATCH_DISABLE_SION
...
static int sion_alloc(struct mem_chunk_list *mcl)
{
	...
	if (mcl->protect_id == SEC_TASK_TINY) {
		if (ddr_sec_cfg(sbuf->sg, DDR_SEC_TINY, DDR_SET_SEC))
			tloge("%s, ddr sec cfg failed\n", __func__);
	}
	sbuf_num++;
...
static int sion_free(struct mem_chunk_list *mcl)
{
	...
	if (mcl->protect_id == SEC_TASK_TINY) {
		if (ddr_sec_cfg(sbuf->sg, DDR_SEC_TINY, DDR_UNSET_SEC))
			tloge("%s, ddr sec cfg unset failed\n", __func__);
	}
	free_sec_id(sbuf->id);
	destory_sglist(sbuf->sg);
	sbuf->sg = NULL;
...
{
	if (protect_id >= SEC_TASK_MAX) {
		tloge("Unsupported id: %u\n", protect_id);
		return NULL;
	}

	return sion_dev[protect_id].sdomain;
}

int sion_create_siommu_domain(u32 protect_id, u64 pgtable_addr,
			      u32 pgtable_size)
{
	struct smmu_domain *sdomain = NULL;

	if (protect_id >= SEC_TASK_MAX) {
		tloge("Unsupported protect_id: %u\n", protect_id);
		return -EINVAL;
	}

	if (protect_id == SEC_TASK_SEC) {
		sdomain = siommu_domain_alloc(pgtable_addr, pgtable_size);
	} else {
		tloge("Unsupported protect_id: %u\n", protect_id);
		return -EINVAL;
	}
	...
	if (sdomain == NULL) {
		tloge("siommu init fail, pgtable addr: %llx, size: %u\n",
		       pgtable_addr, pgtable_size);
		return -EINVAL;
	}

	sion_dev[protect_id].sdomain = sdomain;

	return 0;
}

int sion_destroy_siommu_domain(u32 protect_id)
{
	if (protect_id >= SEC_TASK_MAX) {
		tloge("invalid protect id %u\n", protect_id);
		return -EINVAL;
	}

	if (sion_dev[protect_id].sdomain == NULL) {
		tloge("sdomain of protect id %u is null\n", protect_id);
		return -EINVAL;
	}

	siommu_domain_free(sion_dev[protect_id].sdomain);
	sion_dev[protect_id].sdomain = NULL;
...
void sion_show_pte(u32 protect_id, u32 iova, u32 size)
{
	...
	hisi_dump_pgtable(sdomain, iova, size);
}
#endif
...
static int sion_second_syscall(int swi_id, struct drv_param *params, uint64_t permissions)
{
	...
	HANDLE_SYSCALL(swi_id) {
		...
		ACCESS_CHECK_A64(args[0], sion_sg_size((uintptr_t)tmp));
		...
		sion_permission(SW_SYSCALL_SECMEM_CREATE_DOMAIN, permissions)
		ret = (uint32_t)sion_create_siommu_domain((uint32_t)args[0], args[1],
				(uint32_t)args[2]);

		args[0] = ret;
		SYSCALL_END

		sion_permission(SW_SYSCALL_SECMEM_DESTROY_DOMAIN, permissions)
		ret = (uint32_t)sion_destroy_siommu_domain((u32)args[0]);
		args[0] = ret;
		SYSCALL_END
	default:
		return -EINVAL;
