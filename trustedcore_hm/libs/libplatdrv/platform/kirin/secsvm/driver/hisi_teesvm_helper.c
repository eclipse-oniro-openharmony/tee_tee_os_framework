...
{
	(void)memset_s(cmd, CMDQ_ENT_DWORDS << DWORD_BYTES_NUM, 0, CMDQ_ENT_DWORDS << DWORD_BYTES_NUM);
...
void hisi_smmu_group_tlb_inv_context(struct hisi_sec_smmu_domain *cookie)
{
	...
	list_for_each_safe(p, n, &smmu_grp->smmu_list) {
		...
		smmu = list_entry(p, struct hisi_sec_smmu_device, smmu_node);
		if (!smmu || smmu->status != hisi_sec_smmu_enable) {
			tloge("%s smmuid %d is null or disable\n",
...
void hisi_sec_smmu_domain_draft(struct hisi_sec_smmu_domain *smmu_domain)
{
	...
	/* clear cd decs */
	(void)memset_s(cfg->cdptr, CTXDESC_CD_DWORDS << DWORD_BYTES_NUM,
...
{
	(void)memset_s(q->base, q->q_size, 0, q->q_size);
	(void)sre_unmap((unsigned int)(uintptr_t)q->base, q->q_size);
}
...
static void hisi_sec_smmu_free_strtab(struct hisi_sec_smmu_device *smmu)
{
	...
	(void)memset_s(cfg->strtab, cfg->strtab_size, 0, cfg->strtab_size);
	(void)sre_unmap((unsigned int)(uintptr_t)cfg->strtab, cfg->strtab_size);
}
...
static void hisi_evt_irq_thread(void *data)
{
	...
	while (1) {
		...
		if (pthread_mutex_lock(&g_hisi_svmtlb_mutex)) {
			tloge("%s: pthread_mutex_lock fail\n", __func__);
			(void)pthread_mutex_unlock(&smmu->evt_lock);
			return;
		}
...
int hisi_evt_irq_setup(struct hisi_sec_smmu_device *smmu)
{
	...
	ret = pthread_create(&smmu->evt_thread, NULL, (void *)hisi_evt_irq_thread, smmu);
	if (ret) {
...
int hisi_aicpu_intr_addr_remap(void)
{
	...
	if (g_pgfault_asid_addr) {
		...
		if (g_hisi_mmu_dev)
			g_hisi_mmu_dev->asid_mem_base = (void *)(uintptr_t)asid_mem_base;
	}
