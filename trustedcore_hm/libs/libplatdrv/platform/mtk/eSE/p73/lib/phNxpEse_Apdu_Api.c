...
ESESTATUS phNxpEse_7816_Transceive(pphNxpEse_7816_cpdu_t pCmd,
  ...
  if (NULL == pCmd || NULL == pRsp) {
    ...
    tloge(" %s Invalid response buffer \n", __FUNCTION__);
    ...
    if (ESESTATUS_SUCCESS == status) {
      ...
      if (ESESTATUS_SUCCESS != status) {
        tloge(" %s phNxpEse_Transceive Failed \n", __FUNCTION__);
        ...
        if ((pRspTrans.len > 0) && (pRspTrans.p_data != NULL)) {
          ...
          tloge("pRspTrans.len error = %d", pRspTrans.len);
          status = ESESTATUS_FAILED;
        }
      }
      if (pCmd_data != NULL) {
        tloge("Freeing memory pCmd_data");
...
static ESESTATUS phNxpEse_7816_FrameCmd(pphNxpEse_7816_cpdu_t pCmd,
  ...
  if (pCmd->lc > 0) {
    ...
    if (pCmd->pdata == NULL) {
      tloge("%s Invalide data buffer from application ", __FUNCTION__);
  ...
  if (pCmd->le_type > 0) {
    if (pCmd->le_type == 1) {
      ...
      if ((pCmd->le_type == 3) && (lc_len == 0)) {
        ...
        tloge("%s wrong LE type  %d", __FUNCTION__, pCmd->le_type);
        cmd_total_len += pCmd->le_type;
        le_len = pCmd->le_type;
      }
    } else {
      tloge("%s wrong cpdu_type value %d", __FUNCTION__, pCmd->cpdu_type);
  ...
  if (pbuff == NULL) {
    tloge("%s Error allocating memory", __FUNCTION__);
