...
ESESTATUS phNxpEse_SetEndPoint_Cntxt(uint8_t uEndPoint)
{
  ...
  /*if (GET_CHIP_OS_VERSION() != OS_VERSION_4_0) {
    ...
    tloge( "%s: Enpoint=%d", __FUNCTION__, uEndPoint);
  } else {
    tloge("%s- Function not supported", __FUNCTION__);
  } */
  return status;
}
...
//      (EseConfig_getUnsigned(NAME_SE_DEBUG_ENABLED, 0) != 0) ? true : false;
  ...
  tloge( "%s: level=%u", __func__, ese_debug_enabled);
}
...
ESESTATUS phNxpEse_init(phNxpEse_initParams initParams) {
  ...
  if (EseConfig_hasKey(NAME_NXP_WTX_COUNT_VALUE)) {
    ...
    tloge( "Wtx_counter read from config file - %lu",
  ...
  if (EseConfig_hasKey(NAME_RNACK_RETRY_DELAY)) {
    ...
    tloge( "Rnack retry_delay read from config file - %lu",
             num);
  } else {
  ...
  if (ESESTATUS_SUCCESS == wConfigStatus) {
    tloge( "phNxpEseProto7816_Open completed >>>>>");
    ...
    tloge("phNxpEseProto7816_Open failed with status = %x", wConfigStatus);
  }
...
ESESTATUS phNxpEse_open(phNxpEse_initParams initParams) {
  ...
  tloge( "MW SEAccessKit Version");
  tloge( "Android Version:0x%x", NXP_ANDROID_VER);
  tloge( "Major Version:0x%x", ESELIB_MW_VERSION_MAJ);
  tloge( "Minor Version:0x%x", ESELIB_MW_VERSION_MIN);
  ...
  if (EseConfig_hasKey(NAME_NXP_POWER_SCHEME)) {
    ...
    tloge( "Power scheme read from config file - %lu",
             num);
  } else {
    nxpese_ctxt.pwr_scheme = PN67T_POWER_SCHEME;
    tloge( "Power scheme not defined in config file - %lu",
             num);
  }
#else
  nxpese_ctxt.pwr_scheme = PN67T_POWER_SCHEME;
  ...
  tloge( "Nad poll retry time in us - %lu us",
  ...
  if (GET_CHIP_OS_VERSION() == OS_VERSION_4_0) {
    ...
    if (wConfigStatus != ESESTATUS_SUCCESS) {
      tloge("Failed to open SPI due to VEN pin used by FW download \n");
      wConfigStatus = ESESTATUS_FAILED;
      goto clean_and_return_1;
    }
  }
  ...
  if (GET_CHIP_OS_VERSION() == OS_VERSION_4_0) {
    ...
    if (ESE_MODE_OSU == nxpese_ctxt.initParams.initMode) {
      tloge( "%s Init mode ---->OSU", __FUNCTION__);
      wConfigStatus = phNxpEse_checkJcopDwnldState();
      if (wConfigStatus != ESESTATUS_SUCCESS) {
        tloge("phNxpEse_checkJcopDwnldState failed");
        goto clean_and_return_1;
      }
    }
  }
  ...
  if (wSpmStatus != ESESTATUS_SUCCESS) {
    ...
    tloge( "nxpese_ctxt.spm_power_state true");
...
ESESTATUS phNxpEse_openPrioSession(phNxpEse_initParams initParams) {
  ...
  /* initialize trace level */
  phNxpLog_InitializeLogLevel();
  tloge( "phNxpEse_openPrioSession Enter");
#ifdef SPM_INTEGRATED
  ESESTATUS wSpmStatus = ESESTATUS_SUCCESS;
  spm_state_t current_spm_state = SPM_STATE_INVALID;
#endif
  ...
  if (wSpmStatus != ESESTATUS_SUCCESS) {
    tloge("phNxpEse_SPM_ConfigPwr: enabling power for spi prio Failed");
    if (wSpmStatus == ESESTATUS_BUSY) {
      wConfigStatus = ESESTATUS_BUSY;
    } else if (wSpmStatus == ESESTATUS_DWNLD_BUSY) {
      wConfigStatus = ESESTATUS_DWNLD_BUSY;
...
static ESESTATUS phNxpEse_setJcopDwnldState(phNxpEse_JcopDwnldState state) {
  ...
  ESESTATUS wConfigStatus = ESESTATUS_FAILED;
  tloge( "phNxpEse_setJcopDwnldState Enter");
 ...
 ******************************************************************************/
static ESESTATUS phNxpEse_checkJcopDwnldState(void) {
  tloge( "phNxpEse_checkJcopDwnld Enter");
  ...
  if (wSpmStatus == ESESTATUS_SUCCESS) {
    ...
    if (status == ESESTATUS_SUCCESS) {
      while (ese_dwnld_retry < ESE_JCOP_OS_DWNLD_RETRY_CNT) {
        tloge( "ESE_JCOP_OS_DWNLD_RETRY_CNT retry count");
        wSpmStatus = phNxpEse_SPM_GetState(&current_spm_state);
        if (wSpmStatus == ESESTATUS_SUCCESS) {
          if ((current_spm_state & SPM_STATE_JCOP_DWNLD)) {
  ...
  tloge( "phNxpEse_checkJcopDwnldState status %x", status);
  return status;
  #endif
}
...
ESESTATUS phNxpEse_Transceive(phNxpEse_data* pCmd, phNxpEse_data* pRsp) {
  ...
  if ((pCmd->len == 0) || pCmd->p_data == NULL) {
    tloge(" phNxpEse_Transceive - Invalid Parameter no data\n");
    return ESESTATUS_INVALID_PARAMETER;
  } else if (pCmd->len > MAX_SUPPORTED_DATA_SIZE) {
    tloge(" phNxpEse_Transceive - Invalid data size \n");
    ...
    if (ESESTATUS_SUCCESS != status) {
      tloge(" %s phNxpEseProto7816_Transceive- Failed \n", __FUNCTION__);
...
ESESTATUS phNxpEse_coldReset(void) {
  ...
  /*if (GET_CHIP_OS_VERSION() != OS_VERSION_4_0) {
    ...
    tloge(" %s Function not supported \n", __FUNCTION__);
  }*/
  tloge( " %s Exit status 0x%x \n", __FUNCTION__,
           wSpmStatus);
  return wSpmStatus;
...
ESESTATUS phNxpEse_reset(void) {
  ...
  status = phNxpEseProto7816_IntfReset(
      (phNxpEseProto7816SecureTimer_t*)&nxpese_ctxt.secureTimerParams);
  if (!status) {
    tloge("%s Ese status Failed", __FUNCTION__);
  }
...
ESESTATUS phNxpEse_resetJcopUpdate(void) {
  ...
  if (EseConfig_hasKey(NAME_NXP_POWER_SCHEME)) {
    ...
    if ((num == 1) || (num == 2)) {
      ...
      if (status != ESESTATUS_SUCCESS) {
        tloge("phNxpEse_resetJcopUpdate: reset Failed");
        status = ESESTATUS_FAILED;
      }
    } else if (num == 3) {
      ...
      if (status != ESESTATUS_SUCCESS) {
        tloge("phNxpEse_resetJcopUpdate: chip reset Failed");
        status = ESESTATUS_FAILED;
      }
    } else {
  ...
  status = phPalEse_ioctl(phPalEse_e_ResetDevice, nxpese_ctxt.pDevHandle, 2);
  if (status != ESESTATUS_SUCCESS) {
    tloge("phNxpEse_resetJcopUpdate Failed");
  }
#endif
...
ESESTATUS phNxpEse_chipReset(void) {
  ...
  if (nxpese_ctxt.pwr_scheme == PN80T_EXT_PMU_SCHEME) {
    ...
    if (!bStatus) {
      tloge("Inside phNxpEse_chipReset, phNxpEseProto7816_Reset Failed");
    }
    status = phPalEse_ioctl(phPalEse_e_ChipRst, nxpese_ctxt.pDevHandle, 6);
    if (status != ESESTATUS_SUCCESS) {
      tloge("phNxpEse_chipReset  Failed");
    }
  } else {
    tloge(
  ...
  else
  {
    ...
    if (status == ESESTATUS_SUCCESS) {
      ...
      if (GET_CHIP_OS_VERSION() == OS_VERSION_4_0) {
        status = phNxpEse_SPM_DisablePwrControl(maxTimer);
        if (status != ESESTATUS_SUCCESS) {
          tloge("%s phNxpEseP61_DisablePwrCntrl: failed", __FUNCTION__);
        }
      }
#endif
    }
...
ESESTATUS phNxpEse_close(ESESTATUS deInitStatus) {
  ...
  if (GET_CHIP_OS_VERSION() == OS_VERSION_4_0) {
    if (ESE_MODE_OSU == nxpese_ctxt.initParams.initMode) {
      ...
      if (status != ESESTATUS_SUCCESS) {
        tloge("%s: phNxpEse_setJcopDwnldState failed", __FUNCTION__);
      }
    }
  } else {
    if (NULL != nxpese_ctxt.pDevHandle) {
      if (ESE_PROTOCOL_MEDIA_SPI == nxpese_ctxt.initParams.mediaType) {
        tloge( "Inform eSE that trusted Mode is over");
      ...
      if (nxpese_ctxt.EseLibStatus == ESE_STATUS_RECOVERY ||
        ...
        tloge( "eSE not responding perform hard reset");
...
static int phNxpEse_readPacket(void* pDevHandle, uint8_t* pBuffer, int nNbBytesToRead)
{
    ...
    tloge("read() max_sof_counter: %X ESE_POLL_TIMEOUT %2X",
    ...
    do {
        ...
        if ((pBuffer[0] == nxpese_ctxt.nadInfo.nadRx) ||
            ...
            tloge("%s Read HDR SOF + PCB, pBuffer[0] = 0x%x, pBuffer[1] = 0x%x",
            ...
            tloge("%s Read HDR only SOF", __FUNCTION__);
            pBuffer[0] = pBuffer[1];
    ...
    if ((pBuffer[0] == nxpese_ctxt.nadInfo.nadRx) ||
        (pBuffer[0] == RECIEVE_PACKET_SOF)) {
        tloge("%s SOF FOUND, headerIndex = 0x%x, numBytesToRead = 0x%x", __FUNCTION__,
        ...
        if ((1 + headerIndex > MAX_DATA_LEN) || (1 + headerIndex + numBytesToRead > MAX_DATA_LEN)) {
            ret = ESESTATUS_IOCTL_FAILED;
            tloge("%s buffer is overflow", __FUNCTION__, ret);
            return ret;
        }
        ...
        if ((pBuffer[1] == CHAINED_PACKET_WITHOUTSEQN) ||
            ...
            tloge("poll_sof_chained_delay value is %d ", poll_sof_chained_delay);
        } else {
        ...
        tloge("%s pBuffer[PH_PROPTO_7816_PCB_OFFSET] = 0x%x", __FUNCTION__,
        ...
        if (memcpy_s(&pcb_bits, sizeof(pcb_bits), &pcb, sizeof(uint8_t)) != EOK) {
            ret = ESESTATUS_IOCTL_FAILED;
            tloge("%s memcpy_s is fail", __FUNCTION__, ret);
            return ret;
        }

        /* For I-Frame Only */
        tloge("%s pcb_bits.msb=0x%x, pBuffer[2] = 0x%x", __FUNCTION__, pcb_bits.msb, pBuffer[2]);
        if (0 == pcb_bits.msb) {
        /* length is one or two bytes by read pBuffer[2] */
            if (pBuffer[2] != EXTENDED_FRAME_MARKER) {
                ...
                tloge("_spi_read() %d, pBuffer[3] = 0x%x, pBuffer[4] = 0x%x, total_count = 0x%x",
        ...
        if (ret < 0) {
            ...
            if ((pcb_bits.msb == 0) && ((nNbBytesToRead == 0) ||
                ...
                tloge("I-Frame with invalid len == %d", nNbBytesToRead);
                pBuffer[0] = 0x90;
                pBuffer[1] = RECIEVE_PACKET_SOF;
                ret = 0x02;
            }
        ...
        tloge("_spi_read() corrupted, IFSD size=%d flushing it out!!", ifsd_size);
        ...
        if (ret < 0) {
            ...
            tloge("_spi_read() SUCCESS  ret : %X LRC fail excpected for this frame", ret);
    ...
    tloge("%s Exit ret = %d", __FUNCTION__, ret);
    return ret;
}
...
static void phNxpEse_GetMaxTimer(unsigned long* pMaxTimer) {
  ...
  *pMaxTimer +=
  ...
  tloge( "%s Max timer value = %lu", __FUNCTION__,
           *pMaxTimer);
  return;
}
...
ESESTATUS phNxpEse_DisablePwrCntrl(void) {
  ...
  phNxpEse_GetMaxTimer(&maxTimer);
  {
    tloge("%s phNxpEseP61_DisablePwrCntrl: not supported", __FUNCTION__);
    status = ESESTATUS_FAILED;
  }
  return status;
}
 ...
 ******************************************************************************/
static ESESTATUS phNxpEse_checkFWDwnldStatus(void) {
  tloge( "phNxpEse_checkFWDwnldStatus Enter");
  return ESESTATUS_SUCCESS;
}
...
ESESTATUS phNxpEse_GetEseStatus(phNxpEse_data* timer_buffer) {
 ...
 /* if (GET_CHIP_OS_VERSION() != OS_VERSION_4_0) {
  ...
  tloge( "%s Exit status = 0x%x", __FUNCTION__, status);
  return status;
}
    ...
    for (count = 0; count < 4; count++) {
      if (timer_buffer != NULL) {
        *timer_buffer = (value >> (shift * 8) & mask);
        tloge( "*timer_buffer=0x%x shift=0x%x",
                *timer_buffer, shift);
        timer_buffer++;
        shift--;
      } else {
