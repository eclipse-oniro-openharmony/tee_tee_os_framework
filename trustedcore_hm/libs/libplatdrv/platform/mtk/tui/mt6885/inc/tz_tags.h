...
#define	__packed	__attribute__((__packed__))  /* todo */
...
#ifdef MTK_RPMB_LAYOUT
	unsigned int rpmb_layout[12];
#endif
...
static inline struct tz_tag *find_tag(unsigned long start_addr,
	...
	struct tz_tag *tag = (struct tz_tag *)(uintptr_t)start_addr;

	if (IS_NULL_TAG(tag))
		return (struct tz_tag *)(uintptr_t)NULL;
	...
	do {
		...
		if (OUT_OF_TAG_SEARCH_RANGE(tag, (uintptr_t)start_addr))
			return (struct tz_tag *)(uintptr_t)NULL;
