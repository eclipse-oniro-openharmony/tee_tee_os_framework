...
#define DRAPI_ERROR_MAJOR(a) (uint32_t)(a)
#define DRAPI_ERROR_DETAIL(a) (uint32_t)(a)

int32_t cmdq_tz_map_physical_addr_by_flag(uint8_t **va, const uint64_t pa, const uint32_t length, uint32_t flag)
{
    drApiResult_t drRes = (drApiResult_t)dr_api_map_physical_buffer(pa, length, flag, (void **)va);

    if (drRes != DRAPI_OK)
        return -(CMDQ_REC_ERR_NOMEM);
    else
        CMDQ_ERR("%s map physical buffer fail\n", __func__);
...
{
	const uint32_t flag = (MAP_READABLE | MAP_WRITABLE | MAP_UNCACHED);

    return (int32_t)dr_api_map_physical_buffer(pa, length, flag, va);
}

int32_t cmdq_tz_unmap_physical_addr(const uint8_t *va, const uint32_t length)
{
    const drApiResult_t status = (drApiResult_t)dr_api_unmap_buffer((void *)va, length);

	if (DRAPI_OK != status) {
        CMDQ_ERR("unmap physical addr fail\n");
		return -(CMDQ_REC_ERR_NOMEM);
	}
...
int32_t cmdq_tz_init_module_base_VA(void)
{
    ...
    gCmdqRegBaseVA = GCE_BASE_PA; /* error addr 0x10228000, 0x1000, 0x10210000- dx 0x100000- size */
...
void cmdq_tz_dump_thread(uint32_t thread, uint32_t line)
{
	...
	value[0] = CMDQ_ADDR_REVERT(CMDQ_REG_GET32(CMDQ_THR_CURR_ADDR(thread)));
	value[1] = CMDQ_ADDR_REVERT(CMDQ_REG_GET32(CMDQ_THR_END_ADDR(thread)));
					...
					uint32_t insts[4])
{
	...
	currPC = CMDQ_ADDR_REVERT(CMDQ_AREG_TO_PHYS(CMDQ_REG_GET32(
...
void cmdq_tz_dump_disp_trigger_loop(const char *tag)
{
	...
	trigerCurr = CMDQ_ADDR_REVERT(CMDQ_REG_GET32(CMDQ_THR_CURR_ADDR(
		CMDQ_MAX_HIGH_PRIORITY_THREAD_COUNT)));
	trigerEnd = CMDQ_ADDR_REVERT(CMDQ_REG_GET32(CMDQ_THR_END_ADDR(
		CMDQ_MAX_HIGH_PRIORITY_THREAD_COUNT)));
...
int32_t cmdq_tz_insert_task_to_thread_multiple_tasks_impl(TaskStruct *pTask, int32_t thread)
{
	...
	threadPC = CMDQ_ADDR_REVERT(CMDQ_REG_GET32(CMDQ_THR_CURR_ADDR(thread)));
	...
	curr_pc = CMDQ_ADDR_REVERT(CMDQ_REG_GET32(CMDQ_THR_END_ADDR(thread)));
	if ((threadPC == (curr_pc - 8)) ||	/* PC = END - 8, EOC is executed, PC locates JUMP */
		 (threadPC == curr_pc)) {	/* PC = END - 0, All CMDs are executed */

		CMDQ_MSG("EXEC: Set HW thread(%d) pc:%pa\n", thread, &pTask->MVABase);
		CMDQ_REG_SET32(CMDQ_THR_CURR_ADDR(thread), CMDQ_ADDR_SHIFT(pTask->MVABase));
	}
	else
	{
		...
		pPrevTask->pCMDEnd[-1] = CMDQ_ADDR_SHIFT(pTask->MVABase);	/* Jump to here */
	}

	/* extent thread's end addr */
	CMDQ_REG_SET32(CMDQ_THR_END_ADDR(thread),
		CMDQ_ADDR_SHIFT(pTask->MVABase + pTask->commandSize));
...
int32_t cmdq_tz_exec_task_async(TaskStruct *pTask, int32_t thread)
{
	...
	if (pTask->resetExecCnt) {
		...
		CMDQ_REG_SET32(CMDQ_THR_CURR_ADDR(thread), CMDQ_ADDR_SHIFT(pTask->MVABase));
		CMDQ_REG_SET32(CMDQ_THR_END_ADDR(thread), CMDQ_ADDR_SHIFT(pTask->MVABase + pTask->commandSize));
...
int32_t cmdqTzCancelTask(iwcCmdqCancelTask_t *pCancelTask)
{
	...
	do {
		...
		throwAEE = true;
		/* get PC and err intr */
		pc = CMDQ_ADDR_REVERT(CMDQ_REG_GET32(CMDQ_THR_CURR_ADDR(thread)));
		pcVA = cmdq_tz_get_pc(pTask, thread, insts);
		/* dump error */
		cmdq_tz_attach_error_task(pTask, thread);
