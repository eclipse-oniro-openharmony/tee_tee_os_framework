...
int32_t cmdq_rec_initialize_cmd_buffer(void)
{
    ...
    if ((result = (int32_t)dr_api_map_physical_buffer((uint64_t)TEE_PARAMETER_BASE,
	...
	if (IS_TAG_FMT((struct tz_tag*)(sec_mem_tag_ptr))) {
        ...
        //ptr = malloc_coherent(60 * 1024);

		uint64_t phy_addr = get_spi_dma_addr();
		//malloc_coherent(64K);
		sec_mem_arg->cmdq_mem_start = phy_addr + 2 * 1024 * 1024; /* teeos last addr */
		sec_mem_arg->cmdq_mem_size  = 1 * 1024 * 1024;  /* size is 1M */
    ...
    gCmdBufferBaseVA = ptr;
	status = cmdq_tz_map_physical_addr_by_flag(&gCmdBufferBaseVA, (uint64_t)gCmdBufferBasePA,
		gCmdqBufferShareSize, MAP_READABLE | MAP_WRITABLE | MAP_UNCACHED);
	CMDQ_LOG("legacy flow --> gCmdBufferBaseVA:0x%x\n", gCmdBufferBaseVA);
