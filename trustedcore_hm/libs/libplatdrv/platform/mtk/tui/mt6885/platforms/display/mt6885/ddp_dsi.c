...
void ddp_get_ds0_size(void)
{
    ...
    dsi_size = INREG32(DISPSYS_DSI0_BASE + 0x0038);

    int32_t x_res = dsi_size & 0x7fff;
    int32_t y_res = (dsi_size >> 16) & 0x7fff;

    if ((DISP_REG_GET(DISPSYS_CONFIG_BASE + 0xF48) == 0) &&
        (DISP_REG_GET(DISPSYS_CONFIG_BASE + 0xF80) == 1) &&
        (DISP_REG_GET(DISPSYS_CONFIG_BASE + 0xFF8) == 1) &&
        ((DISP_REG_GET(DISPSYS_CONFIG_BASE + 0xF50) & 0x8) == 8) &&
        (DISP_REG_GET(DISPSYS_CONFIG_BASE + 0xF54) == 3))
        x_res *= 3;

    tloge("ddp_get_ds0_size 0x%x, 0x%x\n", x_res, y_res);
    set_x_res(x_res);
    set_y_res(y_res);
}
