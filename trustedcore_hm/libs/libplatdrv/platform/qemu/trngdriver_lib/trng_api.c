...
int trng_driver_syscall(int swi_id, struct drv_param *params, uint64_t ull_permissions)
{
    ...
    HANDLE_SYSCALL(swi_id) {
        SYSCALL_PERMISSION(SW_SYSCALL_TRNG_GENERATE_RANDOM, ull_permissions, CC_RNG_GROUP_PERMISSION)
            tmp_addr = GET_64BIT_ADDR(args[1], args[0]);
            ACCESS_CHECK_A64(tmp_addr, args[2]);
            ACCESS_WRITE_RIGHT_CHECK(tmp_addr, args[2]);
            ret = trng_get_random((uint8_t *)(uintptr_t)tmp_addr, args[2]);
            args[0] = (uint32_t)ret;
        SYSCALL_END;
