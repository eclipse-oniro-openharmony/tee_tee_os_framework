/*
 * Copyright (c) Huawei Technologies Co., Ltd. 2019-2020. All rights reserved.
 * Description: keymaster default cert
 * Create: 2019-05-04
 */

#include "keymaster_defs.h"
#include "securec.h"
#include "tee_crypto_api.h"
#include "km_attest_default_certs.h"
#include "km_attest_factory.h"
#include "crypto_wrapper.h"
/*
 * The following certificate related parameters are valid when there is no pre-made
 * Google certificate. For devices upgrading N, the key certificate has not been
 * pre-installed on the production line and the default key certificate is required
 * using the untrusted keys/certificates of google softkeymaster. Google AOSP code
 * soft-keymaster implementation in hard-coded untrusted keys/cerificates, you can
 * get it at will. In accordance with google's request, Keymaster TA cites this
 * untrusted keys/cerificates to ensure the functional integrity of key testation.
 * Google CA does not trust the certificate chain signed by trusted keys/cerificates,
 * authentication failure keys cannot be used in the secure business scenario. The
 * parameters are attest key, attest cert, attest root cert for ras and ec.
 */
static uint8_t g_krsa_attest_cert[] = {
    ,
};

static uint8_t g_krsa_attest_rootcert[] = {
    ,
};

static uint8_t g_kec_attest_cert[] = {
    ,
};

static uint8_t g_kec_attest_rootcert[] = {
    ,
};

static uint8_t g_kec_attest_key[] = {
    0x30, 0x77, 0x02, 0x01, 0x01, 0x04, 0x20, 0x21, 0xe0, 0x86, 0x43, 0x2a, 0x15, 0x19, 0x84, 0x59,
    0xcf, 0x36, 0x3a, 0x50, 0xfc, 0x14, 0xc9, 0xda, 0xad, 0xf9, 0x35, 0xf5, 0x27, 0xc2, 0xdf, 0xd7,
    0x1e, 0x4d, 0x6d, 0xbc, 0x42, 0xe5, 0x44, 0xa0, 0x0a, 0x06, 0x08, 0x2a, 0x86, 0x48, 0xce, 0x3d,
    0x03, 0x01, 0x07, 0xa1, 0x44, 0x03, 0x42, 0x00, 0x04, 0xeb, 0x9e, 0x79, 0xf8, 0x42, 0x63, 0x59,
    0xac, 0xcb, 0x2a, 0x91, 0x4c, 0x89, 0x86, 0xcc, 0x70, 0xad, 0x90, 0x66, 0x93, 0x82, 0xa9, 0x73,
    0x26, 0x13, 0xfe, 0xac, 0xcb, 0xf8, 0x21, 0x27, 0x4c, 0x21, 0x74, 0x97, 0x4a, 0x2a, 0xfe, 0xa5,
    0xb9, 0x4d, 0x7f, 0x66, 0xd4, 0xe0, 0x65, 0x10, 0x66, 0x35, 0xbc, 0x53, 0xb7, 0xa0, 0xa3, 0xa6,
    0x71, 0x58, 0x3e, 0xdb, 0x3e, 0x11, 0xae, 0x10, 0x14,
};

static uint8_t g_krsaattestkey[] = {
    ,
};

int32_t get_default_attest_key(keymaster_algorithm_t algorithm, void *attest_key)
{
    int ret;
    if (algorithm == KM_ALGORITHM_EC) {
        ret = ecc_import_priv((ecc_priv_key_t *)attest_key, g_kec_attest_key, sizeof(g_kec_attest_key));
        if (ret < 0) {
            tloge("ecc_import_priv error\n");
            return -1;
        }
    } else {
        ret = rsa_import_priv((rsa_priv_key_t *)attest_key, g_krsaattestkey, sizeof(g_krsaattestkey));
        if (ret < 0) {
            tloge("rsa_import_priv error\n");
            return -1;;
        }
    }
    return 0;
}
static void assignment_untrusted_chain(int alg, keymaster_cert_chain_t *untrusted_chain)
{
    if (alg == ALG_EC) {
        untrusted_chain->entries[0].data_addr = g_kec_attest_cert;
        untrusted_chain->entries[0].data_length = sizeof(g_kec_attest_cert);
        untrusted_chain->entries[1].data_addr = g_kec_attest_rootcert;
        untrusted_chain->entries[1].data_length = sizeof(g_kec_attest_rootcert);
    } else {
        untrusted_chain->entries[0].data_addr = g_krsa_attest_cert;
        untrusted_chain->entries[0].data_length = sizeof(g_krsa_attest_cert);
        untrusted_chain->entries[1].data_addr = g_krsa_attest_rootcert;
        untrusted_chain->entries[1].data_length = sizeof(g_krsa_attest_rootcert);
    }
}
int32_t format_untrusted_chain(uint8_t *chain, uint32_t *out_len, int32_t src, int32_t alg)
{
    tlogd("format_untrusted_chain begin\n");
    bool check = (chain == NULL) || (out_len == NULL) || (*out_len < sizeof(uint32_t)) ||
        (src != SRC_GOOGLE) || ((alg != ALG_EC) && (alg != ALG_RSA));
    if (check) {
        tloge("invalid input params!\n");
        return TEE_ERROR_BAD_PARAMETERS;
    }
    uint32_t i;
    uint32_t chain_len_out = 0;
    uint8_t *tmp = chain;
    /* format chain body; */
    *(uint32_t *)tmp = UNTRUSTED_CERTS_COUNT;
    tmp += sizeof(uint32_t);
    chain_len_out += sizeof(uint32_t);
    keymaster_cert_chain_t untrusted_chain;
    untrusted_chain.entries = TEE_Malloc(UNTRUSTED_CERTS_COUNT * sizeof(keymaster_blob_t), TEE_MALLOC_FILL_ZERO);
    if (untrusted_chain.entries == NULL) {
        tloge("untrusted_chain entries malloc failed\n");
        return TEE_ERROR_OUT_OF_MEMORY;
    }
    assignment_untrusted_chain(alg, &untrusted_chain);
    keymaster_blob_t cert_entry;
    uint32_t next_entry_len;
    for (i = 0; i < UNTRUSTED_CERTS_COUNT; i++) {
        cert_entry     = untrusted_chain.entries[i];
        next_entry_len = sizeof(uint32_t) + cert_entry.data_length;
        check = (((UINT32_MAX - chain_len_out) < next_entry_len) || ((chain_len_out + next_entry_len) > *out_len));
        if (check) {
            tloge("format chain out error: out buf len is invalid[%u]\n", *out_len);
            TEE_Free(untrusted_chain.entries);
            return AT_CHAIN_OUT_ERR;
        }
        *(uint32_t *)tmp = cert_entry.data_length;
        tmp += sizeof(uint32_t);
        chain_len_out += sizeof(uint32_t);
        if (memcpy_s(tmp, *out_len - chain_len_out, cert_entry.data_addr, cert_entry.data_length) != EOK) {
            tloge("[error]memcpy_s failed\n");
            TEE_Free(untrusted_chain.entries);
            return AT_CHAIN_OUT_ERR;
        }
        tmp += cert_entry.data_length;
        chain_len_out += cert_entry.data_length;
    }
    *out_len = chain_len_out;
    TEE_Free(untrusted_chain.entries);
    tlogd("format untrusted chain success outlen=%u\n", chain_len_out);
    return 0;
}

int32_t device_cert(int alg, keymaster_blob_t *cert_entry)
{
    if (cert_entry == NULL)
        return -1;
    if (alg == ALG_EC) {
        cert_entry->data_addr = g_kec_attest_cert;
        cert_entry->data_length = sizeof(g_kec_attest_cert);
    } else if (alg == ALG_RSA) {
        cert_entry->data_addr = g_krsa_attest_cert;
        cert_entry->data_length = sizeof(g_krsa_attest_cert);
    } else {
        tloge("invalid alg\n");
        return -1;
    }
    return 0;
}
