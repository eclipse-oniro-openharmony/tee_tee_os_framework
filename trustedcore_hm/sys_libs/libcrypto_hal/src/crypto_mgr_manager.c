...
static uint32_t get_share_mem_size(const struct memref_t *fill_data, uint32_t fill_data_count)
{
...
static int32_t fill_share_mem(uint8_t *shared_buf, const struct memref_t *fill_data, uint32_t fill_data_count)
{
    for (uint32_t i = 0; i < fill_data_count; i++) {
...
static int32_t prepare_ioctl_parameters(const struct memref_t *data, uint32_t data_count, uint8_t **share_mem,
    uint32_t *share_mem_size, struct crypto_ioctl *ioctl_param)
{
    ...
    *share_mem = tee_alloc_sharemem_aux(&uuid, size);
    if (*share_mem == NULL) {
        tloge("alloc share memory failed\n");
        return CRYPTO_OVERFLOW;
    }

    (void)memset_s(*share_mem, size, 0, size);

    int32_t ret = fill_share_mem(*share_mem, data, data_count);
    if (ret != CRYPTO_SUCCESS) {
    ...
    *share_mem_size = size;
    ioctl_param->buf = (uint64_t)(uintptr_t)(*share_mem);
...
error:
    (void)memset_s(*share_mem, size, 0, size);
    tee_free_sharemem(*share_mem, size);
    *share_mem = NULL;
    return ret;
}
...
static int32_t get_share_mem(uint8_t *shared_buf, struct memref_t *get_data, uint32_t get_data_count)
{
...
{
    return tee_drv_ioctl(fd, IOCTRL_CRYPTO_GET_CTX_SIZE, &alg_type, sizeof(alg_type));
}
...
int32_t crypto_driver_ctx_copy(const struct ctx_handle_t *src_ctx, struct ctx_handle_t *dest_ctx)
{
    if (src_ctx == NULL || dest_ctx == NULL) {
        tloge("src_ctx or dest_ctx is NULL\n");
        return CRYPTO_BAD_PARAMETERS;
    }

    struct memref_t fill_data[] = {
        { .buffer = dest_ctx->ctx_buffer, .size = dest_ctx->ctx_size },
        { .buffer = src_ctx->ctx_buffer,  .size = src_ctx->ctx_size }
    };
    struct crypto_ioctl input = {0};
    uint8_t *ioctl_buf = NULL;
    uint32_t ioctl_size = 0;

    int32_t ret = prepare_ioctl_parameters(fill_data, ARRAY_SIZE(fill_data), &ioctl_buf, &ioctl_size, &input);
    if (ret != CRYPTO_SUCCESS) {
        tloge("ctx copy prepare ioctl parameters failed. ret = %d\n", ret);
        return ret;
    }

    input.arg1 = src_ctx->alg_type;

    ret = tee_drv_ioctl(src_ctx->fd, IOCTRL_CRYPTO_CTX_COPY, (void *)(&input), sizeof(input));
    if (ret != CRYPTO_SUCCESS) {
    ...
    if (ret != CRYPTO_SUCCESS) {
        tloge("ctx copy get share mem failed. ret = %d\n", ret);
        goto end;
    }
...
int32_t crypto_driver_hash_init(struct ctx_handle_t *ctx)
{
    ...
    uint64_t ctx_buffer = ctx->ctx_buffer;
    TEE_UUID uuid = CRYPTOMGR;
    uint32_t *tmp = tee_alloc_sharemem_aux(&uuid, ctx->ctx_size);
    if (tmp == NULL) {
        tloge("alloc share mem failed\n");
        return CRYPTO_OVERFLOW;
    }

    (void)memset_s(tmp, ctx->ctx_size, 0, ctx->ctx_size);
    ctx->ctx_buffer = (uint64_t)(uintptr_t)tmp;

    int64_t ret = tee_drv_ioctl(ctx->fd, IOCTRL_CRYPTO_HASH_INIT, (void *)ctx, sizeof(*ctx));
    if (ret != CRYPTO_SUCCESS) {
        tloge("share buffer failed\n");
        tee_free_sharemem(tmp, ctx->ctx_size);
        ctx->ctx_buffer = ctx_buffer;
        return ret;
    }

    (void)memcpy_s((void *)(uintptr_t)ctx_buffer, ctx->ctx_size, tmp, ctx->ctx_size);
    ctx->ctx_buffer = ctx_buffer;
    tee_free_sharemem(tmp, ctx->ctx_size);
...
int32_t crypto_driver_hash_update(struct ctx_handle_t *ctx, const struct memref_t *data_in)
{
    ...
    struct memref_t fill_data[] = {
        { .buffer = ctx->ctx_buffer, .size = ctx->ctx_size},
        { .buffer = data_in->buffer, .size = data_in->size}
    };
    struct crypto_ioctl input = {0};
    uint8_t *ioctl_buf = NULL;
    uint32_t ioctl_size = 0;
    ...
    ret = get_share_mem(ioctl_buf, fill_data, CRYPTO_PARAM_COUNT_1);
    if (ret != CRYPTO_SUCCESS)
        tloge("hash update get share mem failed. ret = %d\n", ret);
    ...
    struct memref_t *data_out)
{
    ...
    struct memref_t fill_data[] = {
        { .buffer = ctx->ctx_buffer,  .size = ctx->ctx_size},
        { .buffer = data_out->buffer, .size = data_out->size}
    };
    struct crypto_ioctl input = {0};
    uint8_t *ioctl_buf = NULL;
    uint32_t ioctl_size = 0;
    ...
    ret = get_share_mem(ioctl_buf, fill_data, CRYPTO_PARAM_COUNT_2);
    if (ret != CRYPTO_SUCCESS) {
        tloge("hash dofinal get share mem failed. ret = %d\n", ret);
        goto end;
    }

    data_out->size = fill_data[CRYPTO_DATA_OUT_OFFSET_1].size;
end:
    if (ioctl_buf != NULL)
        tee_free_sharemem(ioctl_buf, ioctl_size);
...
int32_t crypto_driver_hmac_init(struct ctx_handle_t *ctx, const struct symmerit_key_t *key)
{
    ...
    struct memref_t fill_data[] = {
        { .buffer = ctx->ctx_buffer, .size = ctx->ctx_size},
        { .buffer = key->key_buffer, .size = key->key_size},
    };
    struct crypto_ioctl input = {0};
    uint8_t *ioctl_buf = NULL;
    uint32_t ioctl_size = 0;
    ...
    ret = get_share_mem(ioctl_buf, fill_data, CRYPTO_PARAM_COUNT_1);
    if (ret != CRYPTO_SUCCESS)
        tloge("hmac init get share mem failed. ret = %d\n", ret);
...
int32_t crypto_driver_hmac_update(struct ctx_handle_t *ctx, const struct memref_t *data_in)
{
    ...
    ret = get_share_mem(ioctl_buf, fill_data, CRYPTO_PARAM_COUNT_1);
    if (ret != CRYPTO_SUCCESS)
        tloge("hmac update get share mem failed. ret = %d\n", ret);
    ...
    const struct memref_t *iv)
{
    ...
    if (iv != NULL) {
        fill_data[CRYPTO_IV_OFFSET_2].buffer = iv->buffer;
        fill_data[CRYPTO_IV_OFFSET_2].size = iv->size;
    }
    ...
    int32_t ret = prepare_ioctl_parameters(fill_data, ARRAY_SIZE(fill_data), &ioctl_buf, &ioctl_size, &input);
    if (ret != CRYPTO_SUCCESS) {
        tloge("cipher init prepare ioctl parameters failed. ret = %d\n", ret);
        return ret;
    }
    ...
    ret = tee_drv_ioctl(ctx->fd, IOCTRL_CRYPTO_CIPHER_INIT, (void *)(&input), sizeof(input));
    if (ret != CRYPTO_SUCCESS) {
    ...
    ret = get_share_mem(ioctl_buf, fill_data, CRYPTO_PARAM_COUNT_1);
    if (ret != CRYPTO_SUCCESS)
        tloge("cipher init get share mem failed. ret = %d\n", ret);
    ...
    const struct ae_init_data *ae_init_param)
{
    ...
    struct memref_t fill_data[] = {
        { .buffer = ctx->ctx_buffer,      .size = ctx->ctx_size },
        { .buffer = key->key_buffer,      .size = key->key_size },
        { .buffer = ae_init_param->nonce, .size = ae_init_param->nonce_len }
    };
    struct crypto_ioctl input = {0};
    uint8_t *ioctl_buf = NULL;
    uint32_t ioctl_size = 0;
    ...
    int32_t ret = prepare_ioctl_parameters(fill_data, ARRAY_SIZE(fill_data), &ioctl_buf, &ioctl_size, &input);
    if (ret != CRYPTO_SUCCESS) {
        tloge("ae init prepare ioctl parameters failed. ret = %d\n", ret);
        return ret;
    }
    ...
    ret = tee_drv_ioctl(ctx->fd, IOCTRL_CRYPTO_AE_INIT, (void *)(&input), sizeof(input));
    if (ret != CRYPTO_SUCCESS) {
    ...
    ret = get_share_mem(ioctl_buf, fill_data, CRYPTO_PARAM_COUNT_1);
    if (ret != CRYPTO_SUCCESS)
        tloge("ae init get share mem failed. ret = %d\n", ret);
...
int32_t crypto_driver_ae_update_aad(struct ctx_handle_t *ctx, const struct memref_t *aad_data)
{
    ...
    struct memref_t fill_data[] = {
        { .buffer = ctx->ctx_buffer,  .size = ctx->ctx_size },
        { .buffer = aad_data->buffer, .size = aad_data->size }
    };
    struct crypto_ioctl input = {0};
    uint8_t *ioctl_buf = NULL;
    uint32_t ioctl_size = 0;
    ...
    int32_t ret = prepare_ioctl_parameters(fill_data, ARRAY_SIZE(fill_data), &ioctl_buf, &ioctl_size, &input);
    if (ret != CRYPTO_SUCCESS) {
        tloge("ae update aad prepare ioctl parameters failed. ret = %d\n", ret);
        return ret;
    }

    ret = tee_drv_ioctl(ctx->fd, IOCTRL_CRYPTO_AE_UPDATE_AAD, (void *)(&input), sizeof(input));
    if (ret != CRYPTO_SUCCESS) {
    ...
    ret = get_share_mem(ioctl_buf, fill_data, CRYPTO_PARAM_COUNT_1);
    if (ret != CRYPTO_SUCCESS)
        tloge("ae update aad get share mem failed. ret = %d\n", ret);
    ...
    struct memref_t *data_out, struct memref_t *tag_out)
{
    ...
    struct memref_t fill_data[] = {
        { .buffer = ctx->ctx_buffer,  .size = ctx->ctx_size },
        { .buffer = data_out->buffer, .size = data_out->size },
        { .buffer = tag_out->buffer,  .size = tag_out->size },
        { .buffer = data_in->buffer,  .size = data_in->size }
    };
    struct crypto_ioctl input = {0};
    uint8_t *ioctl_buf = NULL;
    ...
    ret = get_share_mem(ioctl_buf, fill_data, CRYPTO_PARAM_COUNT_3);
    if (ret != CRYPTO_SUCCESS) {
    ...
    data_out->size = fill_data[CRYPTO_DATA_OUT_OFFSET_1].size;
    tag_out->size = fill_data[CRYPTO_TAG_OUT_OFFSET_2].size;
end:
    if (ioctl_buf != NULL)
        tee_free_sharemem(ioctl_buf, ioctl_size);
    ...
    const struct memref_t *tag_in, struct memref_t *data_out)
{
    ...
    struct memref_t fill_data[] = {
        ...
        { .buffer = data_out->buffer, .size = data_out->size },
        { .buffer = data_in->buffer,  .size = data_in->size },
        { .buffer = tag_in->buffer,   .size = tag_in->size }
    };
    struct crypto_ioctl input = {0};
    uint8_t *ioctl_buf = NULL;
    uint32_t ioctl_size = 0;
    ...
    struct rsa_priv_key_t *key_pair, uint32_t engine)
{
    ...
    struct memref_t fill_data[] = {
        { .buffer = (uint64_t)(uintptr_t)key_pair, .size = sizeof(*key_pair)},
        { .buffer = 0,                             .size = 0},
    };
...
int32_t crypto_rsa_encrypt_ops(uint64_t fd, struct memref_t *fill_data, uint32_t fill_data_count, uint32_t alg_type)
{
    uint8_t *ioctl_buf = NULL;
    uint32_t ioctl_size = 0;
    struct crypto_ioctl input = {0};
    ...
    int32_t ret = prepare_ioctl_parameters(fill_data, fill_data_count, &ioctl_buf, &ioctl_size, &input);
    if (ret != CRYPTO_SUCCESS) {
    ...
    struct memref_t *data_out, uint32_t engine)
{
    ...
    struct memref_t fill_data[] = {
        { .buffer = data_out->buffer,                .size = data_out->size},
        { .buffer = data_in->buffer,                 .size = data_in->size},
        { .buffer = (uint64_t)(uintptr_t)public_key, .size = sizeof(*public_key)},
        { .buffer = (uint64_t)(uintptr_t)attr_buf,   .size = attr_buf_len},
    };
...
int32_t crypto_rsa_decrypt_ops(uint64_t fd, struct memref_t *fill_data, uint32_t fill_data_count, uint32_t alg_type)
{
    uint8_t *ioctl_buf = NULL;
    uint32_t ioctl_size = 0;
    struct crypto_ioctl input = {0};
    ...
    uint32_t engine)
{
    ...
    struct memref_t fill_data[] = {
        { .buffer = data_out->buffer,                 .size = data_out->size},
        { .buffer = data_in->buffer,                  .size = data_in->size},
        { .buffer = (uint64_t)(uintptr_t)private_key, .size = sizeof(*private_key)},
        { .buffer = (uint64_t)(uintptr_t)attr_buf,    .size = attr_buf_len},
    };
...
int32_t crypto_rsa_sign_digest_ops(uint64_t fd, struct memref_t *fill_data, uint32_t fill_data_count, uint32_t alg_type)
{
    uint8_t *ioctl_buf = NULL;
    uint32_t ioctl_size = 0;
    struct crypto_ioctl input = {0};
    ...
    const struct memref_t *digest, struct memref_t *signature, uint32_t engine)
{
    ...
    struct memref_t fill_data[] = {
        { .buffer = signature->buffer,                .size = signature->size},
        { .buffer = digest->buffer,                   .size = digest->size},
        { .buffer = (uint64_t)(uintptr_t)private_key, .size = sizeof(*private_key)},
        { .buffer = (uint64_t)(uintptr_t)attr_buf,    .size = attr_buf_len},
    };
    ...
    const struct memref_t *digest, const struct memref_t *signature, uint32_t engine)
{
    ...
    ret = prepare_ioctl_parameters(fill_data, ARRAY_SIZE(fill_data), &ioctl_buf, &ioctl_size, &input);
    if (ret != CRYPTO_SUCCESS) {
...
int32_t crypto_ecc_encrypt_ops(uint64_t fd, struct memref_t *fill_data, uint32_t fill_data_count, uint32_t alg_type)
{
    uint8_t *ioctl_buf = NULL;
    uint32_t ioctl_size = 0;
    struct crypto_ioctl input = {0};
...
int32_t crypto_ecc_decrypt_ops(uint64_t fd, struct memref_t *fill_data, uint32_t fill_data_count, uint32_t alg_type)
{
    uint8_t *ioctl_buf = NULL;
    uint32_t ioctl_size = 0;
    struct crypto_ioctl input = {0};
...
int32_t crypto_ecc_sign_digest_ops(uint64_t fd, struct memref_t *fill_data, uint32_t fill_data_count, uint32_t alg_type)
{
    uint8_t *ioctl_buf = NULL;
    uint32_t ioctl_size = 0;
    struct crypto_ioctl input = {0};
...
int32_t crypto_ecdh_derive_key_ops(uint64_t fd, struct memref_t *fill_data, uint32_t fill_data_count, uint32_t alg_type)
{
    uint8_t *ioctl_buf = NULL;
    uint32_t ioctl_size = 0;
    struct crypto_ioctl input = {0};
    ...
    const struct asymmetric_params_t *ec_params, struct memref_t *secret, uint32_t engine)
{
    ...
    struct memref_t fill_data[] = {
        { .buffer = secret->buffer,                  .size = secret->size},
        { .buffer = (uint64_t)(uintptr_t)client_key, .size = sizeof(*client_key)},
        { .buffer = (uint64_t)(uintptr_t)server_key, .size = sizeof(*server_key)},
        { .buffer = (uint64_t)(uintptr_t)attr_buf,   .size = attr_buf_len},
    };
    ...
    struct memref_t *pub_key, struct memref_t *priv_key, uint32_t engine)
{
    ...
    struct memref_t fill_data[] = {
        { .buffer = pub_key->buffer,                 .size = pub_key->size},
        { .buffer = priv_key->buffer,                .size = priv_key->size},
        { .buffer = dh_generate_key_data->prime,     .size = dh_generate_key_data->prime_size},
        { .buffer = dh_generate_key_data->generator, .size = dh_generate_key_data->generator_size},
        { .buffer = dh_generate_key_data->dh_param.generate_key_t.q,
            .size = dh_generate_key_data->dh_param.generate_key_t.q_size},
    };
    struct crypto_ioctl input = {0};
    uint8_t *ioctl_buf = NULL;
    uint32_t ioctl_size = 0;
    ...
    struct memref_t *secret, uint32_t engine)
{
    ...
    struct memref_t fill_data[] = {
        { .buffer = secret->buffer,                .size = secret->size},
        { .buffer = dh_derive_key_data->prime,     .size = dh_derive_key_data->prime_size},
        { .buffer = dh_derive_key_data->generator, .size = dh_derive_key_data->generator_size},
        { .buffer = dh_derive_key_data->dh_param.derive_key_t.pub_key,
            .size = dh_derive_key_data->dh_param.derive_key_t.pub_key_size},
        { .buffer = dh_derive_key_data->dh_param.derive_key_t.priv_key,
            .size = dh_derive_key_data->dh_param.derive_key_t.priv_key_size},
    };
...
int32_t crypto_get_buf_ops(uint32_t cmd_id, uint64_t fd, void *buffer, uint32_t size)
{
    ...
    struct memref_t fill_data[] = {
        { .buffer = (uint64_t)(uintptr_t)buffer, .size = size }
    };
    ...
    (void)memset_s(ioctl_buf, ioctl_size, 0, ioctl_size);
    ...
    const struct memref_t *salt, struct memref_t *data_out, struct crypto_ioctl *input)
{
    struct memref_t fill_data[] = {
        { .buffer = data_out->buffer, .size = data_out->size },
        { .buffer = salt->buffer,     .size = salt->size },
        { .buffer = password->buffer, .size = password->size }
    };
