...
bool __attribute__((weak)) is_modload_perm_valid(const TEE_UUID *uuid, const char *name)
{
    (void)uuid;
    (void)name;
    return false;
...
{
    char name_bak[MAX_NAME_LEN] = { 0 };
    char *ptr = name_bak;
    char *prefix = NULL;

    if (memcpy_s(name_bak, MAX_NAME_LEN, mod_info->name, MAX_NAME_LEN) != 0)
        return DRV_CALL_ERROR;
