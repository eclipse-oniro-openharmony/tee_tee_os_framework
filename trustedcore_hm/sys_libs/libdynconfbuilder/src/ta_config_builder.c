...
static int32_t handle_ta_control_info_rpmb_size(uint32_t *rpmb_size, uint32_t size, const char *value)
{
    uint64_t tmp_rpmb_size;

    if (value == NULL || size > MAX_UINT32_LEN || size == 0) {
        hm_error("invalid parm while handle rpmb size\n");
        return TEE_ERROR_BAD_PARAMETERS;
    }

    char buff[MAX_UINT32_LEN + 1];
    if (memcpy_s(buff, sizeof(buff), value, size) != 0) {
        hm_error("memcpy failed while handle rpmb size\n");
        return TEE_ERROR_GENERIC;
    }
    buff[size] = '\0';

    if (trans_str_to_int(buff, size, BASE_OF_TEN, &tmp_rpmb_size) != TEE_SUCCESS) {
        hm_error("get thread limit failed, and thread_limit\n");
        return TEE_ERROR_GENERIC;
    }

    if (tmp_rpmb_size > UINT32_MAX) {
        hm_error("rpmb size is invalied %llx\n", (unsigned long long)tmp_rpmb_size);
        return TEE_ERROR_GENERIC;
    }

    *rpmb_size = (uint32_t)tmp_rpmb_size;
                                               ...
                                               void *obj, uint32_t obj_size)
{
    ...
    struct config_info *cfg_info = NULL;
    if (obj_size != sizeof(*cfg_info)) {
        hm_error("obj size is invalid while build rpmb size\n");
        return TEE_ERROR_BAD_PARAMETERS;
    }
    ...
    switch (node->tag) {
    case CONFIGINFO_TA_CONTROL_INFO_RPMB_INFO_RPMB_SIZE_RPMB_SIZE:
        if (handle_ta_control_info_rpmb_size(&cfg_info->control_info.rpmb_info.size,
                                             node->size, node->value) != TEE_SUCCESS) {
            hm_error("build ta control info rpmb size failed\n");
            return TEE_ERROR_GENERIC;
        }
        break;
    default:
                                                  ...
                                                  void *obj, uint32_t obj_size)
{
    ...
    struct config_info *cfg_info = NULL;
    if (obj_size != sizeof(*cfg_info)) {
        hm_error("obj size is invalid while build rpmb general\n");
        return TEE_ERROR_BAD_PARAMETERS;
    }
    ...
    switch (node->tag) {
    case CONFIGINFO_TA_CONTROL_INFO_RPMB_INFO_RPMB_PERMISSION_RPMB_GENERAL_RPMB_GENERAL:
        if (node->size == 1 && node->value[0] == TLV_TRUE)
            cfg_info->control_info.rpmb_info.permissions |= RPMB_GENERAL_PERMISSION;
        break;
    default:
        break;
    }
                                                      ...
                                                      void *obj, uint32_t obj_size)
{
    ...
    struct config_info *cfg_info = NULL;
    if (obj_size != sizeof(*cfg_info)) {
        hm_error("obj size is invalid while build rpmb permission\n");
        return TEE_ERROR_BAD_PARAMETERS;
    }
    ...
    switch (node->tag) {
    case CONFIGINFO_TA_CONTROL_INFO_RPMB_INFO_RPMB_PERMISSION_RPMB_GENERAL:
        if (handle_conf_node_to_obj(pos, build_ta_control_info_rpmb_general,
                                    cfg_info, sizeof(*cfg_info)) != TEE_SUCCESS) {
            hm_error("build ta control info rpmb info rpmb permission rpmb general failed\n");
            return TEE_ERROR_GENERIC;
        }
        break;
    default:
        hm_debug("skip in build ta control info rpmb permission\n");
        if (handle_conf_node_to_obj(pos, NULL, cfg_info, sizeof(*cfg_info)) != TEE_SUCCESS)
            return TEE_ERROR_GENERIC;
        break;
    }
                                               ...
                                               void *obj, uint32_t obj_size)
{
    ...
    struct config_info *cfg_info = NULL;
    if (obj_size != sizeof(*cfg_info)) {
        hm_error("obj size is invalid while build rpmb info\n");
        return TEE_ERROR_BAD_PARAMETERS;
    }
    ...
    switch (node->tag) {
    case CONFIGINFO_TA_CONTROL_INFO_RPMB_INFO_RPMB_SIZE:
        if (handle_conf_node_to_obj(pos, build_ta_control_info_rpmb_size,
                                    cfg_info, sizeof(*cfg_info)) != TEE_SUCCESS) {
            hm_error("build ta control info rpmb info rpmb size failed\n");
            return TEE_ERROR_GENERIC;
        }
        break;
    case CONFIGINFO_TA_CONTROL_INFO_RPMB_INFO_RPMB_PERMISSION:
        if (handle_conf_node_to_obj(pos, handle_ta_control_info_rpmb_permission,
                                    cfg_info, sizeof(*cfg_info)) != TEE_SUCCESS) {
            hm_error("build ta control info rpmb permission failed\n");
            return TEE_ERROR_GENERIC;
        }
        break;
    default:
        hm_debug("skip in build ta control info rpmb info\n");
        if (handle_conf_node_to_obj(pos, NULL, cfg_info, sizeof(*cfg_info)) != TEE_SUCCESS)
            return TEE_ERROR_GENERIC;
        break;
    }
                                     ...
                                     void *obj, uint32_t obj_size)
{
    ...
    switch (node->tag) {
#if (defined(CONFIG_APP_TEE_RPMB) || defined(CONFIG_APP_TEE_RPMB_A32))
    case CONFIGINFO_TA_CONTROL_INFO_RPMB_INFO:
        if (handle_conf_node_to_obj(pos, build_ta_control_info_rpmb_info,
                                    cfg_info, sizeof(*cfg_info)) != TEE_SUCCESS) {
