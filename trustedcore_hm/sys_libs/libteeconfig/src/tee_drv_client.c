 ...
 * Description: define tee driver client api
 * Create: 2021-03-01
 */
...
static struct list_head g_drv_channel = LIST_HEAD_INIT(g_drv_channel);
...
static struct drv_channel *alloc_and_init_channel(const char *drv_name, int64_t fd)
{
    ...
    init_list_head(&new_ch->drv_list);
...
static struct drv_channel *find_drv_channel(int64_t fd)
{
    ...
    list_for_each(pos, &g_drv_channel) {
        temp = list_entry(pos, struct drv_channel, drv_list);
...
static int32_t alloc_and_get_drv_channel(int64_t fd, const char *drv_name)
{
    ...
    if (drv_ch != NULL) {
        ...
        if (new_ch == NULL)
            goto err_out;
        list_add_tail(&new_ch->drv_list, &g_drv_channel);
        func_ret = 0;
    }
