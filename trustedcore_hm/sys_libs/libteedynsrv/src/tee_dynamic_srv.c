...
int tee_srv_map_from_task(uint32_t in_task_id, uint32_t va_addr, uint32_t size, uint64_t *virt_addr)
{
    uint64_t vaddr = 0;
    int ret;
...
static void tee_srv_dispatch(const char *task_name, const struct srv_dispatch_t *dispatch,
    uint32_t n_dispatch)
{
    cref_t msghdl;
    uint32_t task_id;
