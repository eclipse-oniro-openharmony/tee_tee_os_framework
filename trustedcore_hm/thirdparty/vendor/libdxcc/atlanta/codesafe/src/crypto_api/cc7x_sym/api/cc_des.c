...
static CCError_t DesVerifyWeakKeys(CCDesKey_t* key, CCDesNumOfKeys_t numOfKeys)
{
        ...
        */
        ...
        if ((CC_PalMemCmp(key->key1, key->key2, CC_DES_KEY_SIZE_IN_BYTES) == 0) || (CC_PalMemCmp(key->key2, key->key3, CC_DES_KEY_SIZE_IN_BYTES) == 0))
        {
        ...
        */
        for (i = 0; i < DES_NUM_OF_WEAK_KEYS; ++i) {
                if ((CC_PalMemCmp(DesWeakKeysTable[i].key, key->key1, CC_DES_KEY_SIZE_IN_BYTES) == 0) ||
                        (CC_PalMemCmp(DesWeakKeysTable[i].key, key->key2, CC_DES_KEY_SIZE_IN_BYTES) == 0) ||
                        (CC_PalMemCmp(DesWeakKeysTable[i].key, key->key3, CC_DES_KEY_SIZE_IN_BYTES) == 0)) {
                        return CC_DES_ILLEGAL_PARAMS_ERROR;
                }
        }
				       ...
				       uint8_t 		*DataOut_ptr )
{
	...
	symRc = SymDriverAdaptorProcess((uint32_t *)pDesContext,
					DataIn_ptr, DataOut_ptr, DataInSize, pDesContext->alg);
	return CC_CRYPTO_RETURN_ERROR(symRc, 0, SymAdaptor2CCDesErr);
}
