				     ...
				     CCHashOperationMode_t  OperationMode)
{
	...
	pHashPrivContext = (CCHashPrivateContext_t *)&(((uint32_t*)pHashContext)[CC_HASH_USER_CTX_ACTUAL_SIZE_IN_WORDS-1]);
				       ...
				       size_t                   DataInSize )
{
	...
	CCHashPrivateContext_t tmpHashPrivCtx;
	...
	pHashContext = (struct drv_ctx_hash *)GetUserCtxLocation(ContextID_ptr->buff);
	pHashPrivContext = (CCHashPrivateContext_t *)&(((uint32_t*)pHashContext)[CC_HASH_USER_CTX_ACTUAL_SIZE_IN_WORDS-1]);
	tmpHashPrivCtx = *pHashPrivContext;
	...
	*pHashPrivContext = tmpHashPrivCtx;
	...
	dataToProcessSize = min((hash_block_size_in_bytes - pHashPrivContext->remainingBufSize) % hash_block_size_in_bytes, DataInSize);
	if (dataToProcessSize > 0) {
		/* add the data to the remaining buffer */
		CC_PalMemCopy(&(pHashPrivContext->remainingBuf[pHashPrivContext->remainingBufSize]), DataIn_ptr, dataToProcessSize);
	...
	if (pHashPrivContext->remainingBufSize == hash_block_size_in_bytes) {
		symRc = SymDriverAdaptorProcess((uint32_t *)pHashContext,
			pHashPrivContext->remainingBuf, NULL, pHashPrivContext->remainingBufSize, pHashContext->alg);
		if (symRc != CC_RET_OK) {
			return CC_CRYPTO_RETURN_ERROR(symRc, 0, SymAdaptor2CCHashErr);
		}
		pHashPrivContext->remainingBufSize = 0;
	...
	if (dataToProcessSize > 0) {
		symRc = SymDriverAdaptorProcess((uint32_t *)pHashContext,
			DataIn_ptr, NULL, dataToProcessSize, pHashContext->alg);
		if (symRc != CC_RET_OK) {
			return CC_CRYPTO_RETURN_ERROR(symRc, 0, SymAdaptor2CCHashErr);
		}
					...
					CCHashResultBuf_t        HashResultBuff )
{
	...
	if (pHashContext->alg != DRV_CRYPTO_ALG_HASH ||
	    (int32_t)pHashContext->mode < DRV_HASH_SHA1 ||
	    pHashContext->mode > DRV_HASH_MD5)
	{
	    return CC_HASH_ILLEGAL_PARAMS_ERROR;
	}
	...
	*pHashPrivContext = tmpHashPrivCtx;
	...
	if (pHashPrivContext->remainingBufSize > 0) {
		...
		if ((uint32_t)DataIn_ptr < (uint32_t)ContextID_ptr->buff ||
		    (uint32_t)DataIn_ptr > (uint32_t)ContextID_ptr->buff + sizeof(CCHashUserContext_t) ||
		    DataInSize > sizeof(CCHashUserContext_t) ||
		    (uint32_t)DataIn_ptr + DataInSize > (uint32_t)ContextID_ptr->buff + sizeof(CCHashUserContext_t))
		{
			return CC_HASH_ILLEGAL_PARAMS_ERROR;
		}
	}
	symRc = SymDriverAdaptorFinalize((uint32_t *)pHashContext,
		DataIn_ptr, NULL, DataInSize, pHashContext->alg);
	if (symRc != CC_RET_OK) {
		return CC_CRYPTO_RETURN_ERROR(symRc, 0, SymAdaptor2CCHashErr);
	}
	pHashPrivContext->remainingBufSize = 0;
	...
	CC_PalMemCopy(HashResultBuff, pHashContext->digest, CC_HASH_RESULT_SIZE_IN_WORDS*sizeof(uint32_t));
