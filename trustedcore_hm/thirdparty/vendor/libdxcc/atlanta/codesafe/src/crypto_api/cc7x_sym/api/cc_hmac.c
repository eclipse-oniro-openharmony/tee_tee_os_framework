                             ...
                             size_t                  DataInSize )
{
	struct drv_ctx_hash *pHmacContext;
	CCHmacPrivateContext_t *pHmacPrivContext;
	CCHmacPrivateContext_t tmpPrivCtx;
	int symRc = CC_RET_OK;
	...
	pHmacContext = (struct drv_ctx_hash *)GetUserCtxLocation(ContextID_ptr->buff);
	pHmacPrivContext = (CCHmacPrivateContext_t *)&(((uint32_t*)pHmacContext)[CC_HMAC_USER_CTX_ACTUAL_SIZE_IN_WORDS-1]);
	tmpPrivCtx = *pHmacPrivContext;
	...
	*pHmacPrivContext = tmpPrivCtx;

	if (pHmacPrivContext->isLastBlockProcessed != 0) {
		return CC_HMAC_LAST_BLOCK_ALREADY_PROCESSED_ERROR;
	}
	...
	if ((DataInSize % blockSizeBytes) == 0) {
		symRc = SymDriverAdaptorProcess((uint32_t *)pHmacContext,
					DataIn_ptr, NULL, DataInSize, pHmacContext->alg);
		if (symRc != CC_RET_OK) {
			return CC_CRYPTO_RETURN_ERROR(symRc, 0, SymAdaptor2CCHmacErr);
		}
					...
					CCHashResultBuf_t       HmacResultBuff )
{
	...
	*pHmacPrivContext = tmpPrivCtx;
	...
	if (pHmacPrivContext->isLastBlockProcessed == 0) {
		symRc = SymDriverAdaptorFinalize((uint32_t *)pHmacContext, NULL, NULL, 0, pHmacContext->alg);
		if (symRc != CC_RET_OK) {
			return CC_CRYPTO_RETURN_ERROR(symRc, 0, SymAdaptor2CCHmacErr);
		}
	}
