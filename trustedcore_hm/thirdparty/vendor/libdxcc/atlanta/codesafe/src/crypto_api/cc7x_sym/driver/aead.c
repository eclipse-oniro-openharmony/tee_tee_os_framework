...
//CC_PAL_COMPILER_ASSERT(sizeof(DrvAeadCcmFlow_e)==sizeof(uint32_t), "DrvAeadCcmFlow_e is not 32bit!");
...
static void StoreAeadMac(CCSramAddr_t ctxAddr)
{
	CCSramAddr_t macStateAddr = GET_CTX_FIELD_ADDR(ctxAddr, struct drv_ctx_aead, mac_state);
	HwDesc_s desc;
...
static void StoreAeadCipherState(CCSramAddr_t ctxAddr)
{
	const CCSramAddr_t blockStateAddr = GET_CTX_FIELD_ADDR(ctxAddr, struct drv_ctx_aead, block_state);
	HwDesc_s desc;
...
static void InitCcmCounter(CCSramAddr_t ctxAddr, uint8_t CounterInitialValue, struct drv_ctx_aead *pCtxAead)
{
	CCSramAddr_t blockStateAddr = GET_CTX_FIELD_ADDR(ctxAddr, struct drv_ctx_aead, block_state);
	...
	case DRV_AEAD_FLOW_ADATA_INIT:
		...
		if (dmaInMode == DMA_MLLI) {
			...
			*  could be one table or more but in a single descriptor processing */
			pAeadCtx->nextProcessingState = DRV_AEAD_FLOW_TEXT_DATA_INIT;
			pAeadCtx->headerRemainingBytes = pAeadCtx->headerRemainingBytes - pAeadCtx->header_size;
		} else {
