...
{
	const CCSramAddr_t blockStateAddr = GET_CTX_FIELD_ADDR(ctxAddr, struct drv_ctx_cipher, block_state);
	HwDesc_s desc;
	uint32_t blockSize;
	...
	if (pCipherContext->alg == DRV_CRYPTO_ALG_AES) {
...
static int RevertLastMacBlock(CCSramAddr_t ctxAddr, struct drv_ctx_cipher *pCipherContext)
{
	...
	if ((pCipherContext->mode != DRV_CIPHER_XCBC_MAC) && (pCipherContext->mode != DRV_CIPHER_CMAC)) {
	...
	if ((pCipherContext->mode == DRV_CIPHER_XCBC_MAC) && (pCipherContext->key_size != CC_AES_128_BIT_KEY_SIZE)) {
...
int InitCipher(CCSramAddr_t ctxAddr, uint32_t *pCtx)
{
    ...
    if (pCipherContext->alg == DRV_CRYPTO_ALG_DES) {
	    ...
	    if (pCipherContext->key_size == CC_DRV_DES_DOUBLE_KEY_SIZE){
		    ...
		    WriteContextField((keyAddr + CC_DRV_DES_DOUBLE_KEY_SIZE), tKeybuff, CC_DRV_DES_ONE_KEY_SIZE);
	...
	case DRV_CIPHER_CMAC:
		...
		if(pCipherContext->crypto_key_type == DRV_ROOT_KEY) {
		    ...
		    pCipherContext->key_size = keySize;
		}
		break;
	case DRV_CIPHER_XCBC_MAC:
...
int ProcessCipher(CCSramAddr_t ctxAddr, uint32_t *pCtx, DmaBuffer_s *pDmaInputBuffer, DmaBuffer_s *pDmaOutputBuffer)
{
	...
	if (pCipherContext->mode == DRV_CIPHER_CBC_CTS &&
	   pCipherContext->dataBlockType != LAST_BLOCK){
		pCipherContext->mode = DRV_CIPHER_CBC;
		LoadCipherKey(ctxAddr, pCipherContext);
	 	LoadCipherState(ctxAddr, 0, pCipherContext);
	...
	if (isInplaceOp){
		...
		case DMA_MLLI:
			...
			pOutputData = GetFirstLliPtr(MLLI_OUTPUT_TABLE);
			PrepareMLLITable(pDmaOutputBuffer->pData, pDmaOutputBuffer->size, pDmaOutputBuffer->axiNs, MLLI_OUTPUT_TABLE);
			/* data size should hold the number of LLIs */
	...
	if ((pCipherContext->mode == DRV_CIPHER_CMAC) || (pCipherContext->mode == DRV_CIPHER_XCBC_MAC)) {
	...
	flowMode = (pCipherContext->alg == DRV_CRYPTO_ALG_AES) ? DIN_AES_DOUT : DIN_DES_DOUT;
	...
	/* get machine state */
	StoreCipherState(ctxAddr, pCipherContext);
...
int FinalizeCipher(CCSramAddr_t ctxAddr, uint32_t *pCtx, DmaBuffer_s *pDmaInputBuffer, DmaBuffer_s *pDmaOutputBuffer)
{
	...
	default:
		if (isRemainingData) {
			...
			drvRc = ProcessCipher(ctxAddr, (uint32_t *)pCipherContext, pDmaInputBuffer, pDmaOutputBuffer);
			if (drvRc != CC_RET_OK) {
				goto EndWithErr;
			}
