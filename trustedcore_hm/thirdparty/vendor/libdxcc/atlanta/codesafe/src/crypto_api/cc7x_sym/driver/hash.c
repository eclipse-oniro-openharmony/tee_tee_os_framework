...
/* Real expected size */
const uint32_t gHashDigestSize[DRV_HASH_MODE_NUM] = {CC_SHA1_DIGEST_SIZE, CC_SHA256_DIGEST_SIZE, CC_SHA224_DIGEST_SIZE, CC_SHA512_DIGEST_SIZE, CC_SHA384_DIGEST_SIZE, CC_MD5_DIGEST_SIZE};
/* SHA224 is processed as SHA256! */
const uint32_t gHashHwDigestSize[DRV_HASH_MODE_NUM] = {CC_SHA1_DIGEST_SIZE, CC_SHA256_DIGEST_SIZE, CC_SHA256_DIGEST_SIZE, CC_SHA512_DIGEST_SIZE,  CC_SHA512_DIGEST_SIZE,CC_MD5_DIGEST_SIZE};
/*from the HW side, HASH512 and HASH384 are the same*/
const uint32_t gHashHwMode[DRV_HASH_MODE_NUM] = {DRV_HASH_HW_SHA1, DRV_HASH_HW_SHA256, DRV_HASH_HW_SHA256,DRV_HASH_HW_SHA512,DRV_HASH_HW_SHA512,DRV_HASH_HW_MD5};
 ...
 * \return int One of DX_SYM_* error codes defined in cc_sym_error.h.
 */
int LoadHashState(CCSramAddr_t ctxAddr, enum HashConfig1Padding paddingSelection, struct drv_ctx_hash * pHashCtx)
{
	...
	const CCSramAddr_t currentDigestedLengthAddr = GET_CTX_FIELD_ADDR(ctxAddr, struct drv_ctx_hash, CurrentDigestedLength);
...
int StoreHashState(CCSramAddr_t ctxAddr, struct drv_ctx_hash * pHashCtx)
{
	...
	HW_DESC_INIT(&desc);
	HW_DESC_SET_CIPHER_MODE(&desc, hw_mode);
	HW_DESC_SET_STATE_DOUT_PARAM(&desc, currentDigestedLengthAddr, (DRV_HASH_LENGTH_WORDS * sizeof(uint32_t)));
	HW_DESC_SET_FLOW_MODE(&desc, S_HASH_to_DOUT);
 ...
 * \return int One of DX_SYM_* error codes defined in cc_sym_error.h.
 */
int ProcessHash(CCSramAddr_t ctxAddr, uint32_t *pCtx, DmaBuffer_s *pDmaInputBuffer, DmaBuffer_s *pDmaOutputBuffer)
{
	...
	switch (dmaMode) {
	case DMA_MLLI:
		pInputData = GetFirstLliPtr(MLLI_INPUT_TABLE);
		PrepareMLLITable(pDmaInputBuffer->pData, pDmaInputBuffer->size, pDmaInputBuffer->axiNs, MLLI_INPUT_TABLE);
 ...
 * \return int One of DX_SYM_* error codes defined in cc_sym_error.h.
 */
int FinalizeHash(CCSramAddr_t ctxAddr, uint32_t *pCtx, DmaBuffer_s *pDmaInputBuffer, DmaBuffer_s *pDmaOutputBuffer)
{
	const CCSramAddr_t digestAddr = GET_CTX_FIELD_ADDR(ctxAddr, struct drv_ctx_hash, digest);
	HwDesc_s desc;
	...
	if (isRemainingData == 1) {
		...
		HW_DESC_SET_STATE_DOUT_PARAM(&desc, currentDigestedLengthAddr,
