...
static int ProcessHmacPad( uint32_t constPadData, CCSramAddr_t hashData, uint32_t hashDataSize,
	enum drv_hash_mode hmode, CCSramAddr_t hashCurrentLength, CCSramAddr_t hashResult, CCSramAddr_t ctxAddr)
{
	HwDesc_s desc;
	uint32_t lhmode;
	uint32_t DigestSize;
	...
	HW_DESC_SET_STATE_DOUT_PARAM(&desc, hashCurrentLength, sizeof(uint32_t) * DRV_HASH_LENGTH_WORDS);
	HW_DESC_SET_FLOW_MODE(&desc, S_HASH_to_DOUT);
...
int InitHmac(CCSramAddr_t ctxAddr, uint32_t *pCtx)
{
	...
	drvRc = ProcessHmacPad(HMAC_IPAD_CONST_BLOCK, k0Addr, BlockSize, pHmacCtx->mode,
		currentDigestedLengthAddr, digestAddr, ctxAddr);
	if (drvRc != CC_RET_OK) {
		goto EndWithErr;
	}
	drvRc = ProcessHmacPad(HMAC_OPAD_CONST_BLOCK, k0Addr, BlockSize, pHmacCtx->mode,
