...
typedef int (*ProcessFunc_t)(CCSramAddr_t ctxAddr, uint32_t *pCtx, DmaBuffer_s *pDmaInputBuffer, DmaBuffer_s *pDmaOutputBuffer);
typedef int (*FinalizeFunc_t)(CCSramAddr_t ctxAddr, uint32_t *pCtx, DmaBuffer_s *pDmaInputBuffer, DmaBuffer_s *pDmaOutputBuffer);
 ...
 * \return int One of DX_SYM_* error codes defined in head of file.
 */
int SymDriverDispatchProcess(CCSramAddr_t ctxAddr, uint32_t *pCtx, DmaBuffer_s *pDmaInputBuffer, DmaBuffer_s *pDmaOutputBuffer, enum drv_crypto_alg alg)
{
	int retcode;
	...
	CC_PAL_LOG_INFO("pDmaInputBuffer: pData=%" PRIx64 " DataSize=%08X DmaMode=%08X \n",
		pDmaInputBuffer->pData, pDmaInputBuffer->size, pDmaInputBuffer->dmaBufType);

	CC_PAL_LOG_INFO("pDmaOutputBuffer: pData=%" PRIx64 " DataSize=%08X DmaMode=%08X\n",
		pDmaOutputBuffer->pData, pDmaOutputBuffer->size, pDmaOutputBuffer->dmaBufType);
	...
	retcode = (gFuncDispatchList[alg].processFunc)(ctxAddr, pCtx, pDmaInputBuffer, pDmaOutputBuffer);
 ...
 * \return int One of DX_SYM_* error codes defined in head of file.
 */
int SymDriverDispatchFinalize(CCSramAddr_t ctxAddr, uint32_t *pCtx, DmaBuffer_s *pDmaInputBuffer, DmaBuffer_s *pDmaOutputBuffer, enum drv_crypto_alg alg)
{
	int retcode;
	...
	retcode = (gFuncDispatchList[alg].finalizeFunc)(ctxAddr, pCtx, pDmaInputBuffer, pDmaOutputBuffer);
