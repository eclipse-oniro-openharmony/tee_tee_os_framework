...
uint32_t PkaGet2MsBits(uint32_t rX,        /*!< [in] Register virt. pointer. */
			...
			uint32_t *pW,      /*!< [in] Pointer to 32-bit current word, which must be saved by
							caller through reading bits from the register. */
			uint32_t *pIsNew)  /*!< [in] Pointer to indicator is it a new start ofthe function
							       ...
							       As output - actual available regs, must be at least as required. */
{
        ...
        if (err != CC_SUCCESS) {
                CC_PalMutexUnlock(&CCAsymCryptoMutex);
        }
        return err;
}
                                                               ...
                                                               at least as required. */
{
        ...
        if (opSizeInBits < PKA_MIN_OPERATION_SIZE_BITS ||
            opSizeInBits > PKA_MAX_OPERATION_SIZE_BITS) {
								...
								with word extension. */
{
        ...
        if ((firstReg + countOfRegs)*size > CC_SRAM_PKA_SIZE_IN_BYTES/sizeof(uint32_t)) {
                countOfRegs = ((CC_SRAM_PKA_SIZE_IN_BYTES/sizeof(uint32_t)) / size) - firstReg;
        }
