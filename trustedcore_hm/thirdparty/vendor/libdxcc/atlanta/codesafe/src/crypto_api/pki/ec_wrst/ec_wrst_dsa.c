 ...
 */
void EcWrstDsaTruncateMsg(uint32_t *pMsgOut,   /*!< [out] Pointer to truncated message output. Output format:bytes, LS byte is left most one. */
                ...
                /* copy external key if provided */
                        ...
                        if (countTries > 0) {
                                ...
                                *  try; if it isn't then return error          */
                                err = ECWRST_DSA_SIGN_CALC_CANNOT_CREATE_SIGNATURE;
                                goto End;
                        }
