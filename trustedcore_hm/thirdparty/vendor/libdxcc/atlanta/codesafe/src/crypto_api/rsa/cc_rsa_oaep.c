 ...
 *-----------------------------------------------*/
         ...
         */
        uint16_t hLenModifyForCTS = CC_HASH_SHA1_DIGEST_SIZE_IN_BYTES;
        CCHashOperationMode_t hashFuncModifyForCTS = CC_HASH_SHA1_mode;
        switch (MGF) {
        case CC_PKCS1_MGF1:
 ...
 *---------------------------------------------------------*/
        ...
        if (TmpByte_ptr[I] != 0x01)
                return CC_RSA_OAEP_DECODE_ERROR;
 ...
 *-----------------------------------------------*/
        ...
        if (*MSize_ptr < (uint32_t)(EMSize - 2*HashOutputSize - I - 1)) {
                *MSize_ptr = EMSize - 2*HashOutputSize - I - 1;
                return CC_RSA_DECRYPT_INVALID_OUTPUT_SIZE;
        }
