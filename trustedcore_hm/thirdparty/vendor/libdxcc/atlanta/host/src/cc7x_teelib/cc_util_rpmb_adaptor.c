                         ...
                         uint32_t          		listSize)
{
		...
		if (listSize == RPMB_MAX_BLOCKS_PER_UPDATE) {
			symRc = RpmbSymDriverAdaptorProcess((uint32_t *)pHmacContext,
					pListOfDataFrames, listSize, pHmacContext->alg);
		if (symRc != CC_RET_OK) {
...
int RpmbSymDriverAdaptorProcess(uint32_t *pCtx,
				unsigned long   *pListOfDataFrames, uint32_t listSize, enum drv_crypto_alg alg)
{
		int 			symRc = CC_RET_OK;
		...
		retCode = RpmbBuildDmaFromDataPtr(pListOfDataFrames, listSize, CC_PAL_DMA_DIR_TO_DEVICE,
						&dmaBuffIn, &gDmaBuildBuffer);
		if (retCode != 0) {
...
EndWithErr:
		...
		retCode = RpmbBuildDataPtrFromDma(pListOfDataFrames, listSize, CC_PAL_DMA_DIR_TO_DEVICE,
						  &gDmaBuildBuffer);
		if (retCode != 0) {
...
EndWithErr:
		retCode = RpmbBuildDataPtrFromDma(pListOfDataFrames, listSize, CC_PAL_DMA_DIR_TO_DEVICE, &gDmaBuildBuffer);
		if (retCode != 0) {
				CC_PAL_LOG_ERR("failed to RpmbBuildDataPtrFromDma for pDataIn 0x%x\n", retCode);
				symRc = retCode;
		}
...
uint32_t   RpmbBuildMlliTable(int j, uint32_t numOfBlocks, mlliTable_t *pDevBuffer, RpmbDmaBuffBlocksInfo_t *pUsrBlockList)
{
		uint32_t i;
		CCPalPerfData_t 	perfIdx = 0;
...
void RpmbClearDmaBuildBuffers(RpmbDmaBuildBuffer_t *pDmaBuildBuff)
{
	...
	CC_PalMemSetZero(pDmaBuildBuff->blocksList.pBlockEntry, RPMB_MAX_PAGES_PER_BLOCK * sizeof(CCPalDmaBlockInfo_t));
	CC_PalMemSetZero(pDmaBuildBuff->blocksList.numOfBlocks, RPMB_MAX_BLOCKS_PER_UPDATE * sizeof(uint32_t));
...
uint32_t RpmbAllocDmaBuildBuffers(RpmbDmaBuildBuffer_t *pDmaBuildBuff)
{
	...
	rc = CC_PalDmaContigBufferAllocate(FW_MLLI_TABLE_LEN * sizeof(lliInfo_t), &tmpBuff);
	if (rc != 0) {
		return CC_RET_NOMEM;
	}
