...
extern void __dma_flush_range(uintptr_t start, uintptr_t end);
extern void __dma_inv_range(uintptr_t start, uintptr_t end);
// ----

#define ADDRESS_32BIT_MAX 0xFFFFFFFF
 ...
 */
uint32_t cc_pal_dma_init(uint32_t  buff_size)
{
    gMemVirtBaseAddr = (CCVirtAddr_t)malloc_coherent(buff_size);
 ...
 */
CCDmaAddr_t CC_PalMapVirtualToPhysical(uint8_t *pVirtualAddr);
CCDmaAddr_t CC_PalMapVirtualToPhysical(uint8_t *pVirtualAddr)
{
	CCDmaAddr_t physAddr = (CCDmaAddr_t)(
		((CCVirtAddr_t)ptr_to_uint64(pVirtualAddr) - gMemVirtBaseAddr) + gMemPhysBaseAddr);
	return physAddr;
}
			     ...
			     CC_PalDmaBufferHandle       *dmaBuffHandle)
{
	...
	*(uint32_t*)dmaBuffHandle = 0;
    ...
    if (ADDRESS_32BIT_MAX - buffSize < (uint32_t)(uintptr_t)pDataBuffer) {
        retCode = 1;
        goto pal_mapEnd;
    }
    /* calculate number of blocks(pages) held by pDataBuffer */
    endPage = (uint32_t)(((uintptr_t)pDataBuffer + buffSize) - 1) >> PAL_PAGE_SHIFT;
    startPage = ((uint32_t)(uintptr_t)pDataBuffer) >> PAL_PAGE_SHIFT;
    ...
    startOffset = (uint32_t)(uintptr_t)pDataBuffer & (~PAL_PAGE_MASK);
	*pNumOfBlocks = numPages;
	...
	for (index = 0; index < numPages; index++) {
        ...
        pDmaBlockList[index].blockPhysAddr = (CCDmaAddr_t)__virt_to_phys((uint32_t)(uintptr_t)pTmpBuff);
        if (ADDRESS_32BIT_MAX - pDmaBlockList[index].blockSize < (uint32_t)(uintptr_t)pTmpBuff) {
            retCode = 1;
            goto pal_mapEnd;
        }
        __dma_flush_range((uint32_t)(uintptr_t)pTmpBuff,
                          (uint32_t)((uintptr_t)pTmpBuff + pDmaBlockList[index].blockSize));
			       ...
			       CC_PalDmaBufferHandle       dmaBuffHandle)
{
	...
	for (index = 0; index < numOfBlocks; index++) {
        ...
        /*
         * if buffer was not allocated/copy in CC_PalDmaBufferMap(),
         * nothing left to be doen.Just return OK
         */
    ...
    __dma_inv_range((uint32_t)(uintptr_t)pDataBuffer, (uint32_t)(uintptr_t)pDataBuffer + buffSize);
pal_unmapEnd:
