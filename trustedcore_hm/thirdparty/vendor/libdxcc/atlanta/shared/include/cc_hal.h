...
/*!
 * @brief This function is used to wait for the IRR interrupt signal. The existing implementation performs a "busy wait" on
	  the IRR, and returns its value once it changes to non-zero.
          This implementation can be left as-is. However, it degrades performance significantly. Therefore, it is recommended to
	  change it to an implementation that waits for an actual interrupt.

 * @return uint32_t The IRR value.
 */
uint32_t CC_HalWaitInterrupt(uint32_t data /*!< [in] The interrupt bits to wait upon. */ );
