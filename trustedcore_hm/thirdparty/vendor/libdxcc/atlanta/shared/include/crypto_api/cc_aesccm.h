...
/*!
@file
@brief This file contains all of the enums and definitions that are used for the CryptoCell AESCCM APIs, as well as the APIs themselves.
*/
...
typedef enum {
	CC_AES_Key128BitSize   = 0,
	CC_AES_Key192BitSize   = 1,
	CC_AES_Key256BitSize   = 2,
	CC_AES_Key512BitSize   = 3,

	CC_AES_KeySizeNumOfOptions,

	CC_AES_KeySizeLast    = 0x7FFFFFFF,
...
* + 1 word for offset management */
   uint32_t  buff[CC_AESCCM_USER_CTX_SIZE_IN_WORDS];
...
It formats of the input data, calculates AES-MAC value for the formatted B0 block containing control information and CCM unique value (Nonce),
...
<li> If decrypting:
  ...
  <li> Encrypts the calculated MAC using AES_CTR based on the saved CTR0 value, and compares it with SizeOfT last bytes of input data (i.e. tag value).</li>
...
\note For security reasons it is recommended to use the integrated function only, if the non integrated        												 functions are used the data is decrypted before the full authentication is done.
@return CC_OK on success.
@return A non-zero value on failure as defined cc_aesccm_error.h.
*/
