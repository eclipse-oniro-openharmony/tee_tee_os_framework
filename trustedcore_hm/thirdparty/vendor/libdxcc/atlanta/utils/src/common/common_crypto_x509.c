...
#define MAX_EXT_VAL_LIST 64 // additional data
...
const uint8_t *certType2Str[CC_X509_CERT_TYPE_MAX] = {NULL,
  ...
  /*CC_X509_CERT_TYPE_PRIM_DBG */      (uint8_t *)CC_X509_CERT_DBG1_CERT,
  /*CC_X509_CERT_TYPE_SCND_DBG */      (uint8_t *)CC_X509_CERT_DBG2_CERT
};
...
*/
/*********************************************************/
int32_t CC_CommonX509Free(uint8_t **ppCertBuff)
{
	/* validate inputs */
	if ((NULL == ppCertBuff) ||
	    (NULL == *ppCertBuff)) {
				...
				CCX509CertType_t  certType,CCX509CertHeaderParamsIn_t *pCertHeaderParams)
{
	...
	char* pIssuerName = CERT_ISSUER_NAME;
...
/**
* @brief convert the x.509 certificate to pem format
*
* @param[in/out] ppCertBuff      - x.509 certificate
* @param[out] pOutCertSize 	- certificate size in PEM format
*/
/*********************************************************/
int32_t CC_CommonX509ToPem(uint8_t **ppCertBuff,
			    uint32_t *pOutCertSize)
{
	...
	*pOutCertSize = 0;
	...
	bioDataSize = BIO_get_mem_data(mbio, (uint8_t *)&pTmpCertData);
	//UTIL_LOG_ERR("bioCertSize %d, bioDataSize %d\n", bioCertSize, bioDataSize);
	if (bioDataSize == 0) {
		UTIL_LOG_ERR("failed to BIO_get_mem_data\n");
		rc = 1;
		goto END;
	}
	// Released in main lib after coping the cert to binary file
	pPemCertData = malloc(bioDataSize);
	if (NULL == pPemCertData) {
		UTIL_LOG_ERR("failed to malloc pPemCertData\n");
		rc = 1;
		goto END;
	}
	memcpy(pPemCertData, pTmpCertData, bioDataSize);
	CC_CommonX509Free(ppCertBuff);
	*ppCertBuff = pPemCertData;
	*pOutCertSize = bioDataSize;
				...
				uint8_t *outPkgFile)
{
	...
	((CCX509PkgHeader_t *)pCertPkg)->certInfo.certInfoBits.storeAddr = (sizeof(CCX509PkgHeader_t)+addDataSize) & 0xFFFF;
