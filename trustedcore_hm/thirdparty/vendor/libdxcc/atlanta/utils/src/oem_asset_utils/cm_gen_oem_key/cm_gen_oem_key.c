...
#define	CM_UTIL_MAX_KDF_SIZE_IN_BYTES 	(CM_UTIL_MAX_LABEL_LENGTH_IN_BYTES+ \
					 CM_UTIL_MAX_CONTEXT_LENGTH_IN_BYTES+CM_UTIL_FIX_DATA_MIN_SIZE_IN_BYTES)

#define KEY_PLAT_LABEL		0x4B,0x45,0x59,0x20,0x50,0x4C,0x41,0x54 /* "KEY PLAT" */
#define KEY_PROVISION_LABEL	0x50,0x52,0x4f,0x56,0x49,0x53,0x49,0x4f,0x4e,0x20,0x4B,0x45,0x59 /* PROVISION KEY*/
...
static unsigned char cmSecretsBase64AesBuff[CC_COMMON_CALC_BASE64_ENCODE_SIZE(CC_COMMON_CALC_CBC_ENCODE_SIZE(CM_SECRETS_FILE_CONTENT_MAX_SIZE))] = {0};
...
static unsigned char cmSecretsAesBuff[CC_COMMON_CALC_CBC_ENCODE_SIZE(CM_SECRETS_FILE_CONTENT_MAX_SIZE)] = {0};
...
static unsigned char csrBase64Buff[CC_COMMON_CALC_BASE64_ENCODE_SIZE(sizeof(CsrBuff_t))] = {0};
...
static unsigned char pltKeyBuff[CC_UTIL_KPLT_SIZE_IN_BYTES] = {0};

static unsigned char oemKeyFileName[UTIL_MAX_FILE_NAME] = "oem_key.bin";
static unsigned char oemKeyBuff[CC_UTIL_KOEM_SIZE_IN_BYTES] = {0};
static EncOemKeyBuff_t oemKeyRsa = {0};
static unsigned char oemKeyRsaBase64Buff[CC_COMMON_CALC_BASE64_ENCODE_SIZE(ENC_OEM_KEY_BUFF_SIZE)] = {0};
...
void usage(void)
{
	...
	UTIL_LOG_ERR( "--cm-pwd		      -w:\t optional, pwd of private key file name. If not exist, PEM should be entered manually\n");
...
int parseArguments (const int argc, char *const argv[])
{
	...
	while (1) {
		...
		if (ch == -1)  break; // No more parameters
		switch (ch) {
		case 'c':   // --csr
	...
	rc = CC_CommonUtilCopyDataFromBinFile(csrFileName, csrBase64Buff, &csrBase64BuffSize);
	if ((rc != 0) ||
	    (csrBase64BuffSize !=  (sizeof(csrBase64Buff)))) {
	...
	rc = CC_CommonUtilCopyDataFromBinFile(cmSecretsFileName, cmSecretsBase64AesBuff, &cmSecretsBase64AesBuffSize);
	if ((rc != 0) ||
	    (cmSecretsBase64AesBuffSize > sizeof(cmSecretsBase64AesBuff))) {
...
int getCmSecrets(void)
{
	...
	cmSecretsAesBuffSize = CC_COMMON_CALC_BASE64_MAX_DECODE_SIZE(cmSecretsBase64AesBuffSize);
	rc = CC_CommonBase64Decode(cmSecretsBase64AesBuff, cmSecretsBase64AesBuffSize, cmSecretsAesBuff, &cmSecretsAesBuffSize);
	if (rc != 0) {
		UTIL_LOG_ERR( "failed to CC_CommonBase64Decode() for cm secrets, rc %d\n", rc);
		return 1;
	}
	UTIL_LOG_BYTE_BUFF("cmSecretsAesBuff", cmSecretsAesBuff, (int)sizeof(cmSecretsAesBuff));

	rc = CC_CommonAesCbcDecrypt(cmPwdFilePtr, cmSecretsAesBuff, cmSecretsAesBuffSize, cmSecretsBuff);
	if (rc != 0) {
		UTIL_LOG_ERR( "failed to CC_CommonAesCbcDecrypt() for cm secrets, rc %d\n", rc);
		return 1;
	}
...
int computeHbk(void)
{
	...
	rc = CC_CommonCalcHBKFromBuff(csrBuff.pubKey0, hbkBuff, sizeOfHash);
	if (rc != 0) {
		UTIL_LOG_ERR( "failed to CC_CommonCalcHBKFromBuff %d for pubKey0\n", rc);
		return 1;
	}
	/* If there is a second keypair the hbkbuff is combined from truncated_hbk0 + truncated_hbk1 */
	if (true == isSecKeyExist) {
		/* compute  hbkBuff */
		rc = CC_CommonCalcHBKFromFile(pubKey1FileName, hbkBuff+sizeOfHash, sizeOfHash);
		if (rc != 0) {
			UTIL_LOG_ERR( "failed to CC_CommonCalcHBKFromBuff %d for pubKey1\n", rc);
			return 1;
		}
	}
...
int buildKcp(void)
{
	...
	for (i = OEM_ASSET_KCP_KEY_BUFF_SIZE/2; i < OEM_ASSET_KCP_KEY_BUFF_SIZE; i++) {
		cpKeyBuff[i] = cmSecretsBuff[OEM_ASSET_PROV_KEY_FILE_KPRTL_OFFSET+i] ^ cmSecretsBuff[OEM_ASSET_PROV_KEY_FILE_SCP_OFFSET+j];
		j++;
	}
	return 0;
}
...
int getCsr(void)
{
	...
	rc = CC_CommonBase64Decode(csrBase64Buff, sizeof(csrBase64Buff), (unsigned char *)&csrBuff, &csrBuffSize);
	if ((rc != 0) ||
	    (csrBuffSize != sizeof(csrBuff))) {
		UTIL_LOG_ERR( "failed to CC_CommonBase64Decode() for csr, rc %d, csr length %d\n", rc, csrBuffSize);
		return 1;
	}
	...
	if (rc != 0) {
		UTIL_LOG_ERR( "failed to CC_CommonRsaVerify() for csr, rc %d\n", rc);
		return 1;
	}
...
static int deriveKey(char *pLabel, int labelSize, char *pContext, int contextSize,
		     char *pKey, int keySize, char *pKeyOut)
{
	...
	err = CC_CommonAesCmacEncrypt(dataIn, dataSize, pKey, keySize, pKeyOut);
...
int main (int argc, char *argv[])
{
	...
	rc = deriveKey(	(uint8_t *)keyPlatLabel, sizeof(keyPlatLabel), hbkBuff, sizeof(hbkBuff),
			cpKeyBuff, OEM_ASSET_KCP_KEY_BUFF_SIZE, pltKeyBuff);
	if (rc != 0) {
	...
	rc = deriveKey(	(uint8_t *)keyProvisionLabel, sizeof(keyProvisionLabel), hbkBuff, sizeof(hbkBuff),
			pltKeyBuff, CC_UTIL_KPLT_SIZE_IN_BYTES, oemKeyBuff);
	if (rc != 0) {
	...
	rc = CC_CommonRsaEncrypt(RSA_USE_PKCS_21_VERSION, csrBuff.pubKey0,  oemKeyBuff, (int)sizeof(oemKeyBuff), oemKeyRsa.oemKeyRsaEnc);
	if (rc != 0) {
		UTIL_LOG_ERR("failed to CC_CommonRsaEncrypt(), rc %d\n", rc);
		goto genOemEnd;
	}
	...
	rc = CC_CommonBase64Encode((char *)&oemKeyRsa, sizeof(oemKeyRsa), oemKeyRsaBase64Buff, &oemKeyRsaBase64BuffSize);
	if ((rc != 0) ||
	    (oemKeyRsaBase64BuffSize != sizeof(oemKeyRsaBase64Buff))) {
		UTIL_LOG_ERR( "failed to CC_CommonBase64Encode(), rc %d\n", rc);
		goto genOemEnd;
	}

	UTIL_LOG_BYTE_BUFF("oemKeyRsaBase64Buff", oemKeyRsaBase64Buff, (int)sizeof(oemKeyRsaBase64Buff));
	/* save buffer into file */
	rc = CC_CommonUtilCopyBuffToBinFile(oemKeyFileName, oemKeyRsaBase64Buff, oemKeyRsaBase64BuffSize);
	if (rc != 0) {
