...
#define OEM_KEY_BASE64_SIZE_IN_BYTES (CC_COMMON_CALC_BASE64_ENCODE_SIZE(sizeof(EncOemKeyBuff_t)))
#define OEM_ASSET_HEX_PREFIX_MAX_SIZE	     2
#define OEM_ASSET_MAX_UINT32	     0xFFFFFFFF

static uint64_t assetId = 0;
...
static unsigned char assetBinBuff[CC_UTIL_OEM_ASSET_DATA_MAX_SIZE_IN_BYTES] = {0};
...
static unsigned char assetPkg[CC_UTIL_OEM_ASSET_PACKAGE_MAX_SIZE_IN_BYTES] = {0};
static unsigned char assetIdBuf[UTIL_MAX_FILE_NAME] = {0};
...
void usage(void)
{
	...
	UTIL_LOG_ERR( "--pwd                  -w:\t optional, pwd of private key file name. If not exist, PEM should be entered manually\n");
...
int parseArguments (const int argc, char *const argv[])
{
	...
	char assetIdtempStr[UTIL_MAX_FILE_NAME];
	char *tmpP = assetIdtempStr;
	...
	while (1) {
		...
		case 'b':   // --asset-bin
			...
			break;
		case 'i':   // --asset-id
			strncpy(assetIdBuf, optarg, UTIL_MAX_FILE_NAME-1);
			assetIdBuf[UTIL_MAX_FILE_NAME-1] = '\0';
			UTIL_LOG_ERR("assetIdBuf %s\n", assetIdBuf);
			break;
		case 'p':   // --key-pair
	...
	if (strncmp(assetIdBuf, "0x", OEM_ASSET_HEX_PREFIX_MAX_SIZE)){ //if its not hexadecimal return error
		UTIL_LOG_ERR( "invalid asset_id in decimal and not hex\n");
		return 1;
	}

	assetId = strtoul(assetIdBuf, &tmpP, 16);
	if (assetId > OEM_ASSET_MAX_UINT32){ //asset_id is defined as uin32_t parameter.
		UTIL_LOG_ERR( "invalid asset_id, illegal size 0x%lu\n", assetId);
		return 1;
	}
	if (0 == assetId) {
		UTIL_LOG_ERR( "invalid asset_id 0x%lX\n", assetId);
		return 1;
	}
	if (*tmpP != '\0'){
		UTIL_LOG_ERR( "invalid asset_id, illegal hex value %s\n", tmpP);
		return 1;
	}
	...
	rc = CC_CommonUtilCopyDataFromBinFile(assetBinFileName, assetBinBuff, &assetBinBuffSize);
	if ((rc != 0) ||
	    (assetBinBuffSize & OEM_ASSET_MUL_16_BYTES_MASK) ||
	    (0 == assetBinBuffSize) ||
	    (assetBinBuffSize > CC_UTIL_OEM_ASSET_DATA_MAX_SIZE_IN_BYTES)) {
	...
	rc = CC_CommonUtilCopyDataFromBinFile(oemKeyFileName, oemKeyRsaBase64Buff, &oemKeyRsaBase64BuffSize);
	if ((rc != 0) ||
	    (oemKeyRsaBase64BuffSize > sizeof(oemKeyRsaBase64Buff))) {
...
int getOemKey(void)
{
	...
	rc = CC_CommonBase64Decode(oemKeyRsaBase64Buff, sizeof(oemKeyRsaBase64Buff), (char *)&oemKeyStr, &oemKeyStrSize);
	if ((rc != 0) ||
	    (oemKeyStrSize != sizeof(EncOemKeyBuff_t))) {
		UTIL_LOG_ERR( "failed to CC_CommonBase64Decode(), rc %d oemKeyStrSize %d\n", rc, oemKeyStrSize);
		return 1;
	}
	...
	rc = CC_CommonRsaDecrypt(RSA_USE_PKCS_21_VERSION, keyPairFileName, pwdFilePtr, oemKeyStr.oemKeyRsaEnc, RSA_OAEP_KEY_SIZE_IN_BYTES, oemKeyBuff);
	EVP_cleanup();
	CRYPTO_cleanup_all_ex_data();  /* cleanup application specific data to avoid memory leaks.*/
	if (rc != 0) {
		UTIL_LOG_ERR("failed to CC_CommonRsaDecrypt(), rc %d\n", rc);
		return 1;
	}
...
int main (int argc, char *argv[])
{
	...
	assetId32 = (uint32_t)assetId; // assetId is 64bits but only the lower 32 bits are initialized
	CONVERT_WORD_TO_BYTE_ARR(assetId32, &dataInBuff[i]);

	i+=sizeof(int);
	dataInBuff[i] = KOEM_DATA_IN_SUFIX_DATA;

	provKeyBuffSize = ASSET_PKG_AES_CMAC_RESULT_SIZE_IN_BYTES;
	rc = CC_CommonAesCmacEncrypt(dataInBuff, sizeof(dataInBuff), oemKeyBuff, CC_UTIL_KOEM_SIZE_IN_BYTES, provKeyBuff);
	if (rc != 0) {
		UTIL_LOG_ERR( "failed to CC_CommonAesCmacEncrypt() for provKeyBuff rc %d\n", rc);
		goto packEnd;
	}
	...
	rc = CC_CommonAesCcmEncrypt(provKeyBuff,
				       &assetPkg[ASSET_PKG_CCM_NONCE_OFFSET], ASSET_PKG_CCM_NONCE_SIZE,
				       &assetPkg[ASSET_PKG_CCM_ADDITIONAL_DATA_OFFSET], ASSET_PKG_CCM_ADDITIONAL_DATA_SIZE,
				       assetBinBuff, assetBinBuffSize,
				       &assetPkg[ASSET_PKG_EN_DATA_OFFSET], &assetEnDataSize,
				       &assetPkg[ASSET_PKG_EN_DATA_OFFSET+assetBinBuffSize], ASSET_PKG_MAC_SIZE);
	if (rc != 0) {
		UTIL_LOG_ERR( "failed to CC_CommonAesCcmEncrypt or invalid size max is %d actual is %d\n", assetBinBuffSize, assetEnDataSize);
		goto packEnd;
	}
	UTIL_LOG_BYTE_BUFF("assetPkg", assetPkg, ASSET_PKG_NONE_ASSET_DATA_SIZE+assetEnDataSize);

	/* save buffer into file */
	rc = CC_CommonUtilCopyBuffToBinFile(assetPktFileName, assetPkg, ASSET_PKG_NONE_ASSET_DATA_SIZE+assetEnDataSize);
	if (rc != 0) {
