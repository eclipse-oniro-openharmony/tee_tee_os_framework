...
static unsigned char csrBase64EncBuff[CC_COMMON_CALC_BASE64_ENCODE_SIZE(sizeof(CsrBuff_t))] = {0};
...
int main (int argc, char *argv[])
{
	...
	UTIL_LOG_INFO( "calling CC_CommonGetNbuffFromKeyPair\n");
	len = RSA_OAEP_KEY_SIZE_IN_BYTES;
	rc = CC_CommonGetNbuffFromKeyPair(keyPairFileName, pwdFilePtr, csrBuff.pubKey0, &len);
	if (rc != 0) {
		UTIL_LOG_ERR( "failed to CC_CommonGetNbuffFromKeyPair() for pubkey0, rc %d\n", rc);
		goto genCsrEnd;
	}

	UTIL_LOG_INFO( "calling CC_CommonRsaSign\n");
	/* first RSA_PSS sign using privKey0 , then  encode base64    */
	rc = CC_CommonRsaSign(RSA_USE_PKCS_21_VERSION,		     /* RSA version */
				(char *)&csrBuff,			      /* data */
	...
	if (rc != 0) {
		UTIL_LOG_ERR( "failed to CC_CommonRsaSign() for csr, rc %d\n", rc);
		goto genCsrEnd;
	}
	UTIL_LOG_BYTE_BUFF("csrBuff", (unsigned char *)&csrBuff, sizeof(csrBuff));

	csrBase64EncBuffSize = sizeof(csrBase64EncBuff);
	UTIL_LOG_INFO( "calling CC_CommonBase64Encode() for csr, size %d\n", csrBase64EncBuffSize);
	rc = CC_CommonBase64Encode((unsigned char *)&csrBuff, sizeof(csrBuff), csrBase64EncBuff, &csrBase64EncBuffSize);
	if (rc != 0) {
		UTIL_LOG_ERR( "failed to CC_CommonBase64Encode() for csr, rc %d\n", rc);
		goto genCsrEnd;
	}
	UTIL_LOG_BYTE_BUFF("csrBase64EncBuff", csrBase64EncBuff, sizeof(csrBase64EncBuff));


	rc = CC_CommonUtilCopyBuffToBinFile(csrFileName, csrBase64EncBuff, csrBase64EncBuffSize);
	if (rc != 0) {
