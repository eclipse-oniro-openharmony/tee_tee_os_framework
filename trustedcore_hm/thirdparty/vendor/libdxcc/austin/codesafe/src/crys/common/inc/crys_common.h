...
extern uint8_t DX_GLOBAL_FIPS_MODE;
extern uint16_t DX_GLOBAL_FIPS_SF_STATUS;
#endif
/* *********************** Public Functions ******************* */
...
CRYSError_t CRYS_COMMON_Init(uint32_t param);
 ...
 */
void CRYS_COMMON_AlignedWordsArrayToBytes(uint32_t *in32_ptr, uint32_t *out32_ptr, uint32_t outAlignBits,
                                          uint32_t sizeWords);
 ...
 */
CRYSError_t CRYS_COMMON_ConvertMsbLsbBytesToLswMswWords(uint32_t *out32_ptr, uint32_t sizeOutBuffBytes,
                                                        uint8_t *in8_ptr, uint32_t sizeInBytes);
 ...
 *                        value MODULE_* as defined in .
 */
CRYSError_t CRYS_COMMON_ConvertLswMswWordsToMsbLsbBytes(uint8_t *out8_ptr, uint32_t sizeOutBuffBytes,
 ...
 *
 * When no_rtos is declared then we allow a global data. The random key/AES secret key are hidden as static inside the
 * function
 *
 *
 * @param[in] Globalid       select the buffer
 * @param[in] GlobalDataSizeWords       - the global data buffer size needed in words - this value must be a
 * predetermined value
 * @param[out] GlobalData_ptr - Pointer to the global buffer returned. The buffer must be at least GlobalDataSizeWords
 * size
 *
 ...
 * @param[in] GlobalDataSizeWords       - the global data buffer size needed in words - this value must be a
 * predetermined value
 * @param[in] GlobalData_ptr - Pointer to the global buffer to be saved. The buffer must be at least GlobalDataSizeWords
 * size
 *
 *   Return Value:
 */
 ...
 */
CRYSError_t CRYS_COMMON_CutAndSaveEndOfLliData(uint32_t *SrcLliTab_ptr, uint32_t *SrcLliTabSize_ptr, uint8_t *Dst_ptr,
                                               uint32_t DataSize);
 ...
 */
void CRYS_COMMON_CutAndSaveBeginOfLliData(uint32_t **SrcLliTab_ptr_ptr, uint32_t *SrcLliTabSize_ptr, uint8_t *Dst_ptr,
                                          uint32_t DataSize);
