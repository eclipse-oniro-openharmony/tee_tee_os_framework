...
/* *********************** Macros ***************************** */
#ifndef IS_ALIGNED
#define IS_ALIGNED(val, align) (((DxVirtAddr_t)(val) & ((align)-1)) == 0)
#endif
...
#define CRYS_COMMON_REVERSE32(x) \
    (((CRYS_COMMON_ROT32((x)) & 0xff00ff00UL) >> 8) | ((CRYS_COMMON_ROT32((x)) & 0x00ff00ffUL) << 8))

#define SHIFT_LEFT(x, nBits)  ((x) >> (nBits))
#define SHIFT_RIGHT(x, nBits) ((x) << (nBits))
#define CONVERT_LE_2_CPU_E(x) (x)
#define CONVERT_CPU_E_2_BE(x) CRYS_COMMON_REVERSE32(x)
...
#ifdef BIG__ENDIAN
#define CRYS_COMMON_CONVERT_TO_LE32(in32_ptr, out32_ptr, size_words) \
    {                                                                \
        uint32_t i;                                                  \
...
#define CRYS_COMMON_GET_BIT_VAL_FROM_WORD_ARRAY(ptr, bit_pos) (((ptr)[(bit_pos) >> 5] >> ((bit_pos)&0x1FUL)) & 1UL)

/* exchange a bit on a word array */
#define CRYS_COMMON_EXCHANGE_BIT_ON_WORD_ARRAY(ptr, bit_pos) ((ptr)[(bit_pos) >> 5] ^= (1UL << ((bit_pos)&0x1FUL)))
...
#define CRYS_COMMON_Copy4wordsToBytes(out32_ptr, outAlign, in32_ptr)                                                   \
    if (outAlign != 0) {                                                                                               \
        (out32_ptr)[0] =                                                                                               \
            ((out32_ptr)[0] & (0xFFFFFFFF << (32 - (outAlign)))) | CRYS_COMMON_REVERSE32((in32_ptr)[0]) >> (outAlign); \
        ...
        (out32_ptr)[4] = ((out32_ptr)[4] & (0xFFFFFFFF >> (outAlign))) | CRYS_COMMON_REVERSE32((in32_ptr)[3])          \
                                                                             << (32 - (outAlign));                     \
    } else {                                                                                                           \
...
#define CRYS_COMMON_CopyWordsToBytes(out32_ptr, outAlign, in32_ptr, inOffset) \
    CRYS_COMMON_Copy4wordsToBytes(out32_ptr, outAlign, in32_ptr)
#define CRYS_COMMON_CopyBytesToWords(in32_ptr, inAlign, out32_ptr, outOffset) \
    CRYS_COMMON_Copy16BytesToWords(in32_ptr, inAlign, out32_ptr)

/* *********************** Enums ****************************** */

/* the counter comperation result enum */
typedef enum {
    CRYS_COMMON_CmpCounter1AndCounter2AreIdentical = 0,
...
void CRYS_COMMON_IncMsbUnsignedCounter(uint32_t *CounterBuff_ptr, uint32_t Val, uint32_t CounterSize);
...
uint32_t CRYS_COMMON_IncLsbUnsignedCounter(uint32_t *CounterBuff_ptr, uint32_t Val, uint32_t CounterSize);
...
void CRYS_COMMON_IncLsbUnsignedCounterBigEndian(uint32_t *CounterBuff_ptr, uint32_t Val, uint32_t CounterSize);
...
void CRYS_COMMON_DecrLsbUnsignedCounter(uint32_t *CounterBuff_ptr, uint32_t Val, uint32_t CounterSizeInWords);
...
void CRYS_COMMON_DecrMsbUnsignedCounter(uint32_t *CounterBuff_ptr, uint32_t Val, uint32_t CounterSizeInWords);
...
CRYS_COMMON_CmpCounter_t CRYS_COMMON_CmpMsbUnsignedCounters(const uint8_t *CounterBuff1_ptr, uint32_t Counter1Size,
...
CRYS_COMMON_CmpCounter_t CRYS_COMMON_CmpLsbUnsignedCounters(const uint8_t *CounterBuff1_ptr, uint32_t Counter1Size,
                                                            const uint8_t *CounterBuff2_ptr, uint32_t Counter2Size);
 ...
 *                                2 - counter 2 is larger.
 */
CRYS_COMMON_CmpCounter_t CRYS_COMMON_CmpLsWordsUnsignedCounters(uint32_t *CounterBuff1_ptr, uint32_t Counter1SizeWords,
                                                                uint32_t *CounterBuff2_ptr, uint32_t Counter2SizeWords);
...
uint32_t CRYS_COMMON_GetBytesCounterEffectiveSizeInBits(const uint8_t *CounterBuff_ptr, uint32_t CounterSize);
 ...
 *
 */
uint32_t CRYS_COMMON_GetWordsCounterEffectiveSizeInBits(uint32_t *CounterBuff_ptr, uint32_t CounterSizeWords);
 ...
 * @return no return value.
 */
void CRYS_COMMON_ShiftLeftVector(uint8_t *VecBuff_ptr, uint32_t SizeInBytes, int8_t Shift);
...
uint32_t CRYS_COMMON_SubtractUintArrays(uint32_t *A_ptr, uint32_t *B_ptr, uint32_t SizeInWords, uint32_t *Res_ptr);
...
uint32_t CRYS_COMMON_AddTwoLsbUint8Vectors(uint8_t *A_ptr, uint8_t *B_ptr, uint32_t VectSizeInBytes, uint8_t *Res_ptr);
...
*/
uint8_t CRYS_COMMON_SubtractMSBUint8Arrays(uint8_t *A_ptr, uint32_t sizeA, uint8_t *B_ptr, uint32_t sizeB,
                                           uint8_t *Res_ptr);
