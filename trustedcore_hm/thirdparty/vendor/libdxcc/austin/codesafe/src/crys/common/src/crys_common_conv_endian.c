                                                        ...
                                                        uint8_t *in8_ptr, uint32_t sizeInBytes)
{
    ...
    if ((sizeInBytes == 0) || (((DxVirtAddr_t)out32_ptr == (DxVirtAddr_t)in8_ptr) && (sizeInBytes & 3)))
    ...
    *   buffer starting from non zero MSB */
    if ((DxVirtAddr_t)out32_ptr != (DxVirtAddr_t)in8_ptr) {
        ...
        /* zeroe empty bytes */
        out32_ptr[0] = 0;
        DX_PAL_MemSetZero((uint8_t *)out32_ptr + (sizeInBytes & 3UL), sizeOutBuffBytes - (sizeInBytes & 3UL));
        /* copy */
        DX_PAL_MemCopy((uint8_t *)out32_ptr + ((4 - (sizeInBytes & 3UL)) & 3UL), in8_ptr, sizeInBytes);
    }
                                                        ...
                                                        uint32_t *in32_ptr, uint32_t sizeInBytes)
{
    ...
    if ((DxVirtAddr_t)out8_ptr != (DxVirtAddr_t)in32_ptr) {
        ...
        DX_PAL_MemCopy(out8_ptr, (uint8_t *)in32_ptr + ((4 - (sizeInBytes & 3UL)) & 3UL), sizeInBytes);
