...
static CRYS_KDF_HASH_OpMode_t MakeKDFHashMode(CRYS_DH_HASH_OpMode_t hashMode)
{
    CRYS_KDF_HASH_OpMode_t outMode;

    switch (hashMode) {
    case CRYS_DH_HASH_SHA1_mode:
        outMode = CRYS_KDF_HASH_SHA1_mode;
        break;
    case CRYS_DH_HASH_SHA224_mode:
        outMode = CRYS_KDF_HASH_SHA224_mode;
        break;
    case CRYS_DH_HASH_SHA256_mode:
        outMode = CRYS_KDF_HASH_SHA256_mode;
        break;
    case CRYS_DH_HASH_SHA384_mode:
        outMode = CRYS_KDF_HASH_SHA384_mode;
        break;
    case CRYS_DH_HASH_SHA512_mode:
        outMode = CRYS_KDF_HASH_SHA512_mode;
        break;
    default:
        outMode = CRYS_KDF_HASH_OpModeLast;
    }
...
static CRYS_KDF_DerivFuncMode_t MakeKDFDeriveFuncMode(CRYS_DH_DerivationFunc_Mode deriveFunc)
{
    CRYS_KDF_DerivFuncMode_t outDeriveFunc;

    /* to avoid the problem of two same definitions in CRYS_DH_DerivationFunc_Mode */
    if (deriveFunc == CRYS_DH_Concat_Der_mode)
        return CRYS_KDF_ConcatDerivMode;

    switch (deriveFunc) {
    case CRYS_DH_ASN1_Der_mode:
        outDeriveFunc = CRYS_KDF_ASN1_DerivMode;
        break;
    case CRYS_DH_X963_DerMode:
        outDeriveFunc = CRYS_KDF_ConcatDerivMode;
        break;
    case CRYS_DH_OMADRM_DerMode:
        outDeriveFunc = CRYS_KDF_OMADRM_DerivMode;
        break;
    default:
        outDeriveFunc = CRYS_DH_DerivationFunc_ModeLast;
    }
 ...
 * @brief _DX_DH_GeneratePubPrv has 2 functionalities:
  ...
  @return CRYSError_t - On success CRYS_OK is returned, on failure an ERROR as defined CRYS_DH_error.h:
                        ...
                        CRYS_DH_INVALID_ARGUMENT_OPERATION_MODE_ERROR;
 */
CEXPORT_C CRYSError_t _DX_DH_GeneratePubPrv(uint8_t *Generator_ptr,                     /* generator */
                                            ...
                                            uint16_t QSize, CRYS_DH_OpMode_t DH_mode, /* in */
...

{
    ...
    RETURN_IF_DH_UNSUPPORTED(Generator_ptr, GeneratorSize, Prime_ptr, PrimeSize, L, Q_ptr, QSize, Error /* DH_mode */,
                             tmpPubKey_ptr, tmpPrimeData_ptr, ClientPrvKey_ptr, ClientPrvKeySize_ptr, ClientPub1_ptr,
                             ClientPubSize_ptr, tmpByte, tmpSize, comp, mask, mask1, shift, Error, Error);
    ...
    if (Generator_ptr == DX_NULL || Prime_ptr == DX_NULL || ClientPrvKey_ptr == DX_NULL || ClientPub1_ptr == DX_NULL ||
        ClientPrvKeySize_ptr == DX_NULL || ClientPubSize_ptr == DX_NULL || tmpPubKey_ptr == DX_NULL ||
        tmpPrimeData_ptr == DX_NULL)
    ...
    if (PrimeSize > CRYS_DH_MAX_MOD_SIZE_IN_BYTES || PrimeSize == 0)
    ...
    if (*ClientPubSize_ptr < PrimeSize)
        return CRYS_DH_INVALID_PUBLIC_KEY_SIZE_ERROR;

    /* Compare the generator with zero: requested g > 0 */
    tmpByte = 0;
    comp    = CRYS_COMMON_CmpMsbUnsignedCounters(Generator_ptr, GeneratorSize, &tmpByte, 1);
    ...
    comp = CRYS_COMMON_CmpMsbUnsignedCounters(Generator_ptr, GeneratorSize, Prime_ptr, PrimeSize);
    ...
    CRYS_COMMON_ReverseMemcpy((uint8_t *)tmpPrimeData_ptr->DataIn, Prime_ptr, PrimeSize);

    /* get actual size of prime in bits: min() used to prevent warnings */
    tmpSize =
        min(PrimeSize * 8,
            (uint16_t)CRYS_COMMON_GetBytesCounterEffectiveSizeInBits((uint8_t *)tmpPrimeData_ptr->DataIn, PrimeSize));
    ...
    PKCS#3:  set x private random value according to following:
        ...
        if (L == 0) {
            ...
            CRYS_COMMON_ReverseMemcpy(ClientPrvKey_ptr, (uint8_t *)tmpPrimeData_ptr->DataOut, PrimeSize);
            ...
            *ClientPrvKeySize_ptr = (L + 7) / 8;
            if (*ClientPrvKeySize_ptr > CRYS_DH_MAX_MOD_SIZE_IN_BYTES)
            ...
            if (tmpSize == L) {
                ...
                /* reverse privKey to big endianness */
                CRYS_COMMON_ReverseMemcpy(ClientPrvKey_ptr, (uint8_t *)tmpPrimeData_ptr->DataOut,
    ...
    case CRYS_DH_ANSI_X942_mode:
        ...
        /* check order */
        if (Q_ptr == DX_NULL) {
            Error = CRYS_DH_INVALID_ARGUMENT_POINTER_ERROR;
            goto End;
        }
        ...
        CRYS_COMMON_ReverseMemcpy((uint8_t *)tmpPrimeData_ptr->DataIn, Q_ptr, QSize);

        /* get actual size in bits */
        tmpSize = (uint16_t)CRYS_COMMON_GetBytesCounterEffectiveSizeInBits((uint8_t *)tmpPrimeData_ptr->DataIn, QSize);
        /* private key size in bytes */
        *ClientPrvKeySize_ptr = (tmpSize + 7) / 8;

        /* random generation in range:  0 < x < Q-1  (little endianness) */
        Error =
            CRYS_RND_GenerateVectorInRange(tmpSize /* rndSizeInBits */, (uint8_t *)tmpPrimeData_ptr->DataIn /* maxVect */,
                                           (uint8_t *)tmpPrimeData_ptr->DataOut /* out */);
        if (Error != CRYS_OK)
            goto End;

        /* reverse privKey to big endianness */
        CRYS_COMMON_ReverseMemcpy(ClientPrvKey_ptr, (uint8_t *)tmpPrimeData_ptr->DataOut, *ClientPrvKeySize_ptr);
    ...
    Error = CRYS_RSA_Build_PubKey(tmpPubKey_ptr, ClientPrvKey_ptr, *ClientPrvKeySize_ptr, Prime_ptr, PrimeSize);
    /* check error */
    if (Error != CRYS_OK) {
        goto End;
    }
    ...
    Error = CRYS_RSA_PRIM_Encrypt(tmpPubKey_ptr, tmpPrimeData_ptr, Generator_ptr, GeneratorSize, ClientPub1_ptr);
    if (Error != CRYS_OK) {
        goto End;
    }
...
End:
    ...
    DX_PAL_MemSetZero(tmpPubKey_ptr, sizeof(CRYS_DHUserPubKey_t));
    DX_PAL_MemSetZero(tmpPrimeData_ptr, sizeof(CRYS_DHPrimeData_t));
...
*/
CEXPORT_C CRYSError_t CRYS_DH_GetSecretKey(uint8_t *ClientPrvKey_ptr, uint16_t ClientPrvKeySize,
                                           uint8_t *ServerPubKey_ptr, uint16_t ServerPubKeySize, uint8_t *Prime_ptr,
                                           uint16_t PrimeSize, CRYS_DHUserPubKey_t *tmpPubKey_ptr,
                                           CRYS_DHPrimeData_t *tmpPrimeData_ptr, uint8_t *SecretKey_ptr,
                                           uint16_t *SecretKeySize_ptr)
{
    ...
    RETURN_IF_DH_UNSUPPORTED(ClientPrvKey_ptr, ClientPrvKeySize, ServerPubKey_ptr, ServerPubKeySize, Prime_ptr,
                             PrimeSize, tmpPubKey_ptr, tmpPrimeData_ptr, SecretKey_ptr, SecretKeySize_ptr, cmpResult,
    ...
    /* if an argument pointer is DX_NULL return an error */
    if (ClientPrvKey_ptr == DX_NULL || ServerPubKey_ptr == DX_NULL || Prime_ptr == DX_NULL ||
        tmpPubKey_ptr == DX_NULL || tmpPrimeData_ptr == DX_NULL || SecretKey_ptr == DX_NULL ||
        SecretKeySize_ptr == DX_NULL)

        return CRYS_DH_INVALID_ARGUMENT_POINTER_ERROR;

    /* If an argument buffer size is zero return an error */
    if (PrimeSize == 0 || PrimeSize > CRYS_DH_MAX_MOD_SIZE_IN_BYTES || ClientPrvKeySize == 0 ||
    ...
    cmpResult = CRYS_COMMON_CmpMsbUnsignedCounters(ClientPrvKey_ptr, ClientPrvKeySize, Prime_ptr, PrimeSize);
    ...
    if (cmpResult != CRYS_COMMON_CmpCounter2GraterThenCounter1) {
        ...
        Error = CRYS_RSA_Build_PubKey(tmpPubKey_ptr, (uint8_t *)tmpPrimeData_ptr->DataIn, ClientPrvKeySize, Prime_ptr,
                                      PrimeSize);
    } else {
        /* build the Data for the Exp operation */
        Error = CRYS_RSA_Build_PubKey(tmpPubKey_ptr, ClientPrvKey_ptr, ClientPrvKeySize, Prime_ptr, PrimeSize);
    }
    ...
    Error = CRYS_RSA_PRIM_Encrypt(tmpPubKey_ptr, /* Note: this is the private key */
...
*/
CEXPORT_C CRYSError_t CRYS_DH_X942_GetSecretData(uint8_t *ClientPrvKey_ptr, uint16_t ClientPrvKeySize,
                                                 uint8_t *ServerPubKey_ptr, uint16_t ServerPubKeySize,
                                                 uint8_t *Prime_ptr, uint16_t PrimeSize,
                                                 CRYS_DH_OtherInfo_t *otherInfo_ptr, CRYS_DH_HASH_OpMode_t hashMode,
                                                 CRYS_DH_DerivationFunc_Mode DerivFunc_mode,
                                                 CRYS_DH_Temp_t *tmpBuff_ptr, uint8_t *SecretKeyingData_ptr,
                                                 uint16_t SecretKeyingDataSize)
{
    ...
    RETURN_IF_DH_UNSUPPORTED(ClientPrvKey_ptr, ClientPrvKeySize, ServerPubKey_ptr, ServerPubKeySize, Prime_ptr,
                             PrimeSize, otherInfo_ptr, Error /* hashMode */, Error /* DerivFunc_mode */, tmpBuff_ptr,
                             SecretKeyingData_ptr, SecretKeyingDataSize, SecretKeySize, Error, Error, Error, Error,
    ...
    Error = CRYS_DH_GetSecretKey(ClientPrvKey_ptr, ClientPrvKeySize, ServerPubKey_ptr, ServerPubKeySize, Prime_ptr,
    ...
    Error =
        CRYS_KDF_KeyDerivFunc((uint8_t *)tmpBuff_ptr->TempBuff, SecretKeySize, otherInfo_ptr, MakeKDFHashMode(hashMode),
                              MakeKDFDeriveFuncMode(DerivFunc_mode), SecretKeyingData_ptr, SecretKeyingDataSize);

ExitOnError:

    DX_PAL_MemSetZero(tmpBuff_ptr, sizeof(CRYS_DH_Temp_t));
...
CEXPORT_C CRYSError_t CRYS_DH_X942_HybridGetSecretData(
    ...
    uint8_t *Prime_ptr, uint16_t PrimeSize, CRYS_DH_OtherInfo_t *otherInfo_ptr, CRYS_DH_HASH_OpMode_t hashMode,
    CRYS_DH_DerivationFunc_Mode DerivFunc_mode, CRYS_DH_HybrTemp_t *tmpDhHybr_ptr, uint8_t *SecretKeyingData_ptr,
    uint16_t SecretKeyingDataSize)
{
    /* FUNCTION DECLARATIONS */
    ...
    RETURN_IF_DH_UNSUPPORTED(ClientPrvKey_ptr1, ClientPrvKeySize1, ClientPrvKey_ptr2, ClientPrvKeySize2,
                             ServerPubKey_ptr1, ServerPubKeySize1, ServerPubKey_ptr2, ServerPubKeySize2, Prime_ptr,
                             PrimeSize, otherInfo_ptr, Error /* hashMode */, Error /* DerFunc_mode */, tmpDhHybr_ptr,
                             SecretKeyingData_ptr, SecretKeyingDataSize, SecretKeyDataSize1, SecretKeyDataSize2, Error,
                             Error, Error, Error);
    ...
    Error = CRYS_DH_GetSecretKey(ClientPrvKey_ptr1, ClientPrvKeySize1, ServerPubKey_ptr1, ServerPubKeySize1, Prime_ptr,
                                 PrimeSize, &tmpDhHybr_ptr->UserPubKey, &tmpDhHybr_ptr->PrimeData,
                                 (uint8_t *)&tmpDhHybr_ptr->TempBuff, &SecretKeyDataSize1);
    if (Error != CRYS_OK)
        goto End;

    /* get shared secret key (value) 2 */
    Error = CRYS_DH_GetSecretKey(ClientPrvKey_ptr2, ClientPrvKeySize2, ServerPubKey_ptr2, ServerPubKeySize2, Prime_ptr,
                                 PrimeSize, &tmpDhHybr_ptr->UserPubKey, &tmpDhHybr_ptr->PrimeData,
...
End:
    DX_PAL_MemSetZero(tmpDhHybr_ptr, sizeof(CRYS_DH_HybrTemp_t));
 ...
 */
CEXPORT_C CRYSError_t CRYS_DH_CheckPubKey(uint8_t *modP_ptr,        /* in */
                                          ...
                                          CRYS_DH_Temp_t *tempBuff_ptr /* in */)
{
    ...
    RETURN_IF_DH_UNSUPPORTED(modP_ptr, modPsizeBytes, orderQ_ptr, orderQsizeBytes, pubKey_ptr, pubKeySizeBytes,
                             tempBuff_ptr, modPsizeBits, orderQsizeBits, cmpRes, cmpCounters, pubKeySizeBits,
                             tmpPubKey_ptr, Error, Error, Error, Error, Error, Error, Error, Error, Error);
    ...
    if (modP_ptr == DX_NULL || orderQ_ptr == DX_NULL || pubKey_ptr == DX_NULL || tempBuff_ptr == DX_NULL) {
        return CRYS_DH_INVALID_ARGUMENT_POINTER_ERROR;
    }
    ...
    /* ------------------------------------ */
    DX_PAL_MemSetZero(tempBuff_ptr, sizeof(CRYS_DH_ExpTemp_t));
    ...
    Error = CRYS_COMMON_ConvertMsbLsbBytesToLswMswWords(tempBuff_ptr->PrimeData.DataIn, modPsizeBytes, pubKey_ptr,
                                                        pubKeySizeBytes);
    if (Error)
        return Error;
    ...
    modPsizeBits = min(8 * modPsizeBytes,
                       CRYS_COMMON_GetWordsCounterEffectiveSizeInBits(tmpPubKey_ptr->n, (uint16_t)modPsizeBytes / 4));
    orderQsizeBits =
        min(8 * orderQsizeBytes,
            CRYS_COMMON_GetWordsCounterEffectiveSizeInBits(tmpPubKey_ptr->e, (uint16_t)(orderQsizeBytes + 3) / 4));
    pubKeySizeBits = min(8 * pubKeySizeBytes, CRYS_COMMON_GetWordsCounterEffectiveSizeInBits(
                                                  tempBuff_ptr->PrimeData.DataIn, (uint16_t)(pubKeySizeBytes + 3) / 4));

    /* check sizes */
    if (modPsizeBits < CRYS_DH_MIN_VALID_KEY_SIZE_VALUE_IN_BITS || modPsizeBits % 256 != 0 ||
        modPsizeBits > CRYS_DH_MAX_VALID_KEY_SIZE_VALUE_IN_BITS) {
        Error = CRYS_DH_INVALID_MODULUS_SIZE_ERROR;
        goto End;
    }

    if (orderQsizeBits < CRYS_DH_SEED_MIN_SIZE_IN_BITS || orderQsizeBits % 32 != 0) {
        Error = CRYS_DH_INVALID_ORDER_SIZE_ERROR;
        goto End;
    }
    ...
    cmpCounters = CRYS_COMMON_CmpLsWordsUnsignedCounters(tmpPubKey_ptr->e, /* counter1 - pubKey */
    ...
    DX_PAL_MemSetZero((uint8_t *)&tmpPubKey_ptr->n, modPsizeBytes);
    tmpPubKey_ptr->n[0] = 1;

    /* compare DataOut to 1: */
    cmpRes = CRYS_COMMON_CmpLsWordsUnsignedCounters(tempBuff_ptr->PrimeData.DataOut, modPsizeBytes / 4,
...
End:
    ...
    /* clean temp buffers */
    DX_PAL_MemSetZero(tempBuff_ptr, sizeof(CRYS_DH_Temp_t));
