...
#define RETURN_IF_DH_UNSUPPORTED(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, a1, b1, c1, d1, e1, f1, g1, h1, \
...
CRYSError_t DX_RND_GenerateWordsArrayInRange(uint32_t rndSizeInBits, uint32_t *maxVect_ptr, uint32_t *rndVect_ptr,
                                             uint32_t *tmp_ptr);
 ...
 */
static uint32_t DX_DH_KG_AddValueToMsbLsbBytesArray(uint8_t *arr_ptr, uint32_t val, uint32_t sizeBytes)
{
    int32_t i;
...
#ifndef BIG__ENDIAN
        tmp = curr = CRYS_COMMON_REVERSE32(ptr[i]);
#else
        tmp = curr = ptr[i];
#endif
        tmp += val;
...
#ifndef BIG__ENDIAN
        ptr[i] = CRYS_COMMON_REVERSE32(tmp);
#else
        ptr[i]     = tmp;
#endif
...
uint32_t DX_DH_KG_GetSizeInBitsOfMsbLsbBytesArray(uint8_t *arr_ptr, uint32_t sizeInBytes)
{
 ...
 * bytes. Note: The seed is set inthe buffer as BE bytes array.
 ...
 *
 */
static CRYSError_t DX_DH_X942_FindPrimeQ(uint32_t QsizeBits,      /* in */
                                         ...
                                         uint32_t *TempBuff3_ptr) /* in */
{
    ...
    uint8_t *hashDataIn1_ptr, *hashDataIn2_ptr;
    ...
    uint32_t QsizeBytes, QsizeWords;

    /* exact seed size in bits and in words */
    uint32_t seedSizeBytes, seedSizeWords, remainingSize;
    ...
    if (Q_ptr == DX_NULL || S_ptr == DX_NULL || TempBuff1_ptr == DX_NULL || TempBuff2_ptr == DX_NULL) {
    ...
    /* zeroing  Q buffer */
    DX_PAL_MemSetZero(Q_ptr, QsizeBytes);

    /* set HASH pointers to temp buffer */
    hashDataIn1_ptr = (uint8_t *)TempBuff1_ptr;
    hashDataIn2_ptr = (uint8_t *)TempBuff2_ptr;
    ...
    if (generateSeed != 1) {
        ...
        DX_PAL_MemCopy(hashDataIn1_ptr, S_ptr, seedSizeBytes);
        DX_PAL_MemCopy(hashDataIn2_ptr, S_ptr, seedSizeBytes);
    ...
    while (isPrime != DX_TRUE) {
        ...
        if (generateSeed == 1) {
            ...
            Error = CRYS_RND_GenerateVector((uint16_t)seedSizeBytes, S_ptr);
            ...
            DX_PAL_MemCopy(hashDataIn2_ptr, hashDataIn1_ptr, seedSizeBytes);
        ...
        current_ptr   = &((uint8_t *)Q_ptr)[QsizeBytes - CRYS_HASH_SHA1_DIGEST_SIZE_IN_BYTES];
        ...
        for (i = 0; i < M1; i++) {
            ...
            Error = CRYS_HASH(CRYS_HASH_SHA1_mode, hashDataIn1_ptr, seedSizeBytes, *hashRes1_ptr);
            ...
            Error = CRYS_HASH(CRYS_HASH_SHA1_mode, (uint8_t *)hashDataIn2_ptr, seedSizeBytes, *hashRes2_ptr);
            ...
            if (remainingSize >= CRYS_DH_SEED_MIN_SIZE_IN_BYTES) {
                DX_PAL_MemCopy(current_ptr, hashRes1_ptr, CRYS_DH_SEED_MIN_SIZE_IN_BYTES);
                ...
                DX_PAL_MemCopy((uint8_t *)Q_ptr,
                               (uint8_t *)hashRes1_ptr + CRYS_DH_SEED_MIN_SIZE_IN_BYTES - remainingSize, remainingSize);
            }
        ...
        /* convert Q to words */
        Error = CRYS_COMMON_ConvertMsbLsbBytesToLswMswWords(Q_ptr, QsizeBytes, (uint8_t *)Q_ptr, QsizeBytes);
        if (Error) {
            goto EndWithError;
        }

        Error = LLF_PKI_RSA_primeTestCall(Q_ptr, QsizeBytes / 4, 8 /* rabinTestsCount */, &isPrime, TempBuff3_ptr,
...
EndWithError:
    ...
    DX_PAL_MemSetZero((uint8_t *)Q_ptr, QsizeBytes);
    DX_PAL_MemSetZero((uint8_t *)S_ptr, seedSizeBytes);
 ...
 *
 */
static CRYSError_t DX_DH_X942_FindPrimeP(uint32_t modPsizeBits,     /* in */
                                         ...
                                         uint32_t *TempBuff_ptr)    /* in */
{
    ...
    /* check pointers: modP, generator and tempBuff. Note: other pointers may be NULL  */
    if (P_ptr == DX_NULL || Q_ptr == DX_NULL || S_ptr == DX_NULL || pgenCounter_ptr == DX_NULL ||
        TempBuff1_ptr == DX_NULL || TempBuff2_ptr == DX_NULL) {
        return CRYS_DH_INVALID_ARGUMENT_POINTER_ERROR;
    }
    ...
    /* zeroing of P  */
    DX_PAL_MemSetZero(P_ptr, modSizeBytes + 2);
    ...
    *pgenCounter_ptr = 0;
    ...
    /* set HASH pointers to temp buffer */
    hashDataIn_ptr = (uint8_t *)TempBuff1_ptr;
    ...
    DX_PAL_MemCopy(hashDataIn_ptr, S_ptr, seedSizeBytes);
    ...
    while (isPrime != DX_TRUE) {
        ...
        current_ptr   = &((uint8_t *)P_ptr)[modSizeBytes - CRYS_DH_SEED_MIN_SIZE_IN_BYTES];
        ...
        for (i = 0; i < L1; i++) {
            ...
            Error = CRYS_HASH(CRYS_HASH_SHA1_mode, hashDataIn_ptr, seedSizeBytes, *hashRes_ptr);
            ...
            if (remainingSize >= CRYS_DH_SEED_MIN_SIZE_IN_BYTES) {
                DX_PAL_MemCopy(current_ptr, hashRes_ptr, CRYS_DH_SEED_MIN_SIZE_IN_BYTES);
                remainingSize -= CRYS_DH_SEED_MIN_SIZE_IN_BYTES;
                current_ptr -= CRYS_DH_SEED_MIN_SIZE_IN_BYTES;
            } else {
                DX_PAL_MemCopy((uint8_t *)P_ptr,
                               (uint8_t *)hashRes_ptr + CRYS_DH_SEED_MIN_SIZE_IN_BYTES - remainingSize, remainingSize);
            }
        ...
        Error = CRYS_COMMON_ConvertMsbLsbBytesToLswMswWords(P_ptr, modSizeWords * 4, (uint8_t *)P_ptr, modSizeBytes);
        if (Error) {
            return Error;
        }
        ...
        DX_PAL_MemSetZero((uint8_t *)TempBuff4_ptr, modSizeBytes);

        LLF_PKI_RSA_Call_Div(P_ptr,              /* numerator P */
        ...
        /* add 1 to P */
        CRYS_COMMON_IncLsbUnsignedCounter(P_ptr, 1, (uint8_t)modSizeWords);
        ...
        /* compare */
        cmpRes = CRYS_COMMON_CmpLsWordsUnsignedCounters(P_ptr, (uint16_t)modSizeWords, TempBuff4_ptr,
        ...
        if (cmpRes == CRYS_COMMON_CmpCounter1GraterThenCounter2) {
            Error = LLF_PKI_RSA_primeTestCall(P_ptr, modSizeWords, 8 /* rabinTestsCount */, (int8_t *)&isPrime,
...
EndWithError:
    ...
    DX_PAL_MemSetZero(P_ptr, modSizeBytes);
 ...
 *
 */
static CRYSError_t DX_DH_X942_CreateGenerator(uint32_t *P_ptr,                    /* in */
                                              ...
                                              CRYS_DH_ExpTemp_t *expTempBuff_ptr) /* in */
{
    ...
    DX_PAL_MemCopy((uint8_t *)expTempBuff_ptr->PubKey.n, (uint8_t *)P_ptr, modSizeBytes);
    DX_PAL_MemSetZero((uint8_t *)expTempBuff_ptr->PubKey.n + modSizeBytes,
                      CRYS_DH_MAX_MOD_SIZE_IN_BYTES - modSizeBytes);
    /* copy order Q into aligned buffer */
    DX_PAL_MemCopy((uint8_t *)expTempBuff_ptr->TempBuff, (uint8_t *)Q_ptr, orderSizeBytes);
    DX_PAL_MemSetZero((uint8_t *)expTempBuff_ptr->TempBuff + orderSizeBytes,
                      CRYS_DH_MAX_MOD_SIZE_IN_BYTES - orderSizeBytes);
    ...
    J_effectiveSizeBits = min(modSizeBits, CRYS_COMMON_GetWordsCounterEffectiveSizeInBits(tempBuff1_ptr, modSizeWords));
    ...
    DX_PAL_MemSetZero((uint8_t *)&expTempBuff_ptr->PrimeData, sizeof(CRYS_DHPrimeData_t));

    /* generating rnd vector */
    Error = DX_RND_GenerateWordsArrayInRange(modSizeBits, expTempBuff_ptr->PubKey.n /* P-1 */,
    ...
    /* cleaning of temp buffer */
    DX_PAL_MemSetZero((uint8_t *)&expTempBuff_ptr->PubKey, sizeof(expTempBuff_ptr->PubKey));
    ...
    DX_PAL_MemCopy((uint8_t *)expTempBuff_ptr->PubKey.e, (uint8_t *)tempBuff1_ptr,
                   4 * ((J_effectiveSizeBits + 31) / 32));
    ...
    while (compFlag == 0) {
        ...
        DX_PAL_MemSetZero((uint8_t *)tempBuff1_ptr, modSizeBytes);
        tempBuff1_ptr[0] = 1;

        /* compare DataOut to 1: */
        compFlag = CRYS_COMMON_CmpLsWordsUnsignedCounters(expTempBuff_ptr->PrimeData.DataOut, modSizeWords,
                                                          tempBuff1_ptr, modSizeWords);

        /* if G == 1 change DataIn (by adding 1) for trying next G value */
        if (compFlag == 0) {
            CRYS_COMMON_IncLsbUnsignedCounter(expTempBuff_ptr->PrimeData.DataIn, 1, (uint8_t)modSizeWords);
        }
    }

    /* copy generator into output */
    DX_PAL_MemCopy((uint8_t *)G_ptr, (uint8_t *)expTempBuff_ptr->PrimeData.DataOut, modSizeBytes);
 ...
 *
 */
static CRYSError_t DX_DH_CreateDomainParams(uint32_t modPsizeBits,         /* in */
                                            uint32_t orderQsizeBits,       /* in */
                                            uint32_t seedSizeBits,         /* in */
                                            ...
                                            CRYS_DHKGData_t *DHKGbuff_ptr /* in */)
{
    ...
    /* clean DHKGbuff_ptr */
    DX_PAL_MemSetZero(DHKGbuff_ptr, sizeof(CRYS_DHKGData_t));
    ...
    if (generateSeed == 0) {
        DX_PAL_MemCopy((uint8_t *)S_ptr, seedS_ptr, (seedSizeBits + 7) / 8);
    }
    ...
    Error = DX_DH_X942_FindPrimeQ(orderQsizeBits,           /* in */
                                  seedSizeBits,             /* in */
                                  generateSeed,             /* in */
                                  Q_ptr,                    /* out */
                                  S_ptr,                    /* in/out */
    ...
    Error = DX_DH_X942_FindPrimeP(modPsizeBits,             /* in */
                                  orderQsizeBits,           /* in */
                                  seedSizeBits,             /* in */
                                  P_ptr,                    /* out */
                                  Q_ptr,                    /* out */
    ...
    if (generatorG_ptr != DX_NULL) {
        Error = DX_DH_X942_CreateGenerator(P_ptr,          /* in */
                                           modPsizeBits,   /* in */
                                           Q_ptr,          /* in */
                                           orderQsizeBits, /* in */
                                           G_ptr,          /* out */
        ...
        generatorSizeBits    = CRYS_COMMON_GetWordsCounterEffectiveSizeInBits(G_ptr, (uint16_t)modSizeBytes / 4);
        *generGsizeBytes_ptr = (generatorSizeBits + 7) / 8;

        Error = CRYS_COMMON_ConvertLswMswWordsToMsbLsbBytes(generatorG_ptr, *generGsizeBytes_ptr, G_ptr,
                                                            *generGsizeBytes_ptr);
        if (Error != CRYS_OK) {
            goto EndWithError;
        }
    ...
    Error = CRYS_COMMON_ConvertLswMswWordsToMsbLsbBytes(modP_ptr, modSizeBytes, P_ptr, modSizeBytes);
    if (Error != CRYS_OK) {
        goto EndWithError;
    }

    if (orderQ_ptr != DX_NULL) {
        Error = CRYS_COMMON_ConvertLswMswWordsToMsbLsbBytes(orderQ_ptr, orderQsizeBits / 8, Q_ptr,
                                                            (orderQsizeBits + 7) / 8);
        if (Error != CRYS_OK) {
            goto EndWithError;
        }
    ...
    if (generateSeed == 1) {
        DX_PAL_MemCopy(seedS_ptr, (uint8_t *)S_ptr, (seedSizeBits + 7) / 8);
    }
    ...
    if (factorJ_ptr != DX_NULL) {
        ...
        *JsizeBytes_ptr = CRYS_COMMON_GetWordsCounterEffectiveSizeInBits(J_ptr, (uint16_t)modSizeBytes / 4);
        ...
        *JsizeBytes_ptr = (*JsizeBytes_ptr + 7) / 8;
        ...
        Error = CRYS_COMMON_ConvertLswMswWordsToMsbLsbBytes(factorJ_ptr, *JsizeBytes_ptr, J_ptr, *JsizeBytes_ptr);
        if (Error != CRYS_OK) {
            goto EndWithError;
        }
    }
...
EndWithError:
    ...
    DX_PAL_MemSetZero(modP_ptr, (modPsizeBits + 7) / 8);

    if (generatorG_ptr != DX_NULL) {
        DX_PAL_MemSetZero(generatorG_ptr, *generGsizeBytes_ptr);
    }

    if (orderQ_ptr != DX_NULL) {
        DX_PAL_MemSetZero(orderQ_ptr, (orderQsizeBits + 7) / 8);
    }

    if (factorJ_ptr != DX_NULL) {
        DX_PAL_MemSetZero(factorJ_ptr, *JsizeBytes_ptr);
    }

    if (generateSeed == 1) {
        DX_PAL_MemSetZero(seedS_ptr, (seedSizeBits + 7) / 8);
    }
 ...
 * bytes). The user must set the size of allocated buffer, and the function returns the actual size of the generator in
 * bytes.
 ...
 *
 */
CEXPORT_C CRYSError_t CRYS_DH_CreateDomainParams(uint32_t modPsizeBits,         /* in */
                                                 uint32_t orderQsizeBits,       /* in */
                                                 uint32_t seedSizeBits,         /* in */
                                                 uint8_t *modP_ptr,             /* out */
                                                 uint8_t *orderQ_ptr,           /* out */
                                                 ...
                                                 CRYS_DHKGData_t *DHKGbuff_ptr /* in */)
{
    ...
    RETURN_IF_DH_UNSUPPORTED(modPsizeBits, orderQsizeBits, seedSizeBits, modP_ptr, orderQ_ptr, generatorG_ptr,
                             generGsizeBytes_ptr, factorJ_ptr, JsizeBytes_ptr, seedS_ptr, generateSeed, pgenCounter_ptr,
                             DHKGbuff_ptr, modSizeBytes, orderSizeBytes, Error, Error, Error, Error, Error, Error,
    ...
    if (modP_ptr == DX_NULL || orderQ_ptr == DX_NULL || seedS_ptr == DX_NULL || DHKGbuff_ptr == DX_NULL) {
        return CRYS_DH_INVALID_ARGUMENT_POINTER_ERROR;
    } else if (modPsizeBits < CRYS_DH_MIN_VALID_KEY_SIZE_VALUE_IN_BITS || /* check sizes */
               modPsizeBits % 256 != 0 || modPsizeBits > CRYS_DH_MAX_VALID_KEY_SIZE_VALUE_IN_BITS) {
    ...
    if (orderQsizeBits < CRYS_DH_SEED_MIN_SIZE_IN_BITS || orderQsizeBits > (modPsizeBits - 1) ||
        orderQsizeBits % 32 != 0) {
        return CRYS_DH_INVALID_ORDER_SIZE_ERROR;
    }
    ...
    if ((generatorG_ptr == DX_NULL) || (generGsizeBytes_ptr == DX_NULL) || (*generGsizeBytes_ptr < modSizeBytes)) {
    ...
    /*   call exec function */
    Error = DX_DH_CreateDomainParams(modPsizeBits,        /* in */
                                     orderQsizeBits,      /* in */
                                     seedSizeBits,        /* in */
                                     modP_ptr,            /* out */
                                     orderQ_ptr,          /* out */
 ...
 *
 */
CEXPORT_C CRYSError_t CRYS_DH_CheckDomainParams(uint8_t *modP_ptr,           /* in */
                                                ...
                                                CRYS_DHKG_CheckTemp_t *checkTempBuff_ptr /* in */)
{
    ...
    RETURN_IF_DH_UNSUPPORTED(modP_ptr, modPsizeBytes, orderQ_ptr, orderQsizeBytes, generatorG_ptr, generatorSizeBytes,
                             seedS_ptr, seedSizeBits, pgenCounter, checkTempBuff_ptr, DHKGbuff_ptr, Q_ptr, P_ptr,
                             modPsizeBits, modPsizeWords, orderQsizeBits, pgenCounter1, TempBuff_ptr, Error, Error,
                             Error, Error, Error, Error, Error, Error, Error, Error, Error, Error, Error, Error, Error,
                             Error);
    ...
    /* check pointers: modP, generator and tempBuff. Note: other pointers may be NULL  */
    if (modP_ptr == DX_NULL || orderQ_ptr == DX_NULL || checkTempBuff_ptr == DX_NULL) {
        return CRYS_DH_INVALID_ARGUMENT_POINTER_ERROR;
    }

    /* check modulus and order sizes */
    if (modPsizeBytes < CRYS_DH_MIN_VALID_KEY_SIZE_VALUE_IN_BITS / 8 || modPsizeBytes % 32 != 0 ||
        modPsizeBytes > CRYS_DH_MAX_MOD_SIZE_IN_BYTES) {
        return CRYS_DH_INVALID_MODULUS_SIZE_ERROR;
    }

    if (orderQsizeBytes < CRYS_DH_SEED_MIN_SIZE_IN_BITS / 8 || orderQsizeBytes % 4 != 0 ||
    ...
    if ((seedSizeBits == 0 && seedS_ptr != DX_NULL) || (seedSizeBits != 0 && seedS_ptr == DX_NULL)) {
    ...
    /* clean TempBuff_ptr */
    DX_PAL_MemSetZero(checkTempBuff_ptr, sizeof(CRYS_DHKG_CheckTemp_t));
    ...
    if (seedS_ptr != DX_NULL) {
        ...
        CRYS_COMMON_ReverseMemcpy((uint8_t *)P_ptr, modP_ptr, modPsizeBytes);
        CRYS_COMMON_ReverseMemcpy((uint8_t *)Q_ptr, orderQ_ptr, orderQsizeBytes);

        modPsizeBits = min(8 * modPsizeBytes,
                           CRYS_COMMON_GetBytesCounterEffectiveSizeInBits((uint8_t *)P_ptr, (uint16_t)modPsizeBytes));
        orderQsizeBits =
            min(8 * orderQsizeBytes,
                CRYS_COMMON_GetBytesCounterEffectiveSizeInBits((uint8_t *)Q_ptr, (uint16_t)orderQsizeBytes));
        ...
        if (DX_PAL_MemCmp(modP_ptr, (uint8_t *)P_ptr, modPsizeBytes) != 0) {
            Error = CRYS_DH_CHECK_DOMAIN_PRIMES_NOT_VALID_ERROR;
            goto End;
        }

        else if (DX_PAL_MemCmp(orderQ_ptr, (uint8_t *)Q_ptr, orderQsizeBytes) != 0) {
            Error = CRYS_DH_CHECK_DOMAIN_PRIMES_NOT_VALID_ERROR;
            goto End;
        }
    ...
    if (generatorG_ptr != DX_NULL) {
        Error = CRYS_DH_CheckPubKey(modP_ptr,                 /* in */
                                    modPsizeBytes,            /* in */
                                    orderQ_ptr,               /* in */
                                    orderQsizeBytes,          /* in */
                                    generatorG_ptr,           /* in */
        ...
        if (Error == CRYS_DH_INVALID_PUBLIC_KEY_SIZE_ERROR || Error == CRYS_DH_INVALID_PUBLIC_KEY_ERROR) {
...
End:
    ...
    DX_PAL_MemSetZero(&DHKGbuff_ptr->ExpTemps, sizeof(DHKGbuff_ptr->ExpTemps));
