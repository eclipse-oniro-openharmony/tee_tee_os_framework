                                                ...
                                                CRYS_EC_ELGAMAL_TempData_t *TempData_ptr /* in */)
{
    ...
    uint32_t ModSizeInBytes, ModSizeInWords, OrderSizeInBytes;
    /* pointer to temp buffer for output encrypted data from LLF */
    uint32_t *out32_ptr =
        (uint32_t *)&(((LLF_ecpki_elgamal_db_t *)&TempData_ptr->crysElgamalIntBuff)->OutBuff.encrMessOut);
    ...
    RETURN_IF_ECPKI_UNSUPPORTED(ReceiverUserPublKey_ptr, MessageIn_ptr, MessageInSizeBytes, EncrMessageOut_ptr,
                                IsEphemerKeyInternal, TempData_ptr, EncrMessOutSize_ptr, EphemerPrivKeyIn_ptr,
                                EphemerPrivKeySizeBytes, ReceiverPublKey_ptr, DomainID, DomainInfo.EC_ModulusSizeInBits,
                                ModSizeInBytes, ModSizeInWords, OrderSizeInBytes, out32_ptr, inBuff_ptr, Error, Error,
                                Error, Error, Error);
    ...
    if (DxCcAcl_IsBuffAccessOk(ACCESS_READ, ReceiverUserPublKey_ptr, sizeof(CRYS_ECPKI_UserPublKey_t)) ||
        ...
        DxCcAcl_IsBuffAccessOk(ACCESS_READ_WRITE, EncrMessOutSize_ptr, sizeof(uint32_t)) ||
        ((EphemerPrivKeyIn_ptr != DX_NULL) &&
         DxCcAcl_IsBuffAccessOk(ACCESS_READ, EphemerPrivKeyIn_ptr, EphemerPrivKeySizeBytes)) ||
    ...
    if (DomainID >= CRYS_ECPKI_DomainID_OffMode) {
        ...
        *EncrMessOutSize_ptr = 4 * ModSizeInBytes;
        ...
        /* Clean temp data */
        DX_PAL_MemSetZero((uint8_t*)TempData_ptr, sizeof(CRYS_EC_ELGAMAL_TempData_t));
        ...
        Error = CRYS_COMMON_ConvertMsbLsbBytesToLswMswWords(inBuff_ptr->EphemerPrivKey, (ModSizeInBytes + 4),
...
CEXPORT_C CRYSError_t CRYS_ECPKI_ELGAMAL_Decrypt(CRYS_ECPKI_UserPrivKey_t *ReceiverUserPrivKey_ptr, /* in */
                                                 ...
                                                 uint32_t *DecrMessageOutSize_ptr,                  /* in/out */
                                                 CRYS_EC_ELGAMAL_TempData_t *TempData_ptr /* in */)

{
    ...
    uint32_t *tmp32_ptr = (uint32_t *)TempData_ptr;
    ...
    RETURN_IF_ECPKI_UNSUPPORTED(ReceiverUserPrivKey_ptr, EncrMessageIn_ptr, EncrMessageSizeInBytes, DecrMessageOut_ptr,
                                DecrMessageOutSize_ptr, TempData_ptr, ReceiverPrivKey_ptr, DomainID,
                                DomainInfo.EC_ModulusSizeInBits, ModSizeInBytes, ModSizeInWords, Error, Error, Error,
    ...
    if (DxCcAcl_IsBuffAccessOk(ACCESS_READ, ReceiverUserPrivKey_ptr, sizeof(CRYS_ECPKI_UserPrivKey_t)) ||
        ...
        DxCcAcl_IsBuffAccessOk(ACCESS_READ_WRITE, DecrMessageOutSize_ptr, sizeof(uint32_t)) ||
        DxCcAcl_IsBuffAccessOk(ACCESS_READ_WRITE, TempData_ptr, sizeof(CRYS_EC_ELGAMAL_TempData_t))) {
        return CRYS_ECC_ILLEGAL_PARAMS_ACCORDING_TO_PRIV_ERROR;
    }
    ...
    if (DomainID >= CRYS_ECPKI_DomainID_OffMode
#ifdef CRYS_EC_ELGAMAL_NOT_SUPPORT_ALL_DOMAINS
        || (DomainID != CRYS_ECPKI_DomainID_WMDRM10 && DomainID != CRYS_ECPKI_DomainID_secp192r1 &&
            DomainID != CRYS_ECPKI_DomainID_secp256r1)
#endif
    )
        return CRYS_EC_ELGAMAL_ENCR_INVALID_DOMAIN_ID_ERROR;
    ...
    Error = CRYS_COMMON_ConvertMsbLsbBytesToLswMswWords(tmp32_ptr /* dstReg */, 4 * ModSizeInWords,
                                                        EncrMessageIn_ptr /* src_ptr */, ModSizeInBytes);
    if (Error)
        return Error;

    Error = CRYS_COMMON_ConvertMsbLsbBytesToLswMswWords(tmp32_ptr + ModSizeInWords /* dstReg */, 4 * ModSizeInWords,
    ...
    Error =
        CRYS_COMMON_ConvertMsbLsbBytesToLswMswWords(tmp32_ptr + 2 * ModSizeInWords /* dstReg */, 4 * ModSizeInWords,
                                                    EncrMessageIn_ptr + 2 * ModSizeInBytes /* src_ptr */, ModSizeInBytes);
    if (Error)
        return Error;

    Error =
        CRYS_COMMON_ConvertMsbLsbBytesToLswMswWords(tmp32_ptr + 3 * ModSizeInWords /* dstReg */, 4 * ModSizeInWords,
    ...
    Error = LLF_ECPKI_ELGAMAL_Decrypt(ReceiverPrivKey_ptr, tmp32_ptr /* in */, tmp32_ptr /* out */, *DecrMessageOutSize_ptr,
                                      TempData_ptr);
    if (Error != CRYS_OK)
        goto END;

    /* reverse the decrypted message */
    Error = CRYS_COMMON_ConvertLswMswWordsToMsbLsbBytes(DecrMessageOut_ptr /* out */, sizeof(uint32_t) * ModSizeInWords,
