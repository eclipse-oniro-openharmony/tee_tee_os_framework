                                              ...
                                              CRYS_ECPKI_UserPrivKey_t *UserPrivKey_ptr /* out */)
{
    ...
    RETURN_IF_ECPKI_UNSUPPORTED(DomainID, PrivKeyIn_ptr, PrivKeySizeInBytes, UserPrivKey_ptr, PrivKey_ptr,
                                DomainInfo.EC_ModulusSizeInBits, OrderSizeInBytes, Error, Error, Error, Error, Error,
    ...
    DX_PAL_MemSet(PrivKey_ptr, 0, sizeof(CRYS_ECPKI_PrivKey_t));

    /* loading the private key db to little endian and domain ID */
    Error = CRYS_COMMON_ConvertMsbLsbBytesToLswMswWords(PrivKey_ptr->PrivKey, sizeof(PrivKey_ptr->PrivKey),
...
End:
    ...
    if (Error != CRYS_OK) {
        DX_PAL_MemSet(UserPrivKey_ptr, 0, sizeof(CRYS_ECPKI_UserPrivKey_t));
                                             ...
                                             CRYS_ECPKI_BUILD_TempData_t *TempBuff_ptr /* in */)
{
    ...
    RETURN_IF_ECPKI_UNSUPPORTED(DomainID, PublKeyIn_ptr, PublKeySizeInBytes, CheckMode, UserPublKey_ptr, TempBuff_ptr,
                                PublKey_ptr, DomainInfo.EC_ModulusSizeInBits, ModSizeInBytes, PC, PC1, Error, Error,
    ...
    if (DxCcAcl_IsBuffAccessOk(ACCESS_READ, PublKeyIn_ptr, PublKeySizeInBytes) ||
        ...
        ((TempBuff_ptr != DX_NULL) &&
         DxCcAcl_IsBuffAccessOk(ACCESS_READ_WRITE, TempBuff_ptr, sizeof(CRYS_ECPKI_BUILD_TempData_t)))) {
        return CRYS_ECC_ILLEGAL_PARAMS_ACCORDING_TO_PRIV_ERROR;
    }
    ...
    DX_PAL_MemSet((uint8_t *)UserPublKey_ptr, 0, sizeof(CRYS_ECPKI_UserPublKey_t));

    /* Set DomainID into Builded public key */
    PublKey_ptr->DomainID = DomainID;

    if (CRYS_EC_PointUncompressed == PC1 || CRYS_EC_PointHybrid == PC1) { /*  PC1 = 4 or PC1 = 6 */
    ...
    if (CheckMode >= ECpublKeyPartlyCheck) {
        ...
        Error = LLF_ECPKI_CheckPublKeyCall(PublKey_ptr, CheckMode, (uint32_t *)TempBuff_ptr);
    } else {
    ...
    if (Error != CRYS_OK) {
        ...
        DX_PAL_MemSet(UserPublKey_ptr, 0, sizeof(CRYS_ECPKI_UserPublKey_t));
        return Error;
    }
                                               ...
                                               uint32_t *PublKeySizeInBytes_ptr /* in/out */)
{
    ...
    /* .................. if not supported exit .............................. */
    RETURN_IF_ECPKI_UNSUPPORTED(UserPublKey_ptr, Compression, ExternPublKey_ptr, PublKeySizeInBytes_ptr, PublKey_ptr,
                                DomainInfo.EC_ModulusSizeInBits, DomainID, ModSizeInBytes, Error, Error, Error, Error,
                                Error, Error, Error, Error, Error, Error, Error, Error, Error, Error);
    ...
    if (DxCcAcl_IsBuffAccessOk(ACCESS_READ, UserPublKey_ptr, sizeof(CRYS_ECPKI_UserPublKey_t)) ||
        ...
        DxCcAcl_IsBuffAccessOk(ACCESS_READ_WRITE, PublKeySizeInBytes_ptr, sizeof(uint32_t))) {
        return CRYS_ECC_ILLEGAL_PARAMS_ACCORDING_TO_PRIV_ERROR;
    }
    ...
    case CRYS_EC_PointUncompressed:
        ...
        Error = CRYS_COMMON_ConvertLswMswWordsToMsbLsbBytes(ExternPublKey_ptr + 1, 4 * ((ModSizeInBytes + 3) / 4),
                                                            PublKey_ptr->PublKeyX, ModSizeInBytes);
        if (Error != CRYS_OK)
            return Error;
        Error = CRYS_COMMON_ConvertLswMswWordsToMsbLsbBytes(ExternPublKey_ptr + 1 + ModSizeInBytes,
                                                            4 * ((ModSizeInBytes + 3) / 4), PublKey_ptr->PublKeyY,
                                                            ModSizeInBytes);
        if (Error != CRYS_OK)
            return Error;
