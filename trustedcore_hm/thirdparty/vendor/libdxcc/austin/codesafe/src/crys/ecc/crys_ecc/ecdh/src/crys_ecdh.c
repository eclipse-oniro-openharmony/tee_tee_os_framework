...
*/
CEXPORT_C CRYSError_t CRYS_ECDH_SVDP_DH(CRYS_ECPKI_UserPublKey_t *PartnerPublKey_ptr, /* in */
                                        ...
                                        CRYS_ECDH_TempData_t *TempBuff_ptr /* in */)
{
    ...
    RETURN_IF_ECPKI_UNSUPPORTED(PartnerPublKey_ptr, UserPrivKey_ptr, DomainID, SharedSecretValue_ptr,
                                SharedSecrValSize_ptr, TempBuff_ptr, PublKey_ptr, DomainInfo.EC_ModulusSizeInBits,
                                ModSizeInBytes, Error, Error, Error, Error, Error, Error, Error, Error, Error, Error,
                                Error, Error, Error);
    ...
    if (DxCcAcl_IsBuffAccessOk(ACCESS_READ, UserPrivKey_ptr, sizeof(CRYS_ECPKI_UserPrivKey_t)) ||
        ...
        DxCcAcl_IsBuffAccessOk(ACCESS_READ_WRITE, SharedSecrValSize_ptr, sizeof(uint32_t)) ||
    ...
    /* ------------------------------------------------------------------ */
    Error = LLF_ECPKI_SVDP_DH(PublKey_ptr, PrivKey_ptr->PrivKey, (uint32_t *)TempBuff_ptr /* out: SharedSecret */,
                              TempBuff_ptr);

    if (Error != CRYS_OK)
        return Error;

    Error = CRYS_COMMON_ConvertLswMswWordsToMsbLsbBytes(SharedSecretValue_ptr, 4 * ((ModSizeInBytes + 3) / 4),
                                                        (uint32_t *)TempBuff_ptr /* in32_ptr */, ModSizeInBytes);
