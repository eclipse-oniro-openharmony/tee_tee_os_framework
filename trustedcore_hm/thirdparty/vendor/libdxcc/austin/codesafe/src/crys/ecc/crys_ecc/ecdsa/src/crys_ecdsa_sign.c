...
#define DX_PAL_LOG_CUR_COMPONENT DX_LOG_MASK_ASYM_ECC
                                          ...
                                          CRYS_ECPKI_HASH_OpMode_t HashMode /* in */)
{
    ...
    CRYSError_t Error = CRYS_OK, Error1 = CRYS_OK;
    ...
    RETURN_IF_ECPKI_UNSUPPORTED(SignUserContext_ptr, SignerPrivKey_ptr, HashMode, ccmWorkingContext_ptr, PrivKey_ptr,
                                Error1, Error, Error, Error, Error, Error, Error, Error, Error, Error, Error, Error,
    ...
    DX_PAL_MemSet(ccmWorkingContext_ptr, 0, sizeof(ECDSA_SignContext_t));
    ...
    DX_PAL_MemCopy((uint8_t *)&ccmWorkingContext_ptr->ECDSA_SignerPrivKey, (uint8_t *)SignerPrivKey_ptr,
...
END_WITH_ERROR:
    /* ................. release the context ................................. */
    /* ----------------------------------------------------------------------- */
    Error1 = CRYS_CCM_ReleaseContext(SignUserContext_ptr,    /* the users context space - in */
                                     ccmWorkingContext_ptr,  /* the CCM context returned - in */
    ...
    /* -------------------------------------------------------------------- */
    DX_PAL_MemSet(SignUserContext_ptr, 0, sizeof(CRYS_ECDSA_SignUserContext_t));
                                            ...
                                            uint32_t DataInSize /* in */)
{
    ...
    RETURN_IF_ECPKI_UNSUPPORTED(SignUserContext_ptr, MessageDataIn_ptr, DataInSize, ccmWorkingContext_ptr, Error1,
    ...
    Error = CRYS_CCM_GetContext(SignUserContext_ptr,          /* the users context space - in */
                                (void **)&tempWorkingctx_ptr, /* the CCM context returned - out */
                                DX_ECDSA_SIGN_CONTEXT);       /* need to decrypt context in AES_block */
    ...
    case CRYS_ECPKI_AFTER_HASH_SHA1_mode:
        ...
        if (DataInSize != CRYS_HASH_SHA1_DIGEST_SIZE_IN_BYTES) { /* DataInSize must fit exactly to the size of Hash
                                                                    output that we support */
            Error = CRYS_ECDSA_SIGN_INVALID_MESSAGE_DATA_IN_SIZE_ERROR;
            goto END_WITH_ERROR;
        }
        /* Copy the DataIn_ptr to the HashResult */
        Error = CRYS_Bypass(MessageDataIn_ptr, DataInSize, (uint8_t *)ccmWorkingContext_ptr->HASH_Result);
        if (Error != CRYS_OK)
            goto END_WITH_ERROR;
                                           ...
                                           uint32_t *EphemerKeyData_ptr /* in */)
{
    ...
    uint32_t *SignatureC_ptr, *SignatureD_ptr;
    uint32_t *MessageRepresent_ptr;
    ...
    RETURN_IF_ECPKI_UNSUPPORTED(SignUserContext_ptr, SignatureOut_ptr, SignatureOutSize_ptr, IsEphemerKeyInternal,
                                EphemerKeyData_ptr, ccmWorkingContext_ptr, PrivKey_ptr, DomainID,
                                DomainInfo.EC_ModulusSizeInBits, MessageRepresent_ptr, HashSizeWords, SignatureC_ptr,
                                SignatureD_ptr, TempBuff_ptr, Error, OrderSizeInBytes, OrderSizeInWords, Shift, i,
                                Error1, Error, Error);
    ...
    if (DxCcAcl_IsBuffAccessOk(ACCESS_READ_WRITE, SignUserContext_ptr, sizeof(CRYS_ECDSA_SignUserContext_t)) ||
        ...
        DxCcAcl_IsBuffAccessOk(ACCESS_READ_WRITE, SignatureOutSize_ptr, sizeof(uint32_t)) ||
        ((EphemerKeyData_ptr != DX_NULL) &&
         DxCcAcl_IsBuffAccessOk(ACCESS_READ_WRITE, EphemerKeyData_ptr, sizeof(uint32_t)))) {
        return CRYS_ECC_ILLEGAL_PARAMS_ACCORDING_TO_PRIV_ERROR;
    }
    ...
    DX_PAL_MemSetZero(MessageRepresent_ptr, 4 * OrderSizeInWords);
    ...
    if (DomainInfo.EC_OrderSizeInBits >= 32 * HashSizeWords) { /* Copy HASH_Result into MessageRepresent buffer */
        ...
        if (Shift) {
            for (i = 0; i < OrderSizeInWords - 1; i++) {
                MessageRepresent_ptr[i] =
                    (MessageRepresent_ptr[i] >> Shift) | (MessageRepresent_ptr[i + 1] << (32 - Shift));
            }
    ...
    Error =
        LLF_ECDSA_SignCalcCall(PrivKey_ptr, MessageRepresent_ptr, min(OrderSizeInWords, HashSizeWords),
    ...
    Error = CRYS_COMMON_ConvertLswMswWordsToMsbLsbBytes(SignatureOut_ptr, 4 * OrderSizeInWords, SignatureC_ptr,
                                                        OrderSizeInBytes);
    if (Error != CRYS_OK)
        return Error;

    Error = CRYS_COMMON_ConvertLswMswWordsToMsbLsbBytes(SignatureOut_ptr + OrderSizeInBytes, 4 * OrderSizeInWords,
                                      ...
                                      uint32_t *SignatureOutSize_ptr /* in */)
{
    ...
    RETURN_IF_ECPKI_UNSUPPORTED(SignUserContext_ptr, SignerPrivKey_ptr, HashMode, MessageDataIn_ptr, MessageSizeInBytes,
                                SignatureOut_ptr, SignatureOutSize_ptr, Error, Error, Error, Error, Error, Error, Error,
    ...
    Error = CRYS_ECDSA_SignUpdate(SignUserContext_ptr, MessageDataIn_ptr, MessageSizeInBytes);
    if (Error != CRYS_OK)
    ...
    Error = CRYS_ECDSA_SignFinish(SignUserContext_ptr, SignatureOut_ptr, SignatureOutSize_ptr);
    return Error;
