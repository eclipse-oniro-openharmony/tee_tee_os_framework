...
typedef struct {
    ...
    uint32_t NP[LLF_ECPKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS];
    /* Barrett tag for order R */
    uint32_t RP[LLF_ECPKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS];
} LLF_ecpki_domains_db_t;

/* LLF strucure for ECPKI Private Key temp buffers */
typedef struct {
    uint32_t NP[LLF_ECPKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS]; /* Barrett for modulus */
    uint32_t RP[LLF_ECPKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS]; /* Barrett for order */
...
typedef struct {
    ...
    uint32_t s[CRYS_ECPKI_ORDER_MAX_LENGTH_IN_WORDS];
    uint32_t tmp[CRYS_ECPKI_ORDER_MAX_LENGTH_IN_WORDS];
} LLF_ecpki_key_gen_db_t;

/* Temporary buffers used for the functions called from ECDSA */
typedef struct {
    uint32_t Tempbuff[5 * CRYS_ECPKI_MODUL_MAX_LENGTH_IN_WORDS];
} LLF_ecdsa_sign_db_t;

/* Temporary buffers used for the  functions called from ECDSA */
typedef struct {
    uint32_t Tempbuff[3 * CRYS_ECPKI_MODUL_MAX_LENGTH_IN_WORDS];
