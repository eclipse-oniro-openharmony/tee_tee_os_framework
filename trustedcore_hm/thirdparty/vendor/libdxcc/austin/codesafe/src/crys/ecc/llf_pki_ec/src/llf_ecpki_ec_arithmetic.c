                                    ...
                                    uint32_t VirtualHwBaseAddr /* in */)
{
    ...
    Error = LLF_ECPKI_GFpScalarMultProj(rn, P0, Res, DomainID, regTemps_ptr + 1, countTemps - 1, VirtualHwBaseAddr);
    if (Error != CRYS_OK)
        return Error;

    /* normalize the result point according affine ordinary representation */
    Error = LLF_ECPKI_GFpJacobianToAffine(Res /* In */, Res /* Out */, DomainID, regTemps_ptr + 1, countTemps - 1,
                                          VirtualHwBaseAddr);
    if (Error != CRYS_OK)
        return Error;
...
*/
CRYSError_t LLF_ECPKI_FullAddAff(LLF_ECPKI_PKA_Point_t *P1,      /* in */
                                 ...
                                 uint32_t VirtualHwBaseAddr /* in */)
{
    ...
    if (P1->PointID == EC_PointAffine && P2->PointID == EC_PointAffine) {
...
#ifdef LLF_PKI_PKA_DEBUG
    else if (P1->PointID != EC_PointJacobian || P2->PointID != EC_PointJacobian)
        return LLF_ECPKI_INVALID_POINT_ID_ERROR;
#endif
    ...
    Error = LLF_ECPKI_FullAdd(P1, P2, Res, DomainID, regTemps_ptr + 2, countTemps - 2, VirtualHwBaseAddr);
    if (Error != CRYS_OK)
        return Error;

    /* convert the result point to affine ordinary representation */
    Error = LLF_ECPKI_GFpJacobianToAffine(Res /* in */, Res /* out */, DomainID, regTemps_ptr + 2, countTemps - 2,
                                          VirtualHwBaseAddr);
    if (Error != CRYS_OK)
        return Error;
...
CRYSError_t LLF_ECPKI_GFpAddProj(LLF_ECPKI_PKA_Point_t *P1,      /* in */
                                 LLF_ECPKI_PKA_Point_t *P2,      /* in */
                                 LLF_ECPKI_PKA_Point_t *Res,     /* out */
                                 CRYS_ECPKI_DomainID_t DomainID, /* in */
                                 int8_t *regTemps_ptr,           /* in */
                                 ...
                                 uint32_t VirtualHwBaseAddr /* in */)
{
    ...
    /* virtual pointers to PKA temp registers */
    int8_t rT1, rT2, rT3, rT4, rT5, rT6, rT7;
...
CRYSError_t LLF_ECPKI_GFpDoubleProj(LLF_ECPKI_PKA_Point_t *P0,      /* in */
                                    LLF_ECPKI_PKA_Point_t *Res,     /* out */
                                    CRYS_ECPKI_DomainID_t DomainID, /* in */
                                    int8_t *regTemps_ptr,           /* in */
                                    uint32_t countTemps,            /* in */
                                    uint32_t VirtualHwBaseAddr /* in */)
{
    ...
    /* virtual pointers to PKA temp registers */
    int8_t rT1, rT2, rT3, rT4, rT5, rT6;

    /* PKA status */
    uint32_t status1, status2;

    /* EC domain parameters */
    uint32_t modSizeInWords;
    ...
    LLF_PKI_PKA_CopyDataIntoPkaReg(rT4 /* dstReg */, LenID_n + 1,
                                   (uint32_t *)LLF_EC_DOMAINS_ARRAY[DomainID].EC_Param_A /* src_ptr */, modSizeInWords,
                                   VirtualHwBaseAddr);
...
*/
CRYSError_t LLF_ECPKI_FullAdd(LLF_ECPKI_PKA_Point_t *P1,      /* in */
                              LLF_ECPKI_PKA_Point_t *P2,      /* in */
                              LLF_ECPKI_PKA_Point_t *Res,     /* out */
                              CRYS_ECPKI_DomainID_t DomainID, /* in */
                              int8_t *regTemps_ptr,           /* in */
                              ...
                              uint32_t VirtualHwBaseAddr /* in */)
{
    ...
    Error = LLF_ECPKI_GFpAddProj(P1, P2, Res, DomainID, regTemps_ptr, countTemps, VirtualHwBaseAddr);
    if (Error != CRYS_OK)
        return Error;
    ...
    if (LLF_ECPKI_CompareProjPointToNull(LenID_n, Res, VirtualHwBaseAddr) == 1)
        ...
        Error = LLF_ECPKI_GFpDoubleProj(P1, Res, DomainID, regTemps_ptr, countTemps, VirtualHwBaseAddr);
...
*/
CRYSError_t LLF_ECPKI_SubtractProj(LLF_ECPKI_PKA_Point_t *P1,      /* in */
                                   LLF_ECPKI_PKA_Point_t *P2,      /* in */
                                   LLF_ECPKI_PKA_Point_t *Res,     /* out */
                                   CRYS_ECPKI_DomainID_t DomainID, /* in */
                                   int8_t *regTemps_ptr,           /* in */
                                   ...
                                   uint32_t VirtualHwBaseAddr /* in */)
{
    ...
    Error = LLF_ECPKI_FullAdd(P1, P2, Res, DomainID, regTemps_ptr + 1, countTemps - 1, VirtualHwBaseAddr);
...
@brief  This function performs scalar multiplication  of EC point P0 by integer n.
   ...
   @return CRYSError_t - CRYS_OK,
            ...
            LLF_ECPKI_ScalarMult_ARITHM_OPERATIONS_OFF_MODE_ERROR
*/
CRYSError_t LLF_ECPKI_GFpScalarMultProj(int8_t rn,                      /* in */
                                        LLF_ECPKI_PKA_Point_t *P0,      /* in */
                                        LLF_ECPKI_PKA_Point_t *Res,     /* out */
                                        CRYS_ECPKI_DomainID_t DomainID, /* in */
                                        int8_t *regTemps_ptr,           /* in */
                                        ...
                                        uint32_t VirtualHwBaseAddr /* in */)
{
...
#ifdef LLF_PKI_PKA_DEBUG
    {
        ...
        LLF_PKI_PKA_CopyDataIntoPkaReg(rT /* dstReg */, LenID_r + 1,
                                       (uint32_t *)LLF_EC_DOMAINS_ARRAY[DomainID].EC_GenerOrder /* src_ptr */,
    ...
    if (status1 == 1) {
        ...
        LLF_ECPKI_GFpJacobianToAffine(Res /* in */, &Q /* out */, DomainID, regTemps_ptr + 7, countTemps - 7,
                                      VirtualHwBaseAddr);
    }
    ...
    /* 7. Perform loop  of double - add - subtract operations according to sequence of bit
           values of h and k */
    for (i = (int32_t)hSizeInBits - 2; i >= 1; i--) {
        /* 7.1. Set Res = 2*Res  / Double  */
        Error = LLF_ECPKI_GFpDoubleProj(Res, Res, DomainID, regTemps_ptr + 7, countTemps - 7, VirtualHwBaseAddr);
        if (Error != CRYS_OK)
            return Error;
...
*/
CRYSError_t LLF_ECPKI_IsPointOnCurve(LLF_ECPKI_PKA_Point_t *W_ptr,   /* in */
                                     CRYS_ECPKI_DomainID_t DomainID, /* in */
                                     int8_t *regTemps_ptr,           /* in */
                                     uint32_t countTemps,            /* in */
                                     uint32_t VirtualHwBaseAddr /* in */)

{
    ...
    LLF_PKI_PKA_CopyDataIntoPkaReg(rT2 /* dstReg */, LenID_n + 1,
                                   (uint32_t *)LLF_EC_DOMAINS_ARRAY[DomainID].EC_Param_B /* src_ptr */, modSizeInWords,
                                   VirtualHwBaseAddr);
...
*/
CRYSError_t LLF_ECPKI_GFp_ConvArrayToECaffine(int8_t rArr,                     /* in */
...

{
    ...
    uint32_t changedWord, changedWordNum, changedByteNum;
...
#endif
    /* Domain parameters */
    P_ptr          = (uint32_t *)LLF_EC_DOMAINS_ARRAY[DomainID].GF_Modulus;
    ...
    if (((P_ptr[modSizeInWords - 1] >> 24) & 0xFF) > 100) {
        ...
        if ((modSizeInBytes - 1) % 4 == 0) {
            /* set MSByte = 0 */
            LLF_PKI_PKA_SetWordIntoRegister(LenID_n, 0 /* changedWord */, PAff_ptr->rX, changedWordNum /* i */,
                                            VirtualHwBaseAddr);
    ...
    for (tries = 0; tries < 100; tries++) {
        ...
        /* 1.1.  T1 = X^2 */
        LLF_PKI_PKA_ModMul(LenID_n, PAff_ptr->rX /* OpA */, PAff_ptr->rX /* OpB */, rT1 /* Res */, 0 /* Tag */,
                           VirtualHwBaseAddr);
        ...
        Error = LLF_ECPKI_SquareRootModPrime(rT1 /* in */, PAff_ptr->rY /* out */, &isRootExists /* out */, regTemps_ptr + 3,
                                             countTemps - 3, VirtualHwBaseAddr);
        if (Error != CRYS_OK)
            return Error;
...
*/
CRYSError_t LLF_ECPKI_GFpJacobianToAffine(LLF_ECPKI_PKA_Point_t *PJac,    /* in */
...

{
    ...
    LLF_PKI_PKA_CopyDataIntoPkaReg(rT1 /* dstReg */, LenID_n + 1,
                                   (uint32_t *)LLF_EC_DOMAINS_ARRAY[DomainID].EC_Param_B /* src_ptr */,
                                   (uint32_t)LLF_EC_DOMAINS_ARRAY[DomainID].EC_ModulusSizeInWords, VirtualHwBaseAddr);

    /* If Z = 0 (point on infinity), then output (x,y) as:
           if EC parameter B != 0 - output (0,0), else - output (0,1) */
    ...
    Error = LLF_PKI_PKA_ModInv(LenID_n, rT2 /* OpB */, rT1 /* Res */, 0 /* Tag */, VirtualHwBaseAddr);
    if (Error)
        return Error;
...
*/
void LLF_ECPKI_SetPoint(uint32_t *X_ptr, uint32_t *Y_ptr, uint32_t *Z_ptr, uint32_t sizeInWords, uint32_t LenID,
                        LLF_ECPKI_PKA_Point_t *Res, CRYS_ECPKI_PointID_t pointID, uint32_t VirtualHwBaseAddr)
{
    /* Copy X,Y,Z coordinates  */
    LLF_PKI_PKA_CopyDataIntoPkaReg(Res->rX /* dstReg */, LenID_n + 1, X_ptr /* src_ptr */, sizeInWords, VirtualHwBaseAddr);

    LLF_PKI_PKA_CopyDataIntoPkaReg(Res->rY /* dstReg */, LenID_n + 1, Y_ptr /* src_ptr */, sizeInWords, VirtualHwBaseAddr);

    if (pointID > EC_PointAffine && pointID < EC_PointID_OffMode) {
        if (Z_ptr != DX_NULL) {
            LLF_PKI_PKA_CopyDataIntoPkaReg(Res->rZ /* dstReg */, LenID_n + 1, Z_ptr /* src_ptr */, sizeInWords,
                                           VirtualHwBaseAddr);
        } else {
            /* set Z = 1 */
...
/*
   ...
   @return     No return value.
               ...
               (CRYS_ECPKI_MODUL_MAX_LENGTH_IN_WORDS words).
*/
void LLF_ECPKI_CopyPoint(uint32_t LenID, LLF_ECPKI_PKA_Point_t *To, LLF_ECPKI_PKA_Point_t *From,
...
/*
   @brief  This function sets projective point to infinity that means sets
       point coordinates as following  X=0, Y=0, Z=0.
...
void LLF_ECPKI_SetNullToPoint(uint32_t LenID, LLF_ECPKI_PKA_Point_t *P, uint32_t VirtualHwBaseAddr)
{
    /* Set null to X,Y,Z   */
...
/*
   ...
   @param[in]  LenID - RegSizes entry, containing exact size of coordinates in bits.
   @param[in]  P  - A pointer to structure, containing the point.

   @return     No return value.
*/

void LLF_ECPKI_SetProjPointOnInfinity(uint32_t LenID, LLF_ECPKI_PKA_Point_t *P, uint32_t VirtualHwBaseAddr)
{
    /* Set null to X,Y,Z   */
    LLF_PKI_PKA_Clr(LenID + 1, P->rX /* OpA */, 0 /* Tag */, VirtualHwBaseAddr);
    LLF_PKI_PKA_Clr(LenID + 1, P->rY /* OpA */, 0 /* Tag */, VirtualHwBaseAddr);
...
int32_t LLF_ECPKI_IsProjPointOnInfinity(uint32_t LenID,            /* in */
                                        LLF_ECPKI_PKA_Point_t *P0, /* in */
                                        uint32_t VirtualHwBaseAddr /* in */)
{
    uint32_t status1, status2;
...
int32_t LLF_ECPKI_CompareProjPointToNull(uint32_t LenID,            /* in */
                                         LLF_ECPKI_PKA_Point_t *P0, /* in */
                                         uint32_t VirtualHwBaseAddr /* in */)
{
    uint32_t status1, status2;
