                               ...
                               uint32_t *TempBuff_ptr /* in */)
{
    ...
    int8_t rS, rU, rF, rR, rT, rN, rNp, rRp;
    ...
    uint32_t isModGrateOrder, status;
    ...
    *TempBuff_ptr = *TempBuff_ptr;
    ...
    LLF_PKI_PKA_CopyDataIntoPkaReg(rN /* dstReg */, LenID_r + 1, (uint32_t *)Domain_ptr->GF_Modulus /* src_ptr */,
                                   modSizeInWords, VirtualHwBaseAddr);

    LLF_PKI_PKA_CopyDataIntoPkaReg(
        rNp /* dstReg */, LenID_r + 1,
        ((LLF_ecpki_priv_key_db_t *)(SignerPrivateKey_ptr->crysPrivKeyIntBuff))->NP /* src_ptr */,
        LLF_PKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS, VirtualHwBaseAddr);

    /* EC order and its Barrett tag */
    LLF_PKI_PKA_CopyDataIntoPkaReg(rR /* dstReg */, LenID_r + 1, (uint32_t *)Domain_ptr->EC_GenerOrder /* src_ptr */,
                                   ordSizeInWords, VirtualHwBaseAddr);
    LLF_PKI_PKA_CopyDataIntoPkaReg(
        rRp /* dstReg */, LenID_r + 1,
        ((LLF_ecpki_priv_key_db_t *)(SignerPrivateKey_ptr->crysPrivKeyIntBuff))->RP /* src_ptr */,
        LLF_PKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS, VirtualHwBaseAddr);
    ...
    while (1) {
        ...
        if (IsEphemerKeyInternal == DX_TRUE) {
            if (tryNum > 0x100) {
                Error = LLF_ECDSA_SIGN_CALC_CANNOT_CREATE_SIGNATURE;
                goto End;
            }

            kgTempBuff_ptr = (CRYS_ECPKI_KG_TempData_t *)TempBuff_ptr;

            Error = LLF_ECPKI_GenKeyPair(DomainID, rU /* ephemer.priv */, &V /* ephemer.publ */, regTemps + 9,
                                         28 - 9 /* countTemps */, kgTempBuff_ptr, DX_TRUE /* isKeyInternal */,
                                         VirtualHwBaseAddr);
            if (Error != CRYS_OK) {
                goto End;
            }
            ...
            LLF_PKI_PKA_CopyDataIntoPkaReg(rU /* dstReg */, LenID_r + 1, EphemerKeyData_ptr /* src_ptr */, ordSizeInWords,
                                           VirtualHwBaseAddr);

            /* copy ephemeral public Key X */
            LLF_PKI_PKA_CopyDataIntoPkaReg(V.rX /* dstReg */, LenID_r + 1,
                                           (EphemerKeyData_ptr + modSizeInWords + 1) /* src_ptr */, modSizeInWords,
                                           VirtualHwBaseAddr);
        }
        ...
        if (isModGrateOrder == 1) {
            /* c = Xv mod r  */
            LLF_PKI_PKA_Div(LenID_r, V.rX /* OpA=remainder */, rR /* OpB */, V.rZ /* Res not used */, 0 /* Tag */,
                            VirtualHwBaseAddr);
        }
        ...
        Error = LLF_PKI_PKA_ModInv(LenID_r, rT /* OpB */, rU /* Res */, 0 /* Tag */, VirtualHwBaseAddr);
        if (Error) {
            return Error;
        }
    ...
    LLF_PKI_PKA_CopyDataFromPkaReg(SignatureC_ptr, ordSizeInWords, V.rX /* srcReg */, VirtualHwBaseAddr);
    LLF_PKI_PKA_CopyDataFromPkaReg(SignatureD_ptr, ordSizeInWords, rT /* srcReg */, VirtualHwBaseAddr);
...

{
    ...
    /* virtual pointers to PKA registers: private key rS, ephemeral private key rU,
       message rF, EC order rR, temp registers rD etc. */
    int8_t rC, rD, rF, rR, rT, rT1, rT2, rN, rNp, rRp;
    ...
    /* indication variables */
    uint32_t isOnInfinity, status1, status2;
    ...
    *TempBuff_ptr = *TempBuff_ptr;
    ...
    LLF_PKI_PKA_CopyDataIntoPkaReg(rRp /* dstReg */, LenID_r + 1,
                                   ((LLF_ecpki_pub_key_db_t *)SignerPublKey_ptr->crysPubKeyIntBuff)->RP /* src_ptr */,
                                   LLF_ECPKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS, VirtualHwBaseAddr);

    /* save modulus and its Barrett tag */
    LLF_PKI_PKA_CopyDataIntoPkaReg(rN /* dstReg */, LenID_r + 1, (uint32_t *)&(Domain_ptr->GF_Modulus) /* src_ptr */,
                                   modSizeInWords, VirtualHwBaseAddr);
    LLF_PKI_PKA_CopyDataIntoPkaReg(rNp /* dstReg */, LenID_r + 1,
                                   ((LLF_ecpki_pub_key_db_t *)SignerPublKey_ptr->crysPubKeyIntBuff)->NP /* src_ptr */,
                                   LLF_ECPKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS, VirtualHwBaseAddr);

    /* signature C */
    LLF_PKI_PKA_CopyDataIntoPkaReg(rC /* dstReg */, LenID_r + 1, SignatureC_ptr /* src_ptr */, ordSizeInWords,
                                   VirtualHwBaseAddr);
    /* signature D */
    LLF_PKI_PKA_CopyDataIntoPkaReg(rD /* dstReg */, LenID_r + 1, SignatureD_ptr /* src_ptr */, ordSizeInWords,
                                   VirtualHwBaseAddr);
    ...
    /* 2.1. rT = h = d^-1  mod r */
    Error = LLF_PKI_PKA_ModInv(LenID_r, rD /* OpB */, rT /* Res */, 0 /* Tag */, VirtualHwBaseAddr);
    if (Error) {
        goto End;
    }
    ...
    /* P1 =  h1*G */
    LLF_ECPKI_SetPoint((uint32_t *)(Domain_ptr->EC_Gener_X) /* X */, (uint32_t *)(Domain_ptr->EC_Gener_Y) /* Y */,
                       (uint32_t *)DX_NULL /* Z */, modSizeInWords, LenID_r, &P1 /* Res */, EC_PointJacobian,
                       VirtualHwBaseAddr);

    Error = LLF_ECPKI_GFpScalarMultProj(rT1, &P1 /* EC_generator */, &P1 /* Res */, DomainID, regTemps + 12,
                                        28 - 12 /* countTemps */, VirtualHwBaseAddr);
    if (Error != CRYS_OK) {
        goto End;
    }

    /* 3.2. P2 =  h2*W */
    LLF_ECPKI_SetPoint(SignerPublKey_ptr->PublKeyX /* X */, SignerPublKey_ptr->PublKeyY /* Y */, (uint32_t *)DX_NULL /* Z */,
                       modSizeInWords, LenID_r, &P2 /* Res */, EC_PointJacobian, VirtualHwBaseAddr);

    Error = LLF_ECPKI_GFpScalarMultProj(rT2, &P2 /* EC_generator */, &P2 /* Res */, DomainID, regTemps + 12,
                                        28 - 12 /* countTemps */, VirtualHwBaseAddr);
    if (Error != CRYS_OK) {
        goto End;
    }
    ...
    /* 3.3. P3 = P1 + P2 */
    Error =
        LLF_ECPKI_FullAdd(&P1, &P2, &P2 /* Res */, DomainID, regTemps + 12, 28 - 12 /* countTemps */, VirtualHwBaseAddr);
    ...
    Error = LLF_ECPKI_GFpJacobianToAffine(&P2, &P2, DomainID, regTemps + 12, 28 - 12, VirtualHwBaseAddr);
    if (Error != CRYS_OK) {
        goto End;
    }
