 ...
 */
CRYSError_t _DX_LLF_ECPKI_InitPubPrivKeyDb(CRYS_ECPKI_PublKey_t *PubKey_ptr, CRYS_ECPKI_PrivKey_t *PrivKey_ptr)
{
    /* LOCAL DECLARATIONS */
    ...
    if (PubKey_ptr != DX_NULL && PrivKey_ptr == DX_NULL) {
        ...
        Error = LLF_ECPKI_INIT_PKA_INVALID_PTR_ERROR;
        goto End;
    }
    ...
    DX_PAL_MemCopy((uint8_t *)NP_ptr,
                   (uint8_t *)(((LLF_ecpki_domains_db_t *)(LLF_EC_DOMAINS_ARRAY[DomainID].crysDomainIntBuff))->NP),
                   4 * LLF_ECPKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS);
    DX_PAL_MemCopy((uint8_t *)RP_ptr,
                   (uint8_t *)(((LLF_ecpki_domains_db_t *)(LLF_EC_DOMAINS_ARRAY[DomainID].crysDomainIntBuff))->RP),
                   4 * LLF_ECPKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS);
...
*/
CRYSError_t LLF_ECPKI_CheckPublKeyCall(CRYS_ECPKI_PublKey_t *PublKey_ptr, /* in */
                                       ...
                                       uint32_t *tempBuff_ptr /* in */)
{
    ...
    int8_t regTemps[30] = { 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10,
                            0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0xFF, 0xFF };
    ...
    *tempBuff_ptr = *tempBuff_ptr;
    ...
    LLF_PKI_PKA_CopyDataIntoPkaReg(0 /* dstReg */, LenID_n + 1,
                                   (uint32_t *)LLF_EC_DOMAINS_ARRAY[DomainID].GF_Modulus /* src_ptr */, modSizeInWords,
                                   VirtualHwBaseAddr);
    ...
    LLF_PKI_PKA_CopyDataIntoPkaReg(W.rX /* dstReg */, LenID_n + 1, PublKey_ptr->PublKeyX /* src_ptr */, modSizeInWords,
                                   VirtualHwBaseAddr);
    LLF_PKI_PKA_CopyDataIntoPkaReg(W.rY /* dstReg */, LenID_n + 1, PublKey_ptr->PublKeyY /* src_ptr */, modSizeInWords,
                                   VirtualHwBaseAddr);
    /* set Z = 1 */
    ...
    if (CheckMode == ECpublKeyPartlyCheck) {
        ...
        Error = LLF_ECPKI_CheckPublKeySize(DomainID, PublKey_ptr->PublKeyX, 2 * ((modSizeInBits + 7) / 8));
    } else if (CheckMode < EC_PublKeyCheckModeLast) {
        /* full checking of the public key validity */
        Error =
            LLF_ECPKI_GFpCheckPublKey(DomainID, &W, CheckMode, regTemps + 3, 28 - 3 /* countTemps */, VirtualHwBaseAddr);
    }
...
/*
   ...
   @param[in]  isKeyInternal - used by LLF_ECPKI_GenKeyPair
...
*/
CRYSError_t LLF_ECPKI_GenKeyPairCall(CRYS_ECPKI_DomainID_t DomainID,            /* in */
                                     int8_t isKeyInternal,                      /* in */
                                     ...
                                     CRYS_ECPKI_KG_TempData_t *TempData_ptr /* in */)
{
    ...
    uint32_t modSizeInBits;  /* size of EC modulus in bits */
    uint32_t modSizeInWords; /* size of EC modulus in words */
    ...
    *TempData_ptr = *TempData_ptr;
    ...
    LLF_PKI_PKA_CopyDataIntoPkaReg(0 /* dstReg */, LenID_n + 1, (uint32_t *)Domain_ptr->GF_Modulus /* src_ptr */,
                                   modSizeInWords, VirtualHwBaseAddr);

    LLF_PKI_PKA_CopyDataIntoPkaReg(
        1 /* dstReg */, LenID_n + 1,
        (uint32_t *)((LLF_ecpki_domains_db_t *)(Domain_ptr->crysDomainIntBuff))->NP /* src_ptr */,
        LLF_ECPKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS, VirtualHwBaseAddr);

    /* set Barrett tags into keys data bases  */
    DX_PAL_MemCopy((uint8_t *)(((LLF_ecpki_priv_key_db_t *)(privKey_ptr->crysPrivKeyIntBuff))->NP),
                   (uint8_t *)(((LLF_ecpki_domains_db_t *)(LLF_EC_DOMAINS_ARRAY[DomainID].crysDomainIntBuff))->NP),
                   4 * LLF_ECPKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS);
    DX_PAL_MemCopy((uint8_t *)(((LLF_ecpki_priv_key_db_t *)(privKey_ptr->crysPrivKeyIntBuff))->RP),
                   (uint8_t *)(((LLF_ecpki_domains_db_t *)(LLF_EC_DOMAINS_ARRAY[DomainID].crysDomainIntBuff))->RP),
                   4 * LLF_ECPKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS);
    DX_PAL_MemCopy((uint8_t *)(((LLF_ecpki_pub_key_db_t *)publKey_ptr->crysPubKeyIntBuff)->NP),
                   (uint8_t *)(((LLF_ecpki_domains_db_t *)(LLF_EC_DOMAINS_ARRAY[DomainID].crysDomainIntBuff))->NP),
                   4 * LLF_ECPKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS);
    DX_PAL_MemCopy((uint8_t *)(((LLF_ecpki_pub_key_db_t *)publKey_ptr->crysPubKeyIntBuff)->RP),
                   (uint8_t *)(((LLF_ecpki_domains_db_t *)(LLF_EC_DOMAINS_ARRAY[DomainID].crysDomainIntBuff))->RP),
                   4 * LLF_ECPKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS);
    ...
    Error = LLF_ECPKI_GenKeyPair(DomainID, rS /* privKey */, &W /* pubKey */, regTemps + 4, 28 - 4 /* countTemps */,
                                 TempData_ptr, isKeyInternal, VirtualHwBaseAddr);
    ...
    LLF_PKI_PKA_CopyDataFromPkaReg(publKey_ptr->PublKeyX /* dst */, modSizeInWords, W.rX /* src reg */, VirtualHwBaseAddr);

    LLF_PKI_PKA_CopyDataFromPkaReg(publKey_ptr->PublKeyY /* dst */, modSizeInWords, W.rY /* src reg */, VirtualHwBaseAddr);

    LLF_PKI_PKA_CopyDataFromPkaReg(privKey_ptr->PrivKey /* dst */, Domain_ptr->EC_OrderSizeInWords, rS /* src reg */,
                                   VirtualHwBaseAddr);
...
*/
CRYSError_t LLF_ECPKI_DomainInfoAndCheckKey(CRYS_ECPKI_DomainID_t DomainID,          /* in */
                                            CRYS_ECPKI_DomainInfo_t *DomainInfo_ptr, /* in */
                                            uint32_t *PrivKey_ptr,                   /* in */
                                            uint32_t PrivKeySizeInBytes,             /* in */
                                            uint32_t *PublKey_ptr,                   /* in */
                                            ...
                                            int8_t CheckKey /* in */)
{
    ...
    uint16_t OrderSizeInBits, OrderSizeInBytes;
    uint16_t ModSizeInBits, ModSizeInBytes, ModSizeInWords;
    ...
    if (DomainInfo_ptr != DX_NULL) {
        ...
        DomainInfo_ptr->ModulusMSByte        = (uint8_t)(Domain_ptr->GF_Modulus[ModSizeInWords - 1] & 0xFF);
    }
    ...
    if ((CheckKey & 1) == 1) { /* Check Private Key */
        ...
        if (CRYS_COMMON_CmpLsWordsUnsignedCounters(
                PrivKey_ptr, (uint16_t)(PrivKeySizeInBytes + 3) / 4, Domain_ptr->EC_GenerOrder,
                (uint16_t)Domain_ptr->EC_OrderSizeInWords) != CRYS_COMMON_CmpCounter2GraterThenCounter1) {
            return LLF_ECPKI_DOMAIN_INFO_INVALID_PRIV_KEY_DATA_ERROR;
        }

        if (CRYS_COMMON_GetWordsCounterEffectiveSizeInBits(PrivKey_ptr, (uint16_t)(PrivKeySizeInBytes + 3) / 4) == 0) {
            return LLF_ECPKI_DOMAIN_INFO_INVALID_PRIV_KEY_DATA_ERROR;
        }
    }
    ...
    if ((CheckKey & 2) == 2) { /* Check Public Key */
        ...
        if (PublKeySizeInBytes != 2 * (uint32_t)ModSizeInBytes)
            return LLF_ECPKI_DOMAIN_INFO_INVALID_PUBL_KEY_SIZE_ERROR;

        /* if PublKeyX > Modulus  then Error */
        if (CRYS_COMMON_CmpLsWordsUnsignedCounters(PublKey_ptr, ModSizeInWords, (uint32_t *)Domain_ptr->GF_Modulus,
                                                   ModSizeInWords) != CRYS_COMMON_CmpCounter2GraterThenCounter1) {
            return LLF_ECPKI_DOMAIN_INFO_INVALID_PUBL_KEY_DATA_ERROR;
        }

        /* if PublKeyY > Modulus  then Error */
        if (CRYS_COMMON_CmpLsWordsUnsignedCounters(((CRYS_ECPKI_PointAffine_t *)(void *)PublKey_ptr)->CoordY,
                                                   ModSizeInWords, (uint32_t *)Domain_ptr->GF_Modulus,
                                                   ModSizeInWords) != CRYS_COMMON_CmpCounter2GraterThenCounter1) {
            return LLF_ECPKI_DOMAIN_INFO_INVALID_PUBL_KEY_DATA_ERROR;
        }

        /* if PublKey Y and X = 0, then Error */
        if (CRYS_COMMON_GetWordsCounterEffectiveSizeInBits(((CRYS_ECPKI_PointAffine_t *)(void *)PublKey_ptr)->CoordX,
                                                           ModSizeInWords) == 0 &&
            CRYS_COMMON_GetWordsCounterEffectiveSizeInBits(((CRYS_ECPKI_PointAffine_t *)(void *)PublKey_ptr)->CoordY,
                                                           ModSizeInWords) == 0) {
            return LLF_ECPKI_DOMAIN_INFO_INVALID_PUBL_KEY_DATA_ERROR;
        }
    }
...
CRYSError_t LLF_ECPKI_SVDP_DH(CRYS_ECPKI_PublKey_t *PartnerPublKey_ptr, /* in */
                              uint32_t *UserPrivKey_ptr,                /* in */
                              uint32_t *SharedSecretValue_ptr,          /* out */
                              CRYS_ECDH_TempData_t *TempBuff_ptr /* in */)
{
    ...
    *TempBuff_ptr = *TempBuff_ptr;
    ...
    LLF_PKI_PKA_CopyDataIntoPkaReg(0 /* modulus */, LenID_n + 1, (uint32_t *)Domain_ptr->GF_Modulus /* src_ptr */,
                                   ordSizeInWords, VirtualHwBaseAddr);

    LLF_PKI_PKA_CopyDataIntoPkaReg(1 /* BarTag */, LenID_n + 1,
                                   ((LLF_ecpki_pub_key_db_t *)PartnerPublKey_ptr->crysPubKeyIntBuff)->NP /* src_ptr */,
                                   LLF_ECPKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS, VirtualHwBaseAddr);

    /* private key */
    LLF_PKI_PKA_CopyDataIntoPkaReg(rS /* dstReg */, LenID_n + 1, UserPrivKey_ptr /* src_ptr */, ordSizeInWords,
                                   VirtualHwBaseAddr);
    /* public key */
    LLF_ECPKI_SetPoint(PartnerPublKey_ptr->PublKeyX, PartnerPublKey_ptr->PublKeyY, DX_NULL /* Z */, modSizeInWords,
                       LenID_n, &W /* Res */, EC_PointJacobian, VirtualHwBaseAddr);

    /* ....... calculation of the shared secret point sP .................. */
    Error = LLF_ECPKI_GFpScalarMultProj(rS, &W, &sP /* Res */, DomainID, regTemps + 7, 28 - 7 /* countTemps */,
                                        VirtualHwBaseAddr);
    if (Error != CRYS_OK) {
        goto End;
    }
    ...
    if (isOnInfinity == 1) {
        ...
        Error = LLF_ECPKI_GFpJacobianToAffine(&sP /* PJac */, &W /* PAff */, DomainID, regTemps + 7, 28 - 7 /* countTemps */,
                                              VirtualHwBaseAddr);
        if (Error != CRYS_OK) {
            goto End;
        }
...
/*
   ...
   @return CRYSError_t - CRYS_OK,
                         Error from called functions
*/
CRYSError_t LLF_ECDSA_SignCalcCall(CRYS_ECPKI_PrivKey_t *SignerPrivateKey_ptr, /* in */
                                   uint32_t *MessageRepresent_ptr,             /* in */
                                   uint32_t MessageRepresentSizeWords,         /* in */
                                   uint32_t IsEphemerKeyInternal,              /* in */
                                   uint32_t *EphemerKeyData_ptr,               /* in */
                                   uint32_t *SignatureC_ptr,                   /* out */
                                   uint32_t *SignatureD_ptr,                   /* out */
                                   uint32_t *TempBuff_ptr /* in */)
{
    /* LOCAL DECLARATIONS */
    ...
    /* LLF function call */
    Error = LLF_ECDSA_SignCalc(SignerPrivateKey_ptr, MessageRepresent_ptr, MessageRepresentSizeWords,
                               IsEphemerKeyInternal, EphemerKeyData_ptr, SignatureC_ptr, SignatureD_ptr, TempBuff_ptr);
...
/*
 ...
 @return   CRYSError_t - CRYS_OK,
                         Error from called functions
*/
CRYSError_t LLF_ECDSA_VerifyCalcCall(CRYS_ECPKI_PublKey_t *SignerPublKey_ptr, /* in */
                                     uint32_t *MessageRepresent_ptr,          /* in */
                                     uint32_t *SignatureC_ptr,                /* in */
                                     uint32_t *SignatureD_ptr,                /* in */
                                     uint32_t *TempBuff_ptr /* in */)
{
    /* LOCAL DECLARATIONS */
    ...
    Error = LLF_ECDSA_VerifyCalc(SignerPublKey_ptr, MessageRepresent_ptr, Domain_ptr->EC_OrderSizeInWords,
