...

{
    ...
    uint32_t orderSizeInBits; /* size of EC order in bits */
    ...
    if (isPrivKeyInternal == DX_TRUE) {
        ...
        Error = DX_RND_GenerateWordsArrayInRange(orderSizeInBits,
                                                 (uint32_t *)LLF_EC_DOMAINS_ARRAY[DomainID].EC_GenerOrder /* MaxVect */,
    ...
    LLF_ECPKI_SetPoint((uint32_t *)LLF_EC_DOMAINS_ARRAY[DomainID].EC_Gener_X,
                       (uint32_t *)LLF_EC_DOMAINS_ARRAY[DomainID].EC_Gener_Y, DX_NULL /* Z_ptr */,
                       LLF_EC_DOMAINS_ARRAY[DomainID].EC_ModulusSizeInWords, LenID_n, W_ptr /* Res point */,
                       EC_PointJacobian, VirtualHwBaseAddr);

    /* --------------------------------------------------------------------------- */
    /* .........generation of public key by calling low level function ......... */
    /* --------------------------------------------------------------------------- */

    Error = LLF_ECPKI_GFpScalarMultProj(rS, W_ptr /* EC_Order */, W_ptr /* Res */, DomainID, regTemps_ptr, countTemps,
                                        VirtualHwBaseAddr);
    if (Error != CRYS_OK)
        return Error;

    /* convert public key to affine ordinary representation */
    Error = LLF_ECPKI_GFpJacobianToAffine(W_ptr, W_ptr, DomainID, regTemps_ptr, countTemps, VirtualHwBaseAddr);
...
*/
CRYSError_t LLF_ECPKI_GFpCheckPublKey(CRYS_ECPKI_DomainID_t DomainID,  /* in */
                                      ...
                                      uint32_t VirtualHwBaseAddr /* in */)
{
    ...
    if (checkMode == ECpublKeyFullCheck) {
        ...
        Error = LLF_ECPKI_GFpScalarMultProj(rOrd, W_ptr, &W1, DomainID, regTemps_ptr + 4, countTemps - 4,
                                            VirtualHwBaseAddr);
