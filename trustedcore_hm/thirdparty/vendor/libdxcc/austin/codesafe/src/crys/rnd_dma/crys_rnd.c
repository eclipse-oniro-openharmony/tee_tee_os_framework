 ...
 */
static void DX_RND_AddValToIv(uint32_t *iv_ptr, uint32_t val)
{
    int32_t i;
    uint32_t *ptr = iv_ptr;
    uint32_t tmp, curr;
 ...
 */
static CRYSError_t DX_RND_Update(CRYS_RND_State_t *rndState_ptr, /* in/out */
                                 ...
                                 uint8_t skipSetUp)              /* in */
{
    ...
    error = CRYS_AES_Init(&aesCtxID, /* AES context */
                          iv_ptr,    /* IV */
                          k_ptr,     /* calculated key */
                          keySizeID, CRYS_AES_Encrypt, CRYS_AES_CTR_mode);
    ...
    if (!skipSetUp) {
        ...
        error = CRYS_AES_Block(&aesCtxID, providedData_ptr, CRYS_AES_BLOCK_SIZE_IN_BYTES, seed_ptr);
    ...
    error = CRYS_AES_Finish(&aesCtxID, providedData_ptr,                   /* in */
                            seedSizeInWords * sizeof(uint32_t), seed_ptr); /* out */
 ...
 *                        value MODULE_* as defined in ...
 */
static CRYSError_t DX_RND_DF(CRYS_RND_State_t *rndState_ptr, /* in/out */
                             ...
                             uint32_t outDataSizeBytes)      /* in */
{
    ...
    const uint8_t InitialMac128[2][CRYS_RND_BASIC_BLOCK_SIZE_IN_BYTES] = {
        { 0xc6, 0xa1, 0x3b, 0x37, 0x87, 0x8f, 0x5b, 0x82, 0x6f, 0x4f, 0x81, 0x62, 0xa1, 0xc8, 0xd8, 0x79 },
        { 0x95, 0x03, 0xe3, 0xa2, 0x24, 0x5a, 0x2b, 0xe4, 0x3c, 0x98, 0x74, 0xed, 0xfe, 0x1b, 0xed, 0x9e }
    };
    const uint8_t InitialMac256[3][CRYS_RND_BASIC_BLOCK_SIZE_IN_BYTES] = {
    ...
    ((uint32_t *)inputBuff_ptr)[0] = CRYS_COMMON_REVERSE32(inputDataSizeBytes); /* L */
    ((uint32_t *)inputBuff_ptr)[1] = CRYS_COMMON_REVERSE32(outDataSizeBytes);   /* N */
#endif

    inputPtr[8 + inputDataSizeBytes] = 0x80;
    DX_PAL_MemSet(&inputPtr[8 + 1 + inputDataSizeBytes], 0,
    ...
    inputDataSizeBytes = ((inputDataSizeBytes + CRYS_AES_BLOCK_SIZE_IN_BYTES - 1) / CRYS_AES_BLOCK_SIZE_IN_BYTES) *
    ...
    for (i = 0; i < (outDataSizeBytes >> 4); i++) {
        ...
        /* AES MAC */
        error = CRYS_AES(initMac_ptr, (uint8_t *)&Key[0], keySizeID, CRYS_AES_Encrypt, CRYS_AES_MAC_mode,
                         (uint8_t *)inputBuff_ptr, inputDataSizeBytes,
    ...
    error = CRYS_AES_Init(&aesContext, iv_ptr, /* IV */
                          k_ptr,               /* key */
                          keySizeID, CRYS_AES_Encrypt, CRYS_AES_CBC_mode);
    ...
    error = CRYS_AES_Finish(&aesContext, output_ptr /* in */, outDataSizeBytes, output_ptr /* out */);
 ...
 */
static CRYSError_t DX_RND_InstantiateOrReseed(CRYS_RND_State_t *rndState_ptr,    /* in/out */
                                              ...
                                              CRYS_RND_WorkBuff_t *workBuff_ptr) /* in/out */
{
    ...
    RETURN_IF_RND_UNSUPPORTED(rndState_ptr, isInstantiate, isContinued, workBuff_ptr, entrSource_ptr, keySizeWords,
                              entropySizeBits, seedSizeBytes, sourceSizeBytes, trngParams.TrngMode, error, error, error,
    ...
    if (keySizeWords != CRYS_RND_AES_KEY_128_SIZE_WORDS && keySizeWords != CRYS_RND_AES_KEY_256_SIZE_WORDS)
        return CRYS_RND_ILLEGAL_AES_KEY_SIZE_ERROR;

    if (rndState_ptr->AddInputSizeWords > CRYS_RND_ADDITINAL_INPUT_MAX_SIZE_WORDS)
    ...
    if (isInstantiate == 0)
        ...
        entropySizeBits = (keySizeWords + CRYS_AES_BLOCK_SIZE_IN_WORDS) * 32; /* Instantiation */
    ...
    DX_PAL_MemCopy((uint8_t *)&entrSource_ptr[(sourceSizeBytes >> 2) + CRYS_RND_TRNG_SRC_INNER_OFFSET_WORDS],
    ...
    seedSizeBytes = keySizeWords * sizeof(uint32_t) + CRYS_AES_BLOCK_SIZE_IN_BYTES;

    /* 2.1. if Derivation Function is used, call it */
    error = DX_RND_DF(rndState_ptr,                                 /* in */
    ...
    if (isInstantiate == 1)
        DX_PAL_MemSetZero(rndState_ptr->Seed, sizeof(uint32_t) * (keySizeWords + CRYS_AES_BLOCK_SIZE_IN_WORDS));

    /* 2.2. Call Update for Additional data */
    error = DX_RND_Update(rndState_ptr,                                 /* in/out */
...
EndWithError:
    ...
    DX_PAL_MemSetZero(rndState_ptr, sizeof(CRYS_RND_State_t));

End:
    DX_PAL_MemSetZero((uint8_t *)workBuff_ptr, sizeof(CRYS_RND_WorkBuff_t));
 ...
 *                        value MODULE_* as defined in CRYS_Error.h
 */
static CRYSError_t DX_RND_CPRNGT(uint8_t *prev_ptr,   /* in */
                                 uint8_t *buff_ptr,   /* in */
                                 uint8_t *last_ptr,   /* in */
                                 int32_t countBlocks) /* in */
{
    ...
    if (countBlocks == 1) {
        if (DX_PAL_MemCmp(prev_ptr, /* prev */
                          last_ptr, /* last block */
        ...
        if (DX_PAL_MemCmp(buff_ptr + (countBlocks - 2) * CRYS_AES_BLOCK_SIZE_IN_BYTES, /* prev */
                          last_ptr,                                                    /* last block */
                          CRYS_AES_BLOCK_SIZE_IN_BYTES) == 0) {
            error = CRYS_RND_CPRNG_TEST_FAIL_ERROR;
            goto End;
    ...
    if (countBlocks > 2 && error == CRYS_OK) {
        for (i = 0; i < countBlocks - 2; i++) {
            ...
            if (DX_PAL_MemCmp(buff_ptr + i * CRYS_AES_BLOCK_SIZE_IN_BYTES,
                              buff_ptr + (i + 1) * CRYS_AES_BLOCK_SIZE_IN_BYTES, CRYS_AES_BLOCK_SIZE_IN_BYTES) == 0) {
                error = CRYS_RND_CPRNG_TEST_FAIL_ERROR;
                goto End;
            }
        }
 ...
 *                        value MODULE_* as defined in ...
 */
CEXPORT_C CRYSError_t CRYS_RND_GenerateVector(uint16_t outSizeBytes, /* in */
                                              uint8_t *out_ptr)      /* out */
{
    /* LOCAL DECLARATIONS */
    ...
    uint32_t temp[CRYS_RND_ADDITINAL_INPUT_MAX_SIZE_WORDS + 4];
    ...
    RETURN_IF_RND_UNSUPPORTED(outSizeBytes, out_ptr, keySizeWords, seedSizeWords, remainBytes, countBlocks, keySizeID,
                              AesUserContext.buff[0], rndState_ptr, error, error, error, error, error, error, error,
                              error, error, error, error);

    /* ................... checking parameters validity ..................... */
    /* ---------------------------------------------------------------------- */

    error = DX_PAL_MutexLock(&dxRndCryptoMutex, DX_INFINITE);
    if (error != DX_SUCCESS) {
    ...
    if (outSizeBytes == 0) {
        error = CRYS_OK; /* because of PSS */
        goto End;
    }

    if (rndState_ptr->ValidTag != CRYS_RND_WORK_STATE_VALID_TAG) {
        error = CRYS_RND_STATE_VALIDATION_TAG_ERROR;
        goto End;
    }

    /* Check, that instantiation was done */
    if (!(rndState_ptr->StateFlag & CRYS_RND_Instantiated)) {
        error = CRYS_RND_INSTANTIATION_NOT_DONE_ERROR;
        goto End;
    }
    ...
    if (rndState_ptr->ReseedCounter >= CRYS_RND_MAX_RESEED_COUNTER) {
        error = CRYS_RND_RESEED_COUNTER_OVERFLOW_ERROR;
        goto End;
    }
    ...
    if (keySizeWords != CRYS_RND_AES_KEY_128_SIZE_WORDS && keySizeWords != CRYS_RND_AES_KEY_256_SIZE_WORDS) {
        error = CRYS_RND_ILLEGAL_AES_KEY_SIZE_ERROR;
        goto End;
    }

    if (rndState_ptr->AddInputSizeWords > CRYS_RND_ADDITINAL_INPUT_MAX_SIZE_WORDS) {
        error = CRYS_RND_ADDITIONAL_INPUT_SIZE_ERROR;
        goto End;
    }
    ...
    if (rndState_ptr->AddInputSizeWords > 0) {
        ...
        DX_PAL_MemCopy((uint8_t *)&temp[CRYS_RND_TRNG_SRC_INNER_OFFSET_WORDS],
    ...
    DX_RND_AddValToIv(&rndState_ptr->Seed[keySizeWords], 1 /* val */);

    /* Init AES operation on CTR mode */
    error = CRYS_AES_Init(&AesUserContext, (uint8_t *)&rndState_ptr->Seed[keySizeWords], /* CTR=V */
                          (uint8_t *)&rndState_ptr->Seed[0],                             /* calculated key */
                          keySizeID, CRYS_AES_Encrypt, CRYS_AES_CTR_mode);
    if (error != CRYS_OK) {
        goto End;
    }
    ...
    if (!(rndState_ptr->StateFlag & CRYS_RND_KAT_mode) && !(rndState_ptr->StateFlag & CRYS_RND_PreviousIsValid)) {
        DX_PAL_MemSetZero(rndState_ptr->PreviousRandValue, sizeof(rndState_ptr->PreviousRandValue));

        error = CRYS_AES_Block(&AesUserContext, (uint8_t *)&rndState_ptr->PreviousRandValue[0],
                               CRYS_AES_BLOCK_SIZE_IN_BYTES, (uint8_t *)&rndState_ptr->PreviousRandValue[0]);
        if (error != CRYS_OK) {
            goto End;
        }
    ...
    if (outSizeBytes - remainBytes > 0) {
        ...
        error = CRYS_AES_Block(&AesUserContext, out_ptr, outSizeBytes - remainBytes, out_ptr);
        if (error != CRYS_OK) {
            goto End;
        }
    }
    ...
    error = CRYS_AES_Finish(&AesUserContext, (uint8_t *)&rndState_ptr->PreviousRandValue[0],
                            CRYS_AES_BLOCK_SIZE_IN_BYTES, (uint8_t *)&rndState_ptr->PreviousRandValue[0]);
    if (error != CRYS_OK) {
        goto End;
    }
    ...
    /* output remain bytes */
    DX_PAL_MemCopy(out_ptr + outSizeBytes - remainBytes, (uint8_t *)&rndState_ptr->PreviousRandValue[0], remainBytes);

    /*  Perform CPRNGT - continuous test on each block  */
    /* --------------------------------------------------- */
    if (!(rndState_ptr->StateFlag & CRYS_RND_KAT_mode)) {
        error = DX_RND_CPRNGT((uint8_t *)&temp[0],                            /* prev */
                              out_ptr,                                        /* buff_ptr */
    ...
    DX_RND_AddValToIv(&rndState_ptr->Seed[keySizeWords], countBlocks);
...
End:
    if (DX_PAL_MutexUnlock(&dxRndCryptoMutex) != DX_SUCCESS) {
...
CEXPORT_C CRYSError_t CRYS_RND_Instantiation(CRYS_RND_WorkBuff_t *workBuff_ptr /* in/out */)
{
    ...
    RETURN_IF_RND_UNSUPPORTED(workBuff_ptr, error, error, error, error, error, error, error, error, error, error, error,
    ...
    error = DX_RND_InstantiateOrReseed(&CRYS_RND_WorkingState, DX_TRUE /* isInstantiate */, DX_FALSE /* isContinued */,
                                       workBuff_ptr);
...
CEXPORT_C CRYSError_t CRYS_RND_UnInstantiation(void)
{
    ...
    RETURN_IF_RND_UNSUPPORTED(error, error, error, error, error, error, error, error, error, error, error, error, error,
                              error, error, error, error, error, error, error);
...
CEXPORT_C CRYSError_t CRYS_RND_Reseeding(CRYS_RND_WorkBuff_t *workBuff_ptr /* in/out */)
{
    ...
    error = DX_RND_InstantiateOrReseed(&CRYS_RND_WorkingState,                                              /* in/out */
                                       DX_FALSE /* isInstantiate */, DX_FALSE /* isContinued */, workBuff_ptr); /* in/out */
 ...
 */
CEXPORT_C CRYSError_t CRYS_RND_AddAdditionalInput(uint8_t *AdditonalInput_ptr, uint16_t AdditonalInputSizeBytes)
{
    /* The return error identifiers */
    CRYSError_t error;

    /* ............... if not supported exit .............................. */
    RETURN_IF_RND_UNSUPPORTED(AdditonalInput_ptr, AdditonalInputSizeBytes, error, error, error, error, error, error,
    ...
    if ((AdditonalInput_ptr == DX_NULL) && (AdditonalInputSizeBytes != 0))
     ...
     *  of 4 bytes                                 */
    if (AdditonalInputSizeBytes > sizeof(uint32_t) * CRYS_RND_ADDITINAL_INPUT_MAX_SIZE_WORDS ||
        AdditonalInputSizeBytes % sizeof(uint32_t)) {
        error = CRYS_RND_ADDITIONAL_INPUT_SIZE_ERROR;
        goto End;
    }

    DX_PAL_MemSetZero(CRYS_RND_WorkingState.AdditionalInput, sizeof(CRYS_RND_WorkingState.AdditionalInput));

    if (AdditonalInput_ptr != DX_NULL) {
        /* Copy the data from user to the global buffer: AdditionalInput */
        DX_PAL_MemCopy(CRYS_RND_WorkingState.AdditionalInput, AdditonalInput_ptr, AdditonalInputSizeBytes);
    }

    /* Set the AdditionalInput flag to indicate that data written to the buffer
    and the size of the data */
    CRYS_RND_WorkingState.AddInputSizeWords = AdditonalInputSizeBytes / sizeof(uint32_t);
 ...
 *                        value MODULE_* as defined in ...
 */
CEXPORT_C CRYSError_t CRYS_RND_EnterKatMode(uint8_t *entrData_ptr, uint32_t entrSize, uint8_t *nonce_ptr,
                                            uint32_t nonceSize, CRYS_RND_WorkBuff_t *workBuff_ptr /* out */)
{
    /* FUNCTION DECLARATIONS */
    ...
    /* ........     if not supported exit       ....... */
    RETURN_IF_RND_UNSUPPORTED(entrData_ptr, entrSize, nonce_ptr, nonceSize, workBuff_ptr, buf_ptr, error, error, error,
    ...
    if ((entrData_ptr == DX_NULL) && (entrSize == 0)) {
        ...
        error = CRYS_RND_ILLEGAL_PARAMETER_ERROR;
        goto End;
    }

    if ((nonce_ptr == DX_NULL) && (nonceSize != 0)) {
        error = CRYS_RND_ILLEGAL_DATA_PTR_ERROR;
        goto End;
    }

    if (workBuff_ptr == DX_NULL) {
        error = CRYS_RND_WORK_BUFFER_PTR_INVALID_ERROR;
        goto End;
    }

    /* check entropy size */
    if (entrSize > sizeof(uint32_t) * CRYS_RND_ENTROPY_TEMP_BUFFER_MAX_SIZE_WORDS) {
        error = CRYS_RND_ILLEGAL_DATA_SIZE_ERROR;
        goto End;
    }
    ...
    buf_ptr = (uint8_t *)workBuff_ptr +
              sizeof(uint32_t) * (CRYS_RND_SRC_BUFF_OFFSET_WORDS + CRYS_RND_TRNG_SRC_INNER_OFFSET_WORDS);

    DX_PAL_MemCopy(buf_ptr, entrData_ptr, entrSize);

    if ((nonce_ptr != DX_NULL) && (nonceSize != 0)) {
        /* check nonce size */
        if ((entrSize + nonceSize) > sizeof(uint32_t) * CRYS_RND_ENTROPY_TEMP_BUFFER_MAX_SIZE_WORDS) {
            error = CRYS_RND_ILLEGAL_DATA_SIZE_ERROR;
            goto End;
        }
     ...
     *  two words backward                             */
    *((uint32_t *)workBuff_ptr + CRYS_RND_SRC_BUFF_OFFSET_WORDS) = entrSize;
 ...
 */
CEXPORT_C CRYSError_t CRYS_RND_GenerateVectorInRange(uint32_t rndSizeInBits, uint8_t *maxVect_ptr, uint8_t *rndVect_ptr)
{
    /* FUNCTION DECLARATIONS */
    ...
    /* ............... if not supported exit .............................. */
    RETURN_IF_RND_UNSUPPORTED(rndSizeInBits, maxVect_ptr, rndVect_ptr, i, k, shift, mask, flag0, flag1, rndSizeInBytes,
                              checkingSizeBytes, maxVectSizeBits, maxVectSizeBytes, CompRes, smin, Error, Error, Error,
                              Error, Error);
    ...
    if (rndVect_ptr == DX_NULL)
        return CRYS_RND_VECTOR_OUT_PTR_ERROR;

    /* given size of random vector in bytes */
    rndSizeInBytes = (rndSizeInBits + 7) / 8;

    if (rndSizeInBits <= 1 || (uint32_t)rndSizeInBytes > CRYS_RND_MAX_GEN_VECTOR_SIZE_BYTES)
        return CRYS_RND_VECTOR_OUT_SIZE_ERROR;
    ...
    if (!maxVect_ptr) {
        Error = CRYS_RND_GenerateVector((uint16_t)rndSizeInBytes, rndVect_ptr);
    ...
    maxVectSizeBits = CRYS_COMMON_GetBytesCounterEffectiveSizeInBits(maxVect_ptr, (uint16_t)rndSizeInBytes);

    if (maxVectSizeBits < 2 || (maxVectSizeBits == 2 && maxVect_ptr[0] < 3))
        return CRYS_RND_MAX_VECTOR_IS_TOO_SMALL_ERROR;
    ...
    DX_PAL_MemSetZero(rndVect_ptr + maxVectSizeBytes, extraBytes);

    /* calc. intermediate checking size */
    if ((uint32_t)maxVectSizeBytes > CRYS_AES_BLOCK_SIZE_IN_BYTES) {
    ...
    while (k < 0xFFFF) {
        ...
        Error = CRYS_RND_GenerateVector((uint16_t)maxVectSizeBytes, rndVect_ptr);
        ...
        if (checkingSizeBytes == CRYS_AES_BLOCK_SIZE_IN_BYTES) {
            while (1) {
                CompRes = CRYS_COMMON_CmpLsbUnsignedCounters(
                    rndVect_ptr + maxVectSizeBytes - CRYS_AES_BLOCK_SIZE_IN_BYTES, CRYS_AES_BLOCK_SIZE_IN_BYTES,
                    maxVect_ptr + maxVectSizeBytes - CRYS_AES_BLOCK_SIZE_IN_BYTES, CRYS_AES_BLOCK_SIZE_IN_BYTES);
                ...
                Error = CRYS_RND_GenerateVector(CRYS_AES_BLOCK_SIZE_IN_BYTES,
                                                rndVect_ptr + maxVectSizeBytes - CRYS_AES_BLOCK_SIZE_IN_BYTES);
        ...
        CompRes = CRYS_COMMON_CmpLsbUnsignedCounters(rndVect_ptr, (uint16_t)maxVectSizeBytes, maxVect_ptr,
    ...
    CheckMinimal:
        /* check minimal value */
        CompRes = CRYS_COMMON_CmpLsbUnsignedCounters(rndVect_ptr, (uint16_t)rndSizeInBytes, &smin, 1);
                                             ...
                                             uint32_t *tmp_ptr)
{
    ...
    CRYSError_t err;
    uint32_t rndSizeInWords = (rndSizeInBits + 31) / 32;
     ...
     *  array                                  */
    DX_PAL_MemMove((uint8_t *)tmp_ptr, (uint8_t *)maxVect_ptr, rndSizeInWords * sizeof(uint32_t));
    ...
    err = CRYS_RND_GenerateVectorInRange(rndSizeInBits, (uint8_t *)tmp_ptr, (uint8_t *)rndVect_ptr);
