 ...
 */
CEXPORT_C CRYSError_t CRYS_RSA_Build_PubKey(CRYS_RSAUserPubKey_t *UserPubKey_ptr, uint8_t *Exponent_ptr,
                                            uint16_t ExponentSize, uint8_t *Modulus_ptr, uint16_t ModulusSize)
{
    ...
    uint32_t buffSizeBytes = CRYS_RSA_MAX_VALID_KEY_SIZE_VALUE_IN_BYTES;

    /* FUNCTION LOGIC */

    /* ............... if not supported exit .............................. */
    /* -------------------------------------------------------------------- */
    RETURN_IF_RSA_UNSUPPORTED(UserPubKey_ptr, Exponent_ptr, ExponentSize, Modulus_ptr, ModulusSize, CounterCmpResult,
                              ModulusEffectiveSizeInBits, ExponentEffectiveSizeInBits, PubKey_ptr, buffSizeBytes, Error,
    ...
    DX_PAL_MemSet(PubKey_ptr, 0, sizeof(CRYSRSAPubKey_t));
...
End:
    ...
    if (Error != CRYS_OK) {
        DX_PAL_MemSet(UserPubKey_ptr, 0, sizeof(CRYS_RSAUserPubKey_t));
    }
 ...
 *
 */
CEXPORT_C CRYSError_t CRYS_RSA_Build_PrivKey(CRYS_RSAUserPrivKey_t *UserPrivKey_ptr, uint8_t *PrivExponent_ptr,
                                             ...
                                             uint16_t PubExponentSize, uint8_t *Modulus_ptr, uint16_t ModulusSize)
{
    ...
    RETURN_IF_RSA_UNSUPPORTED(UserPrivKey_ptr, PrivExponent_ptr, PrivExponentSize, PubExponent_ptr, PubExponentSize,
                              Modulus_ptr, ModulusSize, CounterCmpResult, ModulusEffectiveSizeInBits,
                              PrivExponentEffectiveSizeInBits, PubExponentEffectiveSizeInBits, PrivKey_ptr,
                              buffSizeBytes, Error, Error, Error, Error, Error, Error, Error, Error, Error);
    ...
    if (PrivExponentSize > CRYS_RSA_MAX_VALID_KEY_SIZE_VALUE_IN_BYTES)
        return CRYS_RSA_INVALID_EXPONENT_SIZE;

    if (PubExponent_ptr != DX_NULL && PubExponentSize > CRYS_RSA_MAX_VALID_KEY_SIZE_VALUE_IN_BYTES)
        return CRYS_RSA_INVALID_EXPONENT_SIZE;
    ...
    DX_PAL_MemSet(PrivKey_ptr, 0, sizeof(CRYSRSAPrivKey_t));
    ...
    ModulusEffectiveSizeInBits = CRYS_COMMON_GetWordsCounterEffectiveSizeInBits(PrivKey_ptr->n, (ModulusSize + 3) / 4);
    ...
    CounterCmpResult = CRYS_COMMON_CmpMsbUnsignedCounters(PrivExponent_ptr, PrivExponentSize, Modulus_ptr, ModulusSize);
    ...
    if (PrivExponentEffectiveSizeInBits < 32 && PrivKey_ptr->PriveKeyDb.NonCrt.d[0] < CRYS_RSA_MIN_PRIV_EXP_VALUE) {
        Error = CRYS_RSA_INVALID_EXPONENT_VAL;
        goto End;
    }
    ...
    if (PubExponent_ptr != DX_NULL) {
        ...
        if (PubExponentEffectiveSizeInBits < 32 && PrivKey_ptr->PriveKeyDb.NonCrt.e[0] < CRYS_RSA_MIN_PUB_EXP_VALUE) {
            Error = CRYS_RSA_INVALID_EXPONENT_VAL;
            goto End;
        }
        ...
        CounterCmpResult =
            CRYS_COMMON_CmpMsbUnsignedCounters(PubExponent_ptr, PubExponentSize, Modulus_ptr, ModulusSize);
...
End:
    ...
    /* if the structure created is not valid - clear it */
    if (Error)
        DX_PAL_MemSet(UserPrivKey_ptr, 0, sizeof(CRYS_RSAUserPrivKey_t));
...
CEXPORT_C CRYSError_t CRYS_RsaGetPrivKey(CRYS_RSAUserPrivKey_t *UserPrivKey_ptr,
    uint8_t *PrivExponent_ptr, uint16_t *PrivExponentSize_ptr,
    uint8_t *PubExponent_ptr, uint16_t *PubExponentSize_ptr,
    uint8_t *Modulus_ptr, uint16_t *ModulusSize_ptr)
{
    CRYSError_t Error = CRYS_OK;
	...
	Error = CRYS_COMMON_ReverseMemcpy(Modulus_ptr, (uint8_t*)PrivKey_ptr->n, nSizeInBytes);
	if (Error != CRYS_OK)
		return Error;

	Error = CRYS_COMMON_ConvertLswMswWordsToMsbLsbBytes(PubExponent_ptr, *PubExponentSize_ptr,
        PrivKey_ptr->PriveKeyDb.NonCrt.e, eSizeInBytes);
	if (Error != CRYS_OK)
		return Error;


	Error = CRYS_COMMON_ReverseMemcpy(PrivExponent_ptr, (uint8_t*)PrivKey_ptr->PriveKeyDb.NonCrt.d, dSizeInBytes);
	if (Error != CRYS_OK)
		return Error;
...
*/
CEXPORT_C CRYSError_t CRYS_RSA_Build_PrivKeyCRT(CRYS_RSAUserPrivKey_t *UserPrivKey_ptr, uint8_t *P_ptr, uint16_t PSize,
                                                uint8_t *Q_ptr, uint16_t QSize, uint8_t *dP_ptr, uint16_t dPSize,
                                                uint8_t *dQ_ptr, uint16_t dQSize, uint8_t *qInv_ptr, uint16_t qInvSize)
{
    /* FUNCTION DECLARATIONS */
    ...
    RETURN_IF_RSA_UNSUPPORTED(UserPrivKey_ptr, P_ptr, PSize, Q_ptr, QSize, dP_ptr, dPSize, dQ_ptr, dQSize, qInv_ptr,
                              qInvSize, qInv_ptr, qInvSize, CounterCmpResult, P_EffectiveSizeInBits,
                              Q_EffectiveSizeInBits, dP_EffectiveSizeInBits, dQ_EffectiveSizeInBits,
                              qInv_EffectiveSizeInBits, ModulusEffectiveSizeInBits, PrivKey_ptr, Error);
    ...
    if (PSize > CRYS_RSA_MAX_VALID_KEY_SIZE_VALUE_IN_BYTES / 2 ||
        QSize > CRYS_RSA_MAX_VALID_KEY_SIZE_VALUE_IN_BYTES / 2) {
...
#ifndef DX_OEM_FW
    if (DxCcAcl_IsBuffAccessOk(ACCESS_READ_WRITE, UserPrivKey_ptr, sizeof(CRYS_RSAUserPrivKey_t)) ||
        DxCcAcl_IsBuffAccessOk(ACCESS_READ, P_ptr, PSize) || DxCcAcl_IsBuffAccessOk(ACCESS_READ, Q_ptr, QSize) ||
        DxCcAcl_IsBuffAccessOk(ACCESS_READ, dP_ptr, dPSize) || DxCcAcl_IsBuffAccessOk(ACCESS_READ, dQ_ptr, dQSize) ||
    ...
    CounterCmpResult = CRYS_COMMON_CmpMsbUnsignedCounters(dP_ptr, dPSize, P_ptr, PSize);
    ...
    CounterCmpResult = CRYS_COMMON_CmpMsbUnsignedCounters(dQ_ptr, dQSize, Q_ptr, QSize);
    ...
    CounterCmpResult = CRYS_COMMON_CmpMsbUnsignedCounters(qInv_ptr, qInvSize, P_ptr, PSize);
    ...
    Error = CRYS_COMMON_ConvertMsbLsbBytesToLswMswWords(PrivKey_ptr->PriveKeyDb.Crt.qInv, buffSizeBytes, qInv_ptr,
                                                        qInvSize);
    if (Error)
        return Error;
...
CEXPORT_C CRYSError_t CRYS_RsaGetPrivKeyCRT(CRYS_RSAUserPrivKey_t *UserPrivKey_ptr,
    uint8_t *P_ptr, uint16_t *PSize_ptr,
    uint8_t *Q_ptr, uint16_t *QSize_ptr,
    uint8_t *dP_ptr, uint16_t *dPSize_ptr,
    uint8_t *dQ_ptr, uint16_t *dQSize_ptr,
    uint8_t *qInv_ptr, uint16_t *qInvSize_ptr)
{
	...
	Error = CRYS_COMMON_ReverseMemcpy(P_ptr, (uint8_t*)PrivKey_ptr->PriveKeyDb.Crt.P, PSizeInBytes);
	if (Error != CRYS_OK)
		return Error;

	Error = CRYS_COMMON_ReverseMemcpy(Q_ptr, (uint8_t*)PrivKey_ptr->PriveKeyDb.Crt.Q, QSizeInBytes);
	if (Error != CRYS_OK)
		return Error;

	Error = CRYS_COMMON_ReverseMemcpy(dP_ptr, (uint8_t*)PrivKey_ptr->PriveKeyDb.Crt.dP, dPSizeInBytes);
	if (Error != CRYS_OK)
		return Error;

	Error = CRYS_COMMON_ReverseMemcpy(dQ_ptr, (uint8_t*)PrivKey_ptr->PriveKeyDb.Crt.dQ, dQSizeInBytes);
	if (Error != CRYS_OK)
		return Error;

	Error = CRYS_COMMON_ReverseMemcpy(qInv_ptr, (uint8_t*)PrivKey_ptr->PriveKeyDb.Crt.qInv, qInvSizeInBytes);
	if (Error != CRYS_OK)
		return Error;
                                                         ...
                                                         CRYS_RSAConvertKeyToCrtBuffers_t *Buffers_ptr)
{
    ...
    RETURN_IF_RSA_UNSUPPORTED(UserPrivKey_ptr, Buffers_ptr, n_ptr, e_ptr, nSizeInBits, nSizeInBytes, PrivKey_ptr,
                              PrivNonCRTKey_ptr, PrivCRTKey_ptr, CounterCmpResult, Error, Error, Error, Error, Error,
    ...
    /*  check the size of the modulus an exponent */
    ...
    if (PrivNonCRTKey_ptr->eSizeInBits == 0 || PrivNonCRTKey_ptr->eSizeInBits > nSizeInBits)
        return CRYS_RSA_INVALID_EXPONENT_SIZE;
    ...
    Error = LLF_PKI_RSA_ConvertPrivKeyToCRT(PrivNonCRTKey_ptr, n_ptr, nSizeInBits, PrivCRTKey_ptr, Buffers_ptr);
    ...
    if (Error == CRYS_OK) {
        ...
        PrivCRTKey_ptr->PSizeInBits =
            CRYS_COMMON_GetWordsCounterEffectiveSizeInBits(PrivCRTKey_ptr->P, (uint16_t)(nSizeInBytes + 3) / 4);

        PrivCRTKey_ptr->QSizeInBits =
            CRYS_COMMON_GetWordsCounterEffectiveSizeInBits(PrivCRTKey_ptr->Q, (uint16_t)(nSizeInBytes + 3) / 4);

        PrivCRTKey_ptr->dPSizeInBits =
            CRYS_COMMON_GetWordsCounterEffectiveSizeInBits(PrivCRTKey_ptr->dP, (uint16_t)(nSizeInBytes + 3) / 4);

        PrivCRTKey_ptr->dQSizeInBits =
            CRYS_COMMON_GetWordsCounterEffectiveSizeInBits(PrivCRTKey_ptr->dQ, (uint16_t)(nSizeInBytes + 3) / 4);

        PrivCRTKey_ptr->qInvSizeInBits =
            CRYS_COMMON_GetWordsCounterEffectiveSizeInBits(PrivCRTKey_ptr->qInv, (uint16_t)(nSizeInBytes + 3) / 4);
    ...
    DX_PAL_MemSetZero((uint8_t *)Buffers_ptr, sizeof(CRYS_RSAConvertKeyToCrtBuffers_t));
...
*/
CEXPORT_C CRYSError_t CRYS_RSA_Get_PubKey(CRYS_RSAUserPubKey_t *UserPubKey_ptr, uint8_t *Exponent_ptr,
                                          uint16_t *ExponentSize_ptr, uint8_t *Modulus_ptr, uint16_t *ModulusSize_ptr)
{
    /* LOCAL DECLERATIONS */
    ...
    RETURN_IF_RSA_UNSUPPORTED(UserPubKey_ptr, Exponent_ptr, ExponentSize_ptr, Modulus_ptr, ModulusSize_ptr,
                              nSizeInBytes, eSizeInBytes, PubKey_ptr, PubKey_ptr, PubKey_ptr, PubKey_ptr, PubKey_ptr,
                              PubKey_ptr, PubKey_ptr, PubKey_ptr, PubKey_ptr, PubKey_ptr, PubKey_ptr, PubKey_ptr,
    ...
    if (DxCcAcl_IsBuffAccessOk(ACCESS_READ, UserPubKey_ptr, sizeof(CRYS_RSAUserPubKey_t)) ||
        ...
        DxCcAcl_IsBuffAccessOk(ACCESS_READ, ExponentSize_ptr, sizeof(uint32_t)) ||
        DxCcAcl_IsBuffAccessOk(ACCESS_READ_WRITE, ModulusSize_ptr, sizeof(uint32_t))) {
        return CRYS_RSA_ILLEGAL_PARAMS_ACCORDING_TO_PRIV_ERROR;
    }
#endif
    ...
    Error = CRYS_COMMON_ConvertLswMswWordsToMsbLsbBytes(Exponent_ptr, 4 * ((*ExponentSize_ptr + 3) / 4), PubKey_ptr->e,
                                                        eSizeInBytes);
    if (Error != CRYS_OK)
        return Error;

    Error = CRYS_COMMON_ConvertLswMswWordsToMsbLsbBytes(Modulus_ptr, 4 * ((*ModulusSize_ptr + 3) / 4), PubKey_ptr->n,
                                                        nSizeInBytes);
    if (Error != CRYS_OK)
        return Error;
...
*/
CEXPORT_C CRYSError_t CRYS_RSA_Get_ModSizeFromPubKey(CRYS_RSAUserPubKey_t *UserPubKey_ptr, uint16_t *ModulusSize_ptr)
{
    /* LOCAL DECLERATIONS */
    ...
    RETURN_IF_RSA_UNSUPPORTED(UserPubKey_ptr, ModulusSize_ptr, PubKey_ptr, PubKey_ptr, PubKey_ptr, PubKey_ptr,
                              PubKey_ptr, PubKey_ptr, PubKey_ptr, PubKey_ptr, PubKey_ptr, PubKey_ptr, PubKey_ptr,
                              PubKey_ptr, PubKey_ptr, PubKey_ptr, PubKey_ptr, PubKey_ptr, PubKey_ptr, PubKey_ptr,
    ...
    /* calculating the required size in bytes */
    *ModulusSize_ptr = (uint16_t)((PubKey_ptr->nSizeInBits + 7) / 8);
