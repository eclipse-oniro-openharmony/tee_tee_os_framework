 ...
 *
 *  \version llf_pki_hw_defs.h#1:incl:1
 *  \author R.Levin
 */
...
/* PKA word size in bits */
#define LLF_PKI_PKA_WORD_SIZE_IN_BITS 32
...
#define LLF_PKI_PKA_MAX_OPERATION_SIZE_BITS 4096
#else
...
#define LLF_PKI_PKA_MIN_REGISTER_SIZE_WORDS ((LLF_PKI_PKA_MIN_OPERATION_SIZE_BITS / 32) + 1)
...
#define PKA_T0 30UL
#define PKA_T1 31UL
#define LLF_PKI_PKA_N_NP_T0_T1_REG_DEFAULT_VAL                                                 \
    (PKA_N << LLF_PKI_PKA_N_NP_T0_T1_REG_N_POS | PKA_NP << LLF_PKI_PKA_N_NP_T0_T1_REG_NP_POS | \
     PKA_T0 << LLF_PKI_PKA_N_NP_T0_T1_REG_T0_POS | PKA_T1 << LLF_PKI_PKA_N_NP_T0_T1_REG_T1_POS)
...
#define LLF_PKI_PKA_Set_StartMemAddr_Reg(PKA_StartMemAddr, VirtualHwBaseAddr) \
    DX_HAL_WriteCcRegister(DX_CC_REG_OFFSET(CRY_KERNEL, PKA_MON_READ), (uint32_t)(PKA_StartMemAddr))
...
#define LLF_PKI_PKA_Set_N_NP_T0_T1_Reg(N, NP, T0, T1, VirtualHwBaseAddr)                                 \
    ...
    DX_HAL_WriteCcRegister(                                                                              \
        DX_CC_REG_OFFSET(CRY_KERNEL, N_NP_T0_T1_ADDR),                                                   \
        (uint32_t)((N) << LLF_PKI_PKA_N_NP_T0_T1_REG_N_POS | (NP) << LLF_PKI_PKA_N_NP_T0_T1_REG_NP_POS | \
                   (T0) << LLF_PKI_PKA_N_NP_T0_T1_REG_T0_POS | (T1) << LLF_PKI_PKA_N_NP_T0_T1_REG_T1_POS))
...
#define LLF_PKI_PKA_SetDefault_N_NP_T0_T1_Reg(VirtualHwBaseAddr) \
    DX_HAL_WriteCcRegister(DX_CC_REG_OFFSET(CRY_KERNEL, N_NP_T0_T1_ADDR), LLF_PKI_PKA_N_NP_T0_T1_REG_DEFAULT_VAL)
...
#define LLF_PKI_PKA_SetRegSize(SizeBits, EntryNum, VirtualHwBaseAddr) \
    LLF_PKI_WAIT_ON_PKA_DONE(VirtualHwBaseAddr);                      \
    DX_HAL_WriteCcRegister((DX_CC_REG_OFFSET(CRY_KERNEL, PKA_L0) + 4 * (EntryNum)), (SizeBits))
...
#define LLF_PKI_PKA_SetRegAddress(VirtReg, PhysAddr, VirtualHwBaseAddr) \
    LLF_PKI_WAIT_ON_PKA_DONE(VirtualHwBaseAddr);                        \
    DX_HAL_WriteCcRegister((DX_CC_REG_OFFSET(CRY_KERNEL, MEMORY_MAP0) + 4 * (VirtReg)), (PhysAddr))
...
#define LLF_PKI_PKA_FullOpCode(Opcode, LenID, IsAImmed, OpA, IsBImmed, OpB, ResDiscard, Res, Tag) \
    (((uint32_t)(Opcode)&31) << LLF_PKI_PKA_OPCODE_OPERATION_ID_POS |                             \
     ((uint32_t)(LenID)&7) << LLF_PKI_PKA_OPCODE_LEN_POS |                                        \
     ((uint32_t)(IsAImmed)&1) << LLF_PKI_PKA_OPCODE_OPERAND_1_IMMED_POS |                         \
     ((uint32_t)(OpA)&31) << LLF_PKI_PKA_OPCODE_OPERAND_1_POS |                                   \
     ((uint32_t)(IsBImmed)&1) << LLF_PKI_PKA_OPCODE_OPERAND_2_IMMED_POS |                         \
     ((uint32_t)(OpB)&31) << LLF_PKI_PKA_OPCODE_OPERAND_2_POS |                                   \
     ((uint32_t)(ResDiscard)&1) << LLF_PKI_PKA_OPCODE_R_DISCARD_POS |                             \
     ((uint32_t)(Res)&31) << LLF_PKI_PKA_OPCODE_RESULT_POS | ((uint32_t)(Tag)&31) << LLF_PKI_PKA_OPCODE_TAG_POS)

/* *************************************************************** */
/*          Macros for reading and loading PKA memory data        */
/* *************************************************************** */

#ifndef CRYS_PKA_INDIRECT_SRAM_ACCESS_MODE
...
#define LLF_PKI_HW_LOAD_VALUE_TO_PKA_MEM(VirtualHwBaseAddr, Addr, Val)                                     \
    do {                                                                                                   \
        /* *(uint32_t*)((uint8_t*)(Addr) + LLF_PKI_PKA_DATA_REGS_MEMORY_OFFSET_ADDR) = ((uint32_t)Val); \ */ \
        *(uint32_t *)((Addr) + LLF_PKI_PKA_DATA_REGS_MEMORY_OFFSET_ADDR) = ((uint32_t)(Val));              \
    } while (0)
...
#define LLF_PKI_HW_LOAD_BLOCK_TO_PKA_MEM(VirtualHwBaseAddr, Addr, ptr, SizeWords)                        \
    do {                                                                                                 \
        DX_PAL_MemCopy((uint8_t *)((Addr) + LLF_PKI_PKA_DATA_REGS_MEMORY_OFFSET_ADDR), (uint8_t *)(ptr), \
                       (SizeWords) * sizeof(uint32_t));                                                  \
    } while (0)
...
#define LLF_PKI_HW_REVERSE_LOAD_BLOCK_TO_PKA_MEM(VirtualHwBaseAddr, Addr, ptr, SizeWords)                           \
    do {                                                                                                            \
        CRYS_COMMON_ReverseMemcpy((uint8_t *)((Addr) + LLF_PKI_PKA_DATA_REGS_MEMORY_OFFSET_ADDR), (uint8_t *)(ptr), \
                                  (SizeWords) * sizeof(uint32_t));                                                  \
    } while (0)
...
#define LLF_PKI_HW_CLEAR_PKA_MEM(VirtualHwBaseAddr, Addr, SizeWords)                      \
    do {                                                                                  \
        DX_PAL_MemSetZero((uint8_t *)((Addr) + LLF_PKI_PKA_DATA_REGS_MEMORY_OFFSET_ADDR), \
                          (SizeWords) * sizeof(uint32_t));                                \
    } while (0)
...
#define LLF_PKI_HW_READ_VALUE_FROM_PKA_MEM(VirtualHwBaseAddr, Addr, Val)          \
    do {                                                                          \
        (Val) = *(uint32_t *)((Addr) + LLF_PKI_PKA_DATA_REGS_MEMORY_OFFSET_ADDR); \
    } while (0)
...
#define LLF_PKI_HW_READ_BLOCK_FROM_PKA_MEM(VirtualHwBaseAddr, Addr, ptr, SizeWords)                      \
    do {                                                                                                 \
        DX_PAL_MemCopy((uint8_t *)(ptr), (uint8_t *)((Addr) + LLF_PKI_PKA_DATA_REGS_MEMORY_OFFSET_ADDR), \
                       (SizeWords) * sizeof(uint32_t));                                                  \
    } while (0)
...
#define LLF_PKI_HW_REVERSE_READ_BLOCK_FROM_PKA_MEM(VirtualHwBaseAddr, Addr, ptr, SizeWords)                         \
    do {                                                                                                            \
        CRYS_COMMON_ReverseMemcpy((uint8_t *)(ptr), (uint8_t *)((Addr) + LLF_PKI_PKA_DATA_REGS_MEMORY_OFFSET_ADDR), \
                                  (SizeWords) * sizeof(uint32_t));                                                  \
    } while (0)
...
#define LLF_PKI_HW_LOAD_VALUE_TO_PKA_MEM(VirtualHwBaseAddr, Addr, Val)                  \
    do {                                                                                \
        ...
        if (DX_PAL_MutexLock(&dxSymCryptoMutex, DX_INFINITE) != DX_SUCCESS) {           \
            DX_PAL_Abort("Fail to acquire mutex\n");                                    \
        }                                                                               \
        DX_HAL_WriteCcRegister(DX_CC_REG_OFFSET(HOST_RGF, SRAM_ADDR),                   \
                               ((Addr) + LLF_PKI_PKA_DATA_REGS_MEMORY_OFFSET_ADDR));    \
        DX_HAL_WriteCcRegister(DX_CC_REG_OFFSET(HOST_RGF, SRAM_DATA), (Val));           \
        do {                                                                            \
...
#define LLF_PKI_HW_LOAD_BLOCK_TO_PKA_MEM(VirtualHwBaseAddr, Addr, ptr, SizeWords)                   \
    do {                                                                                            \
        ...
        for (ii = 0; ii < (SizeWords); ii++) {                                                      \
            DX_HAL_WriteCcRegister(DX_CC_REG_OFFSET(HOST_RGF, SRAM_DATA), ((uint32_t *)(ptr))[ii]); \
            do {                                                                                    \
                dummy = DX_HAL_ReadCcRegister(DX_CC_REG_OFFSET(HOST_RGF, SRAM_DATA_READY));         \
            } while (!(dummy & 0x1));                                                               \
        }                                                                                           \
...
#define LLF_PKI_HW_REVERSE_LOAD_BLOCK_TO_PKA_MEM(VirtualHwBaseAddr, Addr, ptr, SizeWords)   \
    do {                                                                                    \
        ...
        DX_HAL_WriteCcRegister(DX_CC_REG_OFFSET(HOST_RGF, SRAM_ADDR),                       \
                               ((Addr) + LLF_PKI_PKA_DATA_REGS_MEMORY_OFFSET_ADDR));        \
        for (ii = 0; ii < (SizeWords); ii++) {                                              \
            dummy = (CRYS_COMMON_REVERSE32(((uint32_t *)(ptr))[(SizeWords)-ii - 1]));       \
            DX_HAL_WriteCcRegister(DX_CC_REG_OFFSET(HOST_RGF, SRAM_DATA), dummy);           \
            do {                                                                            \
                dummy = DX_HAL_ReadCcRegister(DX_CC_REG_OFFSET(HOST_RGF, SRAM_DATA_READY)); \
            } while (!(dummy & 0x1));                                                       \
        }                                                                                   \
...
#define LLF_PKI_HW_REVERSE_READ_BLOCK_FROM_PKA_MEM(VirtualHwBaseAddr, Addr, ptr, SizeWords)           \
    do {                                                                                              \
        ...
        for (ii = 0; ii < (SizeWords); ii++) {                                                        \
            ...
            dummy                     = DX_HAL_ReadCcRegister(DX_CC_REG_OFFSET(HOST_RGF, SRAM_DATA)); \
            (ptr)[(SizeWords)-1 - ii] = (CRYS_COMMON_REVERSE32(dummy));                               \
        }                                                                                             \
        do {                                                                                          \
            dummy = DX_HAL_ReadCcRegister(DX_CC_REG_OFFSET(HOST_RGF, SRAM_DATA_READY));               \
        } while (!(dummy & 0x1));                                                                     \
