                                         ...
                                         uint32_t VirtualHwBaseAddr)
{
    ...
    if (7 * (modSizeWords + 1) * sizeof(uint32_t) > LLF_PKI_PKA_MAX_REGS_MEM_SIZE_BYTES)
    ...
    LLF_PKI_PKA_CopyDataIntoPkaReg(0 /* dstReg */, 1 /* LenID */, PrivKey_ptr->n /* src_ptr */, modSizeWords,
                                   VirtualHwBaseAddr);
    /* copy Barrett tag NP into PKA register: NP=>r1 */
    LLF_PKI_PKA_CopyDataIntoPkaReg(
        1 /* dstReg */, 1 /* LenID */,
        ((LLF_pki_priv_key_db_t *)(PrivKey_ptr->crysRSAPrivKeyIntBuff))->NonCrt.NP /* src_ptr */,
        LLF_PKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS, VirtualHwBaseAddr);

    /* copy input data into PKI register: DataIn=>r2 */
    LLF_PKI_PKA_CopyDataIntoPkaReg(2 /* dstReg */, 1 /* LenID */, PrivData_ptr->DataIn /* src_ptr */, modSizeWords,
                                   VirtualHwBaseAddr);
...
/* Low memory CRT is not impelemented */
...
#define LenID_N1   3
    ...
    uint8_t Tag = 1;
    volatile uint32_t r=N_NP_T0_T1;
    ...
    /* check that there is enough memory for 7 PKA registers needed for
       CRT exponentiation */
    if (7 * (modSizeBytes+4) > LLF_PKI_PKA_MAX_REGS_MEM_SIZE_BYTES)
        return LLF_PKI_PKA_NOT_ENOUGH_MEMORY_ERROR;
    ...
    DX_HAL_WriteCcRegister( DX_CC_REG_OFFSET (CRY_KERNEL, N_NP_T0_T1_ADDR), (uint32_t)N_NP_T0_T1);
 ...
 *                        value MODULE_* as defined in .
 */
CRYSError_t LLF_PKI_ExecPrivKeyExpCrt_(CRYSRSAPrivKey_t *PrivKey_ptr, CRYS_RSAPrimeData_t *PrivData_ptr,
                                       uint32_t VirtualHwBaseAddr)
{
    /* LOCAL DECLARATIONS */
    ...
    int8_t rN = 0, rNP = 1, rD = 2, rT = 3, rT1 = 4, rMq = 5, rQ = 6, rqInv = 7;
    ...
    /* check that there is enough memory for 10 PKA registers needed for
       CRT exponentiation */
    if (10 * (modSizeWords + 1) * sizeof(uint32_t) > LLF_PKI_PKA_MAX_REGS_MEM_SIZE_BYTES)
        return LLF_PKI_PKA_NOT_ENOUGH_MEMORY_ERROR;

    /* initialize the PKA engine on default mode */
    Error =
        LLF_PKI_PKA_DefaultInitPKA(PrivKey_ptr->nSizeInBits, (PrivKey_ptr->nSizeInBits >> 5) + 1, VirtualHwBaseAddr);
    if (Error != CRYS_OK)
        return Error;
    ...
    LLF_PKI_PKA_CopyDataIntoPkaReg(rNP /* 1 dstReg */, 1 /* LenID */,
                                   ((LLF_pki_priv_key_db_t *)(PrivKey_ptr->crysRSAPrivKeyIntBuff))->Crt.QP /* src_ptr */,
                                   LLF_PKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS, VirtualHwBaseAddr);
    /* copy DataIn into rT and rT! */
    LLF_PKI_PKA_CopyDataIntoPkaReg(rT /* 3 dstReg */, 1 /* LenID */, PrivData_ptr->DataIn /* src_ptr */, modSizeWords,
                                   VirtualHwBaseAddr);
    ...
    LLF_PKI_PKA_CopyDataIntoPkaReg(rNP /* 1 dstReg */, 1 /* LenID */,
                                   ((LLF_pki_priv_key_db_t *)(PrivKey_ptr->crysRSAPrivKeyIntBuff))->Crt.PP /* src_ptr */,
                                   LLF_PKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS, VirtualHwBaseAddr);
    ...
    LLF_PKI_PKA_Div(0 /* LenID */, rT1 /* 4 OpA and remainder */, rN /* 0 OpB */, rT /* 3 res not used */, 0 /* Tag */,
                    VirtualHwBaseAddr);
