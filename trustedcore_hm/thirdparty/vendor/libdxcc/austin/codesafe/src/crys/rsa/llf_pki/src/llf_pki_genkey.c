                                 ...
                                 uint32_t *d_ptr, uint32_t *temp_ptr, uint32_t VirtualHwBaseAddr)
{
    ...
    /* the primes P, Q size */
    uint32_t pSizeInBits, pSizeInWords;
    ...
    *Success_ptr = DX_FALSE;
    ...
    LLF_PKI_PKA_CopyDataIntoPkaReg(rP /* dstReg */, LenIDpq + 1, p_ptr /* src_ptr */, pSizeInWords, VirtualHwBaseAddr);
    LLF_PKI_PKA_CopyDataIntoPkaReg(rQ /* dstReg */, LenIDpq + 1, q_ptr /* src_ptr */, pSizeInWords, VirtualHwBaseAddr);
    LLF_PKI_PKA_CopyDataIntoPkaReg(rE /* dstReg */, LenIDpq + 1, e_ptr /* src_ptr */, (eSizeInBits + 31) / 32,
                                   VirtualHwBaseAddr);
    ...
    Error = LLF_PKI_KG_FindPrime(rP, pSizeInBits, rE, eSizeInBits, testsCount, regTemps + 3, 28 - 3 /* tempsCount */,
    ...
    Error = LLF_PKI_KG_FindPrime(rQ, pSizeInBits, rE, eSizeInBits, testsCount, regTemps + 3, 28 - 3 /* tempsCount */,
                                 temp_ptr, VirtualHwBaseAddr);
    ...
    LLF_PKI_PKA_BigShift(PKA_SHL0, LenIDpq, rT1 /* OpA */, pSizeInBits - 100 /* S */, rT1 /* res */, 0 /* Tag */,
                         VirtualHwBaseAddr);
    ...
    if (status == 1) {
        LLF_PKI_PKA_CopyDataFromPkaReg(p_ptr /* dst_ptr */, pSizeInWords, rP /* srcReg */, VirtualHwBaseAddr);
        LLF_PKI_PKA_CopyDataFromPkaReg(q_ptr /* dst_ptr */, pSizeInWords, rQ /* srcReg */, VirtualHwBaseAddr);

        /* calculate modulus n and private exponent d (if appropriate pointer is not NULL */
        Error = LLF_PKI_CalculateNandD(e_ptr, eSizeInBits, p_ptr, q_ptr, pSizeInBits, n_ptr, d_ptr, VirtualHwBaseAddr);
                                   ...
                                   uint32_t VirtualHwBaseAddr)
{
    ...
    if (n_ptr != DX_NULL) {
        ...
        Error = LLF_PKI_PKA_DefaultInitPKA(2 * primeSizeInBits, LLF_PKI_PKA_MAX_KEY_GENERATION_SIZE_BITS / 32 + 1,
                                           VirtualHwBaseAddr);
        ...
        LLF_PKI_PKA_CopyDataIntoPkaReg(rP /* dstReg */, LenIDn + 1, p_ptr /* src_ptr */, pSizeInWords, VirtualHwBaseAddr);

        LLF_PKI_PKA_CopyDataIntoPkaReg(rQ /* dstReg */, LenIDn + 1, q_ptr /* src_ptr */, pSizeInWords, VirtualHwBaseAddr);
        /* ----------------------------------------------- */
    ...
    if (d_ptr != DX_NULL) {
        ...
        LLF_PKI_PKA_CopyDataIntoPkaReg(rE /* dstReg */, LenIDn + 1, e_ptr /* src_ptr */, (eSizeInBits + 31) / 32,
                                       VirtualHwBaseAddr);
        ...
        /* calc rD = modInv */
        Error = LLF_PKI_PKA_ExecFullModInv(rE /* OpB */, rD /* Res */, LenIDn, 6 /* rT0 */, 7 /* rT1 */, 8 /* rT2 */, 9 /* rT3 */,
                              ...
                              uint32_t VirtualHwBaseAddr) /* in */
{
    ...
    *Success_ptr = DX_FALSE;
    ...
    /* ------------------------------------------------------------------------------ */
    Error = LLF_PKI_genKeyNonCrt(e_ptr,       /* in */
                                 eSizeInBits, /* in */
                                 nSizeInBits, /* in */
                                 testsCount,  /* in */
                                 Success_ptr, /* out */
                                       ...
                                       uint32_t VirtualHwBaseAddr)
{
    ...
    Error = LLF_PKI_PKA_DefaultInitPKA(nSizeInBits / 2, LLF_PKI_PKA_MAX_KEY_GENERATION_SIZE_BITS / 32 + 1,
                                       VirtualHwBaseAddr);
    ...
    /* ------------------------------ */
    LLF_PKI_PKA_CopyDataIntoPkaReg(rE /* dstReg */, LenID + 1, e_ptr /* src_ptr */, (eSizeInBits + 31) / 32,
                                   VirtualHwBaseAddr);
    LLF_PKI_PKA_CopyDataIntoPkaReg(rP /* dstReg */, LenID + 1, p_ptr /* src_ptr */, pSizeInWords, VirtualHwBaseAddr);
    LLF_PKI_PKA_CopyDataIntoPkaReg(rQ /* dstReg */, LenID + 1, q_ptr /* src_ptr */, pSizeInWords, VirtualHwBaseAddr);
    ...
    Error = LLF_PKI_PKA_ExecFullModInv(rT1 /* OpB */, rdQ /* Res */, LenID, rT1 + 1, rT1 + 2, rT1 + 3, rT1 + 4, 0 /* Tag */,
                                       VirtualHwBaseAddr);
    if (Error != CRYS_OK)
        return Error;
    ...
    LLF_PKI_PKA_Copy(LenID + 1, rT1 /* ds */, rE /* src */, 0 /* Tag */, VirtualHwBaseAddr);
    /* mod inverse for even modulus */
    Error = LLF_PKI_PKA_ExecFullModInv(rT1 /* OpB */, rdP /* Res */, LenID, rT1 + 1, rT1 + 2, rT1 + 3, rT1 + 4, 0 /* Tag */,
                                       VirtualHwBaseAddr);
    if (Error != CRYS_OK)
        return Error;
    ...
    Error = LLF_PKI_PKA_ModInv(LenID, rT1 /* OpB */, rQinv /* Res */, 0 /* Tag */, VirtualHwBaseAddr);
    if (Error != CRYS_OK)
        return Error;
