...
extern uint32_t PLAT_CryptoCellBaseAddr;
uint32_t g_MemOffsetAddr;

#ifdef LLF_PKI_PKA_DEBUG
uint32_t tempRes[LLF_PKI_PKA_MAX_REGISTER_SIZE_WORDS];
uint32_t tempRes1[LLF_PKI_PKA_MAX_REGISTER_SIZE_WORDS];
#endif

#ifdef HW_MOD_INV_CALC_FAILS_COUNT
uint32_t gModInvCountAddedModules;
#endif
 ...
 *
 */
CRYSError_t LLF_PKI_PKA_SetRegsSizesTab(LLF_PKI_PKA_RegsSizes_t RegsSizes_ptr, /* in */
                                        uint32_t CountOfSizes,                 /* in */
                                        uint32_t MaxSizeBits,                  /* in */
                                        uint32_t IsDefaultMap,                 /* in */
                                        uint32_t VirtualHwBaseAddr /* in */)
{
    /* LOCAL DECLARATIONS */
    ...
    if (IsDefaultMap == 0) {
    ...
    } else { /* 2. Case of default settings */
#ifdef LLF_PKI_PKA_DEBUG
        /* check maximal size */
        if (MaxSizeBits < LLF_PKI_PKA_MIN_OPERATION_SIZE_BITS || MaxSizeBits > LLF_PKI_PKA_MAX_OPERATION_SIZE_BITS)
        ...
        DX_HAL_WriteCcRegister(DX_CC_REG_OFFSET(CRY_KERNEL, PKA_L0) + 4, 32 * MaxSizeWords + 32);
 ...
 *
 */
CRYSError_t LLF_PKI_PKA_SetRegsMapTab(LLF_PKI_PKA_RegsMap_t *RegsMap_ptr, /* in */
                                      uint32_t *CountOfRegs,              /* in */
                                      uint32_t MaxSizeWords,              /* in */
                                      uint32_t N_NP_T0_T1,                /* in */
                                      uint32_t IsDefaultMap,              /* in */
                                      uint32_t VirtualHwBaseAddr /* in */)
{
    /* LOCAL DECLARATIONS */
    ...
    if (MaxSizeWords < LLF_PKI_PKA_MIN_REGISTER_SIZE_WORDS || MaxSizeWords > LLF_PKI_PKA_MAX_REGISTER_SIZE_WORDS)
        return LLF_PKI_PKA_REGISTER_SIZES_ERROR;
#endif
    ...
    if (IsDefaultMap == 1) {
        ...
        defaultCountOfRegs = min(32, (uint32_t)(8 * LLF_PKI_PKA_MAX_REGS_MEM_SIZE_BYTES) / defaultMaxSize);
        ...
        for (i = 0; i < 32 - 2; i++) {
            if (i < defaultCountOfRegs - 2) {
                DX_HAL_WriteCcRegister(DX_CC_REG_OFFSET(CRY_KERNEL, MEMORY_MAP0) + 4 * i, CurrentAddr);
        ...
        DX_HAL_WriteCcRegister(DX_CC_REG_OFFSET(CRY_KERNEL, MEMORY_MAP0) + 4 * 30, CurrentAddr);

        CurrentAddr = CurrentAddr + defaultMaxSize / 8;

        DX_HAL_WriteCcRegister(DX_CC_REG_OFFSET(CRY_KERNEL, MEMORY_MAP0) + 4 * 31, CurrentAddr);
        ...
        *CountOfRegs = defaultCountOfRegs;
        ...
        DX_HAL_WriteCcRegister(DX_CC_REG_OFFSET(CRY_KERNEL, N_NP_T0_T1_ADDR),
                               (uint32_t)LLF_PKI_PKA_N_NP_T0_T1_REG_DEFAULT_VAL);
    }
    ...
    if (IsDefaultMap == 0) {
        ...
        if (*CountOfRegs == 0 || *CountOfRegs > 31)
            return LLF_PKI_PKA_ENTRIES_COUNT_ERROR;
 ...
 *
 */
CRYSError_t LLF_PKI_PKA_InitPKA(LLF_PKI_PKA_RegsSizes_t RegsSizes_ptr, /* in */
                                uint32_t CountOfSizes,                 /* in */
                                LLF_PKI_PKA_RegsMap_t *RegsMap_ptr,    /* in */
                                uint32_t CountOfRegs,                  /* in */
                                uint32_t OpSizeBits,                   /* in */
                                uint32_t RegSizeWords,                 /* in */
                                uint32_t N_NP_T0_T1,                   /* in */
                                uint32_t IsDefaultMap,                 /* in */
                                uint32_t VirtualHwBaseAddr /* in */)
{
    ...
    if (g_MemOffsetAddr != DX_SRAM_PKA_BASE_ADDRESS) {
        return LLF_PKI_PKA_ILLEGAL_PKA_ADDRESS_ERROR;
    }
    ...
    /* ----------------------------------------- */
    Error = LLF_PKI_PKA_SetRegsSizesTab(RegsSizes_ptr, /* in */
                                        CountOfSizes,  /* in */
                                        OpSizeBits,    /* in */
                                        IsDefaultMap,  /* in */
    ...
    /* ----------------------------------------- */
    Error = LLF_PKI_PKA_SetRegsMapTab(RegsMap_ptr,  /* in */
                                      &CountOfRegs, /* in/out */
                                      RegSizeWords, /* in */
                                      N_NP_T0_T1,   /* in */
                                      IsDefaultMap, /* in */
    ...
    DX_HAL_WriteCcRegister(DX_CC_REG_OFFSET(CRY_KERNEL, PKA_L0) + 3 * 4, 32 * RegSizeWords);
 ...
 *
 */
CRYSError_t LLF_PKI_PKA_InitPkaEx(uint32_t opSizeBits, /* in */
                                  uint32_t virtualHwBaseAddr /* in */)
{
    /* LOCAL DECLARATIONS */
    ...
    uint32_t regSizeWords, countOfRegs, i;

    /* FUNCTION LOGIC */

#ifdef LLF_PKI_PKA_DEBUG
    /* check maximal size */
    if (opSizeBits < LLF_PKI_PKA_MIN_OPERATION_SIZE_BITS || opSizeBits > LLF_PKI_PKA_MAX_OPERATION_SIZE_BITS)
    ...
    LLF_PKI_HW_CLEAR_PKA_MEM(virtualHwBaseAddr, LLF_PKI_PKA_DATA_REGS_BASE_ADDR,
                             LLF_PKI_PKA_MAX_REGS_MEM_SIZE_BYTES / 4);
    ...
    countOfRegs = min(32, (uint32_t)(LLF_PKI_PKA_MAX_REGS_MEM_SIZE_BYTES) / (4 * regSizeWords));
 ...
 *
 */
void LLF_PKI_PKA_FinishPKA(uint32_t VirtualHwBaseAddr /* in */)
{
    /* FUNCTION LOGIC */
 ...
 *
 */
CRYSError_t LLF_PKI_PKA_ExecCalcNpAndInitModOp(uint32_t LenID, uint32_t modSizeBits, int8_t rT0, int8_t rT1, int8_t rT2,
                                               uint32_t VirtualHwBaseAddr)
{
    /* LOCAL DECLARATIONS */
 ...
 *
 */
CRYSError_t LLF_PKI_PKA_DivLongNum(uint8_t LenID, int8_t OpA, uint32_t S, int8_t OpB, int8_t Res, /* div result - out */
                                   int8_t rT1,                                                    /* remainder -  out */
                                   int8_t rT2, uint32_t VirtualHwBaseAddr)
{
    /* LOCAL DECLARATIONS */
    ...
    uint32_t RegSizeBits, RegSizeBytes;
    ...
    for (i = 0; i < nWords; i++) {
        ...
        DX_PAL_MemSetZero((uint8_t *)tempRes, sizeof(tempRes));
        LLF_PKI_PKA_CopyDataFromPkaReg(tempRes /* dst_ptr */, RegSizeWords, rT1 /* srcReg */, VirtualHwBaseAddr);
#endif
 ...
 *
 */
CRYSError_t LLF_PKI_CalcNpAndInitModOp(uint32_t *N_ptr,         /* in */
                                       uint32_t N_sizeBits,     /* in */
                                       LLF_PKI_PKA_NP_t NP_ptr, /* in/out */
                                       uint8_t NpCreateFlag,    /* in */
                                       int8_t rT0,              /* in */
                                       int8_t rT1,              /* in */
                                       int8_t rT2,              /* in */
                                       uint32_t VirtualHwBaseAddr /* in */)
{
    ...
    if (N_ptr == DX_NULL || NP_ptr == DX_NULL)

        return LLF_PKI_PKA_ILLEGAL_PTR_ERROR;

    if (N_sizeBits < LLF_PKI_PKA_MIN_OPERATION_SIZE_BITS || N_sizeBits > LLF_PKI_PKA_MAX_OPERATION_SIZE_BITS)
    ...
    LLF_PKI_PKA_CopyDataIntoPkaReg(0 /* dstReg */, 1 /* LenID */, N_ptr /* src_ptr */, N_sizeWords, VirtualHwBaseAddr);
    ...
    if (NpCreateFlag == PKA_SetNP) {
        ...
        LLF_PKI_PKA_ExecCalcNpAndInitModOp(0 /* LenID */, N_sizeBits, rT0, rT1, rT2, VirtualHwBaseAddr);

        /* output of NP value */
        LLF_PKI_PKA_CopyDataFromPkaReg(NP_ptr /* dst_ptr */, LLF_PKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS, 1 /* srcReg */,
                                       VirtualHwBaseAddr);
    }
 ...
 */
CRYSError_t _LLF_PKI_PKA_ExecOperation(uint32_t Opcode,    /* in */
                                       uint8_t LenID,      /* in */
                                       uint8_t IsAImmed,   /* in */
                                       int8_t OpA,         /* in */
                                       uint8_t IsBImmed,   /* in */
                                       ...
                                       uint32_t VirtualHwBaseAddr /* in */)
{
    ...
    /* register size */
    uint8_t OpPrint;
    uint32_t OpSizeBits, OpSizeWords;
#endif
    ...
    if (((int)OpA < -15) || (OpA > 31) || ((int)OpB < -15) || (OpB > 31) || ((int)Res < -15) || (Res > 31) ||
        (Tag > 31)) {
    ...
    if (Opcode == PKA_ModInv || Opcode == PKA_Div) {
        ...
        FullOpCode = LLF_PKI_PKA_FullOpCode(PKA_AddIm, LenID, 0 /* isAImmed */, OpT /* N */, 1, 0 /* imm 0 */,
                                            1 /* ResDiscard */, 0, Tag);
    ...
    if (Opcode == PKA_ModInv) {
        ...
        FullOpCode =
            LLF_PKI_PKA_FullOpCode(PKA_Test0, LenID, 0 /* isAImmed */, 0 /* N */, 1, 1 /* imm 1 */, 1 /* ResDiscard */, 0, Tag);
    ...
    if (ResDiscard == 0 && Opcode != PKA_Terminate) {
        DX_PAL_MemSetZero(tempRes, sizeof(tempRes));

        LLF_PKI_PKA_CopyDataFromPkaReg(tempRes /* dst_ptr */, OpSizeWords, Res /* srcReg */, VirtualHwBaseAddr);
        ...
        if (Opcode == PKA_Div || Opcode == PKA_ModInv) {
            ...
            LLF_PKI_PKA_CopyDataFromPkaReg(tempRes1 /* dst_ptr */, OpSizeWords, OpPrint /* srcReg */, VirtualHwBaseAddr);
        }
    }
#endif
 ...
 *
 */
CRYSError_t LLF_PKI_PKA_ExecFullModInv(int8_t OpB,     /* in */
                                       int8_t Res,     /* in */
                                       uint32_t LenID, /* in */
                                       int8_t rT0,     /* in */
                                       int8_t rT1,     /* in */
                                       ...
                                       uint32_t VirtualHwBaseAddr /* in */)
{
    ...
    if (status == 0 /* odd N */) {
        ...
        Error = LLF_PKI_PKA_DivLongNum(LenID,                  /* LenID of exact size */
                                       rT0,                    /* numerator */
                                       32 * ModSizeWords + 32, /* Shift */
                                       OpB,                    /* divider */
                                       Res,                    /* result */
                                       rT1, rT3, VirtualHwBaseAddr);
 ...
 *
 */
void LLF_PKI_PKA_CopyDataFromPkaReg(uint32_t *dst_ptr,  /* out */
 ...
 *
 */
void LLF_PKI_PKA_CopyDataIntoPkaReg(int8_t dstReg,      /* out */
                                    uint8_t LenID,      /* in */
                                    uint32_t *src_ptr,  /* in */
                                    uint32_t sizeWords, /* in */
                                    uint32_t VirtualHwBaseAddr /* in */)
{
    ...
    currAddr = currAddr + sizeof(uint32_t) * sizeWords;
 ...
 *
 */
void LLF_PKI_PKA_ClearBlockOfRegs(uint8_t firstReg,    /* in */
                                  ...
                                  uint32_t virtualHwBaseAddr /* in */)
{
    ...
    if (firstReg > 30) {
        DX_PAL_Abort("firstReg > 30");
    }

    /* size of register in words */
    ...
    if ((firstReg + countOfRegs) * size > LLF_PKI_PKA_MAX_REGS_MEM_SIZE_BYTES / sizeof(uint32_t)) {
        countOfRegs = ((LLF_PKI_PKA_MAX_REGS_MEM_SIZE_BYTES / sizeof(uint32_t)) / size) - firstReg;
    }

    /* clear ordinary and temp registers without PKA operations */
    for (i = 0; i < countOfRegs; i++) {
        LLF_PKI_PKA_GetRegAddress(firstReg + i /* VirtReg */, addr /* physAddr */, virtualHwBaseAddr);
 ...
 *
 */
uint8_t LLF_PKI_PKA_GetBitFromPkaReg(int8_t rX, uint32_t LenID, int32_t i, int8_t rT, uint32_t VirtualHwBaseAddr)
{
    /* LOCAL DECLARATIONS */
 ...
 *
 */
void LLF_PKI_PKA_ModDivideBy2(uint8_t LenID, int8_t rX, int8_t rN, int8_t rRes, uint8_t Tag, uint32_t VirtualHwBaseAddr)
{
    /* DECLARATIONS */
 ...
 *
 */
CRYSError_t LLF_PKI_PKA_BigShift(uint32_t Opcode, /* in */
                                 uint8_t LenID,   /* in */
                                 int8_t OpA,      /* in */
                                 uint32_t S,      /* in */
                                 int8_t Res,      /* out */
                                 ...
                                 uint32_t VirtualHwBaseAddr /* in */)
{
    ...
    /* shift by bits */
    if (numBits != 0) {
        _LLF_PKI_PKA_ExecOperation(Opcode, LenID, 0 /* IsAImmed */, OpA, 0 /* IsBImmed */, (int8_t)numBits - 1 /* OpB */,
                                   0 /* ResDiscard */, Res, 0, VirtualHwBaseAddr);
    }

    /* shift by words */
    for (i = 0; i < numWords; i++) {
        _LLF_PKI_PKA_ExecOperation(Opcode, LenID, 0 /* IsAImmed */, OpA, 0 /* IsBImmed */, 32 - 1 /* OpB */, 0 /* ResDiscard */,
                                   Res, Tag, VirtualHwBaseAddr);
    }
 ...
 *
 */
uint32_t LLF_PKI_PKA_GetRegEffectiveSizeInBits(uint8_t LenID, /* in */
                                               int8_t reg,    /* in */
                                               uint8_t Tag,   /* in */
                                               uint32_t VirtualHwBaseAddr /* in */)
{
    ...
    return CRYS_COMMON_GetWordsCounterEffectiveSizeInBits((uint32_t *)(addr + LLF_PKI_PKA_DATA_REGS_MEMORY_OFFSET_ADDR),
                                                          (uint16_t)size);
 ...
 *
 */
uint32_t LLF_PKI_PKA_GetWordFromRegister(uint8_t LenID, /* in */
                                         int8_t rReg,   /* in */
                                         uint32_t i,    /* in */
                                         uint32_t VirtualHwBaseAddr /* in */)
{
 ...
 *
 */
void LLF_PKI_PKA_SetWordIntoRegister(uint8_t LenID, /* in */
                                     uint32_t word, /* in */
                                     int8_t rReg,   /* in */
                                     uint32_t i,    /* in */
                                     uint32_t VirtualHwBaseAddr /* in */)
 ...
 *
 */
void LLF_PKI_PKA_SwapRegsPhisAddr(int8_t *r0, /* in */
                                  int8_t *r1, /* in */
                                  int8_t *r2, /* in */
                                  int8_t *r3, /* in */
                                  uint32_t VirtualHwBaseAddr /* in */)
...
/*
 * The function fixes HW ModInv result in case, that it is wrong (equal to modulus).
 *           Res = 1/A mod N.
 *  If HW ModInv returns result = modulus, then take PKA reg.30 and if it is
 *  negative, then add modulus till positive result recieved and output it as
 *  result rRes.
 *
 * @author reuvenl (7/14/2016)
 *
 * @param LenId  - ID of regsSizesTable entry, containing register
 *                     length with extension word.
 * @param rA - virt.pointer to PKA reg. containig A.
 * @param rN - virt.pointer to PKA reg. containig N.
 * @param rRes - virt.pointer to PKA reg. containig result.
 * @param tag - PKA tag (now not used, inserted for compatibility with
 *            LLF_PKI_PKA_ModInv API)
 * @param tag - PKA tag (now = 0, inserted for compatibility with
 *            LLF_PKI_PKA_ModInv API)
 * @param virtualHwBaseAddr - HW base address (now = 0, inserted for
 *            compatibility with LLF_PKI_PKA_ModInv API)
 *
 * @return CRYS_OK
 */
CRYSError_t LLF_PKI_PKA_ModInv(uint8_t LenID, int8_t rA, int8_t rRes, int8_t tag, uint32_t virtualHwBaseAddr)
{
    CRYSError_t Error = CRYS_OK;
    uint32_t status;
    int8_t rN;
    ...
    Error = LLF_PKI_PKA_ModInvHw(LenID, rA, rRes, 0 /* Tag */, 0 /* VirtualHwBaseAddr */);
    if (Error) {
        return Error;
    }

    /* check is inverse exists */
    LLF_PKI_PKA_CompareIm(LenID + 1, rA, 1, 0 /* Tag */, 0 /* VirtualHwBaseAddr */);
    LLF_PKI_PKA_GET_StatusAluOutZero(0 /* VirtualHwBaseAddr */, status);
    if (status == 0) {
        /* if inverse not exists, set result = 0 */
        LLF_PKI_PKA_AndIm(LenID + 1, rRes, 0 /* OpBim */, rRes, 0 /* Tag */, 0 /* VirtualHwBaseAddr */);
        return LLF_PKI_PKA_INVERSION_NOT_EXISTS_ERROR;
    }

    /* Get ID of modulus register rN */
    LLF_PKI_WAIT_ON_PKA_DONE(0 /* virtualHwBaseAddr */);
    status = DX_HAL_ReadCcRegister(DX_CC_REG_OFFSET(CRY_KERNEL, N_NP_T0_T1_ADDR));
    rN     = DX_CC_REG_FLD_GET(CRY_KERNEL /* unit_name */, N_NP_T0_T1_ADDR /* reg_name */, N_VIRTUAL_ADDR /* fld_name */,
                           status /* reg_val */);

    /* check exceed result Res == modulus */
    LLF_PKI_PKA_Compare(LenID + 1, rN, rRes, 0 /* Tag */, 0 /* VirtualHwBaseAddr */);
    LLF_PKI_PKA_GET_StatusAluOutZero(0 /* VirtualHwBaseAddr */, status);
    if (status == 1) {
        /* extract the inverse from PKA_T0 by adding modulus */
        LLF_PKI_PKA_Copy(LenID + 1, rRes, PKA_T0, 0 /* Tag */, 0 /* VirtualHwBaseAddr */);
        do {
            LLF_PKI_PKA_Add(LenID + 1, rRes, rN, rRes, 0 /* Tag */, 0 /* VirtualHwBaseAddr */);
            LLF_PKI_PKA_GET_StatusAluSignOut(0 /* VirtualHwBaseAddr */, status);
#ifdef HW_MOD_INV_CALC_FAILS_COUNT
            gModInvCountAddedModules++; /* for test */
#endif
        } while (status);
    }
    return Error;
