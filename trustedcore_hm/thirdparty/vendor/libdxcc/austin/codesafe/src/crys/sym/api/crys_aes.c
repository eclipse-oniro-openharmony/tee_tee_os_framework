 ...
 */
static CRYSError_t SymAdaptor2CrysAesErr(int symRetCode, uint32_t errorInfo)
{
    errorInfo = errorInfo;
    switch (symRetCode) {
 ...
 */
CIMPORT_C CRYSError_t CRYS_AES_Init(CRYS_AESUserContext_t *ContextID_ptr, CRYS_AES_IvCounter_t IVCounter_ptr,
                                    ...
                                    CRYS_AES_EncryptMode_t EncryptDecryptFlag, CRYS_AES_OperationMode_t OperationMode)
{
    ...
    DX_PAL_PERF_OPEN_NEW_ENTRY(perfIdx, PERF_TEST_TYPE_CRYS_AES_INIT);
    ...
    if (((OperationMode == CRYS_AES_CBC_mode) || (OperationMode == CRYS_AES_CTR_mode) ||
         (OperationMode == CRYS_AES_MAC_mode) || (OperationMode == CRYS_AES_XTS_mode) ||
         (OperationMode == CRYS_AES_CBC_CTS_mode) || (OperationMode == CRYS_AES_OFB_mode)) &&
        (IVCounter_ptr == DX_NULL)) {
        return CRYS_AES_INVALID_IV_OR_TWEAK_PTR_ERROR;
    }
    /* in XCBC mode enable only key size = 128 bit */
    if ((OperationMode == CRYS_AES_XCBC_MAC_mode) && (KeySizeID != CRYS_AES_Key128BitSize)) {
        return CRYS_AES_ILLEGAL_KEY_SIZE_ERROR;
    }
    ...
    if (DxCcAcl_IsBuffAccessOk(ACCESS_READ, keyAddr, keySizeBytes) ||
        DxCcAcl_IsBuffAccessOk(ACCESS_READ_WRITE, ContextID_ptr, sizeof(CRYS_AESUserContext_t)) ||
        ((IVCounter_ptr != NULL) &&
         DxCcAcl_IsBuffAccessOk(ACCESS_READ_WRITE, IVCounter_ptr, sizeof(CRYS_AES_IvCounter_t)))) {
        return CRYS_AES_ILLEGAL_PARAMS_ERROR;
    }

    /* Get pointer to contiguous context in the HOST buffer */
    pAesContext = (struct sep_ctx_cipher *)DX_InitUserCtxLocation(ContextID_ptr->buff, sizeof(CRYS_AESUserContext_t),
    ...
    if (pAesContext->mode == SEP_CIPHER_XTS) {
        ...
        pAesContext->key_size = keySizeBytes >> 1;
        DX_PAL_MemCopy(pAesContext->key, keyAddr, pAesContext->key_size);
        /* copy second half of the double-key as XEX-key */
        DX_PAL_MemCopy(pAesContext->xex_key, keyAddr + pAesContext->key_size, pAesContext->key_size);
    } else {
        /* key size 512bit is only in XTS mode */
        if (keySizeBytes == 64) {
            return CRYS_AES_ILLEGAL_KEY_SIZE_ERROR;
        }
        DX_PAL_MemCopy(pAesContext->key, keyAddr, keySizeBytes);
    ...
    DX_PAL_PERF_CLOSE_ENTRY(perfIdx, PERF_TEST_TYPE_CRYS_AES_INIT);
 ...
 */
CIMPORT_C CRYSError_t CRYS_AES_Block(CRYS_AESUserContext_t *ContextID_ptr, uint8_t *DataIn_ptr, uint32_t DataInSize,
                                     uint8_t *DataOut_ptr)
{
    int symRc;
    ...
    DX_PAL_PERF_OPEN_NEW_ENTRY(perfIdx, PERF_TEST_TYPE_CRYS_AES_BLOCK);
    /* if the users context ID pointer is DX_NULL return an error */
    if (ContextID_ptr == DX_NULL) {
        return CRYS_AES_INVALID_USER_CONTEXT_POINTER_ERROR;
    }
    ...
    pAesContext = (struct sep_ctx_cipher *)DX_GetUserCtxLocation(ContextID_ptr->buff, sizeof(CRYS_AESUserContext_t));
    if (pAesContext == DX_NULL)
        return CRYS_AES_ILLEGAL_PARAMS_ERROR;
    ...
    if ((pAesContext->mode == SEP_CIPHER_XTS) && (DataInSize < CRYS_AES_BLOCK_SIZE_IN_BYTES)) {
    ...
    if ((pAesContext->mode == SEP_CIPHER_XTS) && (pAesContext->data_unit_size != DataInSize)) {
        DX_PAL_LOG_ERR("Invalid XTS data size: DataInSize=%u data_unit_size=%u\n", (unsigned int)DataInSize,
                       (unsigned int)pAesContext->data_unit_size);
        return CRYS_AES_DATA_IN_SIZE_ILLEGAL;
    }

    if ((pAesContext->mode == SEP_CIPHER_CMAC) || (pAesContext->mode == SEP_CIPHER_XCBC_MAC) ||
    ...
    symRc = SymDriverAdaptorProcess((struct sep_ctx_generic *)pAesContext, DataIn_ptr, pOutData, DataInSize);

    DX_PAL_PERF_CLOSE_ENTRY(perfIdx, PERF_TEST_TYPE_CRYS_AES_BLOCK);
    return DX_CRYS_RETURN_ERROR(symRc, 0, SymAdaptor2CrysAesErr);
}
 ...
 *
 */
CIMPORT_C CRYSError_t CRYS_AES_Finish(CRYS_AESUserContext_t *ContextID_ptr, uint8_t *DataIn_ptr, uint32_t DataInSize,
                                      uint8_t *DataOut_ptr)
{
    /* pointer on SEP AES context struct */
    struct sep_ctx_cipher *pAesContext = NULL;
    ...
    DX_PAL_PERF_OPEN_NEW_ENTRY(perfIdx, PERF_TEST_TYPE_CRYS_AES_FIN);
    /* if the users context ID pointer is DX_NULL return an error */
    if (ContextID_ptr == DX_NULL) {
        return CRYS_AES_INVALID_USER_CONTEXT_POINTER_ERROR;
    }
    ...
    if ((DataInSize % CRYS_AES_BLOCK_SIZE_IN_BYTES != 0) &&
        ((pAesContext->mode == SEP_CIPHER_ECB) || (pAesContext->mode == SEP_CIPHER_CBC) ||
         (pAesContext->mode == SEP_CIPHER_CBC_MAC))) {
        return CRYS_AES_DATA_IN_SIZE_ILLEGAL;
    }
    /* Check, that in case of CTS mode data size is not less than CRYS_AES_BLOCK_SIZE_IN_BYTES */
    if ((DataInSize < CRYS_AES_BLOCK_SIZE_IN_BYTES) && (pAesContext->mode == SEP_CIPHER_CBC_CTS)) {
        return CRYS_AES_DATA_IN_SIZE_ILLEGAL;
    }
    ...
    if ((pAesContext->mode == SEP_CIPHER_XTS) && (DataInSize != 0)) {
        ...
        if ((DataInSize < CRYS_AES_BLOCK_SIZE_IN_BYTES) || (pAesContext->data_unit_size != DataInSize)) {
            DX_PAL_LOG_ERR("Invalid XTS data size: DataInSize=%u data_unit_size=%u\n", (unsigned int)DataInSize,
                           (unsigned int)pAesContext->data_unit_size);
            return CRYS_AES_DATA_IN_SIZE_ILLEGAL;
        }
    ...
    if ((pAesContext->mode == SEP_CIPHER_CBC_CTS) && ((DataInSize % CRYS_AES_BLOCK_SIZE_IN_BYTES) == 0)) {
        pAesContext->mode = SEP_CIPHER_CBC;
        symRc = SymDriverAdaptorFinalize((struct sep_ctx_generic *)pAesContext, DataIn_ptr, DataOut_ptr, DataInSize);
        pAesContext->mode = SEP_CIPHER_CBC_CTS;
    } else {
        symRc = SymDriverAdaptorFinalize((struct sep_ctx_generic *)pAesContext, DataIn_ptr, DataOut_ptr, DataInSize);
    }

    DX_PAL_PERF_CLOSE_ENTRY(perfIdx, PERF_TEST_TYPE_CRYS_AES_FIN);
    if (symRc != DX_RET_OK) {
        return DX_CRYS_RETURN_ERROR(symRc, 0, SymAdaptor2CrysAesErr);
    }
...
CRYSError_t crys_aes_is_mac_mode(CRYS_AESUserContext_t *context, DxBool_t *mac_mode)
{
    if (context == NULL || mac_mode == NULL)
        return CRYS_AES_INVALID_USER_CONTEXT_POINTER_ERROR;

    struct sep_ctx_cipher *aes_ctx =
        (struct sep_ctx_cipher *)DX_GetUserCtxLocation(context->buff, sizeof(CRYS_AESUserContext_t));
    if (aes_ctx == NULL)
        return CRYS_AES_INVALID_USER_CONTEXT_POINTER_ERROR;

    if ((aes_ctx->mode == SEP_CIPHER_CBC_MAC) || (aes_ctx->mode == SEP_CIPHER_XCBC_MAC) ||
        (aes_ctx->mode == SEP_CIPHER_CMAC))
        *mac_mode = DX_TRUE;
    else
        *mac_mode = DX_FALSE;
    return CRYS_OK;
 ...
 *
 */
CIMPORT_C CRYSError_t CRYS_AES(CRYS_AES_IvCounter_t IVCounter_ptr, CRYS_AES_Key_t Key_ptr, CRYS_AES_KeySize_t KeySize,
                               CRYS_AES_EncryptMode_t EncryptDecryptFlag, CRYS_AES_OperationMode_t OperationMode,
                               uint8_t *DataIn_ptr, uint32_t DataInSize, uint8_t *DataOut_ptr)
{
    CRYS_AESUserContext_t UserContext;
    CRYSError_t Error = CRYS_OK;
    uint8_t *keyAddr  = Key_ptr;
    enum sep_crypto_key_type cryptoKeyType;
    KeyPtrType_t keyPtrType;
    ...
    if (((DataInSize % CRYS_AES_BLOCK_SIZE_IN_BYTES) != 0) &&
        ((OperationMode == CRYS_AES_ECB_mode) || (OperationMode == CRYS_AES_CBC_mode) ||
         (OperationMode == CRYS_AES_MAC_mode))) {
        return CRYS_AES_DATA_IN_SIZE_ILLEGAL;
    }
    ...
    if (OperationMode == CRYS_AES_CMAC_mode) {
        ...
        Error = buildKeyInt((struct sep_ctx_cipher *)UserContext.buff, &KeySize, keyBuff, &keyAddr, DX_AES_CMAC,
                            cryptoKeyType, &keySizeBytes);
        if (Error != CRYS_OK) {
            return CRYS_AES_INVALID_KEY_POINTER_ERROR;
        }
    }
    Error = CRYS_AES_Init(&UserContext, IVCounter_ptr, keyAddr, KeySize, EncryptDecryptFlag, OperationMode);
...
*/
CIMPORT_C CRYSError_t CRYS_AES_SetIv(CRYS_AESUserContext_t *ContextID_ptr, uint8_t *iv_ptr)
{
    struct sep_ctx_cipher *pAesContext;
    ...
    if ((pAesContext->mode != SEP_CIPHER_CBC) && (pAesContext->mode != SEP_CIPHER_CTR) &&
        (pAesContext->mode != SEP_CIPHER_XTS) && (pAesContext->mode != SEP_CIPHER_CBC_CTS) &&
    ...
    if (iv_ptr == DX_NULL ||
        ...
        (IS_SMART_PTR(iv_ptr) && (PTR_TO_DMA_BUFFER((void *)iv_ptr)->dmaBufType != DMA_BUF_SEP))) {
        return CRYS_AES_INVALID_IV_OR_TWEAK_PTR_ERROR;
    }

    if (IS_SMART_PTR(iv_ptr)) /* Translate to plain pointer */
        iv_ptr = (uint8_t *)(DxVirtAddr_t)PTR_TO_DMA_BUFFER((void *)iv_ptr)->pData;
...
*/
CIMPORT_C CRYSError_t CRYS_AES_GetIv(CRYS_AESUserContext_t *ContextID_ptr, uint8_t *iv_ptr)
{
    struct sep_ctx_cipher *pAesContext;
