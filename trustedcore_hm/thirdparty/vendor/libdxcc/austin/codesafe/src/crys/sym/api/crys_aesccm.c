 ...
 */
static CRYSError_t SymAdaptor2CrysAesCcmErr(int symRetCode, uint32_t errorInfo)
{
    errorInfo = errorInfo;
    switch (symRetCode) {
 ...
 *
 */
CRYSError_t CRYS_AESCCM_Init(CRYS_AESCCM_UserContext_t *ContextID_ptr, CRYS_AES_EncryptMode_t EncrDecrMode,
                             CRYS_AESCCM_Key_t CCM_Key, CRYS_AESCCM_KeySize_t KeySizeId, uint32_t AdataSize,
                             uint32_t TextSize, uint8_t *N_ptr, uint8_t SizeOfN, uint8_t SizeOfT)
{
    ...
    if (DxCcAcl_IsBuffAccessOk(ACCESS_READ_WRITE, ContextID_ptr, sizeof(CRYS_AESCCM_UserContext_t)) ||
        DxCcAcl_IsBuffAccessOk(ACCESS_READ, CCM_Key, keySizeInBytes) ||
        DxCcAcl_IsBuffAccessOk(ACCESS_READ, N_ptr, SizeOfN)) {
        return CRYS_AESCCM_ILLEGAL_PARAMS_ERROR;
    }

    /* Get pointer to contiguous context in the HOST buffer */
    pAeadContext = (struct sep_ctx_aead *)DX_InitUserCtxLocation(ContextID_ptr->buff, sizeof(CRYS_AESCCM_UserContext_t),
                                                                 sizeof(struct sep_ctx_aead));
    if (pAeadContext == DX_NULL) {
        return CRYS_AESCCM_INVALID_USER_CONTEXT_POINTER_ERROR;
    }
...
CRYSError_t CRYS_AESCCM_BlockAdata(CRYS_AESCCM_UserContext_t *ContextID_ptr, uint8_t *DataIn_ptr, uint32_t DataInSize)
{
    ...
    /* Get pointer to contiguous context in the HOST buffer */
    pAeadContext = (struct sep_ctx_aead *)DX_GetUserCtxLocation(ContextID_ptr->buff, sizeof(CRYS_AESCCM_UserContext_t));
    ...
    uint8_t pA0Block[CRYS_AES_BLOCK_SIZE_IN_BYTES] = { 0 };
    ...
    symRc = CRYS_Bypass(DataIn_ptr, headerA0BorrowLen, pA0Block + headerA0MetaDataLen);
    if (symRc != DX_RET_OK) {
        return DX_CRYS_RETURN_ERROR(symRc, 0, SymAdaptor2CrysAesCcmErr);
    }
    ...
    if (actualHeaderLen <= CRYS_AES_BLOCK_SIZE_IN_BYTES) {
        ...
        /* process A0 block */
        symRc = SymDriverAdaptorProcess((struct sep_ctx_generic *)pAeadContext, pA0Block, NULL,
        ...
        symRc = SymDriverAdaptorProcess((struct sep_ctx_generic *)pAeadContext, DataIn_ptr, NULL,
 ...
 */
CRYSError_t CRYS_AESCCM_BlockTextData(CRYS_AESCCM_UserContext_t *ContextID_ptr, uint8_t *DataIn_ptr,
 ...
 *         DataIn_ptr = DataOut_ptr.
 */
CEXPORT_C CRYSError_t CRYS_AESCCM_Finish(CRYS_AESCCM_UserContext_t *ContextID_ptr, uint8_t *DataIn_ptr,
                                         uint32_t DataInSize, uint8_t *DataOut_ptr, CRYS_AESCCM_Mac_Res_t MacRes,
                                         uint8_t *SizeOfT)
{
    struct sep_ctx_aead *pAeadContext;
    int symRc = DX_RET_OK;
 ...
 *
 */
CIMPORT_C CRYSError_t CRYS_AESCCM(CRYS_AES_EncryptMode_t EncrDecrMode, CRYS_AESCCM_Key_t CCM_Key,
                                  CRYS_AESCCM_KeySize_t KeySizeId, uint8_t *N_ptr, uint8_t SizeOfN,
                                  ...
                                  CRYS_AESCCM_Mac_Res_t MacRes)
{
    ...
    crysRc = CRYS_AESCCM_Init(&ContextID, EncrDecrMode, CCM_Key, KeySizeId, ADataInSize, TextDataInSize, N_ptr, SizeOfN,
                              SizeOfT);
    if (crysRc != CRYS_OK) {
        return crysRc;
    }
