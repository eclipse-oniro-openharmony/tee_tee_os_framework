...
typedef struct CRYS_COMBINED_UserContext_t {
    uint32_t buff[CRYS_COMBINED_USER_CTX_SIZE_IN_WORDS];
 ...
 */
static CRYSError_t SymAdaptor2CrysCombinedErr(int symRetCode, uint32_t errorInfo)
{
    switch (symRetCode) {
...
static CRYSError_t ValidateSupportedModes(struct sep_ctx_combined *pCombinedCtx)
{
    ...
    for (engineIdx = 0; (engineIdx < CRYS_COMBINED_MAX_NODES) && (pCombinedCtx->sub_ctx[engineIdx] != NULL);
         engineIdx++) {
        ...
        case SEP_CRYPTO_ALG_AES: {
            ...
            if ((pAesCtx->mode != SEP_CIPHER_ECB) && (pAesCtx->mode != SEP_CIPHER_CBC) &&
        ...
        case SEP_CRYPTO_ALG_HASH: {
            ...
            if ((pHashCtx->mode != SEP_HASH_SHA1) && (pHashCtx->mode != SEP_HASH_SHA224) &&
                (pHashCtx->mode != SEP_HASH_SHA256)
#ifdef DX_CONFIG_HASH_MD5_SUPPORTED
                && (pHashCtx->mode != SEP_HASH_MD5)
#endif
#ifdef DX_CONFIG_HASH_SHA_512_SUPPORTED
                && (pHashCtx->mode != SEP_HASH_SHA384) && (pHashCtx->mode != SEP_HASH_SHA512)
#endif
            ) {
                return CRYS_COMBINED_UNSUPPORTED_SUB_OPERATION_MODE_ERROR;
            }
...
static void InitCombinedContext(struct sep_ctx_combined *pCtx, CrysCombinedConfig_t *pConfig)
{
    ...
    for (engineIdx = 0; (engineIdx < CRYS_COMBINED_MAX_NODES) && (!done); engineIdx++) {
        if (pConfig->node[engineIdx].pContext) {
            ...
            pCtx->sub_ctx[engineIdx] = (struct sep_ctx_cache_entry *)DX_GetUserCtxLocation(
                pConfig->node[engineIdx].pContext, sizeof(CrysCombinedNodeConfig_t));
        } else {
 ...
 *             and on failure - a value from crys_combined_error.h
 */
CIMPORT_C CRYSError_t CRYS_Combined_Process(CrysCombinedConfig_t *pConfig, uint32_t cipherOffset, uint8_t *pDataIn,
                                            uint32_t dataInSize, uint8_t *pDataOut)
{
    ...
    pcombinedCtx = (struct sep_ctx_combined *)DX_InitUserCtxLocation(
        combinedUsrCtx.buff, sizeof(CRYS_COMBINED_UserContext_t), sizeof(struct sep_ctx_combined));
    if (pcombinedCtx == DX_NULL) {
        return CRYS_COMBINED_ILLEGAL_PARAMS_ERROR;
    }
    ...
    symRc = SymDriverAdaptorProcess((struct sep_ctx_generic *)pcombinedCtx, pDataIn, pDataOut, dataInSize);
 ...
 *             and on failure - a value from crys_combined_error.h
 */
CIMPORT_C CRYSError_t CRYS_Combined_Finish(CrysCombinedConfig_t *pConfig, uint32_t cipherOffset, uint8_t *pDataIn,
                                           uint32_t dataInSize, uint8_t *pDataOut, uint8_t *pAuthDataOut,
                                           uint32_t *pAuthDataOutSize)
{
    ...
    /* check validity for priv */
    if (DxCcAcl_IsBuffAccessOk(ACCESS_READ, pConfig, sizeof(CrysCombinedConfig_t)) ||
        ((pAuthDataOutSize != NULL) && DxCcAcl_IsBuffAccessOk(ACCESS_READ_WRITE, pAuthDataOutSize, sizeof(uint32_t))) ||
        ((pAuthDataOut != NULL) && DxCcAcl_IsBuffAccessOk(ACCESS_READ_WRITE, pAuthDataOut, *pAuthDataOutSize))) {
        return CRYS_COMBINED_ILLEGAL_PARAMS_ERROR;
    }
    ...
    if ((pcombinedCtx->mode != SEP_COMBINED_DIN_TO_AES_TO_AES_TO_DOUT_MODE) && (pAuthDataOutSize == DX_NULL)) {
    ...
    symRc = SymDriverAdaptorFinalize((struct sep_ctx_generic *)pcombinedCtx, pDataIn, pDataOut, dataInSize);
    if (symRc != DX_RET_OK) {
        return DX_CRYS_RETURN_ERROR(symRc, 0, SymAdaptor2CrysCombinedErr);
    }
 ...
 *             and on failure - a value from crys_combined_error.h
 */
CIMPORT_C CRYSError_t CRYS_Combined(CrysCombinedConfig_t *pConfig, uint32_t cipherOffset, uint8_t *pDataIn,
                                    uint32_t dataInSize, uint8_t *pDataOut, uint8_t *pAuthDataOut,
                                    uint32_t *pAuthDataOutSize)
{
    CRYSError_t crysErr = CRYS_OK;
    ...
    return CRYS_Combined_Finish(pConfig, cipherOffset, pDataIn, dataInSize, pDataOut, pAuthDataOut, pAuthDataOutSize);
}
