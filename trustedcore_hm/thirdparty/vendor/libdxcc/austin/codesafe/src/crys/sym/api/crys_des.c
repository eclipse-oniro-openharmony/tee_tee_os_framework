 ...
 */
static CRYSError_t SymAdaptor2CrysDesErr(int symRetCode, uint32_t errorInfo)
{
    errorInfo = errorInfo;
    switch (symRetCode) {
...
CIMPORT_C CRYSError_t CRYS_DES_Init(CRYS_DESUserContext_t *ContextID_ptr, CRYS_DES_Iv_t IV_ptr, CRYS_DES_Key_t *Key_ptr,
                                    CRYS_DES_NumOfKeys_t NumOfKeys, CRYS_DES_EncryptMode_t EncryptDecryptFlag,
                                    CRYS_DES_OperationMode_t OperationMode)
{
    ...
    pDesContext = (struct sep_ctx_cipher *)DX_InitUserCtxLocation(ContextID_ptr->buff, sizeof(CRYS_DESUserContext_t),
                                                                  sizeof(struct sep_ctx_cipher));
    if (pDesContext == DX_NULL) {
        return CRYS_DES_INVALID_USER_CONTEXT_POINTER_ERROR;
    }
 ...
 */
CIMPORT_C CRYSError_t CRYS_DES_Block(CRYS_DESUserContext_t *ContextID_ptr, uint8_t *DataIn_ptr, uint32_t DataInSize,
                                     uint8_t *DataOut_ptr)
{
    int symRc = DX_RET_OK;
    ...
    /* Get pointer to contiguous context in the HOST buffer */
    pDesContext = (struct sep_ctx_cipher *)DX_GetUserCtxLocation(ContextID_ptr->buff, sizeof(CRYS_DESUserContext_t));
    if (pDesContext == NULL)
        return CRYS_DES_ILLEGAL_PARAMS_ERROR;

    symRc = SymDriverAdaptorProcess((struct sep_ctx_generic *)pDesContext, DataIn_ptr, DataOut_ptr, DataInSize);
    return DX_CRYS_RETURN_ERROR(symRc, 0, SymAdaptor2CrysDesErr);
}
 ...
 *
 */
CIMPORT_C CRYSError_t CRYS_DES(CRYS_DES_Iv_t IV_ptr, CRYS_DES_Key_t *Key_ptr, CRYS_DES_NumOfKeys_t NumOfKeys,
                               CRYS_DES_EncryptMode_t EncryptDecryptFlag, CRYS_DES_OperationMode_t OperationMode,
