 ...
 */
static CRYSError_t SymAdaptor2CrysHashErr(int symRetCode, uint32_t errorInfo)
{
    errorInfo = errorInfo;
    switch (symRetCode) {
 ...
 *
 */
CEXPORT_C CRYSError_t CRYS_HASH_Init(CRYS_HASHUserContext_t *ContextID_ptr, CRYS_HASH_OperationMode_t OperationMode)
{
    ...
    DX_PAL_PERF_OPEN_NEW_ENTRY(perfIdx, PERF_TEST_TYPE_CRYS_HASH_INIT);
    ...
    pHashContext = (struct sep_ctx_hash *)DX_InitUserCtxLocation(ContextID_ptr->buff, sizeof(CRYS_HASHUserContext_t),
                                                                 sizeof(struct sep_ctx_hash));
    if (pHashContext == DX_NULL) {
        return CRYS_HASH_INVALID_USER_CONTEXT_POINTER_ERROR;
    }

    pHashPrivContext =
        (CRYS_HASHPrivateContext_t *)&(((uint32_t *)pHashContext)[CRYS_HASH_USER_CTX_ACTUAL_SIZE_IN_WORDS - 1]);
    ...
    DX_PAL_PERF_CLOSE_ENTRY(perfIdx, PERF_TEST_TYPE_CRYS_HASH_INIT);
 ...
 *
 */
CEXPORT_C CRYSError_t CRYS_HASH_Update(CRYS_HASHUserContext_t *ContextID_ptr, uint8_t *DataIn_ptr, uint32_t DataInSize)
{
    struct sep_ctx_hash *pHashContext;
    CRYS_HASHPrivateContext_t *pHashPrivContext;
    int symRc                    = DX_RET_OK;
    ...
    DX_PAL_PERF_OPEN_NEW_ENTRY(perfIdx, PERF_TEST_TYPE_CRYS_HASH_UPDATE);
    ...
    /* Get pointer to contiguous context in the HOST buffer */
    pHashContext = (struct sep_ctx_hash *)DX_GetUserCtxLocation(ContextID_ptr->buff, sizeof(CRYS_HASHUserContext_t));
    ...
    if ((DataInSize % hash_block_size_in_bytes) == 0) {
        symRc = SymDriverAdaptorProcess((struct sep_ctx_generic *)pHashContext, DataIn_ptr, NULL, DataInSize);
        ...
        symRc = SymDriverAdaptorFinalize((struct sep_ctx_generic *)pHashContext, DataIn_ptr, NULL, DataInSize);
    ...
    DX_PAL_PERF_CLOSE_ENTRY(perfIdx, PERF_TEST_TYPE_CRYS_HASH_UPDATE);
    return CRYS_OK;
}
...
CEXPORT_C CRYSError_t CRYS_HASH_Finish(CRYS_HASHUserContext_t *ContextID_ptr, CRYS_HASH_Result_t HashResultBuff)
{
    struct sep_ctx_hash *pHashContext;
    CRYS_HASHPrivateContext_t *pHashPrivContext;
    int symRc                 = DX_RET_OK;
    ...
    DX_PAL_PERF_OPEN_NEW_ENTRY(perfIdx, PERF_TEST_TYPE_CRYS_HASH_FIN);
    ...
    DX_PAL_MemCopy(HashResultBuff, pHashContext->digest, CRYS_HASH_RESULT_SIZE_IN_WORDS * sizeof(uint32_t));

    DX_PAL_PERF_CLOSE_ENTRY(perfIdx, PERF_TEST_TYPE_CRYS_HASH_FIN);
    return CRYS_OK;
}
 ...
 *
 */
CEXPORT_C CRYSError_t CRYS_HASH(CRYS_HASH_OperationMode_t OperationMode, uint8_t *DataIn_ptr, uint32_t DataSize,
