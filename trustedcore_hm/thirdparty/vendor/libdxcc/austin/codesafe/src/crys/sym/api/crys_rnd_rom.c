 ...
 */
CEXPORT_C CRYSError_t CRYS_RND_GenerateVector(uint16_t RndSize, uint8_t *Output_ptr)
{
    ...
    RETURN_IF_RND_UNSUPPORTED(RndSize, Output_ptr, Error, Error, Error, Error, Error, Error, Error, Error, Error, Error,
                              Error, Error, Error);
...
CEXPORT_C CRYSError_t CRYS_RND_GenerateVectorInRange(uint32_t rndSizeInBits, uint8_t *maxVect_ptr, uint8_t *rndVect_ptr)
{
    ...
    roundedSize = CRYS_AES_BLOCK_SIZE_IN_BYTES *
                  ((rndSizeInBytes + CRYS_AES_BLOCK_SIZE_IN_BYTES - 1) / CRYS_AES_BLOCK_SIZE_IN_BYTES);
    ...
    while (k < 0xFFFF) {
        ...
        if (maxVect_ptr != DX_NULL) {
            ...
            if (checkingSizeBytes == CRYS_AES_BLOCK_SIZE_IN_BYTES) {
                while (1) {
                    CompRes = CRYS_COMMON_CmpLsbUnsignedCounters(
                        rndVect_ptr + rndSizeInBytes - CRYS_AES_BLOCK_SIZE_IN_BYTES, CRYS_AES_BLOCK_SIZE_IN_BYTES,
                        maxVect_ptr + rndSizeInBytes - CRYS_AES_BLOCK_SIZE_IN_BYTES, CRYS_AES_BLOCK_SIZE_IN_BYTES);
                    ...
                    Error = DX_RNG_GetRandomVector(CRYS_AES_BLOCK_SIZE_IN_BYTES,
            ...
            CompRes = CRYS_COMMON_CmpLsbUnsignedCounters(rndVect_ptr, (uint16_t)rndSizeInBytes, maxVect_ptr,
