...
DX_PAL_COMPILER_ASSERT(sizeof(enum sep_cipher_mode) == sizeof(uint32_t), "sep_cipher_mode is not 32bit!");
DX_PAL_COMPILER_ASSERT(sizeof(DataBlockType_t) == sizeof(uint32_t), "DataBlockType_t is not 32bit!");
DX_PAL_COMPILER_ASSERT(sizeof(SepAesCoreEngine_t) == sizeof(uint32_t), "SepAesCoreEngine_t is not 32bit!");
...
{
    const DxSramAddr_t modeAddr          = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_cipher, mode);
    const DxSramAddr_t algAddr           = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_cipher, alg);
    const DxSramAddr_t keySizeAddr       = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_cipher, key_size);
    const DxSramAddr_t directionAddr     = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_cipher, direction);
    const DxSramAddr_t blockStateAddr    = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_cipher, block_state);
    const DxSramAddr_t aesPrivateCtxAddr = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_cipher, reserved);
    ...
    if (ReadContextWord(algAddr) == SEP_CRYPTO_ALG_AES) {
...
{
    HwDesc_s desc;
    const DxSramAddr_t keyAddr           = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_cipher, key);
    const DxSramAddr_t modeAddr          = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_cipher, mode);
    const DxSramAddr_t algAddr           = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_cipher, alg);
    const DxSramAddr_t directionAddr     = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_cipher, direction);
    const DxSramAddr_t keySizeAddr       = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_cipher, key_size);
    const DxSramAddr_t xexKeyAddr        = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_cipher, xex_key);
    const DxSramAddr_t dataUnitSizeAddr  = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_cipher, data_unit_size);
    const DxSramAddr_t cryptoKeyTypeAddr = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_cipher, crypto_key_type);
    const DxSramAddr_t aesPrivateCtxAddr = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_cipher, reserved);
    const DxSramAddr_t isTunnelOpAddr    = GET_CTX_FIELD_ADDR(aesPrivateCtxAddr, SepCipherPrivateContext_s, isTunnelOp);
    const DxSramAddr_t tunnetDirAddr     = GET_CTX_FIELD_ADDR(aesPrivateCtxAddr, SepCipherPrivateContext_s, tunnetDir);
    const DxSramAddr_t engineCoreAddr    = GET_CTX_FIELD_ADDR(aesPrivateCtxAddr, SepCipherPrivateContext_s, engineCore);
    ...
    HW_DESC_SET_CIPHER_MODE(&desc, ReadContextWord(modeAddr));
    if (ReadContextWord(algAddr) == SEP_CRYPTO_ALG_AES) {
        HW_DESC_SET_CIPHER_CONFIG0(&desc,
                                   ReadContextWord(isTunnelOpAddr) ? ReadContextWord(tunnetDirAddr) : encDecFlag);
        HW_DESC_SET_CIPHER_CONFIG1(&desc, ReadContextWord(isTunnelOpAddr));
        switch (ReadContextWord(modeAddr)) {
        case SEP_CIPHER_XCBC_MAC:
            ...
            break;
        default:
            switch (ReadContextWord(cryptoKeyTypeAddr)) {
            case SEP_ROOT_KEY:
                HW_DESC_SET_CIPHER_DO(&desc, ROOT_KEY);
                break;
            case SEP_SESSION_KEY:
...
static int RevertLastMacBlock(int qid, DxSramAddr_t ctxAddr)
{
    ...
    if (ReadContextWord(cryptoKeyTypeAddr) == SEP_ROOT_KEY) {
    ...
    if (ReadContextWord(modeAddr) == SEP_CIPHER_XCBC_MAC) { /* XCBC K1 key is used (always 128b) */
        ...
        HW_DESC_SET_STATE_DIN_PARAM(
            &desc, keyAddr, (ReadContextWord(keySizeAddr) == 24) ? SEP_AES_KEY_SIZE_MAX : ReadContextWord(keySizeAddr));
        HW_DESC_SET_KEY_SIZE_AES(&desc, ReadContextWord(keySizeAddr));
    }
    HW_DESC_SET_FLOW_MODE(&desc, S_DIN_to_AES);
    HW_DESC_SET_SETUP_MODE(&desc, SETUP_LOAD_KEY0);
...
int InitCipher(DxSramAddr_t ctxAddr)
{
    ...
    if (ReadContextWord(algAddr) == SEP_CRYPTO_ALG_DES) {
        /* in caes of double DES k1 = K3, copy k1-> K3 */
        if (ReadContextWord(keySizeAddr) == SEP_DES_DOUBLE_KEY_SIZE) {
            ...
            ReadContextField(keyAddr, tKeybuff, SEP_DES_ONE_KEY_SIZE);
            WriteContextField((keyAddr + SEP_DES_DOUBLE_KEY_SIZE), tKeybuff, SEP_DES_ONE_KEY_SIZE);
            WriteContextWord(keySizeAddr, SEP_DES_TRIPLE_KEY_SIZE);
#else
            DX_PAL_MemCopy((keyAddr + SEP_DES_DOUBLE_KEY_SIZE), keyAddr, SEP_DES_ONE_KEY_SIZE);
            keySizeAddr = SEP_DES_TRIPLE_KEY_SIZE;
#endif
        }
        return DX_RET_OK;
    }
...
int ProcessCipher(DxSramAddr_t ctxAddr, DmaBuffer_s *pDmaInputBuffer, DmaBuffer_s *pDmaOutputBuffer)
{
    ...
    case NO_DMA:
        ...
        break;
    default:
        DX_PAL_LOG_ERR("Invalid DMA Input mode\n");
        drvRc = DX_RET_INVARG;
        goto EndWithErr;
    }
    ...
    if (isInplaceOp) {
        ...
        case NO_DMA:
            ...
            break;
        default:
            DX_PAL_LOG_ERR("Invalid DMA Output mode\n");
            drvRc = DX_RET_INVARG;
            goto EndWithErr;
        }
    }
    ...
    flowMode = (ReadContextWord(algAddr) == SEP_CRYPTO_ALG_AES) ? DIN_AES_DOUT : DIN_DES_DOUT;
