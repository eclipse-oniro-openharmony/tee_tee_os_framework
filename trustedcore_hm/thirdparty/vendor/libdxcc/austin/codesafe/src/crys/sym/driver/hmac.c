...
DX_PAL_COMPILER_ASSERT(sizeof(ZeroBlock) >= (SEP_HASH_LENGTH_WORDS * sizeof(uint32_t)),
                          ...
                          DxSramAddr_t ctxAddr)
{
    ...
    switch (hmode) {
    case SEP_HASH_SHA1:
        digestAddr = (DxSramAddr_t)((DxVirtAddr_t)gLarvalSha1Digest);
        break;
    case SEP_HASH_SHA224:
        digestAddr = (DxSramAddr_t)((DxVirtAddr_t)gLarvalSha224Digest);
        break;
    case SEP_HASH_SHA256:
        digestAddr = (DxSramAddr_t)((DxVirtAddr_t)gLarvalSha256Digest);
        break;
    case SEP_HASH_MD5:
        digestAddr = (DxSramAddr_t)((DxVirtAddr_t)gLarvalMd5Digest);
        break;
#ifdef DX_CONFIG_HASH_SHA_512_SUPPORTED
    case SEP_HASH_SHA384:
        digestAddr = (DxSramAddr_t)((DxVirtAddr_t)gLarvalSha384Digest);
        break;
    case SEP_HASH_SHA512:
        digestAddr = (DxSramAddr_t)((DxVirtAddr_t)gLarvalSha512Digest);
        break;
#endif
    default:
        DX_PAL_LOG_ERR("Unsupported hash mode %d\n", hmode);
...
{
    const DxSramAddr_t modeAddr           = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_hmac, mode);
    const DxSramAddr_t digestAddr         = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_hmac, digest);
    const DxSramAddr_t k0Addr             = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_hmac, k0);
    const DxSramAddr_t k0SizeAddr         = GET_CTX_FIELD_ADDR(ctxAddr, struct sep_ctx_hmac, k0_size);
