...
int createDetailedDump(HwDesc_s *pDesc)
{
    ...
    PRINTF("AXI ID = %u  ",
           (int)DX_CC_REG_FLD_GET(CRY_KERNEL, DSCRPTR_QUEUE0_WORD3, DOUT_VIRTUAL_HOST, (pDesc)->word[3]));
    PRINTF("DOUT Last = %u  ",
           (int)DX_CC_REG_FLD_GET(CRY_KERNEL, DSCRPTR_QUEUE0_WORD3, DOUT_LAST_IND, (pDesc)->word[3]));
#if (pe == CC_DEV_ID)
...
#if (pe == CC_DEV_ID)
    PRINTF("SEL NHASH = %u  ",
           (int)DX_CC_REG_FLD_GET(CRY_KERNEL, DSCRPTR_QUEUE0_WORD4, AES_SEL_N_HASH, (pDesc)->word[4]));
#endif
    PRINTF("XOR KEY = %u  ",
           (int)DX_CC_REG_FLD_GET(CRY_KERNEL, DSCRPTR_QUEUE0_WORD4, AES_XOR_CRYPTO_KEY, (pDesc)->word[4]));
    PRINTF("CMAC S0 = %u\n", (int)DX_CC_REG_FLD_GET(CRY_KERNEL, DSCRPTR_QUEUE0_WORD4, CMAC_SIZE0, (pDesc)->word[4]));
#if (pe == CC_DEV_ID)
    PRINTF("Endian/Swap: ");
    PRINTF("DIN SRAM = %u  ",
           (int)DX_CC_REG_FLD_GET(CRY_KERNEL, DSCRPTR_QUEUE0_WORD4, DIN_SRAM_ENDIANNESS, (pDesc)->word[4]));
    PRINTF("DOUT SRAM = %u  ",
           (int)DX_CC_REG_FLD_GET(CRY_KERNEL, DSCRPTR_QUEUE0_WORD4, DOUT_SRAM_ENDIANNESS, (pDesc)->word[4]));
