...
DX_PAL_COMPILER_ASSERT(sizeof(CRYS_CtxBufProps_t) == DX_CTX_BUFF_PROPS_SIZE_BYTES,
                       "sizeof(CRYS_CtxBufProps_t) should be equal to DX_CTX_BUFF_PROPS_SIZE_BYTES!");
...
void *DX_InitUserCtxLocation(void *bufferStart, unsigned long bufferSize, unsigned long contextSize)
{
    ...
    const unsigned long requested_buf_size = sizeof(CRYS_CtxBufProps_t) + 2 * contextSize;
    ...
    contextOffset =
        GetNonCrossingOffset((unsigned long)bufferStart + sizeof(CRYS_CtxBufProps_t), bufferSize, contextSize);
...
void *DX_GetUserCtxLocation(void *bufferStart, unsigned long bufferSize)
{
    ...
    if (bufProps->bufSize != bufferSize || !((bufProps->ctxSize + bufProps->ctxOffset) > bufProps->ctxSize &&
                                             (bufProps->ctxSize + bufProps->ctxOffset) <= bufferSize))
        return NULL;
    ...
    newContextOffset = GetNonCrossingOffset((unsigned long)bufferStart + sizeof(CRYS_CtxBufProps_t), bufProps->bufSize,
                                            bufProps->ctxSize);
