 ...
 */
static void DescBypass(DmaMode_t inType, DxDmaAddr_t inAddr, uint32_t inSize, uint32_t inAxiNs, DmaMode_t outType,
                       DxDmaAddr_t outAddr, uint32_t outSize, uint32_t outAxiNs)
{
    HwDesc_s desc;
 ...
 */
static Bypass_t GetBypassType(DmaBufType_t dmaType, DxDmaAddr_t addr)
{
    switch (dmaType) {
    case DMA_BUF_DLLI:
        return BYPASS_DLLI;
    ...
    case BYPASS_SRAM:
        ...
        case BYPASS_DLLI:
            if (IS_ALIGNED(pDmaInputBuffer->pData, sizeof(uint32_t)) ||
                ...
                DX_PAL_LOG_ERR("Bad address or bad size. SRAM to DLLI copy -Input address %llxl with %ul B\n",
