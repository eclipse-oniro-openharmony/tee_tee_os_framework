...
#define CRYS_RSA_TMP_BUFF_SIZE                                                       \
    (CRYS_RSA_OAEP_ENCODE_MAX_MASKDB_SIZE + CRYS_RSA_OAEP_ENCODE_MAX_SEEDMASK_SIZE + \
     CRYS_PKA_RSA_HASH_CTX_SIZE_IN_WORDS * sizeof(uint32_t) + sizeof(CRYS_HASH_Result_t))
...
#define CRYS_RSA_MGF_2_POWER_32 \
    65535 /* 0xFFFF */ /* 0x100000000 */ /* This is the 2^32 of the 2^32*hLen boundary check */
#define CRYS_RSA_SIZE_OF_T_STRING_BYTES \
    (CRYS_RSA_MAXIMUM_MOD_BUFFER_SIZE_IN_WORDS * sizeof(uint32_t)) /* Buffer inside RSA context */

/* **********************************************************
 *
 * RSA PKCS#1 v2.1 DEFS
 *
 * ******************************************************** */
...
#define CRYS_RSA_OAEP_ENCODE_MAX_MASKDB_SIZE \
    (CRYS_RSA_MAXIMUM_MOD_BUFFER_SIZE_IN_WORDS * sizeof(uint32_t)) /* For OAEP Encode; the max size is emLen */
#define CRYS_RSA_OAEP_DECODE_MAX_DBMASK_SIZE \
    (CRYS_RSA_MAXIMUM_MOD_BUFFER_SIZE_IN_WORDS * sizeof(uint32_t)) /* For OAEP Decode; the max size is emLen */
...
typedef enum {
    ...
    CRYS_RSA_After_HASH_NOT_KNOWN_mode =
        12, /* used only for PKCS#1 Ver 1.5 - possible to perform verify operation without hash mode input ; the hash
               mode will be derived from the signature */
