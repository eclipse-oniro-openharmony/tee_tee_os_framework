...
#define DX_CC_REG_OFFSET(unit_name, reg_name) (DX_BASE_##unit_name + DX_##reg_name##_REG_OFFSET)
 ...
 *  as the gpr_idx) */
#define _SEP_HOST_GPR_REG_OFFSET(gpr_idx) DX_CC_REG_OFFSET(HOST_RGF, HOST_SEP_HOST_GPR##gpr_idx)
#define SEP_HOST_GPR_REG_OFFSET(gpr_idx)  _SEP_HOST_GPR_REG_OFFSET(gpr_idx)
#define _HOST_SEP_GPR_REG_OFFSET(gpr_idx) DX_CC_REG_OFFSET(HOST_RGF, HOST_HOST_SEP_GPR##gpr_idx)
#define HOST_SEP_GPR_REG_OFFSET(gpr_idx)  _HOST_SEP_GPR_REG_OFFSET(gpr_idx)

/* GPR IRQ bit shift/mask by GPR index */
#define _SEP_HOST_GPR_IRQ_SHIFT(gpr_idx) DX_HOST_IRR_SEP_HOST_GPR##gpr_idx##_INT_BIT_SHIFT
#define SEP_HOST_GPR_IRQ_SHIFT(gpr_idx)  _SEP_HOST_GPR_IRQ_SHIFT(gpr_idx)
#define SEP_HOST_GPR_IRQ_MASK(gpr_idx)   (1 << SEP_HOST_GPR_IRQ_CAUSE_SHIFT(gpr_idx))
...
#define DX_CC_REG_ADDR(unit_name, reg_name) (DX_BASE_CC_PERIF + DX_BASE_##unit_name + DX_##reg_name##_REG_OFFSET)

/* Indexed GPR address macros - note the (not original) preprocessor tricks... */
/* (Using the macro without the "_" prefix is allowed with another macro      *
 *  as the gpr_idx) */
#define _SEP_HOST_GPR_REG_ADDR(gpr_idx)     DX_CC_REG_ADDR(SEP_RGF, SEP_SEP_HOST_GPR##gpr_idx)
#define SEP_HOST_GPR_REG_ADDR(gpr_idx)      _SEP_HOST_GPR_REG_ADDR(gpr_idx)
#define _HOST_SEP_GPR_REG_ADDR(gpr_idx)     DX_CC_REG_ADDR(SEP_RGF, SEP_HOST_SEP_GPR##gpr_idx)
...
#define DX_CC_REG_FLD_GET(unit_name, reg_name, fld_name, reg_val) \
    (DX_##reg_name##_##fld_name##_BIT_SIZE == 0x20 ?              \
         ...
         BITFIELD_GET(reg_val, DX_##reg_name##_##fld_name##_BIT_SHIFT, DX_##reg_name##_##fld_name##_BIT_SIZE))

#define DX_CC_REG_FLD_SET(unit_name, reg_name, fld_name, reg_shadow_var, new_fld_val) \
    do {                                                                              \
