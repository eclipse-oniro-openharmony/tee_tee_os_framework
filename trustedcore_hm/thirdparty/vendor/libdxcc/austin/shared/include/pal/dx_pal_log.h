...
#ifdef ARM_DSM
#define DX_PAL_LogInit() \
    do {                 \
    } while (0)
#define DX_PAL_LogLevelSet(setLevel) \
    do {                             \
    } while (0)
#define DX_PAL_LogMaskSet(setMask) \
    do {                           \
    } while (0)
#else
...
#define _DX_PAL_LOG(level, format, ...)                                                                    \
    ...
    __DX_PAL_LOG_PLAT(DX_PAL_LOG_LEVEL_##level, "%s:%s: " format, DX_PAL_LOG_CUR_COMPONENT_NAME, __func__, \
                      ##__VA_ARGS__)

#if (_DX_PAL_MAX_LOG_LEVEL >= DX_PAL_LOG_LEVEL_ERR)
#define DX_PAL_LOG_ERR(format, ...) _DX_PAL_LOG(ERR, format, ##__VA_ARGS__)
#else
#define DX_PAL_LOG_ERR(format, arg...) \
    do {                               \
    } while (0)
#endif
...
#else
#define DX_PAL_LOG_WARN(format, arg...) \
    do {                                \
    } while (0)
#endif
...
#else
#define DX_PAL_LOG_INFO(format, arg...) \
    do {                                \
    } while (0)
#endif
...
#else
#define DX_PAL_LOG_DEBUG(format, arg...) \
    do {                                 \
    } while (0)
#define DX_PAL_LOG_DUMP_BUF(msg, buf, size) \
    do {                                    \
...
#else
#define DX_PAL_LOG_TRACE(format, arg...) \
    do {                                 \
    } while (0)
#endif
...
#else
#define DX_PAL_LOG_DATA(format, arg...) \
    do {                                \
    } while (0)
#endif
