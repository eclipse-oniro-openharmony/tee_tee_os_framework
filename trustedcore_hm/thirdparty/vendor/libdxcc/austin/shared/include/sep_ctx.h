...
#define SEP_MAX_CTX_SIZE (max(sizeof(struct sep_ctx_rc4), sizeof(struct sep_ctx_cache_entry)))
...
enum sep_c2_mode { SEP_C2_NULL = -1, SEP_C2_ECB = 0, SEP_C2_CBC = 1, SEP_C2_RESERVE32B = INT32_MAX };
...
struct sep_ctx_c2 {
    ...
    uint8_t reserved[SEP_CTX_SIZE - 4 * sizeof(uint32_t) - SEP_C2_KEY_SIZE_MAX];
};
...
struct sep_ctx_hash {
    ...
    /* reserve to end of allocated context size */
    uint8_t reserved[SEP_CTX_SIZE - 2 * sizeof(uint32_t) - SEP_DIGEST_SIZE_MAX];
};
...
struct sep_ctx_hmac {
    ...
    uint32_t k0_size;
    /* reserve to end of allocated context size */
    uint8_t reserved[SEP_CTX_SIZE - 3 * sizeof(uint32_t) - SEP_DIGEST_SIZE_MAX - SEP_HMAC_BLOCK_SIZE_MAX];
};
...
struct sep_ctx_cipher {
    ...
    /* reserve to end of allocated context size */
    uint32_t reserved[SEP_CTX_SIZE_WORDS - 6 - SEP_AES_BLOCK_SIZE / sizeof(uint32_t) -
...
struct sep_ctx_aead {
    ...
    uint8_t key[SEP_AES_KEY_SIZE_MAX];
    /* reserve to end of allocated context size */
    uint32_t reserved[SEP_CTX_SIZE_WORDS - 8 - 3 * (SEP_AES_BLOCK_SIZE / sizeof(uint32_t)) -
