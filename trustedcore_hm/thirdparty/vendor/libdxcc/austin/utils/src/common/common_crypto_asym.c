 ...
 */
/* ****************************************************** */
int Sign_v15(RSA *pRsaPrivKey, char *pDataIn, int dataInSize, char *pSignature, char *Key_ptr)
{
    RSA *pRsaPubKey    = NULL;
    BIO *bio           = NULL;
    int status         = -1;
    ...
    int SignatureSize  = RSA_MOD_SIZE_IN_BYTES;
 ...
 */
/* ****************************************************** */
int Sign_v21(RSA *pRsaPrivKey, char *pDataIn, unsigned int dataInSize, char *pSignature)
{
    unsigned char pDigest[DIGEST_LENGTH] = { 0 };
    unsigned int uDigestLen              = DIGEST_LENGTH;
    EVP_MD_CTX md_ctx;
    unsigned char EM[RSA_MOD_SIZE_IN_BYTES]         = { 0 };
    unsigned char pDecrypted[RSA_MOD_SIZE_IN_BYTES] = { 0 };
    int status                                      = -1;
 ...
 */
/* ****************************************************** */
int Verify_v21(RSA *pRsaPubKey, char *pDataIn, int dataInSize, char *pSignature)
{
    unsigned char pDigest[DIGEST_LENGTH] = { 0 };
    unsigned int uDigestLen              = DIGEST_LENGTH;
    EVP_MD_CTX md_ctx;
...
int DX_Common_RSA_Verify(int pkcsVersion, char *pPubKey, char *pDataIn, int dataInSize, char *pSignature)
{
    RSA *pRsaPubKey           = NULL;
    int status                = -1;
    unsigned char pubKeyExp[] = { 0x01, 0x00, 0x01 };
    ...
    /* set the modulus  and exponent from char * into RSA key as BIGNUM */
    ...
    if (pkcsVersion == RSA_USE_PKCS_21_VERSION) {
        ...
        UTIL_LOG_ERR("\nDX_Common_RSA_Verify: Invalid pkcs version\n");
        goto END;
    }
 ...
 */
/* ****************************************************** */
int DX_Common_RSA_Sign(int pkcsVersion, char *pDataIn, unsigned int dataInSize, char *pPrivKeyFileName,
                       char *pPrivKeyPwd, char *pSignature)
{
    ...
    if ((pPrivKeyPwd != NULL)) {
        if (DX_Common_GetPassphrase(pPrivKeyPwd, &pwd)) {
    ...
    if (DX_Common_GetKeyPair(&pRsaPrivKey, pPrivKeyFileName, pwd) < 0) {
    ...
    if (pkcsVersion == RSA_USE_PKCS_21_VERSION) {
        ...
        UTIL_LOG_ERR("\nDX_Common_RSA_Sign: Invalid pkcs version\n");
        goto END;
    }
 ...
 */
/* ****************************************************** */
int DX_Common_RSA_Decrypt(int pkcsVersion, char *pPrivKeyFileName, char *pPrivKeyPwd, char *pEnDataIn, int enDataInSize,
                          char *pData)
{
    RSA *pRsaPrivKey   = NULL;
    unsigned char *pwd = NULL;
...
int DX_Common_RSA_CalculateH(const char *N_ptr, char *H_ptr)
{
    ...
    int len, i;
    int status      = -1;
    unsigned long s = RSA_MOD_SIZE_IN_BITS + 2;
    ...
    H_res = (char *)malloc((RSA_MOD_SIZE_IN_BYTES * 2 + 2) * sizeof(char));
    if (H_res == NULL) {
    ...
    if (len < RSA_MOD_SIZE_IN_BYTES * 2) {
        ...
        for (i = 0; i < (int)(RSA_MOD_SIZE_IN_BYTES * 2 - len); i++) {
            H_res[i] = '0';
        }
    }
    ...
    memcpy((char *)H_ptr, (char *)H_res, RSA_MOD_SIZE_IN_BYTES * 2 + 2);
...
int DX_Common_RSA_LoadKey(char *PemEncryptedFileName_ptr, char *Key_ptr, char *PemDecryted)
{
    ...
    *PemDecryted = '\0';
    if (DX_Common_GetKeyPair(&rsa_pkey, PemEncryptedFileName_ptr, Key_ptr) < 0) {
        UTIL_LOG_ERR("failed to DX_Common_GetKeyPair\n");
        goto rsaLoadKey_end;
    }
...
int DX_Common_RAND_Bytes(int numBytes, char *buf)
{
    ...
    if (numBytes > 0) {
        ...
        if (result <= 0) {
            UTIL_LOG_ERR("\nDX_Common_RAND_Bytes - Internal error: Function RAND_bytes failed\n");
            return 1;
        }
        return 0;
    }
