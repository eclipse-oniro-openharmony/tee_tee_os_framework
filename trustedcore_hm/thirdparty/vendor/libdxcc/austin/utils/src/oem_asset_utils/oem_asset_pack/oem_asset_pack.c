...
#define OEM_KEY_BASE64_SIZE_IN_BYTES (DX_COMMON_CALC_BASE64_ENCODE_SIZE(sizeof(EncOemKeyBuff_t)))
...
static unsigned char assetBinBuff[DX_UTIL_OEM_ASSET_DATA_MAX_SIZE_IN_BYTES] = { 0 };
...
static unsigned char assetPkg[DX_UTIL_OEM_ASSET_PACKAGE_MAX_SIZE_IN_BYTES] = { 0 };
...
int parseArguments(const int argc, char *const argv[])
{
    ...
    rc               = DX_Common_Util_copyDataFromBinFile(assetBinFileName, assetBinBuff, &assetBinBuffSize);
    if ((rc != 0) || (assetBinBuffSize & OEM_ASSET_MUL_16_BYTES_MASK) || (assetBinBuffSize == 0) ||
        (assetBinBuffSize > DX_UTIL_OEM_ASSET_DATA_MAX_SIZE_IN_BYTES)) {
    ...
    rc = DX_Common_Util_copyDataFromBinFile(oemKeyFileName, oemKeyRsaBase64Buff, &oemKeyRsaBase64BuffSize);
...
int getOemKey(void)
{
    ...
    rc = DX_Common_Base64_Decode(oemKeyRsaBase64Buff, sizeof(oemKeyRsaBase64Buff), (char *)&oemKeyStr, &oemKeyStrSize);
    if ((rc != 0) || (oemKeyStrSize != sizeof(EncOemKeyBuff_t))) {
        UTIL_LOG_ERR("failed to DX_Common_base64_decrypt(), rc %d oemKeyStrSize %d\n", rc, oemKeyStrSize);
        return 1;
    }
    ...
    rc = DX_Common_RSA_Decrypt(RSA_USE_PKCS_21_VERSION, keyPairFileName, pwdFilePtr, oemKeyStr.oemKeyRsaEnc,
                               RSA_OAEP_KEY_SIZE_IN_BYTES, oemKeyBuff);
    EVP_cleanup();
    CRYPTO_cleanup_all_ex_data(); /* cleanup application specific data to avoid memory leaks. */
    if (rc != 0) {
        UTIL_LOG_ERR("failed to DX_Common_RSA_Oaep_decrypt(), rc %d\n", rc);
        return 1;
    }
...
int main(int argc, char *argv[])
{
    ...
    rc =
        DX_Common_AES_CMAC_Encrypt(dataInBuff, sizeof(dataInBuff), oemKeyBuff, DX_UTIL_KOEM_SIZE_IN_BYTES, provKeyBuff);
    if (rc != 0) {
        UTIL_LOG_ERR("failed to DX_Common_AES_CMAC_Encrypt() for provKeyBuff rc %d\n", rc);
        goto packEnd;
    }
    ...
    rc =
        DX_Common_AES_CCM_Encrypt(provKeyBuff, &assetPkg[ASSET_PKG_CCM_NONCE_OFFSET], ASSET_PKG_CCM_NONCE_SIZE,
    ...
    if (rc != 0) {
        UTIL_LOG_ERR("failed to DX_Common_AES_CCM_Encrypt or invalid size max is %d actual is %d\n", assetBinBuffSize,
                     assetEnDataSize);
        goto packEnd;
    }
    ...
    rc = DX_Common_Util_copyBuffToBinFile(assetPktFileName, assetPkg, ASSET_PKG_NONE_ASSET_DATA_SIZE + assetEnDataSize);
    if (rc != 0) {
