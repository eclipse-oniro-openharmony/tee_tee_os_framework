...
// #include <assert.h>    we do not have <assert.h>, just define a nop
#define assert(s)
...
void *bget(requested_size) bufsize requested_size;
{
...
#ifdef FreeWipe
    V memset(((char *)b) + sizeof(struct bfhead), 0x55, (MemSize)(b->bh.bsize - sizeof(struct bfhead)));
#endif
...
void bstats(curalloc, totfree, maxfree, nget, nrel) bufsize *curalloc, *totfree, *maxfree;
long *nget, *nrel;
{
...
void bstatse(pool_incr, npool, npget, nprel, ndget, ndrel) bufsize *pool_incr;
