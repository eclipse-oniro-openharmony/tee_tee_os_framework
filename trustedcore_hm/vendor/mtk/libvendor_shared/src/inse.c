...
int __scard_transmit(int reader_id, unsigned char *p_cmd, unsigned int cmd_len, unsigned char *p_rsp,
                     unsigned int *rsp_len)
{
    ...
    return hm_drv_call_ex(SW_SYSCALL_SCARD_TRANSMIT, args, lens, ARRAY_SIZE(args), (void *)rsp_len,
                          sizeof(unsigned int));
}
...
int __scard_send(int reader_id, unsigned char *p_cmd, unsigned int cmd_len)
{
    ...
    return hm_drv_call_ex(SW_SYSCALL_SCARD_SEND, args, lens, ARRAY_SIZE(args), NULL, 0);
}
...
int __scard_receive(unsigned char *p_rsp, unsigned int *rsp_len)
{
    ...
    return hm_drv_call_ex(SW_SYSCALL_SCARD_RECEIVE, args, lens, ARRAY_SIZE(args), rsp_len, sizeof(unsigned int));
}
